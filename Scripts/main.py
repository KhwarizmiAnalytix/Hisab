...
def replace_file_extension(directory: str, from_ext: str, to_ext: str) -> int:
    """
...
def replace_in_files(
    directory: str,
    replace_list: Sequence[str],
    with_list: Sequence[str],
            ...
            try:
                ...
                if content != original_content:
                    ...
                    replacements_made = sum(
                        original_content.count(old) for old in replace_list
                    )
...
def main() -> None:
    """Parse command-line options and run the requested bulk file operation."""
    ...
    parser.add_argument(
        "--from", dest="from_ext", help="Original file extension to rename"
    )
    parser.add_argument("--to", dest="to_ext", help="New file extension")
    parser.add_argument("--replace", help="Comma-separated list of strings to replace")
    parser.add_argument(
        "--with", dest="with_text", help="Comma-separated list of replacement strings"
    )
    parser.add_argument(
        "--extensions",
    ...
    parser.add_argument(
        "--helper", action="store_true", help="Show extended usage information and exit"
    )
...
Combine --dry-run with either mode to preview changes without touching disk.""")
    ...
    if args.replace and args.with_text:
        ...
        print(
            "Error: Both --replace and --with must be specified for content replacement"
        )
        return
