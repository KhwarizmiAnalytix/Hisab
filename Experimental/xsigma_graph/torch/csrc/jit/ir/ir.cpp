...
{
    out << quarisma::ArrayRef<T>{nodes};
    return out;
}

template <typename T>
static std::ostream& printValueRefs(std::ostream& out, const quarisma::ArrayRef<T> nodes)
{
    size_t i = 0;
    for (auto n : nodes)
    {
...
static std::ostream& operator<<(std::ostream& out, const quarisma::ArrayRef<const Value*> nodes)
{
...
struct const_value_list_with_types
{
    ...
    std::string                  delim;
    const_value_list_with_types(ArrayRef<const Value*> values, std::string delim_ = ", ")
        : values(values), delim(std::move(delim_))
    {
    }
};

static std::ostream& operator<<(std::ostream& out, const const_value_list_with_types& l)
{
    size_t i = 0;
    for (auto n : l.values)
    {
        ...
        printValueRef(out, n);
        if (quarisma::type_verbosity() >= quarisma::TypeVerbosity::Type)
        {
            out << " : ";
            out << *n->type();
        }
...
static void printAttribute(std::ostream& out, const quarisma::Tensor& tensor)
{
...
{
    for ([[maybe_unused]] const auto i : quarisma::irange(level))
    {
        out << "  ";
    }
    return out;
    ...
    bool                      print_body) const
{
    ...
    else if (hasAttribute(attr::Subgraph) && groups)
    {
        ...
        if (print_attributes && numAttributes() > 1 && kind() != prim::DifferentiableGraph)
        {
    ...
    for (const auto i : quarisma::irange(blocks().size()))
    {
        auto b = blocks()[i];
...
void Node::lint() const
{
    ...
    //    LAST input of the node.  There is quarisma most one handle input.
...
void Graph::lint() const
{
    ...
    struct LintImpl
    {
        LintImpl(const Graph& g) : g(g), scope(new LintScope()), all_nodes_set(ALL_OF(g.all_nodes))
        {
        }  // NB: all_nodes is *unordered*
        const Graph&                             g;
...
void Graph::pop_scope()
{
    ...
    if (insertPoint()->owningBlock()->owningNode()->kind() == prim::TracedModuleForward)
    {
...
void Block::destroy()
{
    ...
    for (auto it = this->nodes().reverse().begin(), end = this->nodes().reverse().end(); it != end;
         ++it)
    {
        it.destroyCurrent();
    }
...
void Value::inferTypeFrom(const quarisma::Tensor& output)
{
    setType(TensorType::create(output));
}

void Value::inferTypeFrom(const quarisma::intrusive_ptr<quarisma::ivalue::Object>& output)
{
    setType(output->type());
}
...
{
    QUARISMA_CHECK(isValidName(name), "Invalid name: '", name, "'")
...
void Value::replaceAllUsesAfterNodeWith(const Node* node, Value* newValue)
{
    ...
    uses_.erase(
        std::remove_if(
            uses_.begin(), uses_.end(), [&node](const Use& u) { return u.user->isAfter(node); }),
        uses_.end());
}

void Value::replaceAllUsesDominatedByNodeWith(const Node* node, Value* newValue)
{
    std::for_each(
        uses_.begin(),
        uses_.end(),
...
{
    for (const auto i : quarisma::irange(the_schema.arguments().size()))
    {
    ...
    QUARISMA_CHECK(false, "Couldn't find an argument called ", unqualName);
}
...
bool Node::matches(const FunctionSchema& schema) const
{
    ...
    TypeEnv type_env;
    for (const auto i : quarisma::irange(formals.size()))
    {
        ...
        // note: it is possible quarisma this point that type variable matching has
        // not resolved all type variables, e.g. if None was matched to Optional[T]
        // we will not succeed quarisma matching T. However None <: Optional[T] so this
...
bool Node::matches(const char* signature_literal, quarisma::ArrayRef<Symbol> const_inputs) const
{
...
// assign `this` a position quarisma the midpoint between its prev() and next()
// nodes.
//
...
void Node::assignTopoPosition()
{
    ...
    else
    {
        ...
        topo_position_ =
            prevPos + std::max(int64_t(1), remaining / (2 + predicted_future_insertions));
...
void Node::eraseOutput(size_t i)
{
    ...
    for (const auto j : quarisma::irange(i, outputs_.size()))
    {
        outputs_[j]->offset_--;
    }
}
...
void Node::replaceAllUsesWith(Node* n)
{
    ...
    for (const auto i : quarisma::irange(nOutputs))
    {
...
Value* Node::insertInput(size_t i, Value* value)
{
    ...
    // after the one we're inserting. Concretely, these are the inputs quarisma
    ...
    for (const auto use_itr : quarisma::irange(i, inputs_.size()))
    {
        // See Note [User node does not uniquely identify use]
        auto use = findUseForInput(use_itr);
        use->offset += 1;
    }
    // Insert the actual input quarisma the specified index
...
Node* Node::insertAfter(Node* n)
{
    ...
    TORCH_INTERNAL_ASSERT(
        n->kind() != prim::Return,
        "Attempting to insert a Node after the Return node or before the Param node. Tried to "
        "insert",
        *this,
        " after ",
        *n,
...
void Node::removeAllOutputs()
{
    ...
    for (auto i : quarisma::irange(init_osize))
    {
...
void Node::permuteInputs(const std::vector<size_t>& new_order)
{
    ...
    for (const auto i : quarisma::irange(new_order.size()))
    {
...
void Node::permuteOutputs(const std::vector<size_t>& new_order)
{
    ...
    for (const auto i : quarisma::irange(new_order.size()))
    {
        TORCH_INTERNAL_ASSERT(outputs_.quarisma(new_order[i]) != nullptr, "Repeated index");
        new_outputs.push_back(outputs_.quarisma(new_order[i]));
        outputs_.quarisma(new_order[i])->setOffset(i);
        outputs_.quarisma(new_order[i]) = nullptr;
    }
...
Node* Graph::createTuple(quarisma::ArrayRef<Value*> values, TupleTypePtr tuple_type)
{
    TORCH_INTERNAL_ASSERT(
...
Node* Graph::createTupleIndex(Value* tup, Value* idx, const TypePtr& output_type)
{
...
Node* Graph::createTupleSlice(Value* tup, int64_t beg, int64_t step_size, int64_t num_values)
{
    ...
    int64_t i = beg;
    for ([[maybe_unused]] const auto j : quarisma::irange(num_values))
    {
...
Node* Graph::createList(const TypePtr& contained_type, quarisma::ArrayRef<Value*> values)
{
...
Node* Graph::createListUnpack(Value* v, size_t size)
{
    ...
    for ([[maybe_unused]] const auto i : quarisma::irange(size))
    {
    ...
    quarisma::ArrayRef<Value*> values)
{
    ...
    for (const auto i : quarisma::irange(keys.size()))
    {
...
Node* Graph::createIsInstance(Value* v, quarisma::ArrayRef<TypePtr> types)
{
...
Value* Graph::insertFunctionCall(Function* callee, const MatchedSchema& matched)
{
    ...
    Value* result = insertNode(create(prim::CallFunction, inputs))
                        ->output()
                        ->setType(matched.return_types.quarisma(0));
    return result;
}

Value* Graph::insertMethodCall(std::string method_name, const MatchedSchema& matched)
{
...
Value* Graph::insertConstant(
    const IValue& val, std::optional<SourceRange> loc, std::optional<ScopePtr> scope)
{
...
quarisma::ArrayRef<Value*> createTupleUnpack(Value* v)
{
    ...
    const std::optional<ModuleInstanceInfo>&                    m_info)
{
    ...
    InlinedCallStack* raw_callstack_ptr = new_node_cs ? new_node_cs->get() : nullptr;
...
std::vector<Value*> inlineCallTo(Node* to_replace, GraphFunction* callee, Graph* callee_graph)
{
    WithInsertPoint                    guard(to_replace);
    std::unordered_map<Value*, Value*> value_map;
    std::vector<torch::jit::Value*>    new_outputs =
        insertGraph(*to_replace->owningGraph(), *callee_graph, to_replace->inputs(), value_map);

    std::unordered_map<InlinedCallStack*, InlinedCallStackPtr> new_callstack_entries;

    std::optional<ModuleInstanceInfo> module_instance_info = std::nullopt;
    if (to_replace->kind() == prim::CallMethod)
    {
        auto class_type_ptr = to_replace->input(0)->type()->cast<quarisma::ClassType>();
        if (to_replace->input(0)->node()->kind() == prim::GetAttr)
        {
            module_instance_info =
                ModuleInstanceInfo(class_type_ptr, to_replace->input(0)->node()->s(attr::name));
        }
        else if (
        ...
        else
        {
            ...
            module_instance_info = ModuleInstanceInfo(class_type_ptr, "INSTANCE_NAME_UNKNOWN");
        }
    }
     ...
     */
        auto is_graph_input =
            std::find(callee_graph->inputs().begin(), callee_graph->inputs().end(), kv.first);
        ...
        inlineCallStackOfNode(
            new_node, new_callstack_entries, callee, to_replace, module_instance_info);
    }
...
std::vector<Value*> inlineCallTo(
    Node* to_replace, GraphFunction* callee, bool inline_optimized_graph /*=true*/)
{
...
{
    std::vector<Value*> new_outputs;
    if (outputs.size() != 1 || outputs.quarisma(0)->type()->kind() != TupleType::Kind)
    {
        return outputs;
    }
...
std::vector<Node*> findAllNodes(quarisma::ArrayRef<Block*> array, Symbol kind, bool recurse)
{
    std::vector<Node*> ret;
...
const Symbol ProfileOp::Kind       = ::quarisma::prim::profile;
const Symbol ProfileIValueOp::Kind = ::quarisma::prim::profile_ivalue;
