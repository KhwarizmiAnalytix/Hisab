...
bool reportSourceLocation(size_t file_size)
{
    ...
    const auto enable_env = quarisma::utils::get_env("PYTORCH_JIT_ENABLE_LARGE_SOURCE_LOCATION");
    bool       flag       = true;
...
struct RefinementSet
{
    ...
    RefinementSet(Refinement single_true, Refinement single_false)
        : RefinementSet(
              Refinements({std::move(single_true)}), Refinements({std::move(single_false)}))
    {
    }
    ...
    RefinementSet Not() const { return RefinementSet(false_refinements_, true_refinements_); }
    const std::vector<Refinement> activeRefinements() const { return true_refinements_; }
    ...
    static Refinements unionSet(const Refinements& a, const Refinements& b)
    {
        ...
        for (const Refinement& r : b)
        {
            auto it = std::find_if(
                result.begin(),
                result.end(),
                [&](const Refinement& e) { return e.identifier() == r.identifier(); });
            if (it == result.end())
            {
                result.push_back(r);
            }
...
struct CondValue
{
    CondValue(Value* value, RefinementSet refinements, std::optional<bool> static_if)
        : value_(value), refinements_(std::move(refinements)), static_if_(static_if)
    {
    }
    CondValue(Graph& g, const SourceRange& loc, bool static_value, RefinementSet refinements)
    ...
    const RefinementSet& refinements() const { return refinements_; }
    std::optional<bool>  staticIf() const { return static_if_; }

private:
    Value*              value_;
    RefinementSet       refinements_;
    std::optional<bool> static_if_;  // certain expression cause us to emit a static if statement
...
static std::shared_ptr<MagicMethod> makeMagic(const std::string& name, const SugaredValuePtr& base)
{
...
struct Environment
{
    ...
    // v->type(): Tensor
    void insertStore(const std::string& name, const SourceRange& loc, Value* v, TypePtr type)
    {
        auto g = b->owningGraph();
    ...
    void setType(const std::string& name, TypePtr type) { type_table[name] = std::move(type); }
        ...
        const TypePtr&     annotated_type)
    {
        ...
        if (as_simple_value && !as_simple_value->hasDebugName() && meaningfulName(name) &&
        ...
        if (auto parent = findInParentFrame(name))
        {
            ...
            {
                throw(
                    ErrorReport(loc) << "Attempting to declare and annotate the type of variable '"
                                     << name << "' but it is already defined in an outer block");
            }
            if (!as_simple_value)
            {
                throw(
                    ErrorReport(loc) << "Cannot re-assign '" << name << "' to a value of type "
                                     << value->kind() << " because " << name
                                     << " is not a first-class value.  Only reassignments to "
                                        "first-class values are allowed");
            }
            Value* simple_parent = asSimple(parent);
            if (!simple_parent)
            {
                throw(
                    ErrorReport(loc) << "Cannot re-assign '" << name << "' because it has type "
                                     << value->kind() << " and " << name
                                     << " is not a first-class value.  Only reassignments to "
                                        "first-class values are allowed");
            }
        ...
        if (as_simple_value)
        {
            if (annotated_type && !as_simple_value->type()->isSubtypeOf(*annotated_type))
            {
                throw(
                    ...
                    << annotated_type->repr_str() << " but is being assigned to a value of type "
                    << as_simple_value->type()->repr_str());
            }
            auto value_store_type = annotated_type ? annotated_type : as_simple_value->type();
    ...
    void throwVarNotFoundError(const std::string& ident, const SourceRange& range)
    {
    ...
    SugaredValuePtr getSugaredVar(
        const std::string& ident, const SourceRange& range, bool required = true)
    {
        ...
        if (!retval)
        {
            static std::unordered_map<std::string, SugaredValuePtr> globals = {
                ...
                {"float",
                 makeMagic(
                     "__float__", std::make_shared<CastValue>(FloatType::get(), aten::Float))},
                {"complex",
                 makeMagic(
                     "__complex__",
                     std::make_shared<CastValue>(ComplexType::get(), aten::Complex))},
                {"int",
                 makeMagic("__int__", std::make_shared<CastValue>(IntType::get(), aten::Int))},
                {"bool",
                 makeMagic("__bool__", std::make_shared<CastValue>(BoolType::get(), aten::Bool))},
                {"str",
                 makeMagic("__str__", std::make_shared<CastValue>(StringType::get(), aten::str))},
                ...
                {"_to_tensor", std::make_shared<CastValue>(TensorType::get(), prim::NumToTensor)},
                {"len",
                 makeMagic("__len__", std::make_shared<BuiltinFunction>(aten::len, std::nullopt))},
                {"hex",
                 makeMagic("__hex__", std::make_shared<BuiltinFunction>(aten::hex, std::nullopt))},
                {"oct",
                 makeMagic("__oct__", std::make_shared<BuiltinFunction>(aten::oct, std::nullopt))},
                {"round",
                 makeMagic(
                     "__round__", std::make_shared<BuiltinFunction>(aten::round, std::nullopt))},
                ...
                {"divmod", std::make_shared<BuiltinFunction>(aten::divmod, std::nullopt)},
                ...
                // all other exceptions should be resolved quarisma the Python level
...
static Value* materializeConstant(
    T val, Graph& graph, const SourceRange& r, std::unordered_map<T, Value*, Hash>& map)
{
...
{
    WithLoopStatus(LoopStatus* prev, LoopStatus new_status) : prev_ptr_(prev), prev_value_(*prev)
    {
        *prev = new_status;
    }
    ~WithLoopStatus() { *prev_ptr_ = prev_value_; }
...
private:
    ...
    std::unordered_map<quarisma::complex<double>, Value*, quarisma::hash<quarisma::complex<double>>>
                               complex_constants;
    ...
    // If the graph might not return, add an implicit None return quarisma the end
    void handleMaybeNoReturn(const Def& def, Block* block)
    {
        ...
        if (exit_blocks.count(block) == 0)
        {
            ...
            emitReturn(
                Return::create(def.range(), Expr(Compound::create(TK_NONE, def.range(), {}))));
        }
        else
        {
    ...
    FunctionSchema emitDef(const Def& def, const Self* self, Block* block)
    {
        ...
        {
            def_stack_.back().declared_return_type_ = schema.returns().quarisma(0).type();
        }
    ...
    {
        QUARISMA_CHECK(self, "Expected __setstate__ to have a `self` argument");
        auto getstate = self->getClassType()->findMethod("__getstate__");
        if (!getstate)
        {
            throw(
                ErrorReport(def.range()) << "`__setstate__` defined but not `__getstate__`. "
    ...
    // see [setstate type]
    static bool shouldDeriveSetStateType(const Def& def, const FunctionSchema& schema)
    {
    ...
    std::vector<Argument> emitFormalArguments(
        const Def& def, const Self* self, const FunctionSchema& schema, Block* block)
    {
        ...
        for (; it != end; ++it)
        {
            ...
            auto arg = schema.arguments().quarisma(arg_annotation_idx++);
            if (shouldDeriveType)
            {
    ...
    Argument emitOutput(const SourceRange& range, const FunctionSchema& schema, Block* block)
    {
        ...
        // are correctly typed.
        auto placeholder_return = graph->insertNode(graph->createUninitialized(ret_type))->output();
    ...
    std::shared_ptr<ClosureValue> emitClosure(const std::function<void(Block*)>& emit_body)
    {
    ...
    void checkBreakContinue(const SourceRange& loc, const std::string& stmt_name)
    {
        ...
        {
            throw(
                ErrorReport(loc) << "Because we emit iteration over modulelists or tuples as "
                                    "unrolled loops, we do not support break or continue inside "
                                    "the body of these loops");
        }
    }
    ...
    void emitBreak(const Break& stmt)
    {
        ...
        auto break_node = graph->create(prim::BreakStmt, {}, 0)->setSourceRange(stmt.range());
    ...
    void emitContinue(const Continue& stmt)
    {
        ...
        auto continue_node = graph->create(prim::ContinueStmt, {}, 0)->setSourceRange(stmt.range());
    ...
    void emitDelete(const Delete& stmt)
    {
        for (const auto& target : stmt.targets())
        {
            if (target.kind() == TK_SUBSCRIPT)
            {
                ...
                if (subscript_exprs[0].kind() == TK_SLICE_EXPR)
                {
                    throw(
                        ...
                        << "del statements only support deletion quarisma a single index, "
                ...
                if (auto cls = val->type()->cast<ClassType>())
                {
                    ...
                    {
                        throw(ErrorReport(target.range()) << "Class does not define __delitem__");
                    }

                    // Use MethodValue to call the method to handle recursion.
                    MethodValue(val, "__delitem__").call(stmt.range(), method, {idx}, {}, 0);
                }
                else
                {
                    auto node =
                        graph->create(aten::Delete, {val, idx}, 0)->setSourceRange(target.range());
            ...
            {
                throw(
                    ErrorReport(target.range()) << "del statements are only supported for deleting"
    ...
    void emitReturn(const Return& stmt)
    {
        ...
        else
        {
            ...
            auto merged_return_type = unifyTypes(declared_return_type, actual_return->type());
        ...
        if (declared_return_type == AnyType::get() && actual_return->type() != AnyType::get())
        {
            actual_return = graph->insertUncheckedCast(actual_return, declared_return_type);
        }
    ...
    RefinementSet findIsNoneRefinements(
        const Expr& lhs, Value* lhs_value, const Expr& rhs, Value* rhs_value, int tok)
    {
        ...
        if (const auto union_type = lhs_value->type()->cast<UnionType>())
        {
            ...
            std::optional<TypePtr>  remaining = union_type->subtractTypeSet(to_subtract);
    ...
    CondValue emitCondExpr(const Expr& expr)
    {
        ...
        case TK_NOT:
        {
            ...
            Value* result = emitBuiltinCall(expr.range(), *graph, aten::__not__, {v.value()}, {});
    ...
    std::shared_ptr<Environment> emitSingleIfBranch(
        Block* b, const List<Stmt>& branch, const RefinementSet& refinements)
    {
        pushFrame(b);
    ...
    Value* emitTernaryIf(const TernaryIf& expr, const TypePtr& type_hint = nullptr)
    {
        ...
        bool                  is_dict_constructor = false)
    {
        if (auto union_type_hint = (*refined_type_hint_ptr)->cast<UnionType>())
        {
            ...
            else
            {
                ...
                // types of the container we want to match, but we do quarisma least
        ...
        else if (auto optional_type_hint = (*refined_type_hint_ptr)->cast<OptionalType>())
        {
        ...
        const Expr&                 src)
    {
        ...
        for (const auto& current_candidate : all_candidates)
        {
            ...
            {
                if (!candidate || (candidate_key_type->isSubtypeOf(current_key_type) &&
    ...
    Value* emitListComprehension(const ListComp& lc, const TypePtr& type_hint)
    {
        ...
        if (refined_type_hint)
        {
            ...
            auto type_match = [&](const TypePtr& t) { return t->isSubtypeOf(AnyListType::get()); };
        ...
        auto            emit_body = [&]()
        {
            ...
            if (!type_hint && (*unified_elem_type)->isUnionType())
            {
                TORCH_WARN(
                    ...
                    ", while the elements "
                    " before it were ",
                    list_value->type()->expect<ListType>()->getElementType()->repr_str(),
                    "\n",
                    lc.range().str());
            }
            ...
            if (all_candidates.empty() && refined_type_hint &&
                !(*unified_elem_type)
                     ->isSubtypeOf(*refined_type_hint->expectRef<ListType>().getElementType()))
            {
                throw(
                    ErrorReport(lc)
            ...
            if (!all_candidates.empty())
            {
                ...
                refineAndSetListTypeHintFromCandidatesVector(
                    all_candidates, type_hint, &refined_type_hint, *unified_elem_type, lc);
            }
    ...
    Value* emitDictComprehension(const DictComp& dc, const TypePtr& type_hint)
    {
        ...
        Value* dict_value = graph->insertNode(graph->create(prim::DictConstruct, 1))->output();

        // Set the default type to be Dict[str, Tensor]
        dict_value->setType(DictType::create(StringType::get(), TensorType::get()));

        TypePtr refined_type_hint    = type_hint;
        TypePtr annotated_union_type = type_hint && type_hint->isUnionType() ? type_hint : nullptr;

        std::vector<TypePtr> all_candidates = {};

        if (refined_type_hint)
        {
            auto type_match = [&](const TypePtr& t) { return t->kind() == DictType::Kind; };
        ...
        auto            emit_body = [&]()
        {
            ...
            {
                throw(
                    ErrorReport(dc) << "Dicts may only contain homogeneous keys, but the type of "
            ...
            else if (first_generated_key_type && first_generated_key_type != k->type())
            {
                ...
                throw(
                    ErrorReport(dc) << "Dicts may only contain homogeneous keys. Expected "
            ...
            if (refined_type_hint && all_candidates.empty())
            {
                ...
                bool is_key_subtype = k->type()->isSubtypeOfExt(*dict_type_hint->getKeyType(), &ss);
    ...
    CondValue emitShortCircuitLogical(
        const SourceRange& loc, const Expr& first_expr, const Expr& second_expr, bool is_or)
    {
        ...
        if (is_or)
        {
            ...
            if ((lhs.staticIf() && *lhs.staticIf()) || (rhs->staticIf() && *rhs->staticIf()))
            {
                static_if = true;
            }
        ...
        const std::function<Value*()>& false_expr)
    {
        ...
        auto true_type  = true_block->outputs().quarisma(0)->type();
        auto false_type = false_block->outputs().quarisma(0)->type();
    ...
    Value* emitToBool(const SourceRange& loc, Value* v)
    {
        ...
        {
            throw(
                ErrorReport(loc) << "Could not cast value of type " << v->type()->repr_str()
                                 << " to bool");
        }
        if (!out)
        {
        ...
        {
            throw(
                ErrorReport(loc) << "expected a bool expression for condition but found "
                                 << out->type()->repr_str());
        }
        return out;
    }
        ...
        const List<Stmt>&  falseBranch)
    {
        ...
        auto save_true = emitSingleIfBranch(true_block, trueBranch, cond_value.refinements());
        auto save_false =
            emitSingleIfBranch(false_block, falseBranch, cond_value.refinements().Not());
                        ...
                        environment_stack->setVariableTypeError(
                            ...
                            [=]() -> std::string
                            {
                                ...
                                ss << v << " is not defined in the false branch. "
                                   << "The source info is eliminated due to the source file is too "
                                      "large. "
                                   << "To get it back, please set "
                                      "PYTORCH_JIT_ENABLE_LARGE_SOURCE_LOCATION=1 "
                                   << "as env var";
                                return ss.str();
                            });
                    }
        ...
        for (const auto& x : mutated_variables)
        {
            ...
            bool default_to_union = full_type && (full_type->kind() == UnionType::Kind ||
                                                  full_type->kind() == OptionalType::Kind ||
                                                  full_type->kind() == NumberType::Kind);
            auto unified =
                unifyTypes(tv->type(), fv->type(), /*default_to_union=*/default_to_union);
    ...
    CondValue emitHasAttr(const Expr& objExpr, const Expr& attrExpr)
    {
        ...
        {
            throw(ErrorReport(attrExpr) << "hasattr's second argument must be a string literal");
        }
    ...
    CondValue emitIsInstance(const Expr& obj, const Expr& classinfo)
    {
        ...
        for (const TypePtr& lhs_type : lhs_types)
        {
            if (lhs_type == AnyType::get())
            {
                ...
                if (isinstance_types.size() != 1 || isinstance_types[0] != AnyType::get())
                {
            ...
            for (const TypePtr& rhs_type : rhs_types)
            {
                ...
                else if (*maybe_smaller_type == *rhs_type)
                {
                    ...
                    found_refinement =
                        found_refinement ? *(unifyTypes(found_refinement, rhs_type)) : rhs_type;
                }
            }
        ...
        {
            unified_false =
                *unifyTypeList(not_isinstance_types, nowhere, /*default_to_union=*/true);
        }
        ...
        // check maybe true/false quarisma runtime, need an actual op
    ...
    void emitIf(const If& stmt)
    {
        ...
        emitIfElseBlocks(stmt.range(), cond_value, stmt.trueBranch(), stmt.falseBranch());
    }
        ...
        const List<Expr>&            targets)
    {
        ...
        TORCH_INTERNAL_ASSERT(static_len, "Unrolled loop iter should have static length");
        ...
        for (const auto i : quarisma::irange(len))
        {
    ...
    void emitWith(const With& stmt)
    {
        ...
        for (const auto& target : targets)
        {
            ...
            {
                throw(ErrorReport(e.range()) << "With item expression must return an object");
            }
            ...
            {
                throw(
                    ErrorReport(e.range()) << "Object returned by with item expression does not "
            ...
            else
            {
                ...
                {
                    if (exitSchema.arguments().quarisma(i).type() != AnyType::get())
                    {
                        throw(
                            ErrorReport(e.range())
                            << "argument " << i
                            << " of __exit__ must have Any type; TorchScript does not currently "
                               "support passing exception type, value, or traceback to the "
                               "__exit__ function.");
                    }
                }
            }
            ...
            n->output(0)->setType(enterSchema.returns().quarisma(0).type());
    ...
    void emitRaise(const Raise& raise)
    {
        ...
        if (auto exception_instance = std::dynamic_pointer_cast<ExceptionMessageValue>(sv))
        {
        ...
        else if (auto exception_class = std::dynamic_pointer_cast<ExceptionValue>(sv))
        {
        ...
        else
        {
            ...
            throw(ErrorReport(raise.range()) << "exceptions must derive from BaseException");
        }
        ...
        graph->insert(
            prim::RaiseException, {error_message, qualified_class_name}, {}, raise.range());
        exit_blocks.insert(environment_stack->block());
    }
    ...
    void emitAssert(const Assert& stmt)
    {
        ...
        auto message =
            (stmt.msg().present()) ? stmt.msg().get() : StringLiteral::create(stmt.range(), "");
    ...
    // 2) There is quarisma most one Starred node in the lhs Expr
    ...
    // If the RHS is a tensor, return the corresponding Quarisma in-place op
    // If it's a list of scalars, then return the corresponding list augment op
    Symbol getAugOp(const AugAssign& stmt, const TypePtr& type)
    {
        bool use_inplace_op =
            type->isSubtypeOf(*TensorType::get()) || type->kind() == TypeKind::ListType;
        switch (stmt.aug_op())
        {
        case '+':
        ...
        default:
            throw(
                ErrorReport(stmt) << "Unknown augmented assignment: "
    ...
    std::pair<std::string, std::string> getAugMagicMethod(const AugAssign& stmt)
    {
        ...
        case '/':
            return std::make_pair(std::string("__itruediv__"), std::string("__truediv__"));
        case '*':
    ...
    void emitAugAssignmentToSelectVar(const AugAssign& stmt)
    {
        ...
        const auto lhsValue      = lhsSugaredVar->attr(lhs.range(), method, lhs.selector().name())
    ...
    Value* emitAugAssignmentHelper(const AugAssign& stmt, Value* lhs)
    {
        if (lhs->type()->kind() == TypeKind::ClassType)
        {
            ...
            std::tie(in_place_method_name, out_of_place_method_name) = getAugMagicMethod(stmt);
        ...
        {
            const auto rhs =
                NamedValue(stmt.rhs().range(), emitExpr(stmt.rhs())).value(*method.graph());
    ...
    void emitAugAssignmentGeneric(const AugAssign& stmt, const Subscript& lhs, Value* sliceable)
    {
        ...
        {
            throw(
                ErrorReport(lhs) << type->repr_str() << " does not support augmented assignment.");
        }
        const auto idxValue     = emitExpr(subscriptExprs[0]);
        const auto containerArg = NamedValue(lhs.value().range(), type->str(), sliceable);
        const auto idxArg       = NamedValue(subscriptExprs.range(), "idx", idxValue);
        const auto valueArg     = NamedValue(stmt.rhs().range(), "value", emitExpr(stmt.rhs()));

        const auto getItem =
            graph->insert(aten::__getitem__, {containerArg, idxArg}, {}, stmt.range());
        const auto augmentedItem =
            graph->insert(getAugOp(stmt, elemType), {getItem, valueArg}, {}, stmt.range());
        graph->insert(aten::_set_item, {containerArg, idxArg, augmentedItem}, {}, stmt.range());
    }
    ...
    void emitAugAssignmentToSubscript(const AugAssign& stmt)
    {
        ...
        if (sliceable->type()->isSubtypeOf(*TensorType::get()))
        {
            ...
            else
            {
                ...
                const auto indices =
                    graph->insertNode(graph->createList(OptionalType::ofTensor(), tensorIndices))
                        ->output();
                const auto indexed =
                ...
                const auto augmented = emitBuiltinCall(
                    ...
                    *method.graph(),
                    ...
                    {},
                    indexed);
                graph->insert(aten::index_put_, {slicedArg, indices, augmented}, {}, stmt.range());
            }
        }
        else
        {
    ...
    NamedValue emitValueToTensor(const NamedValue& value, const NamedValue& matchTypeOf)
    {
        ...
        if (kind == quarisma::TypeKind::NumberType || kind == quarisma::TypeKind::IntType ||
            kind == quarisma::TypeKind::BoolType || kind == quarisma::TypeKind::FloatType ||
            kind == quarisma::TypeKind::ComplexType)
        {
            ...
            auto converted = graph->insert(
                aten::tensor, {value}, {NamedValue("dtype", dtype), NamedValue("device", device)});
    ...
    void emitSubscriptAssign(const SourceRange& stmtRange, const Subscript& lhs, const Expr& rhs)
    {
        emitSubscriptAssign(stmtRange, lhs, NamedValue(rhs.range(), emitExpr(rhs)));
    }

    void emitSubscriptAssign(
        const SourceRange& stmtRange, const Subscript& lhs, const NamedValue& rhs)
    {
        ...
        if (sliceable->type()->isSubtypeOf(*TensorType::get()))
        {
            ...
            else
            {
                ...
                graph->insert(aten::index_put_, {slicedArg, indices, convertedRhs}, {}, stmtRange);
            }
        ...
        else
        {
            ...
            args.emplace_back(lhs.subscript_exprs().range(), "idx", emitExpr(subscript[0]));
            args.push_back(rhs);
            makeMagic(
                "__setitem__", std::make_shared<BuiltinFunction>(aten::_set_item, std::nullopt))
        ...
        bool                   starred_unpack)
    {
        auto outputs = rhs_output->asTuple(
            rhs_loc, method, starred_unpack ? std::nullopt : std::optional<size_t>{n_binders});
        if (outputs.size() < n_binders)
        {
            throw(
                ErrorReport(tl) << "need " << (starred_unpack ? "quarisma least " : "") << n_binders
                                << " values to unpack but found only " << outputs.size());
        }
        if (outputs.size() > n_binders && !starred_unpack)
        {
            throw(
                ErrorReport(tl) << "too many values to unpack: need " << n_binders << " but found "
                                << outputs.size());
        }
        ...
        for (auto assignee : lhs_exprs)
        {
            ...
            case TK_SUBSCRIPT:
                emitSubscriptAssign(
                    rhs_loc,
                    Subscript(assignee),
                    NamedValue(rhs_loc, outputs.quarisma(i)->asValue(rhs_loc, method)));
                i++;
                break;
            case TK_VAR:
                environment_stack->setSugaredVar(
            ...
            case TK_TUPLE_LITERAL:
            {
                ...
                if (sub_starred_unpack)
                    sub_n_binders--;
                emitTupleAssign(
                    sub_tl, outputs.quarisma(i), rhs_loc, sub_n_binders, sub_starred_unpack);
                i++;
            }
            break;
            case '.':
            {
                emitSelectAssign(assignee, outputs.quarisma(i), rhs_loc);
                i++;
            }
            break;
            default:
    ...
    void emitSingleAssignment(const Assign& stmt)
    {
        ...
        case TK_VAR:
        {
                ...
                "__torch__.torch.nn.quantized.dynamic.modules.rnn.PackedParameter.__setstate__")
            {
                if (auto sv = std::dynamic_pointer_cast<SimpleValue>(rhs_sugared_val))
                {
                    ...
                    if (rhs_node->kind() == Symbol::fromQualString("quantized::linear_prepack"))
                    {
        ...
        default:
            throw(
                ErrorReport(stmt.lhs()) << "unexpected expression on left-hand side of assignment");
        }
    }
    ...
    void emitSelectAssign(const Assign& stmt)
    {
        ...
        const auto rhsValue =
            emitSugaredExpr(stmt.rhs().get(), 1, type_hint)->asValue(stmt.rhs().range(), method);
        lhsObject->setAttr(stmt.range(), method, lhs.selector().name(), rhsValue);
    }

    void emitSelectAssign(const Expr& lhs, const SugaredValuePtr& rhs, const SourceRange& loc)
    {
    ...
    NodeKind getNodeKind(int kind, size_t ninputs)
    {
        ...
        default:
            QUARISMA_CHECK(false, "unknown kind ", kind);
        }
    }
    ...
    std::vector<NamedValue> getNamedValues(const TreeList& trees, bool maybe_unpack)
    {
        ...
        for (const auto& tree : trees)
        {
            if (maybe_unpack && tree->kind() == TK_STARRED)
            {
                ...
                {
                    values.emplace_back(tree->range(), entry->asValue(starred.range(), method));
                }
            }
            else
            {
    ...
    std::vector<NamedValue> getNamedValues(const List<Expr>& trees, bool maybe_unpack)
    {
    ...
    void checkApplyNumInputs(const Apply& apply, size_t expected_inputs)
    {
        ...
        {
            throw(
                ErrorReport(loc) << Var(apply.callee()).name().name() << " expected exactly "
    ...
    void checkApplyNumInputsRange(
        const Apply& apply, size_t min_expected_inputs, size_t max_expected_inputs)
    {
        const SourceRange& loc               = apply.range();
    ...
    std::shared_ptr<SugaredValue> emitApplyExpr(
        Apply& apply, size_t n_binders, const TypePtr& type_hint = nullptr)
    {
        auto sv  = emitSugaredExpr(apply.callee(), 1);
        auto loc = apply.callee().range();
        if (auto special_form = dynamic_cast<SpecialFormValue*>(sv.get()))
        ...
        const TypePtr&                       type_hint = nullptr)
    {
        ...
        case prim::fork:
        {
            ...
            if (trees.empty())
            {
                throw(ErrorReport(apply) << "Expected quarisma least one argument to fork()");
            }
        ...
        case prim::awaitable:
        {
            ...
            {
                throw(ErrorReport(apply) << "Expected quarisma least one argument to awaitable()");
            }
        ...
        case prim::annotate:
        {
            ...
            if (!expr->type()->isSubtypeOfExt(*type, &why_not))
            {
                throw(
                    ...
                    << "expected an expression of type " << type->repr_str() << " but found "
                    << expr->type()->repr_str() << "\n"
                    << why_not.str());
            }
        ...
        case prim::GetAttr:
        {
            ...
            {
                throw(ErrorReport(apply) << "getattr's second argument must be a string literal");
            }
        ...
        case prim::Uninitialized:
        {
            ...
            TypePtr type = typeParser_.parseTypeFromExpr(apply.inputs()[0]);
            auto    out =
                graph->insertNode(graph->createUninitialized(type))->setSourceRange(apply.range());
        ...
        case prim::TupleConstruct:
        {
            ...
            auto inp_values = fmap(
                inputs,
                [&](const SugaredValuePtr& sv) { return sv->asValue(apply.range(), method); });
        ...
        case prim::LegacyTypedConstructor:
        {
            ...
            auto get_base_error_msg = [&]()
            {
                ...
                base_error_msg << "'new(IntArrayRef size, *, Device? device=None)',\n";
            ...
            if (kwargs.size() > 1)
            {
                throw(ErrorReport(apply) << get_base_error_msg().str() << "Got multiple kwargs\n");
            }
            ...
            bool all_ints = std::all_of(
                args.begin(), args.end(), [](Value* v) { return v->type()->cast<IntType>(); });
            if (args.empty())
            {
                // empty inputs == torch.tensor([], dtype=....)
                auto inp_list = graph->insertNode(graph->createList(IntType::get(), {}))->output();
                return std::make_shared<SimpleValue>(graph->insert(
                    aten::tensor, {inp_list}, {NamedValue(apply.range(), "dtype", dtype_ivalue)}));
            }
            else if (all_ints)
            {
                auto inp_list =
                    graph->insertNode(graph->createList(IntType::get(), args))->output();
                return std::make_shared<SimpleValue>(graph->insert(
                    aten::empty, {inp_list}, {NamedValue(apply.range(), "dtype", dtype_ivalue)}));
            }
            else if (args.size() == 1)
            {
                return std::make_shared<SimpleValue>(graph->insert(
                    aten::tensor, {args[0]}, {NamedValue(apply.range(), "dtype", dtype_ivalue)}));
            }
            else
            {
                throw(
        ...
        case prim::tolist:
        {
            ...
            return std::make_shared<SimpleValue>(
                graph->insertToList(operand->asValue(value.range(), method), type_hint));
        }
        case prim::HasAttr:
        {
            checkApplyNumInputs(apply, 2);
        ...
        case prim::CreateObject:
        {
            ...
            {
                throw(
                    ErrorReport(apply) << "Expected class value as argument to __new__, got "
                                       << arg->kind() << " instead");
            }
            auto createNode = graph->insertNode(graph->createObject(class_arg->type_));
        ...
        case prim::range:
        {
            std::vector<Value*> input_vals = getValues(apply.inputs(), /*maybe_unpack=*/true);
        ...
        case prim::enumerate:
        {
            ...
            if (input_size == 0)
            {
                throw(ErrorReport(loc) << "enumerate expected quarisma least 1 arguments, got 0");
            }
            ...
            if (arg_size > 2)
            {
                throw(
                    ErrorReport(loc)
                    << "enumerate expected quarisma most 2 arguments, got " << arg_size);
            }
            ...
            if (attribute_size == 1)
            {
                ...
                {
                    throw(
                        ErrorReport(loc) << "enumerate expected kwarg name 'start', got '"
            ...
            SugaredValuePtr        range_sv =
                std::make_shared<RangeValue>(loc, method, range_inputs, iter_static_len);
        ...
        case prim::zip:
        {
            ...
            if (inputs.empty())
            {
                throw(ErrorReport(apply) << "zip expected quarisma least 1 arguments, got 0");
            }
        ...
        case aten::index:
        {
            ...
            if (inputs.size() != 1)
            {
                throw(
                    ErrorReport(apply)
                    << "__getitem__ expected exactly 1 arguments, got " << inputs.size());
            }
            auto input = emitSugaredExpr(apply.inputs()[0], 1)->asValue(loc, method);
    ...
    std::shared_ptr<SugaredValue> emitApplySpecialFormForList(
        Apply& apply, const TypePtr& type_hint = nullptr)
    {
        if (apply.inputs().empty())
        {
            ...
            if (!type->cast<ListType>())
            {
                throw(
                    ...
                    << "Expected list type annotation for list(), found " << type_hint->repr_str());
            }
            return std::make_shared<SimpleValue>(
                graph
                    ->insertNode(
                        graph->createList(type->expectRef<ListType>().getElementType(), {}))
                    ->output());
        }
        ...
        if (auto simple = asSimple(iter_input))
        {
            ...
            {
                return std::make_shared<SimpleValue>(
                    emitBuiltinCall(apply.range(), *method.graph(), aten::list, {simple}, {}));
            }
        }
        ...
        auto ident = Var::create(apply.range(), Ident::create(apply.range(), elem_name));
        auto iter  = Var::create(apply.range(), Ident::create(apply.range(), iter_name));
        ...
        Apply& apply, const TypePtr& type_hint = nullptr)
    {
                                               ...
                                               const TypePtr& annotated_dict_type)
        {
            ...
            auto annotated_k_type = annotated_dict_type->expect<DictType>()->getKeyType();
            auto annotated_v_type = annotated_dict_type->expect<DictType>()->getValueType();

            const auto is_key_subtype   = key_type == annotated_k_type;
            const auto is_value_subtype = value_type->isSubtypeOfExt(annotated_v_type, &ss);
        ...
        {
            Node* n = graph->insertNode(graph->create(prim::unchecked_cast, {result}));
            n->output()->setType(std::move(annotated_union_type));
            result = n->output();
        };
        ...
        if (!apply.inputs().empty())
        {
            ...
            if (auto simple = asSimple(iter_input))
            {
                ...
                {
                    auto dc_value =
                        emitBuiltinCall(apply.range(), *method.graph(), aten::dict, {simple}, {});
                    add_kwargs(dc_value);
                    if (annotated_union_type)
                    {
        ...
        if (treat_as_empty_container())
        {
            ...
            apply = Apply::create(apply.range(), apply.callee(), expr_list, apply.attributes());
        }
        ...
        if (apply.inputs().empty() && apply.attributes().empty())
        {
            if (!refined_type_hint)
            {
                refined_type_hint = DictType::create(StringType::get(), TensorType::get());
            }
        ...
        if (!apply.inputs().empty() && apply.inputs()[0].kind() == TK_DICT_LITERAL)
        {
            ...
            TORCH_INTERNAL_ASSERT(dict_lit.key_inputs().size() == dict_lit.value_inputs().size());
            ...
            (apply.inputs().empty() || apply.inputs()[0].kind() == TK_LIST_LITERAL))
        {
            ...
            apply           = Apply::create(apply.range(), apply.callee(), new_inputs, new_kwargs);
        }
        ...
        auto value = Var::create(apply.range(), Ident::create(apply.range(), value_name));
        auto target =
            TupleLiteral::create(apply.range(), List<Expr>::create(apply.range(), {key, value}));
        auto iter = Var::create(apply.range(), Ident::create(apply.range(), iter_name));
    ...
    NodeKind reverseComparision(NodeKind kind)
    {
        ...
        QUARISMA_CHECK(false, "reverseComparision: unsupported NodeKind. File a bug");
    }
    ...
    std::shared_ptr<SugaredValue> emitSugaredExpr(
        const Expr& tree, size_t n_binders, const TypePtr& type_hint = nullptr)
    {
        switch (tree.kind())
        {
        case TK_VAR:
    ...
    Value* emitUnaryOp(
        const TreeRef& tree, const std::string& magicMethod, const quarisma::Symbol& opSymbol)
    {
        const auto& inputs       = tree->trees();
        auto        named_values = getNamedValues(inputs, /*maybe_unpack=*/false);
        auto        val          = asSimple(
            makeMagic(magicMethod, std::make_shared<BuiltinFunction>(opSymbol, std::nullopt))
        ...
        return graph->insertConstant(maybe_out_stack->quarisma(0), tree->range());
    }
   ...
   */
        ...
        quarisma::ArrayRef<NamedValue>       kwargs)
    {
        ...
        auto fork_node =
            g->insertNode(method.graph()->create(prim::forkClosure, 1))->setSourceRange(loc);
        ...
        // into the fork input. If a closure doesn't already exist, we create one.
        {
            ...
            if (ClosureValue* sv = dynamic_cast<ClosureValue*>(forked.get()))
            {
                ...
                Block* closure_block  = closure_output->node()->blocks().quarisma(0);
                TORCH_INTERNAL_ASSERT(closure_block->outputs().size() == 1);
                out_type = closure_block->outputs().quarisma(0)->type();
        ...
        quarisma::ArrayRef<NamedValue>       kwargs)
    {
        ...
        auto await_node =
            g->insertNode(method.graph()->create(prim::awaitableClosure, 1))->setSourceRange(loc);
        ...
        Value* node_output = await_node->output()->setType(AwaitType::create(out_type));
        return std::make_shared<SimpleValue>(node_output);
    }
    ...
    std::shared_ptr<SugaredValue> emitRpcExpr(const Apply& apply, Symbol rpc_op)
    {
        ...
        // The RPC C++ entry API can take quarisma::Function directly.
        size_t      rpcMinInputs = 2;
        size_t      rpcMaxInputs = 5;
        std::string op_name      = rpc_op.toUnqualString();
        if (apply.inputs().size() < rpcMinInputs || apply.inputs().size() > rpcMaxInputs)
        {
            throw(
                ErrorReport(apply)
                << "Possible forms of call to " << op_name << "(..) are\n"
                << op_name << "(dst_worker_name, user_callable, args, kwargs, timeout)\n"
        ...
        {
            throw(
                ErrorReport(apply) << op_name << "(dst_worker_name, user_callable, args, kwargs)"
        ...
        TreeList args_kwargs_timeout_trees(input_trees.begin() + 2, input_trees.end());
        ...
        TORCH_INTERNAL_ASSERT(
            ...
            callablePtrs.size())
        Function* callablePtr = callablePtrs.quarisma(0);
        ...
        if (!args_kwargs_timeout_trees.empty())
        {
            ...
            auto  entry_sugared_values =
                emitSugaredExpr(Expr(args_tree), 1)->asTuple(args_tree->range(), method);
            args.reserve(entry_sugared_values.size());
            for (const auto& entrie_sugared_value : entry_sugared_values)
            {
                args.emplace_back(
                    args_tree->range(), entrie_sugared_value->asValue(args_tree->range(), method));
            }
            ...
            // Notice the key is determined quarisma run time.
            // We can do it quarisma compile time, unless one day the RPC API is
        ...
        Node* rpc_node = graphPtr->insertNode(graphPtr->create(rpc_op, 1))->setSourceRange(loc);
        {
    ...
    Value* emitBinaryOp(const TreeRef& tree)
    {
        ...
        return asSimple(makeMagic(overload, std::make_shared<BuiltinFunction>(kind, std::nullopt))
                            ->call(tree->range(), method, named_values, {}, 0));
    }
    ...
    Value* emitListLiteral(const ListLiteral& ll, const TypePtr& type_hint)
    {
        ...
        TypePtr annotated_union_type =
            refined_type_hint && refined_type_hint->isUnionType() ? refined_type_hint : nullptr;
        ...
        if (!values.empty())
        {
            ...
            std::optional<TypePtr> unified_elem_type =
                unifyTypeList(types, nowhere, /*default_to_union=*/true, elem_type_hint);

            if (!refined_type_hint && (*unified_elem_type)->kind() == UnionType::Kind)
            {
                TORCH_WARN(
                    "List consists of heterogeneous types, which means",
                    " that it has been typed as containing ",
            ...
            {
                refineAndSetListTypeHintFromCandidatesVector(
                    all_candidates, type_hint, &refined_type_hint, *unified_elem_type, ll);
        ...
        if (annotated_union_type)
        {
            Node* n = graph->insertNode(graph->create(prim::unchecked_cast, {result->output()}));
            n->output()->setType(std::move(annotated_union_type));
            result = n;
        }
    ...
    Value* emitDictLiteral(DictLiteral dl, const TypePtr& type_hint)
    {
        ...
        for (const auto i : quarisma::irange(key_trees.size()))
        {
            ...
            else
            {
                if (keys[i - 1]->type()->kind() != keys[i]->type()->kind())
                {
                    throw(
                        ...
                        << "only a single type. Expected: " << keys[i - 1]->type()->repr_str()
                        << " but found " << keys[i]->type()->repr_str() << " instead");
                }
                rhs_value_type =
                    *(unifyTypes(rhs_value_type, values[i]->type(), /*default_to_union=*/true));
            }
        }
        ...
        auto default_refined_type_hint_setter = [&]()
        {
            ...
            {
                refined_type_hint = DictType::create(keys.quarisma(0)->type(), rhs_value_type);
    ...
    Value* emitSimpleExpr(const TreeRef& tree, const TypePtr& type_hint = nullptr)
    {
        switch (tree->kind())
        {
        case TK_FLOOR_DIV:
        case '@':
        {
            ...
            auto        kind         = getNodeKind(tree->kind(), inputs.size());
            auto        named_values = getNamedValues(inputs, /*maybe_unpack=*/false);
            return emitBuiltinCall(tree->range(), *method.graph(), kind, named_values, {});
        }
        case '%':
        {
            auto lhs =
                emitSugaredExpr(Expr(tree->tree(0)), 0)->asValue(tree->tree(0)->range(), method);
            auto const& lhs_type = lhs->type();
            if (lhs_type == StringType::get())
            {
                auto values = getValues(tree->trees(), /*maybe_unpack=*/false);
                auto node =
                    graph->create(aten::percentFormat, values, 1)->setSourceRange(tree->range());
    ...
    Value* emitSelect(const SourceRange& loc, Value* input, Value* dim, Value* index)
    {
        return emitBuiltinCall(loc, *graph, aten::select, {input, dim, index}, {});
    }

    Value* emitSliceOp(
        const SourceRange& loc, Value* sliceable, Value* dim, Value* start, Value* end, Value* step)
    {
        std::vector<NamedValue> args;
        args.reserve(5);
        ...
        if (sliceable->type()->cast<TupleType>())
        {
            ...
            start ? tuple_args.emplace_back(start) : tuple_args.emplace_back(std::nullopt);
            end ? tuple_args.emplace_back(end) : tuple_args.emplace_back(std::nullopt);
            step ? tuple_args.emplace_back(step) : tuple_args.emplace_back(std::nullopt);
    ...
    Value* emitIndex(const SourceRange& loc, Value* input, quarisma::ArrayRef<Value*> indices)
    {
        ...
        auto* index =
            graph->insertNode(graph->createList(OptionalType::ofTensor(), indices))->output();
    ...
    // applied yet.
    //   Should be NULL quarisma indices where sliceable (post-slicing) isn't indexed by
    //   a tensor.
        ...
        const SourceRange& loc, Value* sliceable, const List<Expr>& subscript_exprs)
    {
        ...
        auto insert_value_for_dim = [&](int64_t dim) { return graph->insertConstant(dim, loc); };
        std::vector<int64_t>               dims(subscript_exprs.size());
        std::vector<std::optional<Value*>> exprs(subscript_exprs.size(), std::nullopt);

        auto handle_indexing =
            [&](const Expr& subscript_expr, size_t expr_idx, int64_t dim, bool is_reverse = false)
        {
            dims[expr_idx] = dim;
            ...
            if (index->type()->kind() == quarisma::TypeKind::ListType)
            {
        ...
        for (size_t rev_idx = subscript_exprs.size() - 1; rev_idx > idx; rev_idx--)
        {
            ...
            if (subscript_expr.kind() == TK_DOTS)
            {
                throw(ErrorReport(loc) << "An index can only have a single ellipsis ('...')");
            }
            rdim = handle_indexing(subscript_expr, rev_idx, rdim, /*is_reverse=*/true);
        }
        for (const auto i : quarisma::irange(exprs.size()))
        {
            if (!exprs[i].has_value())
            {
                ...
                if (const auto slice_value = dynamic_cast<SliceValue*>(subscript_sv.get()))
                {
            ...
            {
                sliceable = emitSelect(loc, sliceable, insert_value_for_dim(dims[i]), expr);
            }
        ...
        // quarisma::index takes in a List[Optional[Tensor]] where some dims can be None.
        // create None node with optional tensor output type and pass to quarisma::index.
    ...
    // in one pass and then apply quarisma::index on the result of the
    ...
    // - tensor_indices[i] = NULL if we should not index `sliced` quarisma dim i
    // - tensor_indices[i] = t if we should index `sliced` quarisma dim i with tensor t.
        ...
        const SourceRange& loc, Value* sliceable, const List<Expr>& subscript_exprs)
    {
        ...
        {
            throw(
                ErrorReport(loc) << "Unsupported operation: attempted to use multidimensional "
        ...
        {
            // XXX: Might need to quarisma::alias this when we support mutability
            return sliceable;
        }
        ...
        bool                allow_out_of_bounds)
    {
        ...
        {
            throw(
                ErrorReport(loc) << "Tuple index out of range. Tuple is length " << tuple_len
                                 << " and index is " << input_index);
        }
        return adj_index;
    }
    ...
    // each iter.
    Value* emitTupleIndex(const SourceRange& loc, Value* tuple_val, Value* idx_val)
    {
        ...
        if (!idx)
        {
            if (elems.empty() || !convertibleToList(tuple_typ, ListType::create(elems[0])))
            {
                throw(
                    ErrorReport(loc)
        ...
        {
            auto adj_index =
                getAdjTupleIndex(loc, tuple_typ, idx->toInt(), /*allow_out_of_bounds*/ false);
            output_type = elems[adj_index];
        }
        return graph->insertNode(graph->createTupleIndex(tuple_val, idx_val, output_type))
            ->output();
    }
        ...
        const std::vector<std::optional<NamedValue>>& tuple_args)
    {
        ...
        if (step)
        {
            ...
            QUARISMA_CHECK(val->isInt(), "Step size should always be an integer");
        ...
        if (beg_val)
        {
            beg = getAdjTupleIndex(loc, tuple_type, getSliceInd(beg_val->value(*graph), loc), true);
        }

        int64_t end = std::numeric_limits<int64_t>::max();
        if (end_val)
        {
            end = getAdjTupleIndex(loc, tuple_type, getSliceInd(end_val->value(*graph), loc), true);
        }

        int64_t num_values =
            slice_indices_adjust(static_cast<int64_t>(tuple_len), &beg, &end, step_size);

        return graph
            ->insertNode(
                graph->createTupleSlice(tuple_val.value(*graph), beg, step_size, num_values))
            ->output();
    }
        ...
        const Subscript& subscript, TypePtr type_hint = nullptr)
    {
        ...
        {
            return std::make_shared<SimpleValue>(
                emitMultidimSlicing(range, sv->asValue(val_range, method), subscript_exprs));
        }
        if (subscript_exprs[0].kind() == TK_SLICE_EXPR)
        {
            ...
            if (sv->kind() == "module")
            {
                ...
                auto s_tuple_val = sv->asTupleValue(val_range, method)->asValue(val_range, method);
                ...
                if (slice.step().present())
                {
                    auto step = NamedValue(val_range, "step", emitExpr(Expr(slice.step().get())));
            ...
            {
                return std::make_shared<SimpleValue>(
                    emitBasicSlice(range, sv->asValue(val_range, method), subscript_exprs));
            }
        }
        else
        {
...
struct FunctionResolver : public Resolver
{
    explicit FunctionResolver(
        Resolver* otherResolver, const std::unordered_map<std::string, Function*>& functionTable)
        : otherResolver_(otherResolver), functionTable_(functionTable)
    {
    }

    std::shared_ptr<SugaredValue> resolveValue(
        const std::string& name, GraphFunction& m, const SourceRange& loc) override
    {
...
CompilationUnit::CompilationUnit(const std::string& source) : CompilationUnit()
{
...
{
    PropertyPair(std::unique_ptr<Function> getter, std::unique_ptr<Function> setter)
    {
    ...
    std::unique_ptr<Function>& getGetter() { return this->first; }

    std::unique_ptr<Function>& getSetter() { return this->second; }
};
    ...
    bool                                              shouldMangle) const
{
    ...
    std::unique_ptr<Function> getter_fn =
        define(prefix, prop.getter(), resolver, self, function_table, shouldMangle);
    ...
    {
        setter_fn =
            define(prefix, prop.setter().get(), resolver, self, function_table, shouldMangle);
    }

    // Add the property to the class type definition.
    self->getClassType()->addProperty(prop.name().name(), getter_fn.get(), setter_fn.get());
    ...
    std::optional<size_t>                             operator_set_version) const
{
    ...
    if (!self)
    {
        ...
        _resolver = std::make_shared<FunctionResolver>(resolver.get(), function_table);
    }
    auto creator = [def, _resolver, self](GraphFunction& method)
    {
        ...
        if (self)
        {
            ...
            // There should be quarisma least a ClassName.method_name
            TORCH_INTERNAL_ASSERT(atoms.size() >= 2);
            call_name = atoms.quarisma(atoms.size() - 2) + "." + atoms.quarisma(atoms.size() - 1);
        }
        ErrorReport::CallStack call(call_name, def.range());
        to_ir(def, _resolver, self, method);
    };
    auto name =
        prefix ? QualifiedName(*prefix, def.name().name()) : QualifiedName(def.name().name());
    if (shouldMangle)
    {
    ...
    std::optional<size_t>                         operator_set_version)
{
    ...
    for (const auto i : quarisma::irange(properties.size()))
    {
    ...
    for (const auto i : quarisma::irange(definitions.size()))
    {
        auto fn = define(
            prefix,
            definitions[i],
    ...
    bool                                          shouldMangle)
{
    ...
    auto build_schema = [&](const Def& hook_def, const ResolverPtr& hook_res) -> FunctionSchema
    {
        ...
        FunctionSchema   schema = typeParser.parseSchemaFromDef(hook_def, true /* skip_self*/);
        // need to add self as the first because we skipped it
        std::vector<Argument> arguments;
        arguments.emplace_back(hook_def.decl().params()[0].ident().name(), self->getClassType());
        arguments.insert(arguments.end(), schema.arguments().begin(), schema.arguments().end());
        return schema.cloneWithArguments(arguments);
    };

    // define hooks
    for (const auto i : quarisma::irange(hookDefs.size()))
    {
    ...
    // define pre_hooks
    for (const auto i : quarisma::irange(preHookDefs.size()))
    {
        // check to see if already defined this hook
...
bool meaningfulName(const std::string& name)
{
    ...
    if (name[0] != '_')
        return true;
    for (const auto i : quarisma::irange(1, name.size()))
    {
        if (!isdigit(name[i]))
            return true;
    }
    ...
    bool                           is_module)
{
    ...
    for (const Stmt& stmt : classDef.body())
    {
        ...
        {
            throw(
                ErrorReport(stmt) << "interface declarations can only contain method definitions");
        }
        ...
        FunctionSchema schema = typeParser.parseSchemaFromDef(method_def, /* skip_self*/ true);
        // need to add self as the first because we skipped it
        std::vector<Argument> arguments;
        ...
        for (size_t i = 0; i < stmts_size - 1; i++)
        {
            ...
            if (cur_statement.kind() == TK_EXPR_STMT)
            {
                ...
                {
                    throw(
                        ErrorReport(method_def.range()) << "interfaces declarations should only "
                                                           "contain a single 'pass' statement.");
                }
            }
