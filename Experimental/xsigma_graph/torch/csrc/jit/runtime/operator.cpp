...
private:
    ...
    // have static storage duration and so its address won't change quarisma runtime.
    ...
    // still need to do the complete string matching quarisma the first time, which is
    ...
    // which one is picked quarisma runtime.
#ifdef QUARISMA_MOBILE
    std::unordered_set<quarisma::OperatorName> registered_operator_names;
#endif
...
public:
    void registerOperator(Operator&& op)
    {
        ...
        TORCH_INTERNAL_ASSERT(
            ...
            op.schema(),
            "\" to JIT but the operator name was already registered before. Please add or change "
            "the overload name.");
    ...
    std::vector<Symbol> findSimilarOperators(Symbol input_op)
    {
        ...
        auto cmp = [](const EntryPair& lhs, const EntryPair& rhs) { return lhs.first > rhs.first; };
...
bool printerHasSpecialCaseFor(Symbol sym)
{
    ...
    const static std::unordered_set<Symbol> handled = {
        prim::Constant,      prim::Uninitialized,  prim::fork,           prim::awaitable,
        prim::ListConstruct, prim::DictConstruct,  prim::ListUnpack,     prim::Print,
        prim::PythonOp,      prim::TupleConstruct, prim::TupleIndex,     prim::TupleSlice,
        prim::TupleUnpack,   prim::CreateObject,   prim::GetAttr,        prim::SetAttr,
        prim::CallFunction,  prim::isinstance,     prim::unchecked_cast, prim::tolist,
        prim::rpc_async,     prim::rpc_sync,       prim::rpc_remote};
    ...
    const static std::unordered_set<Symbol> required_namespaces = {
        quarisma::namespaces::prim,
        quarisma::namespaces::aten,
        quarisma::namespaces::onnx,
    };
...
std::shared_ptr<Operator> findOperatorFor(const quarisma::OperatorName& full_name)
{
...
std::string canonicalSchemaString(const FunctionSchema& schema)
{
    ...
    {
        out += schema.returns().quarisma(0).type()->str();
    }
