...
struct TORCH_API ProfilingGraphExecutorImpl : public GraphExecutorImplBase
{
    ...
    const ExecutionPlan& getPlanFor(
        Stack& stack, std::optional<size_t> remaining_bailout_depth) override;
    GraphExecutorState getDebugState() override;
    ~ProfilingGraphExecutorImpl() override = default;

    void debugFlushCompilationCache();

    bool isOptimized() const override { return optimized_plan_.has_value(); }

private:
    const ExecutionPlan& getOptimizedPlanFor(
        Stack& stack, std::optional<size_t> remaining_bailout_depth);
    ...
    std::optional<ExecutionPlan>     profiling_plan_;  // plan to run in order to profiling the code
