...
struct ExecutionPlan
{
    ...
    ExecutionPlan(std::shared_ptr<Graph> graph, std::string function_name)
        : code(graph, std::move(function_name)),
          graph(FLAGS_torch_jit_execution_plan_reuse_code_graph ? code.graph() : std::move(graph))
    {
    }

    operator bool() const { return static_cast<bool>(graph); }
...
struct TORCH_API GraphExecutor
{
    ...
    const ExecutionPlan& getPlanFor(
        Stack& inputs, std::optional<size_t> remaining_bailout_depth = std::nullopt);
...
TORCH_API Node* replaceBlockWithFallbackGraph(Block* b, ArrayRef<Value*> inputs);
...
{
    GraphOptimizerEnabledGuard(bool state) : old_state_(getGraphExecutorOptimize())
    {
        setGraphExecutorOptimize(state);
    }

    ~GraphOptimizerEnabledGuard() { setGraphExecutorOptimize(old_state_); }
...
// This interface still allows our tests to look quarisma optimized graphs, but
// with less plumbing.
}  // namespace detail
