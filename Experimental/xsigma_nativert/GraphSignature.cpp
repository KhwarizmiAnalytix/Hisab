...
std::pair<std::string, std::string> getSpecDetails(const torch::_export::InputSpec& inputSpec)
{
    // Retrieve the argument name and spec tag name
    switch (inputSpec.tag())
    {
    case torch::_export::InputSpec::Tag::PARAMETER:
        return std::make_pair(inputSpec.get_parameter().get_arg().get_name(), "PARAMETER");
        break;
    case torch::_export::InputSpec::Tag::BUFFER:
        return std::make_pair(inputSpec.get_buffer().get_arg().get_name(), "BUFFER");
        break;
    case torch::_export::InputSpec::Tag::TENSOR_CONSTANT:
        return std::make_pair(
            inputSpec.get_tensor_constant().get_arg().get_name(), "TENSOR_CONSTANT");
        break;
    case torch::_export::InputSpec::Tag::CUSTOM_OBJ:
        return std::make_pair(inputSpec.get_custom_obj().get_arg().get_name(), "CUSTOM_OBJ");
        break;
    case torch::_export::InputSpec::Tag::USER_INPUT:
        if (inputSpec.get_user_input().get_arg().tag() == torch::_export::Argument::Tag::AS_TENSOR)
        {
            return std::make_pair(
                inputSpec.get_user_input().get_arg().get_as_tensor().get_name(), "USER_INPUT");
        }
        else if (
        ...
        {
            return std::make_pair(
                inputSpec.get_user_input().get_arg().get_as_custom_obj().get_name(), "USER_INPUT");
        }
        else
        {
            QUARISMA_CHECK(false, "Unsupported USER_INPUT argument type.");
        }
        break;
    ...
    case torch::_export::InputSpec::Tag::TOKEN:
        QUARISMA_CHECK(false, "Token inputs not implemented yet.");
    default:
        QUARISMA_CHECK(false, "Unknown InputSpec tag encountered.");
    }
}

void checkInputOrders(const std::vector<torch::_export::InputSpec>& inputSpecs)
{
    // Map each tag to its index in the expected order
    static constexpr std::array<std::pair<torch::_export::InputSpec::Tag, uint32_t>, 5>
        tagOrderArray = {
    ...
    for (const auto& inputSpec : inputSpecs)
    {
        ...
        QUARISMA_CHECK(it != tagOrderArray.end(), "Unknown InputSpec tag encountered.");
    ...
    const quarisma::FastSet<std::string>& graphNames)
{
    ...
    std::string errorMsg = fmt::format(
        ...
        fmt::join(sigNames, ", "),
        fmt::join(graphNames, ", "));
    QUARISMA_CHECK(false, errorMsg);
}

void checkOutputNames(
    const quarisma::FastSet<std::optional<std::string>>& sigNames,
    const quarisma::FastSet<std::string>&                graphNames)
{
...
GraphSignature::GraphSignature(const torch::_export::GraphSignature& storage)
{
    ...
    for (const torch::_export::InputSpec& inputSpec : storage.get_input_specs())
    {
        ...
        case torch::_export::InputSpec::Tag::USER_INPUT:
        {
            ...
            else if (userInputArg.tag() == torch::_export::Argument::Tag::AS_CUSTOM_OBJ)
            {
            ...
            else
            {
                ...
                QUARISMA_CHECK(false, "Non tensor inputs not implemented yet.");
            }
            break;
        }
        case torch::_export::InputSpec::Tag::PARAMETER:
        ...
        {
            numTensorConstants_++;
            const auto& inputName  = inputSpec.get_tensor_constant().get_arg().get_name();
            const auto& weightName = inputSpec.get_tensor_constant().get_tensor_constant_name();
            inputsToWeights_.emplace_back(inputName, weightName);
            break;
        }
        case torch::_export::InputSpec::Tag::CUSTOM_OBJ:
        {
            ...
            const auto& customObjName = inputSpec.get_custom_obj().get_custom_obj_name();
    ...
    for (const torch::_export::OutputSpec& outputSpec : storage.get_output_specs())
    {
            ...
            if (isSymbolicOutput(outputSpec.get_user_output().get_arg().tag()))
            {
                ...
                case torch::_export::Argument::Tag::AS_TENSOR:
                {
                    userOutputs_.emplace_back(
                        outputSpec.get_user_output().get_arg().get_as_tensor().get_name());
                    break;
                }
                case torch::_export::Argument::Tag::AS_SYM_INT:
                {
                    userOutputs_.emplace_back(
                        outputSpec.get_user_output().get_arg().get_as_sym_int().get_as_name());
                    break;
                }
                default:
                {
                    QUARISMA_CHECK(false, "Unsupported symbolic user output type encountered.");
                }
                }
            }
            else
        ...
        {
            QUARISMA_CHECK(false, "Token outputs not implemented yet.");
        }
        default:
            QUARISMA_CHECK(false, "Unknown OutputSpec tag encountered.");
        }
    }
...
quarisma::FastSet<std::string> GraphSignature::inputNames() const
{
    quarisma::FastSet<std::string> ret;
...
void GraphSignature::lint(
    const quarisma::FastSet<std::string>& graphInputs,
    const quarisma::FastSet<std::string>& graphOutputs) const
{
    checkInputNames(inputNames(), graphInputs);
    checkOutputNames(outputNames(), graphOutputs);
}

void GraphSignature::replaceAllUses(std::string_view old, std::string_view replacement)
{
    if (old == replacement)
    {
        return;
...
std::ostream& operator<<(std::ostream& out, const GraphSignature& sig)
{
    ...
    if (!sig.inputsToTensorConstants().empty())
    {
        ...
        for (const auto& [inputName, tensorConstantName] : sig.inputsToTensorConstants())
        {
    ...
    if (!sig.userInputsToMutate().empty())
    {
        ...
        for (const auto& [outputName, mutatedUserInputName] : sig.userInputsToMutate())
        {
    ...
    if (sig.hasBackward())
    {
        ...
        if (!sig.gradientsToUserInputs().empty())
        {
            ...
            for (const auto& [outputName, userInputName] : sig.gradientsToUserInputs())
            {
