...
size_t expectImpl(std::string_view source, std::string_view expected, size_t curPos)
{
...
Value* Graph::addValue(const std::optional<std::string>& name, const Type& type, Node* node)
{
...
{
    auto& node = nodesOwner_.emplace_back(
        std::make_unique<Node>(this, std::move(target), std::move(inputs), std::move(metadata)));
    return node.get();
}
...
Node* Graph::insertNode(
    ...
    std::unordered_map<std::string, std::string> metadata)
{
    auto node = createNode(std::move(target), std::move(inputs), std::move(metadata));
...
bool Node::isBefore(const Node* n) const
{
    ...
    for (const Node* cursor = this->next(); cursor != nullptr; cursor = cursor->next())
    {
        if (cursor == n)
        {
            return true;
...
std::vector<Node*> Node::producers() const
{
    ...
    if (this->next() == nullptr /* prim.Output */)
    {
        ...
        {
            if (node.next() == nullptr /* prim.Output */ || node.prev() == nullptr /* prim.Input */)
            {
                continue;
            }
...
std::vector<Node*> Node::users() const
{
    ...
    if (this->prev() == nullptr /* prim.Input */)
    {
        for (auto& node : owningGraph_->nodes())
        {
            if (node.prev() == nullptr /* prim.Input */ || node.next() == nullptr /* prim.Output */)
            {
                continue;
            }
...
bool Graph::cleanupDeadNodes()
{
    ...
    for (auto& n : nodes())
    {
        if (n.target() == "prim.Input" || n.target() == "prim.Output" || visited.count(&n))
        {
            continue;
        }
...
void Graph::replaceAllUsesAfterNode(Value* old, Value* replacement, Node* afterThis)
{
    ...
    std::unordered_map<const Value*, Value*>& valueMap)
{
    ...
    for (const auto& n : subgraph.nodes())
    {
        ...
        Node* newNode = insertNode(std::string(n.target()), std::move(clonedInputs), n.metadata());
...
{
    users_.erase(
        std::remove_if(users_.begin(), users_.end(), [&](Node* el) { return el == node; }),
        users_.end());
}
...
std::ostream& printList(std::ostream& out, bool encloseInSquareBrackets, const T& list, F formatter)
{
...
std::ostream& operator<<(std::ostream& out, const Constant& constant)
{
    ...
    std::visit(
        [&](auto&& arg)
        {
            ...
            else if constexpr (is_same_v<T, vector<int64_t>> || is_same_v<T, vector<bool>>)
            {
...
std::ostream& operator<<(std::ostream& out, const Node& node)
{
    ...
    if (node.target() == "prim.Output")
    {
        out << "return(";
        printList(
            out, false, node.inputs(), [](std::ostream& out, const auto& nv) { out << *nv.value; });
        out << ")";
        return out;
    }
...
{
public:
    explicit Parser(std::string_view source) : source_(source), graph_(Graph::createGraph()) {}
    std::unique_ptr<Graph> parse();

private:
    template <typename T>
    std::vector<T> parseList(char open, char close, const std::function<T()>& parseFn);

    std::string_view parseUntil(const std::function<bool()>& fn, bool includeEnd = false);
...
template <typename T>
std::vector<T> Parser::parseList(char open, char close, const std::function<T()>& parseFn)
{
    std::vector<T> ret;
    expect(open);
...
std::string_view Parser::parseUntil(const std::function<bool()>& fn, bool includeEnd)
{
...
{
    expect("return");
    const auto returns = parseList<Value*>('(', ')', [&]() { return parseSymbolicArgument(); });
...
Value* Parser::parseSymbolicArgument()
{
    ...
    {
        listElements =
            parseList<Value*>('[', ']', [&]() { return graph_->getValue(parseAtomicSymbol()); });
    }
...
std::variant<Value*, Constant> Parser::parseArgument()
{
    ...
    if (cur() == '[')
    {
        const auto symbol = parseUntil([&]() { return cur() == ']'; }, /*includeEnd=*/true);
    ...
    const auto symbol = parseUntil([&]() { return cur() == ',' || cur() == ')'; });
