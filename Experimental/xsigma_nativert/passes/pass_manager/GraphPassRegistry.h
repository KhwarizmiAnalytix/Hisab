...
public:
    ...
    const GraphPassIdentifier& name() const { return name_; }

    const PassSignature& get() const { return pass_; }
...
public:
    ...
    {
        GraphPassRegistry::get().add_pass(GraphPass(std::move(name), std::move(pass)));
    }
...
private:
    GraphPassRegistry() { LOG(INFO) << "Creating GraphPassRegistry"; }
