...
using MatchFilter =
    std::function<bool(const Match&, const quarisma::FastMap<std::string, const Value*>&)>;
...
inline std::ostream& operator<<(std::ostream& out, const Match& match)
{
    ...
    for (const auto& kv : match.node_map)
    {
        ...
        out << "  Pattern Node: " << *patternNode << " -> Target Node: " << *targetNode << "\n";
    }
    ...
    for (const auto& kv : match.value_map)
    {
        ...
        out << "  Pattern Value: " << *patternValue << " -> Target Value: " << *targetValue << "\n";
    }
...
public:
    ...
    /// Attempt to match the pattern quarisma a given node in the target graph.
...
private:
   ...
   *`match`
   **/
    bool tryMatchNodeInputs(const Node* pattern_node, Node* target_node, Match& match);
...
public:
   ...
   */
    void registerRewritePattern(const std::string& pattern, const std::string& replacement);
   ...
   */
    ...
    bool /* mutated? */ run(
        ...
        const MatchFilter& filter =
            [](const Match&, const quarisma::FastMap<std::string, const Value*>&) { return true; })
    {
        return run(graph, std::vector<MatchFilter>({filter}));
    }

    bool /* mutated? */ run(Graph* graph, const std::vector<MatchFilter>& filters);
...
private:
    ...
    // Helper methods
    bool overlapsWithUsedNodes(const Match& match, const std::unordered_set<Node*>& replacedNodes);
    void rewriteMatch(
        Graph* graph, const Match& match, const Graph& pattern, const Graph& replacement);

    quarisma::FastMap<std::string, const Value*> getVmap(const Graph& pattern);
};
}  // namespace torch::nativert
