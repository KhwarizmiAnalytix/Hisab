...
{
    return std::holds_alternative<int64_t>(c) || std::holds_alternative<double>(c);
}
...
bool schemaTypeMatch(const quarisma::FunctionSchema& schema, const Node& node)
{
    ...
    for (const auto& input : node.inputs())
    {
        ...
        for (const auto& schemaArg : schema.arguments())
        {
            ...
            {
                if (schemaArg.type() == quarisma::TensorType::get() && input.value &&
    ...
    for (const auto& constant : node.attributes())
    {
        for (const auto& schemaArg : schema.arguments())
        {
            ...
            {
                if (schemaArg.type() == quarisma::TensorType::get() && isScalar(constant.value))
                {
                    return false;
                }
                break;
...
std::string selectScalarOverloadName(const Node& node)
{
    ...
    // some linking issues.
    static std::unordered_set<std::string> allowed = {"add",
                                                      "add_",
                                                      "add_out",
                                                      "div",
                                                      "div_",
                                                      ...
                                                      "floor_divide_out",
                                                      "_conj"};
    std::vector<std::string_view>          atoms   = quarisma::split(node.target(), '.');
    ...
    auto op = quarisma::Dispatcher::singleton().findSchemaOrThrow(
    ...
    {
        if (auto schema = quarisma::Dispatcher::singleton().findSchema(
...
void selectScalarOverload(Graph* graph)
{
    for (auto& node : graph->nodes())
    {
        for (auto& attr : node.attributes())
        {
            ...
            {
                selectScalarOverload(std::get<std::unique_ptr<Graph>>(attr.value).get());
            }
        }

        auto                          target = node.target();
        std::vector<std::string_view> atoms  = quarisma::split(target, '.');
        ...
        {
            node.setTarget(fmt::format("torch.ops.{}.{}.{}", ns, opName, overloadName));
        }
