 ...
 * torch/_export/serde/schema.py See more quarisma:
...
public:
    ...
    const auto& lossOutput() const { return lossOutput_; }

    const auto& gradientsToParameters() const { return gradientsToParameters_; }

    const auto& gradientsToUserInputs() const { return gradientsToUserInputs_; }

    auto inputsToParameters() const
    {
        quarisma::FastMap<std::string_view, std::string_view> inputsToParameters;
        inputsToParameters.reserve(numParameters_);
        for (int i = 0; i < numParameters_; ++i)
        {
            inputsToParameters.emplace(inputsToWeights_[i].first, inputsToWeights_[i].second);
        }
    ...
    {
        quarisma::FastMap<std::string_view, std::string_view> inputsToBuffers;
        ...
        for (int i = numParameters_;
             ...
             ++i)
        {
            inputsToBuffers.emplace(inputsToWeights_[i].first, inputsToWeights_[i].second);
        }
    ...
    {
        quarisma::FastMap<std::string_view, std::string_view> inputsToTensorConstants;
        ...
        for (int i = numParameters_ + numPersistentBuffers_ + numNonPersistentBuffers_;
             ...
             ++i)
        {
            inputsToTensorConstants.emplace(inputsToWeights_[i].first, inputsToWeights_[i].second);
        }
        return inputsToTensorConstants;
    }

    const auto& inputsToCustomObjs() const { return inputsToCustomObjs_; }
    ...
    auto buffers() const
    {
        ...
        for (int i = numParameters_; i < numParameters_ + numPersistentBuffers_; i++)
        {
    ...
    const auto& userInputs() const { return userInputs_; }

    const auto& userOutputs() const { return userOutputs_; }

    const auto& buffersToMutate() const { return buffersToMutate_; }

    const auto& userInputsToMutate() const { return userInputsToMutate_; }
...
private:
    quarisma::FastSet<std::string>                inputNames() const;
    quarisma::FastSet<std::optional<std::string>> outputNames() const;

    quarisma::FastMap<std::string, std::string> gradientsToParameters_;
    quarisma::FastMap<std::string, std::string> gradientsToUserInputs_;
    quarisma::FastMap<std::string, std::string> buffersToMutate_;
    quarisma::FastMap<std::string, std::string> userInputsToMutate_;
