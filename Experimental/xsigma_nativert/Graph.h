...
public:
    ...
    void eraseAllUsers() { users_.clear(); }
    ...
    void setIsFolded() { isFolded_ = true; }
...
public:
    ...
    void setTarget(std::string_view target) { target_ = target; }
    ...
    Graph* owningGraph() { return owningGraph_; }

    const Graph* owningGraph() const { return owningGraph_; }
...
{
public:
    static std::unique_ptr<Graph> createGraph() { return std::unique_ptr<Graph>(new Graph()); }
    ...
    Value* addValue(const std::optional<std::string>& name, const Type& type, Node* producer);
    ...
    ValueId getNextValueId() { return uniqueValueId_++; }
    ...
    Node* inputNode() { return inputNode_; }

    Node* outputNode() { return outputNode_; }

    const Node* outputNode() const { return outputNode_; }
    ...
    const GraphSignature& signature() const { return signature_; }

    void setSignature(GraphSignature signature) { signature_ = std::move(signature); }
    ...
    const std::unordered_map<std::string, TensorMeta>& weightsMeta() const { return weightsMeta_; }
...
{
public:
    explicit InsertingAfter(Node* n) : insertAfter_(n), prev_(n->owningGraph()->insertionPoint())
    {
        insertAfter_->owningGraph()->setInsertionPointAfter(insertAfter_);
    }
    ~InsertingAfter() { insertAfter_->owningGraph()->setInsertionPoint(prev_); }
