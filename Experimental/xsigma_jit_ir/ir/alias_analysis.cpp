...
private:
    std::optional<AliasTypeSet> mapTypeToAliasTypeSetImpl(const TypePtr& type)
    {
        ...
        {
            if (auto maybe_mut_types =
                    mapTypeToAliasTypeSet(type->castRaw<FutureType>()->getElementType()))
            {
        ...
        {
            if (auto maybe_mut_types =
                    mapTypeToAliasTypeSet(type->castRaw<AwaitType>()->getElementType()))
            {
                return {AliasTypeSet{AwaitType::create(*toSingleType(*maybe_mut_types))}};
            }
            return std::nullopt;
        }
    ...
    const TypePtr& type, quarisma::flat_hash_map<TypePtr, AliasTypeSet>* mutable_type_cache)
{
    ...
    auto kind = type->kind();
    if (kind == TypeKind::TensorType || kind == TypeKind::ListType || kind == TypeKind::ClassType ||
        kind == TypeKind::DictType)
    {
        return true;
    }
...
const AliasTypeSet* AliasDb::mapTypeToAliasTypeSetPtr(const TypePtr& type) const
{
...
{
    void registerWrite(const Value* v, Node* n) { writes_[n].emplace_back(v); }
    void registerWriteToAllContained(const Value* v, Node* n)
    {
        containedWrites_[n].emplace_back(v);
    }
    void registerWriteToAllWildcards(Node* n) { writesToAllWildcards_.insert(n); }
      ...
      writeRegistry_(std::make_unique<AliasDb::WriteRegistry>())
{
    ...
    memoryDAG_->setWildcards(
        wildcards_,
        elementMap_,
        [&](const Value* v) -> Element* { return getWildcard(v->type()); });
    ...
    for (const auto& write : writeRegistry_->writes_)
    {
        ...
        for (const Value* writtenValue : writtenValues)
        {
            ...
            const auto& writtenMemoryLocations = memoryDAG_->getMemoryLocations(it->second);
...
bool AliasDb::hasWriters(const Value* v) const
{
    ...
    return writtenToLocationsIndex_->intersects(memoryDAG_->getMemoryLocations(el));
}
...
std::string AliasDb::toGraphviz() const
{
    ...
    for (const auto& ptrPair : elementMap_)
    {
        ...
        if (!element->containedElements.empty())
        {
            for (const auto contained : element->containedElements)
            {
                dot << "  " << name(element) << " -> " << name(memoryDAG_->fromIndex(contained))
...
void AliasDb::analyzeImpl(Node* node)
{
    ...
    if (node->kind().is_aten() || node->kind().is_prim() || node->kind().is_cuda())
    {
    ...
    TORCH_INTERNAL_ASSERT(
        analysis == AliasAnalysisKind::FROM_SCHEMA,
        "AliasAnalysisKind::CONSERVATIVE/PURE_FUNCTION/INTERNAL_SPECIAL_CASE should already have "
        "been handled above");
...
void AliasDb::analyzeSubgraph(Node* node, const std::shared_ptr<Graph>& subgraph)
{
    ...
    // subgraph block.
    TORCH_INTERNAL_ASSERT(subgraphBlock->outputs().size() >= node->outputs().size());
...
namespace
{
std::optional<bool> getConstantBooleanInput(Node* node, const std::string& inputName)
{
    TORCH_INTERNAL_ASSERT(node->hasNamedInput(inputName), inputName + " input is expected");
    auto value = node->namedInput(inputName);
    TORCH_INTERNAL_ASSERT(
        value->type() == BoolType::get(), inputName + "training input is expected to be a bool");
...
{
    TORCH_INTERNAL_ASSERT(
        node->kind() == prim::ListConstruct || node->kind() == prim::DictConstruct ||
...
void AliasDb::giveFreshAlias(const Value* value, bool add_wildcard_to_contained_elems)
{
...
{
public:
    explicit WorkingSet(Node* mover, const AliasDb& aliasDb) : aliasDb_(aliasDb), mover_(mover)
    {
    ...
    const std::vector<Node*>& dependentNodes() { return nodes_; }
...
private:
    ...
    bool consumesFrom(Node* n) const
    {
        ...
        return std::any_of(
            users.begin(),
            users.end(),
            [&](Node* user) { return node_to_index_.find(user) != node_to_index_.end(); });
    }
...
bool AliasDb::tryMove(Node* toMove, Node* movePoint, MoveSide moveSide, bool dryRun)
{
    ...
    const bool splitToMoveAndDeps = (moveSide == MoveSide::BEFORE && toMove->isBefore(movePoint)) ||
                                    (moveSide == MoveSide::AFTER && toMove->isAfter(movePoint));
    ...
    if (splitToMoveAndDeps)
    {
        ...
        const auto reversed = moveSide == MoveSide::BEFORE ? MoveSide::AFTER : MoveSide::BEFORE;
...
{
    std::optional<Element*> maybe_wildcardElement = tryGetOrCreateWildcard(v->type());
...
bool AliasDb::writesToAlias(Node* n, const ValueAndMemoryLocationSet& vls) const
{
    const auto writtenTo = getWrites(n);
    if (writtenTo.empty())
    {
