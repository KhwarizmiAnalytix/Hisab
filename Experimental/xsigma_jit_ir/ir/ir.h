...
#include <Quarisma/core/dynamic_type.h>
#include <Quarisma/core/enum_type.h>
...
#define QUARISMA_USING(T) using ::quarisma::T;
QUARISMA_FORALL_TYPES(QUARISMA_USING)
#undef QUARISMA_USING

#define QUARISMA_USING(T) using ::quarisma::T##Ptr;
QUARISMA_FORALL_TYPES(QUARISMA_USING)
#undef QUARISMA_USING

using ::quarisma::Type;
using ::quarisma::TypeEnv;
using ::quarisma::TypePtr;

using ::quarisma::getTypePtr;
using ::quarisma::MatchTypeReturn;
using ::quarisma::TypeKind;
...
namespace attr
{
using namespace ::quarisma::attr;
}
namespace aten
{
using namespace ::quarisma::aten;
}
namespace cuda
{
using namespace ::quarisma::cuda;
}  // namespace cuda
...
struct Use
{
    ...
    bool operator==(const Use& b) { return user == b.user && offset == b.offset; }
};
...
template <typename T>
using ArrayRef        = quarisma::ArrayRef<T>;
...
public:
    ...
    TORCH_API void inferTypeFrom(const quarisma::Tensor& output);
    TORCH_API void inferTypeFrom(const quarisma::intrusive_ptr<quarisma::ivalue::Object>& output);
    const TypePtr& type() const
    {
        AT_ASSERT(type_ != nullptr);
        return type_;
    }
    bool requires_grad() const { return type()->requires_grad(); }
    ...
    bool             hasDebugName() const { return !unique_name_.empty(); }
   ...
   */
    ...
    TORCH_API void replaceAllUsesDominatedByNodeWith(const Node* node, Value* newValue);
...
public:
    ...
    const std::optional<std::string> getHistoricSchemaName() { return historic_schema_name_; }

    void setHistoricSchemaName(const std::string& name) { historic_schema_name_ = name; }
   ...
   */
    ...
    Block*       owningBlock() { return owning_block_; }
    const Block* owningBlock() const { return owning_block_; }
    ScopePtr     scope() { return scope_; }
    void         setScope(ScopePtr scope) { scope_ = std::move(scope); }
    ...
    std::optional<InlinedCallStackPtr> callstack() const { return callstack_; }
    void setCallStack(InlinedCallStackPtr cs) { callstack_ = std::move(cs); }
    ...
    // different.)
    quarisma::ArrayRef<Value*>       inputs() { return inputs_; }
    quarisma::ArrayRef<const Value*> inputs() const
    {
    ...
    // implementation of std::vector could make this representationally
    // different.)
    quarisma::ArrayRef<Value*>       outputs() { return outputs_; }
    quarisma::ArrayRef<const Value*> outputs() const
    {
        // Vectors are not convertible in const-ness of elements, but
        // raw pointers are.
        return {outputs_.data(), outputs_.size()};
    }
    Value* output(size_t i) const { return outputs_.quarisma(i); }
    bool   hasUses() const
    {
        for (auto o : outputs())
        {
    ...
    Value* input()
    {
        ...
        return inputs_.quarisma(0);
    }
    Value* output()
    {
        AT_ASSERT(outputs_.size() == 1);
        return outputs_.quarisma(0);
    }
    ...
    Value* input(size_t i) const { return inputs_.quarisma(i); }
    ...
    bool is_constant(Symbol name) const { return static_cast<bool>(get(name)); }
    ...
    {
        return kind_ == prim::Constant || kind_ == prim::profile || kind_ == prim::profile_ivalue;
    }
    ...
    // Add 'node' as an input to 'this' quarisma the end of existing
    ...
    // Add 'value' as an input to 'this' quarisma the specified position in the
    // arguments. Returns the added value for ease of chaining.
    Value* insertInput(size_t i, Value* value);

    // Replace the input of 'this' quarisma position 'i' with
    ...
    quarisma::ArrayRef<Block*>       blocks() { return blocks_; }
    quarisma::ArrayRef<const Block*> blocks() const
    {
        // Vectors are not convertible in const-ness of elements, but
        // raw pointers are.
    ...
    // Remove the input quarisma 'i' from this node.
    //
    ...
    // iterators of the node list starting quarisma this node
    // useful for resuming a search starting quarisma this node
    ...
    inline const_graph_node_list_iterator reverseIterator() const { return iterator().reverse(); }
    ...
    bool matches(const char* signature_literal, quarisma::ArrayRef<Symbol> const_inputs = {}) const;
    ...
    bool hasAttributeS(const std::string& name) const { return hasAttribute(Symbol::attr(name)); }
    AttributeKind kindOf(Symbol name) const
    {
        AT_ASSERT(name.is_attr());
        return (*findAttr(name, true))->kind();
    }
    AttributeKind kindOfS(const std::string& name) const { return kindOf(Symbol::attr(name)); }
    ...
    Node*  removeAttributeS(const std::string& name) { return removeAttribute(Symbol::attr(name)); }
    bool   hasAttributes() const { return !values_.empty(); }
    size_t numAttributes() const { return values_.size(); }
...
#define CREATE_ACCESSOR(Kind, method)                                                   \
    ...
    {                                                                                   \
        return setAttr<Kind##Attr>(name, std::forward<Kind##Attr::ConstructorType>(v)); \
    }                                                                                   \
    const Kind##Attr::ValueType& method(Symbol name) const { return getAttr<Kind##Attr>(name); }
...
#undef CREATE_ACCESSOR
    ...
    GraphAttr::ValueType& g(Symbol name) { return getAttr<GraphAttr>(name); }
    ...
    const TensorAttr::ValueType& t(Symbol name) const { return getAttr<TensorAttr>(name); }

    Node* ts_(Symbol name, TensorsAttr::ConstructorType v)
    {
        return setAttr<TensorsAttr>(name, std::forward<TensorsAttr::ConstructorType>(v));
    }
    const TensorsAttr::ValueType& ts(Symbol name) const { return getAttr<TensorsAttr>(name); }
...
private:
    ...
    std::vector<AVPtr>::iterator findAttr(Symbol name, bool required)
    {
        AT_ASSERT(name.is_attr());
        auto it = std::find_if(
            values_.begin(), values_.end(), [&](const AVPtr& v) { return v->name == name; });
...
struct Block
{
    ...
    quarisma::ArrayRef<Value*>       inputs() { return input_->outputs(); }
    quarisma::ArrayRef<const Value*> inputs() const
    {
        const auto& inputs = input_->outputs();
        return {inputs.data(), inputs.size()};
    }
    quarisma::ArrayRef<Value*>       outputs() { return output_->inputs(); }
    quarisma::ArrayRef<const Value*> outputs() const
    {
        return static_cast<const Node*>(output_)->inputs();
    }
    graph_node_list       nodes() { return {input_, kNextDirection}; }
    const_graph_node_list nodes() const { return {input_, kNextDirection}; }
    Node*                 return_node() { return output_; }
    const Node*           return_node() const { return output_; }
    Node*                 param_node() { return input_; }
    const Node*           param_node() const { return input_; }
    /**
   * @warning NEVER pass raw pointer of smart pointer managed Graph to Python.
   * Check #87343 for details.
   */
    Graph*       owningGraph() { return graph_; }
    const Graph* owningGraph() const { return graph_; }
    Node*        owningNode() { return owning_node_; }
    const Node*  owningNode() const { return owning_node_; }
    ...
    void   removeAllInputs() { input_->removeAllOutputs(); }
    ...
    void eraseOutput(size_t i) { output_->removeInput(i); }
    void removeAllOutputs() { output_->removeAllInputs(); }

    void replaceOutput(size_t i, Value* n) { output_->replaceInput(i, n); }
    void permuteOutputs(const std::vector<size_t>& new_inputs)
    {
...
public:
    Graph(ScopePtr scope_root = quarisma::make_intrusive<Scope>())
    ...
    quarisma::ArrayRef<Value*>       inputs() { return block_->inputs(); }
    quarisma::ArrayRef<const Value*> inputs() const
    {
        const Block& block = *block_;
        return block.inputs();
    }
    quarisma::ArrayRef<Value*>       outputs() { return block_->outputs(); }
    quarisma::ArrayRef<const Value*> outputs() const
    {
        const Block& block = *block_;
        return block.outputs();
    }
    graph_node_list       nodes() { return block_->nodes(); }
    const_graph_node_list nodes() const
    {
        const Block& block = *block_;
        return block.nodes();
    }
    Node*       param_node() { return block_->param_node(); }
    const Node* param_node() const { return block_->param_node(); }
    Node*       return_node() { return block_->return_node(); }
    const Node* return_node() const { return block_->return_node(); }
    const std::unordered_map<std::string, Value*>& debugNames() const { return unique_names_; }

    TORCH_API void push_scope(const std::string& scope_name);
    TORCH_API void pop_scope();

    ScopePtr current_scope() { return current_scope_; }

    void set_op_version(std::optional<size_t> version) { op_version_ = version; }

    std::optional<size_t> get_op_version() { return op_version_; }

    void set_current_scope(ScopePtr scope) { current_scope_ = std::move(scope); }

    Value* addInput(const std::string& name = "") { return block_->addInput(name); }
    Value* insertInput(size_t i, const std::string& name = "")
    {
        return block_->insertInput(i, name);
    }
    void   eraseInput(size_t i) { block_->eraseInput(i); }
    size_t registerOutput(Value* n) { return block_->registerOutput(n); }
    void   eraseOutput(size_t i) { block_->eraseOutput(i); }
    ...
    TORCH_API Node* createTuple(
        quarisma::ArrayRef<Value*> values, TupleTypePtr optional_named_tuple = nullptr);
    ...
    TORCH_API Node* createIsInstance(Value* v, quarisma::ArrayRef<TypePtr> types);
    ...
    TORCH_API Value* insertFunctionCall(Function* callee, const MatchedSchema& matched);
    TORCH_API Value* insertMethodCall(std::string method_name, const MatchedSchema& matched);
    ...
    TORCH_API Node* createClone(
        Node* n, const std::function<Value*(Value*)>& value_map, bool copy_blocks = true);
    ...
    // initialized to insert quarisma the end of the top level block
    // can be changed with setInsertPoint()
    Node* insertNode(Node* n)
    {
        AT_ASSERT(
            insert_before_->inBlockList() && "insert point node is no longer in a block list");
    ...
    Node* insertPoint() { return insert_before_; }
    ...
    TORCH_API std::ostream& print(std::ostream& out, bool print_source_locations = true) const;
...
struct WithInsertPoint
{
    ...
    ~WithInsertPoint() { prev_->owningGraph()->setInsertPoint(prev_); }
...
{
    WithCurrentScope(Graph& g, ScopePtr scope) : graph_(&g), prev_scope_(g.current_scope())
    {
        g.set_current_scope(std::move(scope));
    }
    ~WithCurrentScope() { graph_->set_current_scope(prev_scope_); }
...
{
    AT_ASSERT(type);
    if (auto dyn = type->castRaw<quarisma::DynamicType>())
    {
        type = dyn->fallback();
    }
    type_ = std::move(type);
...
struct ProfileOp : public Node
{
    ...
    ProfileOp(Graph* graph, std::function<void(std::vector<IValue>&)> callback)
        : Node(graph, ::quarisma::prim::profile), callback_(std::move(callback))
    {
    }
    ...
    const std::function<void(std::vector<IValue>&)>& getCallback() const { return callback_; }
    ...
    void setHasSeenTensor(bool has_seen_tensor) { has_seen_tensor_ = has_seen_tensor; }
...
struct TORCH_API ProfileIValueOp : public Node
{
    ...
    ProfileIValueOp(Graph* graph, std::function<void(std::vector<IValue>&)> callback)
        : Node(graph, ::quarisma::prim::profile_ivalue), callback_(std::move(callback))
    {
    }
...
TORCH_API quarisma::ArrayRef<Value*> createTupleUnpack(Value* v);

/** Insert graph \p CALLEE into graph \p G using \p INPUTS as input values.
 * The insertion happens quarisma the current insertion point.
 * Optionally, one can also pass \p VALUE_MAP to get a map between \p CALLEE
 * values and their cloned copies in \p G.
 */
TORCH_API std::vector<Value*> insertGraph(Graph& g, Graph& callee, ArrayRef<Value*> inputs);
...
TORCH_API std::vector<Value*> inlineCallTo(
    Node* to_replace, GraphFunction* callee, bool use_graph = true);

TORCH_API std::vector<Value*> inlineCallTo(
    Node* to_replace, GraphFunction* callee, Graph* callee_graph);
...
TORCH_API std::vector<Node*> findAllNodes(quarisma::ArrayRef<Block*> a, Symbol kind, bool recurse);
...
struct OperatorMap
{
    ...
    OperatorMap() = default;
    explicit OperatorMap(std::initializer_list<std::pair<std::shared_ptr<Operator>, T>> init)
    {
        insert(init);
    }
    explicit OperatorMap(std::initializer_list<std::pair<const char*, T>> init) { insert(init); }

    void insert(const std::shared_ptr<Operator>& op, T val)
    {
        // Remove if exists before insert
        erase(op);
        map[Symbol::fromQualString(op->schema().name())].emplace_back(std::make_pair(op, val));
    }
    ...
    void insert(std::initializer_list<std::pair<std::shared_ptr<Operator>, T>> v)
    {
        for (auto& el : v)
        {
    ...
    bool contains(const Node* n) const { return n->maybeOperator() && contains(n->getOperator()); }
...
struct FunctionSchemaMap
{
    ...
    void insert(const FunctionSchema& schema, T val)
    {
        // Remove if exists before insert
        erase(schema);
        map[Symbol::fromQualString(schema.name())].emplace_back(std::make_pair(schema, val));
    }
