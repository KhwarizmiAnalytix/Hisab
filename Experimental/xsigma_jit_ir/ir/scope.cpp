...
Scope::Scope(ScopePtr parent, Symbol name) : parent_(std::move(parent)), name_(name) {}

ScopePtr Scope::push(Symbol name)
{
    return quarisma::make_intrusive<Scope>(intrusive_from_this(), name);
}

ScopePtr Scope::parent()
{
    QUARISMA_CHECK(parent_, "Cannot get parent from Scope with no parent");
    return parent_;
}
...
InlinedCallStack::InlinedCallStack(Function* fn, SourceRange source_range)
    : fn_(fn), fn_name_(fn_ ? fn_->name() : ""), source_range_(std::move(source_range))
{
}

InlinedCallStack::InlinedCallStack(
    Function* fn, SourceRange source_range, std::optional<ModuleInstanceInfo> module_instance_info)
    : fn_(fn),
...
InlinedCallStack::InlinedCallStack(
    InlinedCallStackPtr callee, Function* fn, SourceRange source_range)
...
std::vector<InlinedCallStackEntry> InlinedCallStack::vec()
{
    ...
    {
        r.emplace_back(
            (*current)->fn_, (*current)->source_range_, (*current)->module_instance_info_);
