...
public:
    TORCH_API explicit AliasDb(
        std::shared_ptr<Graph> graphi, bool isFrozen = false, bool descendFunctionCalls = false);
    TORCH_API ~AliasDb();
    ...
    TORCH_API bool writesToAlias(Node* n, const ValueAndMemoryLocationSet& vls) const;
