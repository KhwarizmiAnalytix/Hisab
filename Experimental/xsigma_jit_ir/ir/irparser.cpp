...
void parseIR(const std::string& str, torch::jit::Graph* graph, bool parse_tensor_constants)
{
    std::unordered_map<std::string, Value*> vmap;
...
VarWithType IRParser::parseVarWithType(bool allow_optional)
{
    ...
    if (L.nextIf(':'))
    {
        ...
        TORCH_INTERNAL_ASSERT(!type_alias.second, "Parsing IR with Alias Info not handled");
...
void IRParser::parseOperatorOutputs(std::vector<VarWithType>* outs)
{
    ...
    parseList(TK_NOTHING, ',', TK_NOTHING, [&] { outs->push_back(parseVarWithType(true)); });
    L.expect('=');
}
...
ParsedLiteral IRParser::parseScalarLiteral(Node* n)
{
    ...
    case '-':
        ...
        {
            throw(
                ErrorReport(token.range) << "Expected a number after '-' but got:" << token.text());
        }
        [[fallthrough]];
    case TK_NUMBER:
        str += L.cur().text();
        if (str.find('j') != std::string::npos)
        {
            ...
            {
                throw(ErrorReport(token.range) << "Number cannot be converted to double");
            }
        ...
        else
        {
            ...
            {
                throw(ErrorReport(token.range) << "Number cannot be converted to integer");
            }
            catch (const std::out_of_range& e)
            {
    ...
    case '<':
    {
        ...
        if (text.text() != "Tensor")
        {
            throw(ErrorReport(token.range) << "Could not parse literal" << token.text());
        }
    ...
    case '{':
    {
        ...
        if (L.cur().kind != TK_NUMBER)
        {
            ...
            throw(
                ErrorReport(token.range)
                << "Expected single-element tensor constant to contain a number" << token.text());
        }
...
void IRParser::parseAttr(Node* n)
{
    ...
    else if (L.cur().text() == "torch")
    {
        ...
        if (function == "Generator")
        {
            ...
            while (!L.nextIf(')'))
            {
                ...
                else if (arg == "seed")
                {
                    ...
                    {
                        throw(
                            ErrorReport(L.cur().range) << "Expected int literal for seed argument");
                    }
                    if (r.i < 0)
                    {
                        throw(ErrorReport(L.cur().range) << "Seed must be a non-negative integer");
                    }
                    seed = r.i;
                }
                else
        ...
        {
            throw(
                ErrorReport(L.cur().range) << "Expected one of the following torch functions:\n"
                                           << "- Generator\n"
                                           << "Got: " << function);
        }
    }
...
void IRParser::parseOperator(Block* b)
{
    ...
    Node*       n =
        g->create(Symbol::fromQualString(name), {}, outs.size())->setSourceRange(source_range);
            ...
            else
            {
                    ...
                    !v.type->isSubtypeOf(*schema_return_type))
                {
                    throw(
                        ...
                        << "Annotated type " << v.type->repr_str() << " does not match schema type "
...
void IRParser::parseList(int begin, int sep, int end, const std::function<void()>& callback)
{
    if (begin != TK_NOTHING)
    {
        L.expect(begin);
