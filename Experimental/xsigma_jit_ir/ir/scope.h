...
public:
    ...
    {
        return (class_type() == rhs.class_type()) && (instance_name() == rhs.instance_name());
    }
};
...
public:
    ...
    InlinedCallStack(InlinedCallStackPtr callee, Function* fn, SourceRange source_range);
    ...
    bool operator==(const InlinedCallStack& rhs) const
    {
        ...
        // should suffice.
        return (module_instance().has_value() == rhs.module_instance().has_value()) &&
               (module_instance().has_value() &&
                module_instance().value() == rhs.module_instance().value()) &&
               callee() == rhs.callee() && source_range() == rhs.source_range();
    }

    bool operator!=(const InlinedCallStack& rhs) const { return !(*this == rhs); }
};
