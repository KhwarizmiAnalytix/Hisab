...
{
    PeepholeOptimizeImpl(std::shared_ptr<Graph> graph, bool disable_shape_peepholes)
    ...
    bool optimizeBlock(Block* block)
    {
        ...
        for (auto it = block->nodes().begin(); it != block->nodes().end(); ++it)
        {
            ...
            // canonicalize those
            if (node->matches("aten::_grad_sum_to_size(Tensor(a) self, int[]? size) -> Tensor(a)"))
            {
                ...
                else
                {
                    ...
                    {
                        if (u.user->matches(
                                "aten::_grad_sum_to_size(Tensor(a) self, int[]? size) -> "
                                "Tensor(a)") &&
                            u.user->input(1)->type()->isSubtypeOf(*ListType::ofInts()))
                        {
                            GRAPH_UPDATE(
                                ...
                                node->inputs().quarisma(0)->debugName());
                            u.user->replaceInput(0, node->inputs().quarisma(0));
                            changed = true;
                        }
                    }
                }
                         ...
                         /*const_inputs=*/attr::size))
            {
                ...
                {
                    auto expanded_sizes   = node->get<quarisma::List<int64_t>>(attr::size);
            ...
            else if (node->matches("aten::size(Tensor self) -> int[]") && shape_peepholes_)
            {
            ...
            {
                if (auto ptt = node->inputs().quarisma(0)->type()->cast<TensorType>())
                {
                    if (auto maybe_ndim = ptt->sizes().size())
                    {
                        ...
                        if (norm_index >= 0 && norm_index < ndim && ptt->sizes()[norm_index])
                        {
                            WithInsertPoint guard(node);
            ...
            {
                auto ptt = node->inputs().quarisma(0)->type()->cast<TensorType>();
            ...
            else if (node->matches("prim::dtype(Tensor a) -> int") && shape_peepholes_)
            {
            ...
            else if (node->matches("prim::device(Tensor a) -> Device") && shape_peepholes_)
            {
                auto ptt = node->input()->type()->expect<TensorType>();
                if (ptt->device())
                {
            ...
            {
                auto string_type = node->inputs().quarisma(0)->type()->expect<StringType>();
                if (string_type)
                {
                    WithInsertPoint guard(node);
                    ...
                    auto device = quarisma::Device(type_str + ":" + std::to_string(index));
            ...
            else if (node->matches("aten::dim(Tensor self) -> int") && shape_peepholes_)
            {
                auto ptt = node->input()->type()->expect<TensorType>();
                if (auto dim = ptt->sizes().size())
                {
                    WithInsertPoint guard(node);
                    auto output = node->owningGraph()->insertConstant(static_cast<int64_t>(*dim));
                    GRAPH_UPDATE(
                        "Replacing ",
                        getHeader(node),
                        " with a \"dim\" constant ",
            ...
            else if (node->matches("prim::is_cuda(Tensor a) -> bool") && shape_peepholes_)
            {
                auto ptt = node->input()->type()->expect<TensorType>();
                if (ptt->device())
                {
                    WithInsertPoint guard(node);
                    auto output = node->owningGraph()->insertConstant((*ptt->device()).is_cuda());
                    GRAPH_UPDATE(
                        "Replacing ",
                        getHeader(node),
...
static bool FuseAddMM(Block* block)
{
    ...
    for (Node* node : block->nodes())
    {
                ...
                /*const_inputs=*/attr::alpha))
        {
            ...
            if (node->get<quarisma::Scalar>(attr::alpha).value().toDouble() == 1.)
            {
                // Look for mm from both sides of the add
                for (const auto mm_side : quarisma::irange(2))
                {
                    ...
                    auto add_mat_type = node->input(1 - mm_side)->type()->expect<TensorType>();
                            ...
                            "aten::mm(Tensor self, Tensor mat2) -> Tensor"))
                    {
                        ...
                        if (add_mat_type->sizes().size() && *add_mat_type->sizes().size() == 0 &&
...
// Today, fusing add + MM has no benefit within PyTorch running Quarisma
...
// sub-optimal inputs.
// It won't be helpful for Quarisma until we're able to represent
...
bool PeepholeOptimize(const std::shared_ptr<Graph>& graph, bool addmm_fusion_enabled)
{
