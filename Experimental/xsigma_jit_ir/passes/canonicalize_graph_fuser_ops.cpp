...
static std::optional<std::vector<ChunkOutput>> getChunkOutputs(Node* chunk)
{
    ...
    {
        if (list_use.user->matches("aten::select(t[] list, int idx) -> t", attr::idx) &&
            list_use.user->output()->type()->cast<TensorType>())
        {
            outputs.emplace_back(
                list_use.user->output(), list_use.user->get<int64_t>(attr::idx).value());
        }
        else if (list_use.user->kind() == prim::ListUnpack)
        {
            ...
            for (const auto i : quarisma::irange(unpack_outputs.size()))
            {
...
static void CanonicalizeOps(Block* block)
{
    for (auto it = block->nodes().begin(), end = block->nodes().end(); it != end; ++it)
    {
        for (auto sub : it->blocks())
            CanonicalizeOps(sub);
        if (it->matches("aten::add(Tensor self, Tensor other, *, Scalar alpha) -> Tensor") ||
            ...
            it->matches("aten::div(Tensor self, Tensor other) -> Tensor"))
        {
            ...
            if (auto other = it->get<quarisma::Tensor>(attr::other))
            {
                if (other->dim() == 0)
                {
