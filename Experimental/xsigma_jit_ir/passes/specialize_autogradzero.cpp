...
static bool hasGradSumToSizeUses(Value* v)
{
    return std::any_of(
        v->uses().begin(),
        v->uses().end(),
        [](const Use& use) { return use.user->kind() == aten::_grad_sum_to_size; });
}

static void insertProfileNodesForSpecializeAutogradZero(Block* block, ProfilingRecord* pr)
{
    for (auto it = block->nodes().begin(); it != block->nodes().end(); ++it)
    {
        ...
        for (const auto offset : quarisma::irange(n->inputs().size()))
        {
            ...
            if (i->type()->cast<OptionalType>() && hasGradSumToSizeUses(i))
            {
                ...
                std::function<void(Stack&)> optional_profiler = [pr, opt_pn](Stack& stack)
                {
...
private:
    ...
    void setStatesOnGraphInputs()
    {
        for (Value* input : graph_->inputs())
        {
            ...
            else if (tp->isSubtypeOf(*TensorType::get()) || tp->isSubtypeOf(*ListType::ofTensors()))
            {
                state_[input] = State::Nonzero;
            }
            else
    ...
    Node* guardSpecializations()
    {
        ...
        Value* bool_list =
            graph_->insertNode(graph_->createList(BoolType::get(), checks))->output();
        ...
        for (auto it = graph_->block()->nodes().reverse().begin(); *it != versioning_if;)
        {
            Node* n = *it;
            it++;
            n->destroy();
    ...
    void specializeGradSumToSize(Block* b)
    {
        for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it)
        {
            ...
            if (n->kind() == aten::_grad_sum_to_size)
            {
                ...
                {
                    profiled_none_flag = profiled_none_flag || profiled_none_.count(node->input(0));
