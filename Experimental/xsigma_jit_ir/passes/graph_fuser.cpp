...
bool isSimpleMap(Node* node)
{
    static OperatorSet simple_mappable{{
        ...
        "aten::rand_like(Tensor self, *, ScalarType? dtype=None, Layout? layout=None, Device? "
...
struct GraphFuser
{
    ...
    GraphFuser(AliasDb* aliasDb, Block* block, bool strict_fuser_check)
        : block_(block), aliasDb_(aliasDb), strict_fuser_check_(strict_fuser_check)
    {
    }
    ...
    void setInputArgLimit(size_t limit) { subgraph_arg_limit_ = limit; }

    value_list tensorInputs(Node* node)
    {
        return filter(
            node->inputs(), [](Value* v) { return v->type()->isSubtypeOf(*TensorType::get()); });
    }

    bool isFusable(Node* node) { return callback_(this, node); }
    ...
    bool isFusableCatNode(Node* node)
    {
        ...
        if ((tensors_node->inputs().size() + node->outputs().size()) > subgraph_arg_limit_)
        {
            return false;
        }
    ...
    bool calculatesSize(Node* node) { return node->matches("aten::size(Tensor self) -> int[]"); }
    ...
    void mergeFusionGroups(Node* consumer_group, Node* producer_group)
    {
        ...
        for (const auto i : quarisma::irange(inner_inputs.size()))
        {
        ...
        for (auto inner : producer_subgraph->nodes())
        {
            Node* outer = block_->owningGraph()->createClone(
                inner, [&](Value* k) -> Value* { return inner_to_outer.quarisma(k); });
            ...
            for (const auto i : quarisma::irange(inner_outputs.size()))
            {
        ...
        for (const auto i : quarisma::irange(subgraph_outputs.size()))
        {
            auto outer_output = inner_to_outer.quarisma(subgraph_outputs[i]);
    ...
    Node* mergeNodeIntoGroup(Node* group, Node* n)
    {
            ...
            if (inputs_map.count(input) == 0)
            {
                ...
                else
                {
                    ...
                    Node* in_const = subgraph.createClone(
                        input->node(),
                        [](Value*) -> Value* { throw std::runtime_error("unexpected input"); });
        ...
        Node* in_graph = subgraph.createClone(n, [&](Value* k) -> Value* { return inputs_map[k]; });
    ...
    std::optional<Node*> findFusedChunk(Node* group, Value* input)
    {
        ...
        auto&  subgraph       = getSubgraph(group);
        auto*  subgraph_input = subgraph.inputs().quarisma(input_index);
        // If subgraph_input is an input to prim::ConstantChunk, it will have 1 use
        auto* node = subgraph_input->uses().quarisma(0).user;
    ...
    void fuseChunkByReusingExistingFusedChunk(Node* group, Node* chunk, Node* existingFusedChunk)
    {
        ...
        for (size_t i = 0; i < chunk->outputs().size(); ++i)
        {
            ...
            auto* replacement_val = existingFusedChunk->outputs().quarisma(i);
            auto* val             = chunk->outputs().quarisma(i);
            ...
            auto group_input = subgraph.inputs().quarisma(input_index);
    ...
    value_list sortReverseTopological(ArrayRef<Value*> inputs)
    {
        ...
        std::sort(
            result.begin(),
            result.end(),
            [&](Value* a, Value* b) { return a->node()->isAfter(b->node()); });
        return result;
    }
    ...
    quarisma::ArrayRef<Value*> broadcast_tensors(value_list inputs)
    {
        AT_ASSERT(!inputs.empty());
        auto* g          = inputs[0]->owningGraph();
        auto* input_list = g->insertNode(g->createList(TensorType::get(), inputs))->output();
        ...
        auto* unpack_node =
            g->insertNode(g->create(prim::ListUnpack, {output_list}, inputs.size()));
    ...
    Node* promoteChunkToBroadcastingChunk(Node* chunk)
    {
        ...
        for (const auto i : quarisma::irange(nchunks))
        {
            auto* old_output = chunk->outputs().quarisma(i);
            auto* new_output = bchunk->outputs().quarisma(i);
    ...
    bool tryToMoveChunk(Node* consumer, Value* producer)
    {
        ...
        for (auto input : producer_for_chunk_node->inputs())
        {
            ...
            if (it != bchunk_inputs.end())
            {
                ...
                for (const auto chunki : quarisma::irange(nchunks))
                {
                    chunked_inputs.back().push_back(
                        bchunk->outputs().quarisma(nchunks * input_index + chunki));
                }
                continue;
            }
            ...
            for (Value* original_input : original_inputs)
            {
                if (original_input->type()->isSubtypeOf(*TensorType::get()))
                {
                    ...
                    chunked_op->addInput(
                        ...
                        chunked_inputs_it->quarisma(chunk_sel->offset() % nchunks));
                    ++chunked_inputs_it;
                }
                else
                {
        ...
        for ([[maybe_unused]] const auto i : quarisma::irange(nchunks))
        {
        ...
        if (!size_calc_uses.empty())
        {
            ...
            Value* output_size =
                tensor_sizes.size() == 1 ? tensor_sizes[0] : broadcastSizes(tensor_sizes, aliasDb_);
    ...
    void replaceIntermediateBroadcastingChunks()
    {
        for (auto it = block_->nodes().rbegin(); it != block_->nodes().rend();)
        {
            ...
            {
                auto* input = bchunk->inputs().quarisma(input_offset);

                Node* new_chunk = graph->insertNode(graph->create(prim::ConstantChunk, input, 0));
                new_chunk->copyAttributes(*bchunk);
                for (const auto output_offset : quarisma::irange(nchunks))
                {
                    auto new_output = new_chunk->addOutput();
                    auto old_output =
                        bchunk->outputs().quarisma(input_offset * nchunks + output_offset);
                    new_output->copyMetadata(old_output);
                    aliasDb_->replaceWithNewValue(old_output, new_output);
                    old_output->replaceAllUsesWith(new_output);
                }
        ...
        for (Node* n : subgraph->nodes())
        {
            ...
            if (n->kind() == prim::ConstantChunk)
            {
                Node* sizes_node = graph->insertNode(
                    graph->create(prim::ChunkSizes, shape_of.quarisma(n->input()), 2));
                ...
                Value* regular_size = sizes_node->outputs().quarisma(0);
                Value* last_size    = sizes_node->outputs().quarisma(1);
                ...
                shape_of.emplace(outputs.quarisma(outputs.size() - 1), last_size);
                continue;
            }
            auto tensor_inputs = filter(
                n->inputs(), [](Value* v) { return v->type()->isSubtypeOf(*TensorType::get()); });
            auto shapes = fmap(tensor_inputs, [&](Value* v) { return shape_of.quarisma(v); });
        ...
        for (int64_t i = static_cast<int64_t>(outputs.size()) - 1; i >= 0; --i)
        {
            ...
            if (usedOnlyInSize(output) && shape_of.count(soutput) > 0)
            {
                ...
                for (Use u : uses)
                {
                    ...
                    u.user->output()->replaceAllUsesWith(shape_of.quarisma(soutput));
                    u.user->destroy();
                }
    ...
    bool canFuseWithConcat(Value* producer, Node* before_check)
    {
        ...
        if (!aliasDb_->couldMoveBeforeTopologically(producer->node(), before_check))
        {
            return false;
        }
        ...
        if (producer->node()->kind() == prim::FusionGroup)
        {
            ...
            auto* node     = subgraph->outputs().quarisma(producer->offset())->node();
    ...
    Node* createFusedConcat(Node* node)
    {
        ...
        Node* fused_cat =
            graph->create(prim::FusedConcat, list_construct->inputs())->i_(attr::dim, dim);
    ...
    void fuseConcats()
    {
        for (auto it = block_->nodes().rbegin(); it != block_->nodes().rend(); ++it)
        {
            Node* cat = *it;
...
// location
        ...
        for (Node* node : block_->nodes())
        {
            ...
            {
                GraphFuser(aliasDb_, sub_block, callback_, kind_, strict_fuser_check_).run();
            }
        }
    }
};
