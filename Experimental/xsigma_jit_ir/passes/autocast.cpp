...
//
std::optional<AutocastScope> parseAutocast(Value* value, const AutocastContext& context)
{
    ...
    if (value->node()->kind() == prim::CreateObject)
    {
        ...
        for (Use use : value->uses())
        {
            ...
            if (use.user->kind() == prim::SetAttr && use.user->s(attr::name) == "_enabled")
            {
...
void handleBlock(Block* block, AutocastContext initial_state)
{
    ...
    auto current_state = [&]
    { return autocast_stack.empty() ? initial_state : autocast_stack.top().context; };
    ...
    for (Node* node : block->nodes())
    {
        ...
        case prim::CallFunction:
            ...
            if (current_state() == initial_state)
            {
                ...
                if (current_state())
                {
                    castTensorInputs(node, aten::_autocast_to_full_precision, current_state());
                }
                break;
            }
        ...
        case aten::rnn_relu_cell:
            ...
            {
                castTensorInputs(node, aten::_autocast_to_reduced_precision, current_state());
            }
            break;
