...
{
    return n->kind() == prim::TracedModuleForward || n->kind() == prim::TracedFork;
}

// This pass does several things:
// 1) It looks quarisma TracedModuleForward nodes and resolves the type of `self`
...
struct ConvertTracedAttrReferences
{
    void run(const std::shared_ptr<Graph>& graph)
    {
        ...
        // Step 2
        convertAttrReferencesToLocalGetAttrs(graph->block(), "__module", graph->inputs()[0]);
        // Step 3
...
private:
    ...
    void addSelfArgToTracedForwardNodes(Block* b)
    {
        for (Node* n : b->nodes())
        {
            ...
            {
                n->addInput(attr_qualname_to_value.quarisma(n->s(attr::scope)));
                n->blocks()[0]->addInput("self")->setType(
                    attr_qualname_to_value.quarisma(n->s(attr::scope))->type());
    ...
    std::vector<Value*> convertAttrReferencesToLocalGetAttrs(
        Block* b, const quarisma::QualifiedName& prefix, Value* self)
    {
        ...
        std::vector<Value*>&                unresolved_tracedattrs)
    {
        ...
        {
            auto attr_qualname = quarisma::QualifiedName(inp_node->s(attr::scope));
            if (prefix.isPrefixOf(attr_qualname))
            {
                ...
                for (const auto i : quarisma::irange(attr_atoms.size()))
                {
    ...
    void processNode(Node* n, Block* b)
    {
        for (size_t i = 0; i < n->inputs().size(); ++i)
        {
            ...
            if (inp_remapped->node()->owningBlock() != b)
            {
                ...
                Block* common_ancestor = n->findCommonAncestorBlockWith(inp_remapped->node());
                ...
                while (b_itr != common_ancestor)
                {
                    ...
                    Value* remapped = b_itr->owningNode()->addOutput()->setType(v_itr->type());
                    v_itr           = remapped;
            ...
            {
                {
                    ...
                    Node* return_tup = g->insertNode(g->createTuple(sub_block->outputs()));
...
std::string mangleMethodName(const std::string& method_name, const ClassTypePtr& mod_type)
{
...
void createMethodCalls(const std::shared_ptr<Graph>& g)
{
    for (auto node_itr = g->nodes().begin(); node_itr != g->nodes().end();)
    {
        ...
        else if (n->kind() == prim::TracedModuleForward)
        {
            ...
            auto qualname =
                quarisma::QualifiedName(callee_mod_type->name().value(), mangled_method_name);
...
void convertTracedForksToRealForks(const std::shared_ptr<Graph>& g)
{
    for (auto itr = g->nodes().begin(); itr != g->nodes().end();)
    {
        ...
        if (n->kind() == prim::TracedFork)
        {
            ...
            Node*           new_fork_node =
                g->insertNode(g->create(prim::fork, n->outputs().size()))->copyAttributes(*n);
            for (Value* i : n->inputs())
            {
