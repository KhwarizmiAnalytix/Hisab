...
public:
          ...
          preserveParameters_(preserveParameters)
    {
        ...
        {
            QUARISMA_CHECK(checkName(name), "Unknown name: " + name);
        }
    }

    void optimizeSubGraphs(
        std::shared_ptr<Graph>& graph, const std::function<void(std::shared_ptr<Graph>&)>& func)
    {
        func(graph);
    ...
    void run()
    {
        ...
        std::unordered_map<std::string, std::unordered_set<std::string>> interfacesToReassignType;
...
private:
    ...
    std::optional<ResolvedName> resolveName(const std::string& name)
    {
        ...
        if (cur_module.hasattr(attr_name) || cur_module.find_method(attr_name))
        {
            ...
            for (auto& attr_info : attr_infos)
            {
                ...
                userPreservedAttrs_[parent_module._ivalue()].insert(std::move(sub_name));
            }
    ...
    template <typename Iter>
    bool getModuleFromPath(Module& attrModule, const Iter& begin, const Iter& end)
    {
        for (Iter it = begin; it != end; ++it)
        {
    ...
    // attribute lives (in this example it is <SubModule2 quarisma ...>).
    //
    ...
    bool findConstantAttr(
        Value* input, std::string& name, Module& attrModule, std::shared_ptr<Graph>& graph)
    {
    ...
    void insertMutableAttr(const std::string& name, const IValue& attr, const ModulePtr& attrModule)
    {
        if (AliasDb::isMutableType(attr.type()))
        {
    ...
    {
        std::stack<Block*>       blocks({graph->block()});
        std::unique_ptr<AliasDb> aliasDb = std::make_unique<AliasDb>(graph, /* isFrozen */ true);
        while (!blocks.empty())
        {
            Block* block = blocks.top();
            blocks.pop();
            for (auto n : block->nodes())
            {
                ...
                if (n->kind() == prim::SetAttr || n->kind() == prim::GetAttr)
                {
                    ...
                    if (n->kind() == prim::SetAttr || aliasDb->hasOutputWriters(n))
                    {
                        GRAPH_DEBUG(
                            n->kind() == prim::GetAttr
                                ? "attribute: " + name + " in %" + n->output()->debugName() +
                                      " has inplace writer"
    ...
    IValue overrideGradient(IValue attr)
    {
        ...
        else if (attr.isTuple())
        {
            ...
            for (const auto idx : quarisma::irange(elems.size()))
            {
        ...
        {
            quarisma::List<IValue> elems = std::move(attr).toList();
            for (const auto i : quarisma::irange(elems.size()))
            {
    ...
    bool inlineInterfaceCall(Node* n, const IValue& attr)
    {
        ...
        for (auto use : n->output()->uses())
        {
            ...
            if (user_node->kind() == prim::CallMethod)
            {
                ...
                {
                    GRAPH_UPDATE(
                        "Inlining interface method '", function.name(), "' to ", *user_node);
    ...
    void inlineInterfaceCalls(
        ...
        std::unordered_map<std::string, std::unordered_set<std::string>>& interfacesToRetype)
    {
    ...
    void reassignInterfaceTypes(
        const std::unordered_map<std::string, std::unordered_set<std::string>>& interfacesToRetype)
    {
    ...
    {
        std::unordered_map<ModulePtr, std::unordered_map<std::string, Value*>> attrValues;
        ...
        while (!blocks.empty())
        {
            ...
            for (auto it = block->nodes().begin(); it != block->nodes().end();)
            {
                ...
                if (n->kind() == prim::GetAttr)
                {
                    ...
                    n->outputs().quarisma(0)->replaceAllUsesWith(paramConst);
                    n->removeAllInputs();
                }
                else if (n->kind() == prim::fork)
                {
    ...
    {
        QUARISMA_CHECK(n->kind() == prim::fork);
        auto attrModule = module_;
        ...
        if (node->kind() == prim::GetAttr && node->output()->type()->cast<ClassType>())
        {
    ...
    void handleSharedClassType(Module& module, std::shared_ptr<Graph>& graph)
    {
        ...
        for (const auto i : quarisma::irange(N))
        {
            ...
            {
                isMutable = it2 != preservedScalarAttrs_.end() && it2->second.count(name);
            }
            if (isMutable)
            {
    ...
    // IValues.
    std::unordered_map<ModulePtr, std::unordered_set<std::string>> preservedScalarAttrs_;
    ...
    std::unordered_map<ClassTypePtr, IValue::HashAliasedIValues> SharedTypeSubModules_;
    ...
    std::unordered_map<
        quarisma::intrusive_ptr<quarisma::ivalue::Object>,
        quarisma::intrusive_ptr<quarisma::ivalue::Object>>
        object_memo_;

    // Contains names of attributes that the user wants to preserve with
    // their owning modules.
    std::unordered_map<ModulePtr, std::unordered_set<std::string>> userPreservedAttrs_;
...
void freeze_module_inplace(
    ...
    bool                     preserveParameters)
{
    QUARISMA_CHECK(module != nullptr, "module cannot be nullptr");
    checkModuleDoesNotReturnSelf(*module);
    AttributePropagator attrPropagator(
