...
public:
    explicit DeadCodeEliminator(std::shared_ptr<Graph> graph, DCESideEffectPolicy sideEffectPolicy)
        : sideEffectPolicy_(sideEffectPolicy), graph_(std::move(graph)), useAliasDb_(true)
    {
    }
    DeadCodeEliminator(DCESideEffectPolicy sideEffectPolicy) : sideEffectPolicy_(sideEffectPolicy)
    {
    }
    ...
    void setDeleteCallback(
        std::function<void(const std::unordered_set<const Value*>&)> deleteCallback)
    {
...
private:
    void eliminateDeadForkInputs(Block* block, bool recurse)
    {
        for (Node* node : block->nodes())
        {
            ...
            {
                if (!g.inputs().quarisma(i)->hasUses())
                {
                    GRAPH_UPDATE(
                        "Dead ",
                        i,
    ...
    bool markReturnNode(Node* node)
    {
        ...
        {
            // If there's no outer node, we're looking quarisma the graph's top-level
        ...
        if (outerNode->kind() == prim::Loop || outerNode->kind() == quarisma::onnx::Loop)
        {
            // Special handling to deal with loop carried dependencies.
            auto loop = LoopView(outerNode);
            for (const auto i : quarisma::irange(loop.carriedOutputs().size()))
            {
                if (outerNode->kind() == quarisma::onnx::Loop)
                {
                    ...
                    insertLiveValue(loop.bodyCarriedOutputs().quarisma(i));
                    continue;
                }
                auto innerInput  = loop.bodyCarriedInputs().quarisma(i);
                auto innerOutput = loop.bodyCarriedOutputs().quarisma(i);
                auto outerOutput = loop.carriedOutputs().quarisma(i);
        ...
        else
        {
            ...
            for (const auto i : quarisma::irange(outerNode->outputs().size()))
            {
    ...
    bool markLoop(Node* node)
    {
        ...
        do
        {
            marked = mark(node->blocks().quarisma(0));
            anyMarked |= marked;
        } while (marked);
        return anyMarked;
    }
    ...
    bool mark(Block* block)
    {
        ...
        {
            if (sideEffectPolicy_ == DCESideEffectPolicy::DONT_DELETE_NODES_WITH_SIDE_EFFECTS &&
                hasSideEffects(node))
            {
                anyMarked |= mark(node);
            }
    ...
    bool markIfLive(Node* node)
    {
        ...
        if (useAliasDb_)
        {
            if (getOrCreateAliasDb()->writesToAlias(node, getLiveValuesAndMemoryLocations()))
            {
                return mark(node);
            }
        }
        ...
        for (auto it = nodes.begin(); it != nodes.end(); it++)
        {
            ...
            if (!(marked_.count(node) || node->hasUses()))
            {
                GRAPH_UPDATE(
                    ...
                    " which outputs ",
                    (!node->outputs().empty() ? node->outputs().quarisma(0)->debugName() : "n/a"),
                    " will be removed");
                it.destroyCurrent();
            }
        }
                                ...
                                std::any_of(
                                    ...
                                    [&](Block* b)
                                    {
                                        return std::any_of(
                                            b->nodes().begin(),
                                            b->nodes().end(),
                                            [&](Node* n) { return hasSideEffects(n); });
                                    }) ||
    ...
    void removeDeadBlockOutputs(Node* node)
    {
        ...
        {
            size_t i = i_1 - 1;
            if (!node->outputs().quarisma(i)->hasUses())
            {
                GRAPH_UPDATE(
                    "Dead ",
                    i,
                ...
                for (Block* b : node->blocks())
                {
                    GRAPH_UPDATE(
                        ...
                        b->outputs().quarisma(i)->debugName(),
                        " will be removed");
                    b->eraseOutput(i);
                }
            }
    ...
    void removeDeadLoopOutputs(Node* node)
    {
        ...
        for (size_t i_1 = node->outputs().size(); i_1 > 0; --i_1)
        {
            size_t i = i_1 - 1;
            if (!node->outputs().quarisma(i)->hasUses() &&
                !loop_body->inputs().quarisma(loop_body_offset + i)->hasUses())
            {
    ...
    void logDeadLoopOutputs(Node* node, size_t i, size_t loop_input_offset, size_t loop_body_offset)
    {
        ...
        GRAPH_UPDATE(
            "\tDead block output ",
            loop_body->outputs().quarisma(loop_body_offset + i)->debugName(),
            "quarisma offset ",
            loop_body_offset + i,
            " will be removed");
    }
    ...
    ValueAndMemoryLocationSet& getLiveValuesAndMemoryLocations()
    {
        ...
        {
            liveValuesAndMemoryLocations_ = std::make_unique<ValueAndMemoryLocationSet>(
    ...
    // we should have quarisma most 1 of these as a non-nullptr; they are lazily
    ...
    std::unique_ptr<ValueAndMemoryLocationSet> liveValuesAndMemoryLocations_ = nullptr;
