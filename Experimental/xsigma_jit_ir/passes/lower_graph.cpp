...
static std::pair<std::shared_ptr<Graph>, std::vector<Slot>> lower_graph(
    const ModulePtr& self, Graph& g_, size_t self_offset = 0)
{
    ...
    struct SlotHash
    {
        ...
        {
            auto obj_hash    = std::hash<quarisma::ivalue::Object*>{}(slot.obj.get());
            auto offset_hash = std::hash<size_t>{}(slot.offset);
            return quarisma::hash_combine(obj_hash, offset_hash);
        }
    };
    ...
    std::vector<Node*>  to_clean;  // nodes that should be dead quarisma the end
    ...
    auto getOrAddSlot = [&](const Slot& slot) -> Value*
    {
        ...
        if (it != slot_to_offset.end())
        {
            ...
            return g->inputs().quarisma(ivalues_start + it->second);
        }
    ...
    auto self_value = g->inputs().quarisma(self_offset);
...
static std::vector<IValue> loadTensors(const std::vector<Slot>& slots)
{
    ...
    for (const Slot& slot : slots)
    {
        ...
        else
        {
            ...
            QUARISMA_CHECK(
                (type ==
                 getCustomClass("__torch__.torch.classes.quantized.Conv2dPackedParamsBase")) ||
                    (type ==
                     getCustomClass("__torch__.torch.classes.quantized.Conv3dPackedParamsBase")) ||
                    (type ==
            ...
            result.emplace_back(script::Object(obj.toObject()).run_method("__getstate__"));
        }
    }
    return result;
}
...
std::pair<std::shared_ptr<Graph>, std::vector<IValue>> LowerGraph(
    Graph& graph, const ModulePtr& self)
{
