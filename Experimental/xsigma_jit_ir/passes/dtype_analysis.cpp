...
static bool canBeInferredWithMetaTensor(Node* n)
{
    ...
    bool args_valid = std::all_of(n->inputs().begin(), n->inputs().end(), MTensorNodeArgValid);
...
{
    explicit DtypePropagationPass(std::shared_ptr<Graph> graph) : graph_(std::move(graph))
    {
...
private:
    ...
    bool processNode(Node* n)
    {
        ...
        bool has_tensor_output = std::any_of(
            n->outputs().begin(),
            n->outputs().end(),
            [](Value* v) { return (bool)v->type()->cast<TensorType>(); });
        ...
        case prim::ListUnpack:
            TORCH_INTERNAL_ASSERT(
                false, "List Construct and Unpack is not supported in Dtype Propagation");
            break;
        default:
            if (n->kind().is_aten())
            {
            ...
            {
                TORCH_INTERNAL_ASSERT(
                    false, n->kind().toDisplayString(), "Op is not supported in Dtype Propagation");
            }
        }
        return false;
    }
    ...
    bool processIf(Node* node)
    {
        ...
        changed |= mergeTensorProperties(true_block->outputs(), false_block->outputs());
    ...
    void buildDtypeRuleRegistry()
    {
        ...
        dtype_prop_registry_->insert(*nn_ops_first_input_preserving(), setIfAllDtypeMatch);
        dtype_prop_registry_->insert(*ops_one_tensor_in_shape_transform(), setIfAllDtypeMatch);
    }
