...
void fuseReluWithPackedOps(std::shared_ptr<Graph>& graph)
{
    ...
    rewriter.RegisterRewritePattern(linear_prepack_run_relu, linear_prepack_run_relu_fused);
  ...
  graph(%input, %weight, %bias, %stride:int[], %padding:int[],
    ...
    rewriter.RegisterRewritePattern(conv2d_prepack_run_relu, conv2d_prepack_run_relu_fused);
  ...
  graph(%input, %weight, %bias, %stride:int[], %padding:int[],
    ...
    rewriter.RegisterRewritePattern(linear_prepack_run_relu_inplace, linear_prepack_run_relu_fused);
    rewriter.RegisterRewritePattern(conv2d_prepack_run_relu_inplace, conv2d_prepack_run_relu_fused);
...
void fuseHardtanhWithPackedOps(std::shared_ptr<Graph>& graph)
{
    ...
    rewriter.RegisterRewritePattern(linear_prepack_run_hardtanh, linear_prepack_run_hardtanh_fused);
    ...
    rewriter.RegisterRewritePattern(conv2d_prepack_run_hardtanh, conv2d_prepack_run_hardtanh_fused);
...
script::Module metalOptimizeForMobile(
    const script::Module& m, const std::vector<std::string>& preserved_methods)
{
