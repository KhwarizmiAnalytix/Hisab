    ...
    for (auto it = block->nodes().begin(), end = block->nodes().end(); it != end; ++it)
    {
        ...
        if (it->kind().is_caffe2())
        {
            ...
            for (const auto& arg : args)
            {
                ...
                if (type->kind() == TypeKind::OptionalType && origin_input->mustBeNone())
                {
                    continue;
                }
...
std::shared_ptr<Graph> ToONNX(
    std::shared_ptr<Graph>& graph, ::torch::onnx::OperatorExportTypes operator_export_type)
{
    ...
    try
    {
        BlockToONNX(graph->block(), new_graph->block(), operator_export_type, env, values_in_env);
    }
    catch (std::runtime_error& ex)
    {
        ONNX_LOG("ONNX graph being constructed during exception:\n", new_graph->toString());
        throw;
    }
...
void NodeToONNX(
    ...
    py::set&                           values_in_env)
{
    py::object onnx_utils = py::module::import("torch.onnx._internal.torchscript_exporter.utils");
    py::object onnx_globals =
        py::module::import("torch.onnx._internal.torchscript_exporter._globals");
    py::object onnx_registration =
        py::module::import("torch.onnx._internal.torchscript_exporter.registration");
        ...
        for (const auto i : quarisma::irange(num_old_outputs))
        {
            ...
            if (outputs[i])
            {
                ...
                else
                {
                    ...
                    // should be reliable.
                    MergeInferredTypeAndSetMap(outputs[i], old->type(), outputs[i]->type());
        ...
        [&](const std::string& op_name, Node* n, const py::object& raw_output)
    {
        ...
        catch (const std::exception& ex)
        {
            ...
            ss << "Error casting results of symbolic for " << op_name
               ...
               << py::str(py::type::handle_of(raw_output)) << "': " << py::str(raw_output);
            throw std::runtime_error(ss.str());
        }
    ...
    auto callPySymbolicMethod = [&](ConcretePythonOp* op)
    {
        ...
        py::object opset_version = onnx_globals.attr("GLOBALS").attr("export_onnx_opset_version");
        // NOTE(justinchuby): Call the internal registry to register the symbolic
        // method defined in the module.
        bool is_registered_op = onnx_registration.attr("registry")
