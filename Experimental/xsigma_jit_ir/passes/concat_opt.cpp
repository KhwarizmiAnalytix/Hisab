...
bool equal(quarisma::ArrayRef<Value*> list1, quarisma::ArrayRef<Value*> list2)
{
...
{
public:
    explicit ConcatCommonInputsEliminator(std::shared_ptr<Graph> graph) : graph_(std::move(graph))
    {
    }
...
private:
    ...
    void handleCat(Node* node)
    {
        ...
        auto curr_tensor_inputs = curr_all_inputs.slice(0, curr_all_inputs.size() - 1);
        ...
        auto curr_tensor_inputs_prefix = curr_tensor_inputs.slice(0, curr_tensor_inputs.size() - 1);
        for (const auto& prev : concated_outputs_)
        {
            auto prev_all_inputs    = prev->inputs();
            auto prev_tensor_inputs = prev_all_inputs.slice(0, prev_all_inputs.size() - 1);
            auto prev_dim           = prev_all_inputs.back();
            if (equal(curr_tensor_inputs_prefix, prev_tensor_inputs) && curr_dim == prev_dim)
            {
                ...
                auto new_concat = node->owningGraph()->create(prim::VarConcat, new_inputs);
        ...
        //    ...
        //        = %13 ... // Use %13
        auto curr_tensor_inputs_suffix = curr_tensor_inputs.slice(1, curr_tensor_inputs.size() - 1);
        for (const auto& prev : concated_outputs_)
        {
            auto prev_all_inputs    = prev->inputs();
            auto prev_tensor_inputs = prev_all_inputs.slice(0, prev_all_inputs.size() - 1);
            auto prev_dim           = prev_all_inputs.back();
            if (equal(curr_tensor_inputs_suffix, prev_tensor_inputs) && curr_dim == prev_dim)
            {
                if (!node->isDominatedBy(prev))
                {
...
{
public:
    explicit ConcatExpander(std::shared_ptr<Graph> graph) : graph_(std::move(graph)) {}
...
private:
    ...
    void expandCat(Node* node)
    {
        ...
        auto cat_out_empty = graph_->create(
            aten::empty, {cat_out_size_list->output(), none, none, none, none, none});
        ...
        for (auto cat_inp : cat_inp_list->inputs())
        {
            ...
            auto cat_inp_tensor_type = dynamic_cast<TensorType*>(cat_inp->type().get());
            ...
            auto slice = graph_->create(aten::slice, {cat_out_value, cat_dim, start, end, one});
            GRAPH_UPDATE("Inserting\n", *slice, "before\n", *node);
...
size_t determineUsageIdx(Value* value, Node* user)
{
    ...
    using quarisma::ssize;
    QUARISMA_CHECK(idx != ssize(user->inputs()));
    return idx;
}

std::vector<Value*> getConcatInputs(Node* concat)
{
    QUARISMA_CHECK(concat->kind() == aten::cat);
    auto* list           = concat->input(0);
    auto* list_construct = list->node();
    QUARISMA_CHECK(list_construct->kind() == prim::ListConstruct);
...
private:
    ...
    std::unordered_map<Node*, ListConstructInputs> getListConstructInputs()
    {
        ...
        for (const auto& combinable : combinable_concats_)
        {
            ...
            if (cur_list->empty())
            {
                ...
                cur_list->insert(cur_list->end(), starting_values.begin(), starting_values.end());
            }

            cur_list->insert(
                cur_list->begin(), inputs_to_add.begin(), inputs_to_add.begin() + combinable.idx);

            cur_list->insert(
                cur_list->end(), inputs_to_add.begin() + combinable.idx + 1, inputs_to_add.end());
        }
    ...
    {
        CombinableConcat(Node* a, Node* b, size_t i) : first_concat(a), second_concat(b), idx(i) {}
