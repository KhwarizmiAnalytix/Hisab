...
struct ShapeArg : public std::pair<std::optional<quarisma::ShapeSymbol>, std::optional<int64_t>>
{
    ...
    static ShapeArg unknownInteger() { return ShapeArg(); }
...
public:
    ...
    ShapeArguments(std::vector<ShapeArg> ss) : has_dim_(true), maybe_shape_symbols_(std::move(ss))
    {
    }
...
{
    return type->cast<ListType>() && type->cast<ListType>()->getElementType()->cast<IntType>();
}
...
{
    return type->cast<ListType>() && type->cast<ListType>()->getElementType()->cast<TensorType>();
}
...
struct SymbolicShapeOpAnalyzer
{
    ...
    {
        for (size_t op_in_index = 0; op_in_index < shape_compute_graph_->inputs().size();
             op_in_index++)
        {
    ...
    void substituteConstantInputs()
    {
        ...
        while (op_in_index < shape_compute_graph_->inputs().size())
        {
            ...
            TORCH_INTERNAL_ASSERT(
                !seen_tensor_list, "SSA doesn't handle case with multiple tensor lists")
            seen_tensor_list = true;

            uint64_t            li_length = inputs_.size() - (schema_->arguments().size() - 1);
            std::vector<Value*> li_inputs;

            TypePtr element_type = graph_in_var->type()->cast<ListType>()->getElementType();
    ...
    void substituteSymbolicProperties(std::unordered_map<Value*, int64_t>* symbolic_shape_values)
    {
        // clang-format off
        ...
        for (const auto index : quarisma::irange(shape_compute_graph_->inputs().size()))
        {
            ...
            for (const Use& use : shape_compute_graph_->inputs().quarisma(index)->uses())
            {
                ...
                case aten::__getitem__:
                {
                    ...
                    {
                        symbolic_shape_values->emplace(use.user->output(), shape_symbol.value());
                    }
                    else
                    {
    ...
    std::vector<quarisma::SymbolicShape> propagateShapesInGraph()
    {
        ...
        for (unsigned attempt_num = 0; made_change && attempt_num < MAX_ATTEMPTS; attempt_num++)
        {
...
public:
    ...
    SymbolicShapeOpAnalyzer(const FunctionSchema* schema, const std::shared_ptr<Graph>& graph)
        : schema_(schema)
    {
    ...
    std::shared_ptr<Graph> getShapeComputeGraph() { return shape_compute_graph_; }
};
...
std::vector<SSArgument> getNodeInputShapes(Node* n, const AliasDb& db)
{
    ...
    for (size_t node_index = 0; node_index < n->inputs().size(); ++node_index)
    {
        ...
        if (type->cast<ListType>() && type->cast<ListType>()->getElementType()->cast<IntType>())
        {
            ...
            if (input_src_node->kind() == aten::size && !db.hasWriters(n->input(node_index)))
            {
...
std::shared_ptr<Graph> PropagateShapesWithShapeFunction(Node* n, const AliasDb& db)
{
...
struct SymbolicShapeGraphAnalyzer
{
    ...
    {
        TORCH_INTERNAL_ASSERT(beg_->owningBlock() == end_->owningBlock() && end_->isAfter(beg_));
    }
    ...
    std::optional<ShapeComputeGraphMapping> run()
    {
        ...
        for (auto it = beg_->iterator(); it != end_->iterator(); it++)
        {
            ...
            if (curr->kind() == prim::ListConstruct)
            {
                ...
                if (!std::all_of(
                        uses.begin(),
                        uses.end(),
                        [](const Use& use) { return use.user->kind() == aten::cat; }))
                {
        ...
        for (size_t i = 0; i < stitched_shape_compute_graph->outputs().size(); ++i)
        {
            ...
            {
                graph_output_to_symbolic_shape_dim[output] = output_index_to_symbolic_shape_[i];
            }
        }
        ...
        const std::shared_ptr<Graph>& stitched_shape_compute_graph)
    {
        ...
        for (size_t i = 0; i < node_inputs.size(); ++i)
        {
            ...
            if (auto tt = node_input->type()->cast<TensorType>())
            {
                ...
                for (size_t j = 0; j < rank; ++j)
                {
                    ...
                    auto            index =
                        stitched_shape_compute_graph->insertConstant(static_cast<int64_t>(j));
                    auto li_index =
                        stitched_shape_compute_graph->insert(aten::__getitem__, {input, index});
        ...
        insertGraph(
            *stitched_shape_compute_graph, *partial_eval_graph, partial_eval_inputs, value_map);
        ...
        for (size_t i = 0; i < curr->outputs().size(); ++i)
        {
            ...
            auto symbolic_sizes = curr->output(i)->type()->expect<TensorType>()->symbolic_sizes();
...
void PropagateShapesOnBlock(Block* b, const AliasDb& db)
{
    for (Node* n : b->nodes())
    {
        ...
        else if (n->kind() == prim::TupleConstruct)
        {
            ...
            n->output()->setType(orig_type->createWithContained(std::move(new_types)));
        }
    }
}
}  // namespace
...
std::optional<ShapeComputeGraphMapping> PropagateShapesAndBuildLargeShapeComputeGraph(
    std::shared_ptr<Graph>& graph, Node* beg, Node* end)
{
    ...
    const FunctionSchema* schema, const std::vector<SSAInput>& inputs)
{
    ...
    if (bounded_graphs)
    {
        auto lower_bound     = SymbolicShapeOpAnalyzer(schema, bounded_graphs->lower_bound);
        auto lower_bound_res = lower_bound.run(ssa_args);
        auto upper_bound     = SymbolicShapeOpAnalyzer(schema, bounded_graphs->upper_bound);
