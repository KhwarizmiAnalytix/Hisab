...
bool FoldConvBatchNormHelper::tryExtractingConvBNParameters(
    Module& conv, Module& bn, ConvBNParameters& r)
{
    if (!hastensor(conv, "weight") || !conv.hasattr("bias") || !hastensor(bn, "running_mean") ||
...
void FoldConvBatchNormHelper::analyze(Module& module, const PatternInfo& pattern)
{
        ...
        for (auto& method : current.get_methods())
        {
            ...
            if (!conv_bn_paths_.count(g))
            {
                ...
                for (const Match& match : matches)
                {
                    ...
                    {
                        GRAPH_DEBUG(
                            "Conv and BN modules didn't have all required parameters or "
                            "attributes...");
                        continue;
                    }
            ...
            for (const auto& conv_bn : conv_bn_paths_.quarisma(g))
            {
                ...
                TORCH_INTERNAL_ASSERT(
                    tryExtractingConvBNParameters(conv_submodule, bn_submodule, params));
...
graph(%self, %input, %conv, %batchnorm):
    ...
    const std::vector<std::reference_wrapper<const PatternInfo>> patterns = {pattern2d, pattern3d};
