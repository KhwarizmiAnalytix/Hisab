...
bool supportedConvNode(Node* n)
{
    ...
    {
        auto transposed_conv = constant_as<bool>(n->namedInput("transposed")).value_or(true);
...
bool FoldFrozenConvBatchnorm(Block* b)
{
    ...
    for (Node* n : b->nodes())
    {
        ...
        if (n->kind() == aten::batch_norm && supportedConvNode(n->inputs().quarisma(0)->node()))
        {
            ...
            if (bn_rm_ivalue->type() == NoneType::get() && bn_rv_ivalue->type() == NoneType::get())
            {
                continue;
            }
...
bool FoldFrozenConvAddOrSub(Block* b)
{
    ...
    for (Node* n : b->nodes())
    {
        ...
        if (supportedAddOrSub(n) && supportedConvNode(n->inputs().quarisma(0)->node()))
        {
            ...
            add_or_sub->replaceInputWith(conv->output(), b->owningGraph()->insertConstant(bias));
            add_or_sub->replaceInput(1, b->owningGraph()->insertConstant(add_or_sub_tensor));
            ...
            fused_conv_b->setDebugName(
                conv_b_value->debugName() + "_fused_" + add_or_sub->kind().toUnqualString());
            conv->replaceInputWith(conv_b_value, fused_conv_b);
    ...
    for (Node* n : b->nodes())
    {
        ...
        if (supportedMulOrDiv(n) && supportedConvNode(n->inputs().quarisma(0)->node()))
        {
            ...
            fused_conv_weight->setDebugName(
                conv_weight_value->debugName() + "_fused_" + mul_or_div->kind().toUnqualString());
            ...
            if (conv->namedInput("bias")->type() != NoneType::get())
            {
                ...
                fused_conv_weight->setDebugName(
                    conv_b_value->debugName() + "_fused_" + mul_or_div->kind().toUnqualString());
