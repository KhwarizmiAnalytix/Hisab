...
Value* ConvertSliceToIndex(Node* slice, Value* size, Node* insertBefore)
{
    ...
    auto sliced_index_n = graph->create(
        aten::slice,
        {index,
         graph->insertConstant(scalar_to_tensor(quarisma::Scalar(0)), std::nullopt, slice->scope()),
         start,
         end,
         step});
...
{
    ConvertedIndex(Value* index, quarisma::Symbol orig_node_kind)
    ...
    const py::dict&           env)
{
    ...
    for (auto it = slice_and_select_nodes.rbegin(); it != slice_and_select_nodes.rend(); ++it)
    {
        ...
        int64_t dim = node->inputs().quarisma(1)->node()->t(attr::value).item().toLong();

        if (dim < 0)
        {
            // auto input_type = env.quarisma(orig_data)->type()->expect<TensorType>();
        ...
        while (cur_dim < dim)
        {
                ...
                index_put_node->input(1)->node()->input(cur_dim - dim_offset)->node()->mustBeNone())
            {
                ...
                auto            index_tensor =
                    graph->insert(aten::arange, {size}, {NamedValue("dtype", quarisma::kLong)});
...
std::vector<Value*> ReshapeToAdvancedIndexingFormat(
    Graph* graph, Node* index_put_node, std::unordered_map<int64_t, ConvertedIndex>& dim_index_map)
{
    ...
    for (const auto i : quarisma::irange(dim_index_map.size()))
    {
    ...
    for (const auto i : quarisma::irange(dim_index_map.size()))
    {
        ...
        default:
            QUARISMA_CHECK(false, "Unexpected node kind ", index_i->second.orig_node_kind);
        }
...
std::vector<Value*> ConvertIndexPutToONNX(
    Block* new_block, Node* old_node, py::dict& env, py::set& values_in_env)
{
    if (old_node->kind() != Symbol::fromQualString("onnx::Placeholder") ||
        (old_node->s(attr::name) != "index_put" && old_node->s(attr::name) != "index_put_"))
    {
        return {};
    }
    ...
    Node* last_node =
        !slice_and_select_nodes.empty() ? slice_and_select_nodes.back() : index_put_node;
    ...
    std::unordered_map<int64_t, ConvertedIndex> dim_index_map = MergeSliceAndSelectToIndices(
    ...
    const auto list_indices = old_graph->createList(OptionalType::ofTensor(), indices)
    ...
    // New onnx nodes are appended quarisma the end of new_block.
    for (auto at_n : subblock->nodes())
    {
        ...
        NodeToONNX(at_n, new_block, torch::onnx::OperatorExportTypes::ONNX, env, values_in_env);
    }
