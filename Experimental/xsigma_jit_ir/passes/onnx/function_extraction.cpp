...
static bool HasSameAttribute(const Node* a, const Node* b, const quarisma::Symbol& attr);
...
private:
    ...
    struct FunctionContext
    {
        ...
        std::unordered_map<Node*, std::unordered_map<Symbol, std::unordered_set<Node*>>>
            attribute_map_;
    ...
    static void HandleNoScopeNodes(scope_ctx_map& /*scope_ctxs*/, const node_list& no_scope_nlist);
...
FunctionExtractor::FunctionContext::FunctionContext(
    ScopePtr key, const scope_list& scopes, scope_ctx_map& scope_ctxs)
    : scope_key_(std::move(key))
{
    GRAPH_UPDATE("Process function context for scope ", scope_key_->name().toDisplayString());
    ...
    GRAPH_DEBUG("Initialized function context for scope ", scope_key_->name().toDisplayString());

    for (const auto& scope : scopes)
    {
        GRAPH_DEBUG("Process function context for scope ", scope->name().toDisplayString());
    ...
    GRAPH_DEBUG("Process function context complete. ", scope_key_->name().toDisplayString());
    DebugPrint();
}
...
void FunctionExtractor::FunctionContext::DebugPrint() const
{
    ...
    for (const auto& it : attribute_map_)
    {
        ...
        {
            GRAPH_DEBUG(
                "Attribute value difference for attribute ", attr_it.first.toDisplayString());
...
{
    auto v_it = scope_ctxs_[scope_key_]->env_to_subgraph_.find(ref_n->outputs().quarisma(0));
...
void FunctionExtractor::DebugPrintScopeContexts(const scope_ctx_map& scope_ctxs)
{
    ...
    {
        GRAPH_UPDATE(
            "Scope name: ", it.first->namesFromRoot(), " ", it.first->name().toDisplayString());
        GRAPH_UPDATE(
            "Children scopes: ",
            [&]()
            {
...
{
    if (!IsValidScope(parent) || !IsValidScope(child) || parent->getDepth() >= child->getDepth())
    {
        return false;
    }
    do
...
std::optional<ScopePtr> FunctionExtractor::FindCommonAncestor(ScopePtr a, ScopePtr b)
{
    ...
    auto diff = static_cast<int64_t>(a->getDepth()) - static_cast<int64_t>(b->getDepth());
    if (diff != 0)
    {
...
std::optional<ScopePtr> FunctionExtractor::FindCommonAncestor(const scope_list& scopes)
{
    if (scopes.empty())
    {
        return std::nullopt;
    ...
    std::optional<ScopePtr> common_ancestor = scopes.quarisma(0);
    for (const auto& scope : scopes)
    {
...
std::optional<ScopePtr> FunctionExtractor::InferScope(Node* n)
{
    ...
    if (!output_scopes.empty() &&
        std::all_of(
            ...
            { return IsValidScope(scope) && scope == output_scopes.quarisma(0); }))
    {
        return output_scopes.quarisma(0);
    }
    else if (
        !input_scopes.empty() &&
        std::all_of(
            ...
            { return IsValidScope(scope) && scope == input_scopes.quarisma(0); }))
    {
        return input_scopes.quarisma(0);
    }
    else
    {
        scope_list scopes;
        std::copy_if(
            input_scopes.begin(), input_scopes.end(), std::back_inserter(scopes), IsValidScope);
        std::copy_if(
            output_scopes.begin(), output_scopes.end(), std::back_inserter(scopes), IsValidScope);
        if (!scopes.empty())
        {
            auto common_ancestor = FindCommonAncestor(scopes);
            if (common_ancestor.has_value() && IsValidScope(common_ancestor.value()))
            {
                return common_ancestor;
            }
        }
...
std::shared_ptr<Graph> FunctionExtractor::ConstructFuncGraph(FunctionContext& func_ctx)
{
    ...
    for (auto* n : nlist)
    {
        ...
        for (const auto i : quarisma::irange(clone_n->outputs().size()))
        {
    ...
    const std::string&            func_name)
{
    ...
    for (const auto& n_it : func_ctx.attribute_map_)
    {
        ...
        for (const auto& attr_it : n_it.second)
        {
            ...
            auto attr_name = "inferred::" + std::string(n->kind().toUnqualString()) + '_' +
                             attr.toUnqualString();
            ...
            else
            {
                ...
                QUARISMA_CHECK(
                    ...
                    *unseen_attr_name,
                    " from module ",
                    scope->name(),
                    ". Please ensure module instances of the same class have the same set of "
    ...
    const std::string&            func_name)
{
    ...
    GRAPH_DEBUG("Create and insert local function for scope: ", func_scope->namesFromRoot());
    scope_ctx.PopulateInputsOutputs(param_names_);
    auto last_n = *scope_ctx.nlist_.rbegin();
    auto func_n = graph->create(
        Symbol::fromQualString(domain_name + "::" + func_name), scope_ctx.outputs_.size());
    func_n->copyMetadata(last_n);
    for (auto* v : scope_ctx.inputs_)
    {
        func_n->addInput(v);
    }
    for (const auto i : quarisma::irange(scope_ctx.outputs_.size()))
    {
    ...
    const std::shared_ptr<Graph>& graph)
{
    ...
    func_ctxs_.insert(
        std::make_pair(scope_key, new FunctionContext(scope_key, scope_list, scope_ctxs)));
    auto& func_ctx = *func_ctxs_[scope_key];

    const std::string module_class_name(ONNXScopeName::className(func_ctx.scope_key_));
    ...
    const auto func_name   = construct_unique_module_name(module_class_name.substr(pos + 1));
    ...
    for (const auto& it : func_ctx.scope_ctxs_)
    {
        ...
        std::unordered_set<Node*> old_nodes(scope_ctx.nlist_.begin(), scope_ctx.nlist_.end());
        ...
        for (auto& it : scope_ctxs)
        {
            ...
            ctx_nlist.erase(
                std::remove_if(
                    ctx_nlist.begin(),
                    ctx_nlist.end(),
                    [&old_nodes](Node* n) { return old_nodes.find(n) != old_nodes.end(); }),
                ctx_nlist.end());
...
bool FunctionExtractor::ScopeContext::IsIdenticalFuncion(const ScopeContext& other_ctx) const
{
    ...
    if (ONNXScopeName::className(this->scope_) != ONNXScopeName::className(other_ctx.scope_))
    {
        return false;
    }
...
void FunctionExtractor::HandleNoScopeNodes(
    scope_ctx_map& scope_ctxs, const node_list& no_scope_nlist)
{
...
std::tuple<FunctionExtractor::scope_ctx_map, node_list> FunctionExtractor::PartitionNodesByScope(
    Block* b)
{
    scope_ctx_map scope_ctxs = {};
    ...
    for (auto* n : b->nodes())
    {
        ...
        {
            auto [subblock_scope_ctxs, subblock_no_scope_nlist] = PartitionNodesByScope(sub_b);
...
std::unordered_map<ScopePtr, scope_list> FunctionExtractor::PartitionIdenticalScopes(
    FunctionExtractor::scope_ctx_map& scope_ctxs)
{
    std::unordered_map<ScopePtr, scope_list> identical_scope_map;
...
#undef COMP_ATTR
    ...
    default:
        TORCH_INTERNAL_ASSERT(
            false, "Unexpected attribute type ", static_cast<int>(a_kind), " from node ", *a);
        break;
    }
...
NodeAttrNameMap FunctionExtractor::run()
{
    ...
    {
        if (module_names_.find(ONNXScopeName::className(scope_key)) != module_names_.end())
        {
...
void ONNXTrackScopeAttributes(
    std::shared_ptr<Graph>& graph, std::map<std::string, IValue>& attributes)
{
