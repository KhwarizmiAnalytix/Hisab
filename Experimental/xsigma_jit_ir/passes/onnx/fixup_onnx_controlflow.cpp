...
Node* InsertCastForCond(Value* cond_val, Graph* graph, Node* consumer_node, int opset_version)
{
...
void ReplaceBlockOutputWithOptional(const OptionalTypePtr& opt_type, Block* block, size_t i)
{
...
void FixupONNXLoopNodeInputs(Node* node, int opset_version)
{
    ...
    {
        auto* cast_node =
            InsertCastForCond(next_cond_val, graph, sub_block->return_node(), opset_version);
        cast_node->copyMetadata(node);
    }
    ...
    for (const auto i : quarisma::irange(2, node->inputs().size()))
    {
        ...
        if (!input->type()->cast<OptionalType>() && sub_block_input_optional)
        {
            ...
            {
                auto [merged_type, inferred] =
                    MergeInferredType(sub_block_input_optional->getElementType(), input->type());
                if (inferred)
                {
...
static void InferShapeTypeForUninitializedOutput(
    Graph* graph, Block* block, Value* uninitialized_output, Value* other_output, int opset_version)
{
    ...
    else if (auto output_type = other_output->type()->cast<ListType>())
    {
        ...
        {
            TORCH_WARN("UninitializedOutput - Invalid elem Type of ListTensor found.");
            const_node->output()->setType(other_output->type());
        }
    }
...
static void ONNXFixupUninitializedOutput(Node* node, int opset_version)
{
    ...
    {
        Node* cast_node = InsertCastForCond(if_node->input(), graph, if_node, opset_version);
...
static void ONNXMergeIfBlockOutputShapes(Node* node)
{
    ...
    auto mergeListType = [&mergeTensorType](ListTypePtr a, ListTypePtr b) -> ListTypePtr
    {
        if (a && b)
        {
    ...
    for (const auto i : quarisma::irange(else_block->outputs().size()))
    {
        ...
        {
            ReplaceBlockOutputWithOptional(output_i->type()->cast<OptionalType>(), then_block, i);
        }

        if (else_none_type && !then_optional_type)
        {
            ReplaceBlockOutputWithOptional(output_i->type()->cast<OptionalType>(), else_block, i);
        }
    }
}
