...
static void ReplaceAddWithConcat(Block* b)
{
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        if (it->kind() == aten::add)
        {
            if (!it->input(0)->type()->cast<ListType>() || !it->input(1)->type()->cast<ListType>())
            {
                continue;
            }

            const auto& elem = it->input(0)->type()->castRaw<ListType>()->getElementType();
        ...
        if (it->kind() == prim::ListUnpack)
        {
            for (const auto i : quarisma::irange(it->outputs().size()))
            {
                    ...
                    it->input()->type()->castRaw<ListType>()->getElementType()->cast<IntType>())
                {
                    ...
                    gather_indices->t_(
                        attr::value, quarisma::scalar_to_tensor(quarisma::Scalar(int(i))));
