...
#else
#include <Quarisma/ops/full.h>
#include <Quarisma/ops/ones_like_native.h>
#endif
...
// `ret[i] = t1[t2[i]]` says "the output of t2 quarisma position i takes the value of
// the input tensor index contained in t1 quarisma position `t2[i]``".
static std::vector<int64_t> composeTransposes(
    const std::vector<int64_t>& t1, const std::vector<int64_t>& t2)
{
...
// Determine whether `from` can broadcast to `to`, and if so quarisma which
// position. `from` must be a suffix of `to`, except that any
// occurrences of 1 in `from` are treated as wildcards.
static std::optional<size_t> fusibleExpandTo(quarisma::IntArrayRef from, quarisma::IntArrayRef to)
{
    ...
    for (const auto i : quarisma::irange(from.size()))
    {
...
static void fuseBroadcast(Block* b)
{
    for (auto n : b->nodes())
    {
        ...
        for (size_t position : broadcast_positions)
        {
            ...
            if (!unexpanded_input->isCompleteTensor() || !n->output()->isCompleteTensor())
            {
                continue;
            }
...
static void fuseConsecutiveTransposes(Block* b)
{
    for (auto n : b->nodes())
    {
            ...
            n->owningBlock() == n->input()->node()->owningBlock())
        {
            ...
            n->is_(
                attr::perm,
                composeTransposes(origInput->node()->is(attr::perm), n->is(attr::perm)));
...
static void pushPackingPastRnn(Block* b)
{
    for (auto it = b->nodes().begin(); it != b->nodes().end(); ++it)
    {
        ...
        if (n->outputs().quarisma(0)->uses().size() != 1)
        {
        ...
        Node* next = rnn->outputs().quarisma(0)->uses().quarisma(0).user;
        if (next->kind() == onnx::Transpose)
        {
            next = next->outputs().quarisma(0)->uses().quarisma(0).user;
        ...
        Value* batch_sizes = n->outputs().quarisma(1);
        while (!batch_sizes->uses().empty())
        {
            Use   use_0 = batch_sizes->uses().quarisma(0);
            ...
            if (use_0.offset == 0 && user->kind() == onnx::Gather && user->i(attr::axis) == 0 &&
                user->inputs().quarisma(1)->node()->kind() == onnx::Constant &&
                user->inputs().quarisma(1)->node()->hasAttribute(attr::value))
            {
                const quarisma::Tensor& const_val_t =
                    user->inputs().quarisma(1)->node()->t(attr::value);
                if (const_val_t.item().toInt() != 0)
                {
                    // We'll likely produce an invalid graph if this happens.
                    break;
                }
                Value* rnn_input = rnn->inputs().quarisma(0);
                ...
                gather_indices->t_(attr::value, quarisma::native::ones_like(const_val_t));
            ...
            {
                batch_sizes->replaceFirstUseWith(n->inputs().quarisma(1));
            }
            else
            {
        ...
        next->outputs().quarisma(0)->replaceAllUsesWith(newPackPadded->outputs().quarisma(0));
        n->outputs().quarisma(1)->replaceAllUsesWith(newPackPadded->outputs().quarisma(1));

        // set up the new PackPadded's inputs
        newPackPadded->addInput(next->outputs().quarisma(0));
        newPackPadded->addInput(n->inputs().quarisma(1));
        ...
        TensorTypePtr oldType = rnn->inputs().quarisma(0)->type()->cast<TensorType>();
        if (oldType && oldType->isComplete())
        {
            ...
            TensorTypePtr newType =
                TensorType::createContiguous(*oldType->scalarType(), *oldType->device(), new_sizes);
            next->outputs().quarisma(0)->setType(newType);
        }
...
static void fixDefaultRNNState(Graph* graph, Node* n, int input_index, int opset_version)
{
    ...
    gather_indices->t_(attr::value, quarisma::scalar_to_tensor(quarisma::Scalar(1)));
    ...
    hidden_size->t_(
        ...
        quarisma::full(
            ...
            quarisma::kLong));  // quarisma::Scalar(n->i(attr::hidden_size)).toTensor());
    ...
    for (auto* input : n->inputs())
    {
        if (input->node()->kind() == prim::ListConstruct)
        {
            ...
            else
            {
                ...
                {
                    quarisma::Symbol seq_node_kind =
                        !lc_node->inputs().empty() ? onnx::SequenceConstruct : onnx::SequenceEmpty;
                    Node* seq_node =
                        block->owningGraph()->create(seq_node_kind, {lc_node->inputs()}, 1);
...
static void eraseListUnpack(Node* n, int opset_version)
{
    ...
    if (n->kind() == prim::ListUnpack)
    {
        ...
        for (size_t i = 0; i < n->outputs().size(); ++i)
        {
            ...
            seq_idx_n->t_(attr::value, quarisma::scalar_to_tensor(quarisma::Scalar(int64_t(i))));
...
static void fuseListConstructListUnpack(Block* b)
{
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        if (it->kind() == prim::ListUnpack && it->input()->node()->kind() == prim::ListConstruct)
        {
            for (const auto i : quarisma::irange(it->outputs().size()))
            {
                auto output = it->outputs().quarisma(i);
                output->replaceAllUsesWith(it->input()->node()->inputs().quarisma(i));
            }
        }
    }
}
...
static void eraseTupleConstruct(Block* block)
{
    ...
    // be quarisma the output of the blocks.
...
static void removeMaxPoolUnusedOutput(Block* b)
{
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        {
            if (n->outputs().size() == 2 && n->outputs().quarisma(1)->uses().empty())
            {
                it->eraseOutput(1);
            }
        }
    ...
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
        ...
        if (it->kind() == onnx::NegativeLogLikelihoodLoss)
        {
                ...
                prev->input(0)->node()->kind() == onnx::LogSoftmax)
            {
                ...
                {
                    origLogSoftmaxNode->replaceInput(0, transpose->inputs().quarisma(0));
                }
            }
            else if (
                ...
                prev->input(0)->node()->input(0)->node()->kind() == onnx::LogSoftmax)
            {
                ...
                origNllLossNode->replaceInput(1, reshape->inputs().quarisma(0));
            ...
            if (castNode != nullptr)
            {
                ...
                cast_node->addInput(origLogSoftmaxNode->inputs().quarisma(0));
                ...
                origLogSoftmaxNode->replaceInputWith(
                    origLogSoftmaxNode->inputs().quarisma(0), cast_node->output());
            }

            Node* softmaxCrossEntropyNode =
                b->owningGraph()->create(onnx::SoftmaxCrossEntropyLoss, it->outputs().size());
            ...
            softmaxCrossEntropyNode->addInput(origLogSoftmaxNode->inputs().quarisma(0));
            softmaxCrossEntropyNode->addInput(origNllLossNode->inputs().quarisma(1));
            softmaxCrossEntropyNode->copyMetadata(origNllLossNode);
            // optional weight input is provided
            if (origNllLossNode->inputs().size() == 3)
            {
                softmaxCrossEntropyNode->addInput(origNllLossNode->inputs().quarisma(2));
            }
...
static void removeSequenceSplitConcat(Block* b)
{
    for (auto it = b->nodes().begin(), end = b->nodes().end(); it != end; ++it)
    {
            ...
            it->input()->node()->kind() == onnx::SplitToSequence)
        {
            ...
            const auto split_keepdims =
                split_node->hasAttribute(attr::keepdims) ? split_node->i(attr::keepdims) : 1;
            const auto concat_axis = concat_node->i(attr::axis);
            const auto concat_new_axis =
                concat_node->hasAttribute(attr::new_axis) ? concat_node->i(attr::new_axis) : 0;
...
// optimization on Quarisma operators"?  If so, you should seriously consider
...
void PeepholeOptimizeONNX(std::shared_ptr<Graph>& graph, int opset_version, bool fixed_batch_size)
{
    ...
    EliminateDeadCode(
        graph->block(), true, DCESideEffectPolicy::ALLOW_DELETING_NODES_WITH_SIDE_EFFECTS);
