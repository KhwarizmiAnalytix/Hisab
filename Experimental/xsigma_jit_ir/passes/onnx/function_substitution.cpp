...
std::string TidyClassNameFromTorchScript(const std::optional<quarisma::QualifiedName>& class_name)
{
    if (!class_name)
    {
...
std::string GetCallNodeVariableName(const Node* call_node)
{
    TORCH_INTERNAL_ASSERT(
        call_node->kind() == prim::CallFunction || call_node->kind() == prim::CallMethod);
    ...
    while (parent_module_value)
    {
        ...
        if (parent_module_type &&
            parent_module_type->name() == "__torch__.torch.nn.modules.container.ModuleList")
        {
            auto parent_module_node = parent_module_value->node();
            module_name             = parent_module_node->s(attr::name) + "." + module_name;
            parent_module_value =
                !parent_module_node->inputs().empty() ? parent_module_node->input(0) : nullptr;
        }
        else
        {
            break;
...
void functionCallSubstitution(Block* block)
{
    ...
    for (auto it = block->nodes().begin(), end = block->nodes().end(); it != end;)
    {
        ...
        case prim::CallFunction:
        {
            ...
            if ((fun_type->function()->qualname().qualifiedName().find("torch.nn.functional") !=
                 std::string::npos) &&
                (fun_type->function()->qualname().qualifiedName().find("interpolate") !=
                 std::string::npos))
            {
        ...
        case prim::CallMethod:
        {
            ...
            if (auto class_type = cur->input(0)->type()->cast<ClassType>())
            {
                ...
                {
                    GRAPH_DEBUG(
                        "Inner graph for method call ", name, ": ", *graphFunction->graph());
                    WithCurrentScope inner_graph_scope_guard(*graphFunction->graph(), call_scope);
                    functionCallSubstitution(graphFunction->graph()->block());
                    inlineCallTo(cur, graphFunction, false);
                }
            }
        ...
        GRAPH_DEBUG(
            "Graph current scope after node process: ", graph->current_scope()->namesFromRoot());
    }
}
...
ScopePtr ONNXGraphTopLevelScope(Graph& graph)
{
    ...
    if (auto top_module_type = graph.inputs().quarisma(0)->type()->cast<ClassType>())
    {
