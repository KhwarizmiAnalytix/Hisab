...
{
    InplaceConverter(std::shared_ptr<Graph> graph, MutationRemover* mr, Module* model = nullptr)
...
Node* addDummyClone(Graph* graph, Value* orig_data, bool insertBefore, Node* referenceNode)
{
        ...
        orig_data->type()->kind() == TypeKind::BoolType)
    {
        ...
        if (orig_data->type()->kind() == TypeKind::IntType && insertBefore == false)
        {
            orig_data->setType(TensorType::fromNumberType(*IntType::get()));
        }
        else if (orig_data->type()->kind() == TypeKind::FloatType && insertBefore == false)
        {
            orig_data->setType(TensorType::fromNumberType(*FloatType::get()));
        }
        else if (orig_data->type()->kind() == TypeKind::BoolType && insertBefore == false)
        {
...
{
    TORCH_INTERNAL_ASSERT(node->kind() == aten::index_put || node->kind() == aten::index_put_);
...
std::pair<Value*, Value*> PrepareCopyForONNX(Node* node)
{
    ...
    auto dummy_list = graph->insertNode(graph->createList(OptionalType::ofTensor(), {}))->output();

    auto expanded_value = graph->insert(aten::expand_as, {node->input(1), node->input(0)});
    ...
    auto index_put = graph->insert(aten::index_put_, {node->input(0), dummy_list, expanded_value});
...
static std::pair<Value*, Value*> PrepareListPopForONNX(Node* n)
{
    ...
    Node* getitem_node = n->owningGraph()->create(aten::__getitem__, {n->inputs()});
        ...
        do
        {
            ...
            for (auto use : input->uses())
            {
                ...
                if (it != node->inputs().end())
                {
                    ...
                    Node* newNode = addDummyClone(b->owningGraph(), input, false, b->return_node());
...
std::deque<std::string> findSubModuleAttr(
    Value* input, std::string& name, Module& attrModule, const std::shared_ptr<Graph>& graph)
{
...
Value* findArgumentAsInputParam(
    const std::shared_ptr<Graph>& graph, std::string& name, IValue& attr)
{
...
void InplaceConverter::ValueTracker::recordSetValue(Value* old_v, Value* new_v)
{
    GRAPH_UPDATE(
        "Calling recordSetValue with old_v: ", old_v->debugName(), " new_v: ", new_v->debugName());
    ...
    bool registered = std::any_of(
        ...
        [&sorted_alias](Value* out)
        {
            return std::any_of(
                sorted_alias.begin(),
                sorted_alias.end(),
                [&out](Value* alias) { return alias == out; });
        });
...
Value* InplaceConverter::ValueTracker::findAliasForValueAtNode(Value* v, const Node* n) const
{
    ...
    TORCH_INTERNAL_ASSERT(
        ...
        *n,
        ...
        "Please report a bug to PyTorch, and/or try to avoid using in-place operators on this "
        "value.");
...
void InplaceConverter::convertInplaceOpsAndTrackAlias()
{
    ...
    GRAPH_UPDATE("Graph after convertInplaceOpsAndTrackAlias: ", graph_->toString());
