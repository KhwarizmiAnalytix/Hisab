...
std::optional<quarisma::SymbolicShape> ConstantValueMap::GetShape(const std::string& tensorName)
{
...
void ConstantValueMap::SetValue(const std::string& tensorName, const quarisma::Tensor& value)
{
...
std::optional<quarisma::Tensor> ConstantValueMap::GetValue(const std::string& tensorName)
{
    ...
    const std::string& value_name)
{
    if (ConstantValueMap::HasShape(value_name))
    {
        ...
        {
            auto shape_value = ConstantValueMap::GetCompleteShapeInto1DInt64Vector(shape_size);
            return shape_value;
        }
    }
    return std::nullopt;
...
std::optional<std::vector<int64_t>> ConstantValueMap::GetShapeInto1DInt64VectorWithOneUnknown(
    const std::string& value_name)
{
    if (ConstantValueMap::HasShape(value_name))
    {
        ...
        if (shape_size.isComplete())
        {
            shape_value = ConstantValueMap::GetCompleteShapeInto1DInt64Vector(shape_size);
            return shape_value;
        }
        else
        {
...
std::vector<int64_t> ConstantValueMap::GetValueInto1DInt64Vector(const std::string& value_name)
{
    ...
    auto                 value_int64_t = value.toType(quarisma::ScalarType::Long);
    ...
    for (const auto i : quarisma::irange(value_int64_t.size(0)))
    {
...
void ConstantValueMap::SetTypeReliable(const std::string& tensorName, bool value)
{
...
std::optional<bool> ConstantValueMap::GetTypeReliable(const std::string& tensorName)
{
...
void ConstantValueMap::SetUseInferredType(const std::string& tensorName, bool value)
{
...
std::optional<bool> ConstantValueMap::GetUseInferredType(const std::string& tensorName)
{
...
static void UpdateStrKey(Map& map, const std::string& old_key, const std::string& new_key)
{
...
void ConstantValueMap::PrintMaps()
{
    ...
    for (const auto& x : ConstantValueMap::getInstance().rankMap)
    {
        ...
        if (ConstantValueMap::getInstance().shapeMap.find(x.first) !=
            ConstantValueMap::getInstance().shapeMap.end())
        {
            auto shape_symbols = ConstantValueMap::getInstance().shapeMap[x.first].sizes();
