...
struct TreeToken
{
    ...
    explicit operator bool() { return is_root; }
    ...
    std::vector<Node*> removeTransposesAndGatherMatmuls()
    {
        ...
        while (!queue.empty())
        {
            ...
            else if (n->matches("aten::t(Tensor self) -> Tensor"))
            {
                ...
                AT_ASSERT(input_node->matches("aten::mm(Tensor self, Tensor mat2) -> Tensor"));
                // (AB)^T == B^TA^T
            ...
            {
                TORCH_INTERNAL_ASSERT(false, "Unsupported node found in a BatchMM tree!");
            }
        }
        return matmuls;
    }
...
static void BatchMMTreeReduce(Block* block, AliasDb& alias_db)
{
    ...
    for (auto node : block->nodes())
    {
        ...
        else if (node->matches("aten::t(Tensor self) -> Tensor") && !alias_db.hasWriters(node))
        {
    ...
    Value* value, AliasDb& alias_db)
{
    ...
    return std::make_pair(postprocess(std::move(lhses)), postprocess(std::move(rhses)));
}
