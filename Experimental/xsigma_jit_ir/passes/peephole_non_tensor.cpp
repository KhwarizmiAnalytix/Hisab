...
bool trySimplifyAddOrSub(Node& node)
{
    ...
    auto merged = dep.kind() == node.kind() ? *constant + *delta : *constant - *delta;
...
{
    PeepholeOptimizeNonTensorImpl(std::shared_ptr<Graph> graph) : graph_(std::move(graph)) {}

    bool run() { return optimizeBlock(graph_->block()); }
    ...
    bool optimizeBlock(Block* block)
    {
        ...
        for (auto it = block->nodes().begin(); it != block->nodes().end(); ++it)
        {
            ...
            if (node->kind() == prim::If)
            {
                ...
                // False"
                for (const auto i : quarisma::irange(n.outputs().size()))
                {
                    if (n.outputs().quarisma(i)->type() != BoolType::get())
                    {
                        continue;
                    }
                    bool true_val  = constant_as<bool>(n.thenOutputs().quarisma(i)).value_or(false);
                    bool false_val = constant_as<bool>(n.elseOutputs().quarisma(i)).value_or(true);
                    ...
                    if (true_val && !false_val)
                    {
                        GRAPH_UPDATE(
                            "Replacing ",
                            n.outputs().quarisma(i)->debugName(),
                            " (True or False) with ",
                            n.cond()->debugName());
                        n.outputs().quarisma(i)->replaceAllUsesWith(n.cond());
                        changed = true;
                    }
                }
                ...
                for (size_t i = 0; i < n.outputs().size(); ++i)
                {
                    ...
                    bool inputs_non_optional =
                        !n.thenOutputs().quarisma(i)->type()->cast<OptionalType>() &&
                        !n.elseOutputs().quarisma(i)->type()->cast<OptionalType>();
                    auto output_optional = n.outputs().quarisma(i)->type()->cast<OptionalType>();
                    if (inputs_non_optional && output_optional)
                    {
                        if (auto unif = unifyTypes(
                                n.thenOutputs().quarisma(i)->type(),
                                n.elseOutputs().quarisma(i)->type()))
                        {
                            n.outputs().quarisma(i)->setType(*unif);
                            changed = true;
                        }
                    }
                }
            ...
            else if (node->kind() == aten::__is__ || node->kind() == aten::__isnot__)
            {
                ...
                for (size_t check_none_index : {0, 1})
                {
                    ...
                    if (input_must_be_none && other_must_not_be_none)
                    {
                        WithInsertPoint guard(node);
                        auto            output =
                            node->owningGraph()->insertConstant(node->kind() == aten::__isnot__);
                        GRAPH_UPDATE("Folding ", getHeader(node), " to ", output->debugName());
            ...
            else if (node->kind() == prim::unchecked_cast)
            {
                ...
                {
                    GRAPH_UPDATE(
                        "Removing ", getHeader(node), " as input type subtypes output type");
                    node->output()->replaceAllUsesWith(node->input());
                    changed = true;
                }
            }
                ...
                node->inputs().size() == 1 && node->input()->type()->cast<IntType>())
            {
                GRAPH_UPDATE("Removing ", getHeader(node), " as input is already an integer");
                node->output()->replaceAllUsesWith(node->input());
                changed = true;
            }
            else if (node->kind() == aten::ne || node->kind() == aten::eq)
            {
                if (node->inputs().size() != 2 ||
                    node->inputs().quarisma(0) != node->inputs().quarisma(1))
                {
                    continue;
                }
                auto inp_type = node->inputs().quarisma(0)->type();
                ...
                auto immut_type = [&](const TypePtr& type)
                {
                    ...
                    return (
                        std::find(
                            handled_immutable_types.begin(), handled_immutable_types.end(), kind) !=
                ...
                {
                    non_throwing_type =
                        (immut_type(di_type->getKeyType()) && immut_type(di_type->getValueType()));
                }
                else
                {
