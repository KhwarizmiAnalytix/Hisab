...
struct QuantOpParams
{
    ...
    std::string back() const { return "AttributeDoesNotExist"; }
};

quarisma::QScheme toAffine(quarisma::QScheme qscheme)
{
    switch (qscheme)
    {
    case quarisma::kPerTensorAffine:
    case quarisma::kPerTensorSymmetric:
        return quarisma::kPerTensorAffine;
    case quarisma::kPerChannelAffine:
    case quarisma::kPerChannelSymmetric:
        return quarisma::kPerChannelAffine;
    default:
        return qscheme;
    }
}

bool isPerChannel(quarisma::QScheme qscheme)
{
    return qscheme == quarisma::kPerChannelAffine || qscheme == quarisma::kPerChannelSymmetric;
}
...
Node* insertChooseQParams(Graph* graph, Value* original_val)
{
    ...
    Node* choose_qparams = graph->create(
        quarisma::Symbol::aten(choose_qparams_func),
    ...
    choose_qparams->output(1)->setDebugName(original_val->debugName() + ".zero_point");
...
Node* insertDeQuant(Graph* graph, Value* quantized_val, Value* original_val, size_t id = 0)
{
    Node* dequant = graph->create(Symbol::aten("dequantize"), {quantized_val});
    dequant->output()
        ->setDebugName(original_val->debugName() + ".dequant." + std::to_string(id))
        ->setType(original_val->type());
    graph->insertNode(dequant);
    return dequant;
}

std::vector<Value*> insertDeQuantForAllUse(Graph* graph, Value* quantized_val, Value* original_val)
{
    ...
    for (const auto i : quarisma::irange(uses.size()))
    {
...
Node* insertScalarToTensor(Graph* graph, Value* scalar_value)
{
    ...
    Value*          float_scalar_type = graph->insertConstant(IValue(quarisma::kFloat));
    Value*          none              = graph->insertConstant(IValue());
    Node*           tensor_node       = graph->create(
        Symbol::aten("scalar_tensor"), {scalar_value, float_scalar_type, none, none, none});
...
DynamicQuantOps insertChooseQParamQuantDequant(
    Graph* graph, Value* original_val, Value* dtype, NodeKind quant_kind)
{
    ...
    Node* quant =
        insertQuant(graph, quant_inputs, quant_kind, original_val->debugName() + ".quant");
...
std::optional<std::string> findObserverName(Value* v)
{
    ...
    {
        auto module_instance = n->inputs().quarisma(0);
        if (module_instance->node()->kind() == prim::GetAttr &&
            module_instance->node()->s(attr::name).find("_observer_") != std::string::npos)
        {
...
quarisma::ScalarType getObserverDtype(Module& module, Value* v)
{
...
std::optional<std::string> getEmbeddingBagObsName(script::Module& module, Node* n)
{
    Value* v               = n->output();
...
bool isEmbeddingBagOp(Node* observer, std::optional<std::string> embedding_bag_name)
{
...
Node* insertQuantDequantNodes(
    Value* self, Node* observer, T& qparams, const std::string& quantize_func);
    ...
    const std::string&        quantize_func)
{
    ...
    Node* quant = insertQuant(
        g, inputs, quarisma::Symbol::aten(quantize_func), original_val->debugName() + ".quant");
...
Node* insertEmbeddingBagOps(Node* observer, const std::string& op_name)
{
    ...
    else
    {
        TORCH_INTERNAL_ASSERT(
            false,
            "Graph Mode Quantization currently supports 4-bit and 8-bit embedding bag "
            "quantization.");
    }
    ...
    std::vector<Value*> embedding_bag_inputs  = embedding_bag_float_op->inputs().vec();
    std::vector<Value*> qembedding_bag_inputs = {prepack->output()};
    const auto          inputs_size           = embedding_bag_float_op->inputs().size();
    const bool is_aten_op = embedding_bag_float_op->kind() == Symbol::aten("embedding_bag");
    ...
    {
        QUARISMA_CHECK(
            inputs_size == 9, "Expecting FP aten::embedding_bag operator to have 9 inputs");
    ...
    {
        QUARISMA_CHECK(inputs_size == 12, "Expecting F.embedding_bag operator to have 12 inputs");
        ...
        qembedding_bag_inputs.push_back(embedding_bag_inputs[6]);  // scale_grad_by_freq
        qembedding_bag_inputs.push_back(zero);                     // mode
        qembedding_bag_inputs.push_back(pruned_const);             // pruned_weights
        qembedding_bag_inputs.push_back(embedding_bag_inputs[9]);  // per_sample_weights
    }
    ...
    Node* qembedding_bag = g->create(Symbol::fromQualString(quant_fn), qembedding_bag_inputs);
    if (is_aten_op)
    {
        ...
        for (const auto i : quarisma::irange(1, embedding_bag_float_op->outputs().size()))
        {
            QUARISMA_CHECK(
    ...
    embedding_bag_float_op->output(0)->replaceAllUsesWith(qembedding_bag->output());
    ...
    QuantType quant_type = QuantType::STATIC)
{
    ...
    {
        if (getObserverDtype(module, observer_out) == quarisma::ScalarType::Half)
        {
        ...
        else if (!isWeight(module, observer_out))
        {
            ...
            if (observer_dtype == quarisma::ScalarType::QUInt8 ||
                observer_dtype == quarisma::ScalarType::QInt8)
            {
                ...
                dequant      = std::get<2>(insertChooseQParamQuantDequant(
                    g, observer_out, dtype, quarisma::Symbol::aten(quantize_func)));
            }
            else
            {
...
void ReplicateChooseQParamsQuantDequant(std::shared_ptr<Graph>& graph)
{
    ...
    Value*      dequant_val           = vmap.quarisma("a_dequant");
    Node*       pattern_dequant       = dequant_val->node();
    Value*      quant_val             = vmap.quarisma("a_quant");
    Node*       pattern_quant         = quant_val->node();
    Value*      choose_qparam_val     = vmap.quarisma("a_scale");
    Node*       pattern_choose_qparam = choose_qparam_val->node();

    std::vector<DynamicQuantOps> nodes_to_rewrite;
    for (const Match& match : matches)
    {
        Node* matched_dequantize    = match.nodes_map.quarisma(pattern_dequant);
        Node* matched_quantize      = match.nodes_map.quarisma(pattern_quant);
        Node* matched_choose_qparam = match.nodes_map.quarisma(pattern_choose_qparam);
...
void RemoveRedundantDequantize(std::shared_ptr<Graph>& graph)
{
    ...
    auto filter = [&](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
    {
        ...
        auto        dequant_node = match_vmap.quarisma(vmap.quarisma("a_dequant"))->node();
        Value*      dequant_out  = dequant_node->output();
...
public:
    ...
    void buildObserverSubgraph(const std::vector<Node*>& src, std::shared_ptr<Graph> dest);

private:
    // Clone node in the destination Graph g.
    void cloneNodeInGraph(
        Node* node, std::shared_ptr<Graph>& g, std::unordered_map<Value*, Value*>& remap_values);
};
...
private:
    ModuleMethodVector getInvokedMethods(Module& module, const std::string& method_name);
    ...
    std::tuple<quarisma::QScheme, QParamVector> getQSchemeAndQParamVector(
        script::Module& module, Node* n);
    QuantOpParams insertCalculateQParams(script::Module& module, Graph* g, Node* n);

    void checkQScheme(Graph* g, quarisma::QScheme qscheme)
    {
        if (qscheme_for_graph_.count(g))
        {
            ...
            /*
      QUARISMA_CHECK(
          qscheme_for_graph_.quarisma(g) == qscheme,
          "Quantizing same graph with different types of "
          "QSchemes is not supported.\n",
          " Expecting:",
          quarisma::toString(qscheme_for_graph_.quarisma(g)),
          " Got:",
          quarisma::toString(qscheme));
      */
        }
        else
        {
    ...
    void insertCalculateQParamsAndQuantizationOps(Module& module, Graph* g, Value* self);

    // Function that extracts and runs the weight observer in a separate
    // subgraph.
    void extractAndRunWeightObserver(Module& module, Value* self, Value* weight_value);
    ...
    std::unordered_map<Graph*, quarisma::QScheme> qscheme_for_graph_;
    ...
    std::unordered_map<Value*, std::unique_ptr<GraphFunction>> weight_to_graph_fn_;

    QuantType quant_type_ = QuantType::STATIC;
    bool      debug_      = false;
};

void InsertQuantDeQuantHelper::collectObserverNodesAndValueToQuantize(Module& module, Value* v)
{
    ...
    TORCH_INTERNAL_ASSERT(
        observer->kind() == prim::CallMethod && observer->s(attr::name) == "forward" &&
...
void InsertQuantDeQuantHelper::removeObserverNodes(Graph* g)
{
    ...
    {
        for (auto& n : nodes_to_destroy_.quarisma(g))
        {
            n->removeAllInputs();
        }
        for (auto& n : nodes_to_destroy_.quarisma(g))
        ...
        nodes_to_destroy_.quarisma(g).clear();
    }
}
...
void InsertQuantDeQuantHelper::cleanup(Module& module, Graph* g)
{
    ...
    {
        for (auto slot : removed_observer_slots_.quarisma(g))
        {
    ...
    {
        auto& observers = observer_modules_to_remove_.quarisma(g);
...
void SubGraphCloneHelper::cloneNodeInGraph(
    Node* node, std::shared_ptr<Graph>& g, std::unordered_map<Value*, Value*>& remap_old_to_new)
{
    auto* block    = g->block();
...
void SubGraphCloneHelper::buildObserverSubgraph(
    const std::vector<Node*>& weight_subgraph, std::shared_ptr<Graph> dest_graph)
{
    ...
    const std::vector<Node*>& nodes, const std::string& name)
{
    ...
    return std::make_unique<GraphFunction>(name, observer_subgraph, build_observer_graph);
}

void InsertQuantDeQuantHelper::findSubgraph(
    Value* self, Value* input_val, std::vector<Node*>& weight_subgraph)
{
...
void InsertQuantDeQuantHelper::extractAndRunWeightObserver(
    Module& module, Value* self, Value* weight_value)
{
...
void InsertQuantDeQuantHelper::quantizeTensors(Module& module, Graph* g, Value* self)
{
    if (!observer_nodes_for_graph_.count(g))
    {
        return;
    }
    for (auto* n : observer_nodes_for_graph_.quarisma(g))
    {
        ...
        for (auto& pr : qparam_map)
        {
            ...
            size_t      uid    = 0;
            auto qparam_name   = original_value->debugName() + name + "_" + std::to_string(uid++);
            while (module.hasattr(qparam_name))
            {
                qparam_name = original_value->debugName() + name + "_" + std::to_string(uid++);
            }
        ...
        insertQuantizationOps(module, self, n, isPerChannel(qscheme), qparam_names, quant_type_);
    }
}

std::tuple<quarisma::QScheme, QParamVector> InsertQuantDeQuantHelper::getQSchemeAndQParamVector(
    script::Module& module, Node* n)
{
    ...
    else if (scalar_type == quarisma::ScalarType::Half)
    {
        return std::make_tuple(qscheme, std::move(qparams));
    }
    ...
    const std::optional<std::tuple<quarisma::QScheme, QParamVector>>& qparams_opt)
{
    ...
    WithInsertPoint ins(
        quantized_input_node->isAfter(n) ? quantized_input_node->next() : n->next());
    ...
    else
    {
        ...
        Node* scale = insertQParam(
            graph, quantized_input, quarisma::Symbol::aten("q_scale"), FloatType::get(), "q_scale");
        Node* zero_point = insertQParam(
            graph,
            quantized_input,
            quarisma::Symbol::aten("q_zero_point"),
            IntType::get(),
            "q_zero_point");
        Node* dtype  = insertQParam(graph, quantized_input, prim::dtype, IntType::get(), "dtype");
        quant_inputs = {original_output, scale->output(), zero_point->output(), dtype->output()};
    }
    Node* quant =
        insertQuant(graph, quant_inputs, quant_kind, original_output->debugName() + ".quant");
...
void removeDequantizeFromInputs(const std::unordered_set<Value*>& inputs)
{
    ...
    for (auto* dequantized_val : inputs)
    {
        ...
        TORCH_INTERNAL_ASSERT(
            dequantized_val->uses().size() == 1, "Expect to have one dequantize node for each use");
...
void InsertQuantDeQuantHelper::propagateQuantizationOps(Block* block)
{
    for (Node* n : block->nodes())
    {
        ...
        if (isBinaryOpWithScalarInput(n))
        {
            ...
            // the IR:
            // https://github.com/pytorch/pytorch/blob/master/aten/src/Quarisma/native/quantized/cpu/BinaryOps.cpp#L64-L74
            if (debug_)
            {
                TORCH_WARN_ONCE(
...
void InsertQuantDeQuantHelper::runWeightObserver(Module& module, const std::string& method_name)
{
...
void InsertQuantDeQuantHelper::run(Module& module, const std::string& method_name)
{
    for (auto& invoked_methods : getInvokedMethods(module, method_name))
    {
        auto&       invoked_module      = std::get<0>(invoked_methods);
    ...
    {
        for (auto* n : observer_nodes_for_graph_.quarisma(graph.get()))
        {
            auto tp = getQSchemeAndQParamVector(module, n);
            ...
            if (!qparam_map.empty())
            {
                TORCH_INTERNAL_ASSERT(
                    ...
                    *n);
                auto qparam_name_map = qparam_name_map_for_node_.quarisma(n);
                for (auto& pr : qparam_map)
                {
                    const auto& name   = pr.first;
                    const auto& qparam = pr.second;
                    module._ivalue()->setAttr(qparam_name_map.quarisma(name), qparam);
                }
            }
        }
        return;
    ...
    for (const auto idx : quarisma::irange(1, method.num_inputs()))
    {
...
Node* insertQuantDequantNodes<QuantOpParams>(
    Value* self, Node* observer, QuantOpParams& qparams, const std::string& quantize_func)
{
    (void)self;
    Graph*              g            = observer->owningGraph();
    Value*              observer_out = observer->output();
...
void checkCalculateQParamsResultTypes(const Node* out)
{
    ...
    QUARISMA_CHECK(scale->type()->expect<TensorType>(), "Scale value should be of Tensor type.");
    QUARISMA_CHECK(zp->type()->expect<TensorType>(), "Scale value should be of float type.");
}

QuantOpParams InsertQuantDeQuantHelper::insertCalculateQParams(
    script::Module& module, Graph* g, Node* n)
{
    // TODO: refactor findObserverName to take Node* as input
    ...
    if (scalar_type == quarisma::ScalarType::Half)
    {
    ...
    MatchedSchema matched_schema           = matchSchema(
        calculate_qparams_schema, v->node()->sourceRange(), *g, {observer_module_value}, {});
    ...
    quant_op_params.qparams.push_back(scale_zp_node->output(1));  // zero_point Value*
    if (isPerChannel(qscheme))
    {
...
void InsertQuantDeQuantHelper::insertCalculateQParamsAndQuantizationOps(
    Module& module, Graph* graph, Value* self)
{
    if (!observer_nodes_for_graph_.count(graph))
    {
        return;
    }
    for (auto* n : observer_nodes_for_graph_.quarisma(graph))
    {
        ...
        insertQuantizationOps(
            module, self, n, isPerChannel(quant_op_params.qscheme), quant_op_params, quant_type_);
    }
}

void InsertQuantDeQuantHelper::runForOnDevicePTQ(Module& module, const std::string& method_name)
{
...
void ReplicateDeQuant(std::shared_ptr<Graph>& graph)
{
    ...
    while (!blocks_to_visit.empty())
    {
        ...
        for (Node* n : b->nodes())
        {
            if (n->kind() == Symbol::aten("dequantize") && n->output()->uses().size() > 1)
            {
