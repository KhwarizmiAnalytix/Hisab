...
using QConfigTypePtrMap = std::unordered_map<std::optional<QConfig>, TypePtr, OptionalQConfigHash>;
    ...
    const std::optional<QConfig>& parent_qconfig = std::nullopt)
{
    ...
    if (qconfig_dict.find(key) != qconfig_dict.end())
    {
        ...
        qconfig = qconfig_dict.quarisma(key);
    }
    else
    {
...
public:
   ...
   */
    Module clone(
        const Module& module, const ModuleQConfigMap& module_qconfig_map, bool inplace = false)
    {
        std::unordered_map<TypePtr, QConfigTypePtrMap> type_remap;
        IValue::HashIdentityIValueMap                  memo;
        return clone_impl(module, module_qconfig_map, type_remap, inplace, std::move(memo));
    }
...
private:
    Module clone_impl(
        ...
        IValue::HashIdentityIValueMap                   memo)
    {
        auto qconfig = module_qconfig_map.quarisma(module._ivalue());
        ...
        bool type_already_cloned =
            ...
            type_remap.quarisma(type).find(qconfig) != type_remap.quarisma(type).end();
        Module r;
        if (type_already_cloned)
        {
            ...
            Module new_module(
                ...
                type_remap.quarisma(type).quarisma(qconfig)->cast<ClassType>());
            r = new_module;
        }
        else
        {
        ...
        for (const auto i : quarisma::irange(N))
        {
    ...
    void remapTypes(
        ...
        Module&                                                        target,
        const ModuleQConfigMap&                                        module_qconfig_map,
        const std::function<TypePtr(TypePtr, std::optional<QConfig>)>& type_remap_fn)
    {
        ...
        for (Node* node : block->nodes())
        {
            ...
            if (node->kind() == prim::CallMethod || node->kind() == prim::GetAttr)
            {
                ...
                {
                    auto qconfig = module_qconfig_map.quarisma(child_opt->_ivalue());
            ...
            {
                remapTypes(sub_block, self, source, target, module_qconfig_map, type_remap_fn);
            }
            for (Symbol name : node->attributeNames())
            {
                ...
                {
                    remapTypes(
                        node->g(name).get(), source, target, module_qconfig_map, type_remap_fn);
                }
                else if (node->kindOf(name) == AttributeKind::gs)
                {
                    for (const auto& g : node->gs(name))
                    {
                        remapTypes(g.get(), source, target, module_qconfig_map, type_remap_fn);
                    }
                }
            }
        }
    ...
    {
        remapTypes(
            graph->block(), graph->inputs()[0], source, target, module_qconfig_map, type_remap_fn);
    }
    ...
    {
        auto type_remap_fn = [&](TypePtr type_ptr, const std::optional<QConfig>& qconfig)
        {
            if (type_remap.find(type_ptr) != type_remap.end())
            {
                const auto& qconfig_map = type_remap.quarisma(type_ptr);
                if (qconfig_map.find(qconfig) != qconfig_map.end())
                {
                    return qconfig_map.quarisma(qconfig);
                }
            }
            return type_ptr;
        };
...
{
public:
    explicit InsertObserversHelper(const ModuleQConfigMap& map, QuantType quant_type)
   ...
   * since we need to insert these values quarisma call site.
   ...
   */
    std::tuple<OptionalModuleVector, OptionalModuleVector, std::vector<size_t>> insertObservers(
        Module&                    module,
    ...
    void setInsertResetObserverMethod(
        bool insert_reset_observer_method, const std::string& method_name)
    {
...
private:
    ...
    // (per delay rules) instead. The observers are inserted quarisma a later stage
    ...
    bool isObserved(Value* v, const std::unordered_set<Value*>& block_observed_values)
    {
    ...
    void insertObserverResetMinMax(
        Module& module, const NameModuleVector& observer_name_and_modules);
    ...
    // are observed
    bool shouldObserve(
        Node* n, const std::unordered_set<Value*>& block_observed_values, QuantType quant_type)
    {
    ...
    void addValuesToDelayObservation(const Module& module, const std::string& method_name);
    ...
    bool insertResetObserverMethod() { return insert_reset_observer_method_; }
...
graph(%self, %a, %b):
    ...
    const std::vector<std::reference_wrapper<const PatternInfo>> delay_patterns = {
        nn_linear_f_relu,   nn_linear_nn_relu,   nn_linear_aten_relu,   nn_linear_aten_relu_,
        aten_linear_f_relu, aten_linear_nn_relu, aten_linear_aten_relu, aten_linear_aten_relu_,

        nn_conv1d_f_relu,   nn_conv1d_nn_relu,   nn_conv1d_aten_relu,   nn_conv1d_aten_relu_,
        nn_conv2d_f_relu,   nn_conv2d_nn_relu,   nn_conv2d_aten_relu,   nn_conv2d_aten_relu_,
        nn_conv3d_f_relu,   nn_conv3d_nn_relu,   nn_conv3d_aten_relu,   nn_conv3d_aten_relu_,

        add_nn_relu,        add_f_relu,          inplace_add_nn_relu,   inplace_add_f_relu,
        add_aten_relu,      add_aten_relu_,      inplace_add_aten_relu, inplace_add_aten_relu_,

        nn_bn2d_nn_relu,    nn_bn2d_f_relu,      nn_bn2d_aten_relu,     nn_bn2d_aten_relu_,
        nn_bn3d_nn_relu,    nn_bn3d_f_relu,      nn_bn3d_aten_relu,     nn_bn3d_aten_relu_,

        mul_nn_relu,        mul_f_relu,          inplace_mul_nn_relu,   inplace_mul_f_relu,
        mul_aten_relu,      mul_aten_relu_,      inplace_mul_aten_relu, inplace_mul_aten_relu_,
    };
    ...
    Module& module, const NameModuleVector& observer_name_and_modules)
{
    ...
    if (!reset_min_max_opt.has_value())
    {
        ...
        const auto method_name =
            quarisma::QualifiedName(*(module.type()->name()), reset_observer_method_name_);
        auto reset_observer_fn = module._ivalue()->compilation_unit()->create_function(
            method_name, std::move(reset_observer_graph));
        auto self_arg   = quarisma::Argument("self", module.type());
        auto output_arg = quarisma::Argument("none", output_node->output()->type());
        auto schema     = quarisma::FunctionSchema(
    ...
    auto   reset_min_max_graph = module.get_method(reset_observer_method_name_).graph();
    ...
    for (const auto& pair : observer_name_and_modules)
    {
        ...
        Value*        observer_value      = reset_min_max_graph->insertGetAttr(self, observer_name);
        MatchedSchema reset_minmax_schema = matchSchema(
            ...
            *reset_min_max_graph,
            {observer_value},
            {});
        reset_min_max_graph->insertMethodCall("reset_min_max_vals", reset_minmax_schema);
    }
}

void InsertObserversHelper::delayObservingValuesInPattern(Graph& graph, const PatternInfo& pattern)
{
    ...
    for (const auto& match : matches)
    {
        ...
        auto first_output  = match.values_map.quarisma(vmap.quarisma("first_output"));
        auto second_output = match.values_map.quarisma(vmap.quarisma("second_output"));
        GRAPH_DEBUG(
...
void InsertObserversHelper::addValuesToDelayObservation(
    const Module& module, const std::string& method_name)
{
    Method method = module.get_method(method_name);
    auto   graph  = method.graph();
...
void InsertObserversHelper::fillBoundaryValueMap(Module& module, const std::string& method_name)
{
...
void InsertObserversHelper::preprocess(Module& module, const std::string& method_name)
{
...
void InsertObserversHelper::analyze(Module& module, const std::string& method_name)
{
    for (auto& invoked_method : getInvokedMethods(module, method_name))
    {
        auto&       invoked_module      = std::get<0>(invoked_method);
...
bool InsertObserversHelper::valueNeedsToBeQuantized(Value* v, const QConfig& qconfig)
{
    ...
    // For dynamic quantization we only insert observers quarisma the input
    ...
    if (quant_type_ == QuantType::DYNAMIC)
    {
        ...
        if (scalar_type == quarisma::ScalarType::Half && !isWeight(v))
        {
            return false;
        }
    }
...
{
    std::vector<std::unordered_map<Value*, Module>::iterator> values_to_be_removed;
    for (auto it = observer_for_value_.begin(); it != observer_for_value_.end(); it++)
    {
...
void InsertObserversHelper::fillValueObserverMap(Module& module, const std::string& method_name)
{
    Method method = module.get_method(method_name);
    auto   graph  = method.graph();
    ...
    std::stack<Block*> blocks_to_visit;
    auto               qconfig_opt = module_qconfig_map_.quarisma(module._ivalue());
    if (!qconfig_opt)
    {
        return;
    }
...
std::optional<Module> InsertObserversHelper::getObserverFor(Value* v)
{
    ...
    {
        auto observer = observer_for_value_.quarisma(v);
    ...
    {
        for (Value* next : boundary_value_map_.quarisma(v))
        {
    ...
    GRAPH_DEBUG("Observer module config for ", v->debugName(), ":", result.has_value());
    return result;
}
    ...
    std::unordered_set<Value*>&         block_observed_values)
{
    ...
    {
        to_observe = delay_observation_map_.quarisma(v);
    }
    ...
    bool                        is_user_defined_function)
{
    ...
    // we have to insert the observers quarisma call site because
    ...
    if (visited)
    {
        ...
        for (const auto& observer_attrs : block_observer_map_.quarisma(block))
        {
        ...
        for (Node* n : b->nodes())
        {
            ...
            for (Value* v : n->outputs())
            {
                ...
                if (!inputs_outputs.count(v) && !isObserved(v, block_observed_values))
                {
                    ...
                    if (observer_opt && shouldObserve(n, block_observed_values, quant_type_))
                    {
                        recordObserved(v, *observer_opt, values_to_observe, block_observed_values);
                    }
                }
    ...
    Value* output, std::unordered_set<Value*>& block_observed_values)
{
    if (pass_through_value_map_.count(output))
    {
        ...
        for (Value* v : pass_through_value_map_.quarisma(output))
        {
    ...
    QuantType          quant_type)
{
    ...
    helper.insertObservers(cloned_module, observer_method_name, /* is_entry_point */ true);
