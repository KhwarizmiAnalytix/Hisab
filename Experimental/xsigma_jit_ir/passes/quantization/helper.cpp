...
// for example: aten/src/Quarisma/native/quantized/cpu/qsigmoid.cpp
static std::tuple<quarisma::QScheme, QParamVector> _per_tensor_asym_qparam = std::make_tuple(
    quarisma::kPerTensorAffine,
    QParamVector(
        {std::make_pair(".scale", IValue(_asym_scale)),
         std::make_pair(".zero_point", IValue(_asym_zero_point)),
         std::make_pair(".scalar_type", IValue(quarisma::kQUInt8))}));

// quantization parameters for ops with range -1 to 1
// for example: aten/src/Quarisma/native/quantized/cpu/qtanh.cpp
static std::tuple<quarisma::QScheme, QParamVector> _per_tensor_sym_qparam = std::make_tuple(
    quarisma::kPerTensorAffine,
    QParamVector(
...
static std::vector<std::string> _tensor_info_funcs = {"size", "len", "dim", "numel"};
...
// if the second input is a Scalar, we'll only look quarisma the first input to decide
...
bool matchAtenFuncToUse(const Use& use, const std::string& func_name, std::optional<int> n)
{
    Node* node = use.user;
...
bool matchCallFuncToUse(const Use& use, const std::string& func_name, std::optional<int> n)
{
    Node* node = use.user;
    return node->kind() == prim::CallFunction && getFuncName(node->inputs()[0]) == func_name &&
           (!n.has_value() || static_cast<size_t>(n.value()) == use.offset);
}
...
static bool matchArgPattern(
    Value* v, const AtenFuncArgs& aten_func_args, const CallFuncArgs& call_func_args)
{
...
{
    bool result = matchArgPattern(
        v, AtenFuncArgs({{"embedding_bag", 2}, {"embedding_bag", 6}}), CallFuncArgs({}));
    return result;
}
...
std::optional<Use> getClampScalarInputUse(Value* v)
{
    for (const auto& use : v->uses())
    {
        ...
        {
            if (matchAtenFuncToUse(use, aten_func, 1) || matchAtenFuncToUse(use, aten_func, 2))
            {
                return use;
            }
        }
    ...
    Module& module, const std::string& orig_method_name, const std::string& new_method_name)
{
    ...
    const auto this_method_name = quarisma::QualifiedName(*module.type()->name(), new_method_name);
    auto       copied =
...
std::vector<Value*> getPassThroughInputs(Value* v)
{
    ...
    else if (n->kind() == prim::ListConstruct && v->type()->isSubtypeOf(*ListType::ofTensors()))
    {
        std::vector<Value*> inputs;
        for (auto* v : n->inputs())
        {
...
static std::vector<NodeKind> toAtenSymbol(const std::vector<std::string>& func_names)
{
    ...
    std::transform(func_names.begin(), func_names.end(), std::back_inserter(symbols), Symbol::aten);
    return symbols;
}

static bool isAtenFunc(Node* n, const std::vector<NodeKind>& aten_funcs)
{
    return std::find(aten_funcs.begin(), aten_funcs.end(), n->kind()) != aten_funcs.end();
}
...
static bool isFunctionNode(
    Node* n, const std::vector<std::string>& call_funcs, const std::vector<std::string>& aten_funcs)
{
    ...
    if (n->kind() == prim::CallFunction)
    {
        ...
        is_func_node |=
            std::find(call_funcs.begin(), call_funcs.end(), func_name) != call_funcs.end();
    }
    return is_func_node;
}
...
{
    return isAtenFunc(n, _single_input_general_value_aten_funcs) || isBinaryOpWithScalarInput(n);
}
...
bool isSingleInputGeneralAtenFunction(Node* n)
{
    ...
    return isSingleInputGeneralValueAtenFunction(n) || isSingleInputGeneralShapeAtenFunction(n) ||
...
std::optional<std::tuple<quarisma::QScheme, QParamVector>> getFixedQParams(Node* n)
{
    ...
    {
        return _fixed_qparams_map.quarisma(n->kind());
    }
    return std::nullopt;
}
...
{
    return n->kind() == prim::CallFunction && !isSingleInputGeneralCallFunction(n) &&
...
{
    return isFunctionNode(
        n, _static_weight_only_quant_call_funcs, _static_weight_only_quant_aten_funcs);
}
...
bool nodeQuantizable(Node* n, QuantType quant_type)
{
    ...
    return isFunctionNode(
        n,
        /* call_funcs = */
        is_dynamic ? _dynamic_quantizable_call_funcs : _static_quantizable_call_funcs,
        /* aten_funcs = */
        is_dynamic ? _dynamic_quantizable_aten_funcs : _static_quantizable_aten_funcs);
}
...
std::shared_ptr<Graph> getCallFunctionGraph(Node* n)
{
    ...
    QUARISMA_CHECK(graphFunc, "Quantization only works for graph function");
    ...
    int                                            value)
{
    ...
    auto        v          = toIValue(match_vmap.quarisma(vmap.quarisma(vname)));
...
{
    const auto& match_vmap = match.values_map;
    Value*      v          = match_vmap.quarisma(vmap.quarisma(vname));
...
{
    return is_module(match, vmap, "relu", "__torch__.torch.nn.modules.activation.ReLU");
}

bool is_linear_module(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "linear", "__torch__.torch.nn.modules.linear.Linear");
}

bool is_conv1d_module(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "conv", "__torch__.torch.nn.modules.conv.Conv1d");
}

bool is_conv2d_module(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "conv", "__torch__.torch.nn.modules.conv.Conv2d");
}

bool is_conv3d_module(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "conv", "__torch__.torch.nn.modules.conv.Conv3d");
}

bool is_conv_transpose1d_module(
    const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "conv", "__torch__.torch.nn.modules.conv.ConvTranspose1d");
}

bool is_conv_transpose2d_module(
    const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "conv", "__torch__.torch.nn.modules.conv.ConvTranspose2d");
}

bool is_batchnorm2d_module(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    bool regnorm =
        is_module(match, vmap, "batchnorm", "__torch__.torch.nn.modules.batchnorm.BatchNorm2d");
    bool naivenorm = is_module(
        match, vmap, "batchnorm", "__torch__.mobile_cv.arch.layers.batch_norm.NaiveSyncBatchNorm");
    return (regnorm || naivenorm);
}

bool is_batchnorm3d_module(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    return is_module(match, vmap, "batchnorm", "__torch__.torch.nn.modules.batchnorm.BatchNorm3d");
}
