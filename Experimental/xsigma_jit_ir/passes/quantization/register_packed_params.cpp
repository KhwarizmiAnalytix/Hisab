    ...
    const std::string&          attr_prefix)
{
    ...
    std::string attr_name_base = attr_prefix + "_" + method_name + "_ondevice_ptq_packed_weight_";
    ...
    while (!blocks_to_visit.empty())
    {
        ...
        for (Node* n : b->nodes())
        {
            if (is_packed_param(n))
            {
                ...
                Node* set_attr =
                    graph->createSetAttr(graph->inputs()[0], attr_name, packed_param_value);
                ...
                // or SetAttr
                Value* packed_param_attr = graph->insertGetAttr(graph->inputs()[0], attr_name)
