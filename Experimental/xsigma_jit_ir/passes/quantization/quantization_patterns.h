...
std::string getExtraArgList(std::vector<std::string> extra_args)
{
    return std::accumulate(
        ...
        extra_args.end(),
        std::string(),
        [](const std::string& acc, const std::string& arg) { return acc + ", " + arg; });
}
...
std::string getQuantizeForScalar(const std::string& value)
{
    ...
    for ([[maybe_unused]] const auto i : quarisma::irange(3))
    {
    ...
    quantize_pattern +=
        ...
        getExtraArgList({value + "_scale", value + "_zero_point", value + "_dtype"}) + ")";
    ...
    const std::string& op_name, const std::vector<std::string>& extra_op_args)
{
    ...
    std::string op_replacement    = getAtenOpPattern(graph_header, op_name, extra_op_args);
    ...
    const std::string& op_name, const std::vector<std::string>& extra_op_args)
{
    ...
    op_pattern +=
        op_name + "(" + "%a_dequant" + getExtraArgList(std::move(scalar_extra_args)) + ")";
    // IR pattern common to all ops that inherit qparam from input
    op_pattern += R"(
          %r_quant = aten::quantize_per_tensor(%r, %r_scale, %r_zero_point, %r_dtype)
          return (%r_quant) )";

    std::string aten_op_pattern = getAtenOpPattern(graph_header, op_name, extra_op_args);
...
QuantFusionInfo getFixedQParamOpFusionInfo(
    const std::string& op_name, const std::vector<std::string>& extra_op_args, bool is_symmetric)
{
    const auto& extra_op_arg_list = getExtraArgList(extra_op_args);
    std::string graph_header      = "graph(%a_quant" + extra_op_arg_list + "):";
    std::string op_pattern        = graph_header;
    ...
    std::string sym_fixed_qparam_op_suffix = R"(
          ...
          %r_quant = aten::quantize_per_tensor(%r, %r_scale, %r_zero_point, %r_dtype)
          return (%r_quant) )";
    op_pattern += is_symmetric ? sym_fixed_qparam_op_suffix : asym_fixed_qparam_op_suffix;
...
bool input_b_is_scalar(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    ...
    auto        b_scalar   = match_vmap.quarisma(vmap.quarisma("b_scalar"));
    ...
    const std::vector<std::string>& q_extra_args)
{
    ...
    std::string aten_op_pattern =
        "graph(%a_quant" + fp_extra_arg_list + ", %r_scale, %r_zero_point, %r_dtype):" + R"(
          %r_quant = )" +
        q_op_name + "(%a_quant" + q_extra_arg_list + ", %r_scale, %r_zero_point)" + R"(
          return (%r_quant) )";
...
graph(%a_quant, %b_quant, %scale, %zero_point, %dtype):
    ...
    auto mul_scalar = getBinaryOpScalarFusionInfo(
        "aten::mul", {"%b_scalar"}, "quantized::mul_scalar", {"%b_scalar"}, {input_b_is_scalar});
...
graph(%a_quant, %alpha, %scale, %input_scale, %r_scale, %r_zero_point, %r_dtype):
    ...
    auto adaptive_avg_pool1d =
        getInputTensorQParamOpFusionInfo("aten::adaptive_avg_pool1d", {"%output_size"});

    auto adaptive_avg_pool2d =
        getInputTensorQParamOpFusionInfo("aten::adaptive_avg_pool2d", {"%output_size"});

    auto adaptive_avg_pool3d =
        getInputTensorQParamOpFusionInfo("aten::adaptive_avg_pool3d", {"%output_size"});
    ...
    auto upsample_nearest1d =
        getInputTensorQParamOpFusionInfo("aten::upsample_nearest1d", {"%output_size", "%scales"});
    ...
    auto upsample_nearest3d = getInputTensorQParamOpFusionInfo(
        "aten::upsample_nearest3d", {"%output_size", "%scale_d", "%scale_h", "%scale_w"});
    ...
    auto upsample_bilinear2d = getInputTensorQParamOpFusionInfo(
        "aten::upsample_bilinear2d", {"%output_size", "%align_corners", "%scale_h", "%scale_w"});
    ...
    auto leaky_relu = getInputTensorQParamOpFusionInfo("aten::leaky_relu", {"%negative_slope"});

    auto leaky_relu_ = getInputTensorQParamOpFusionInfo("aten::leaky_relu_", {"%negative_slope"});

    // Ops with fixed quantization parameters
    auto hardsigmoid = getFixedQParamOpFusionInfo("aten::hardsigmoid", {}, false);

    auto hardsigmoid_ = getFixedQParamOpFusionInfo("aten::hardsigmoid_", {}, false);
    ...
    auto hardswish =
        getObservedQParamOpFusionInfo("aten::hardswish", "quantized::hardswish", {}, {});

    auto hardswish_ =
        getObservedQParamOpFusionInfo("aten::hardswish_", "quantized::hardswish", {}, {});
    ...
    return {
        ...
        {"quantized::add_relu", std::move(add_relu), quantized_add_relu, {aten_add_alpha_is_one}},
        {"quantized::add_relu",
        ...
        {"quantized::add_scalar_relu_out",
         ...
         quantized_add_scalar_relu_out_replacement},
        {"quantized::add", std::move(add), quantized_add, {aten_add_alpha_is_one}},
        {"quantized::add",
         std::move(inplace_add),
         std::move(quantized_add),
         {aten_add_alpha_is_one}},
        {"quantized::cat", std::move(cat), std::move(quantized_cat)},
        {"quantized::batch_norm", std::move(batch_norm), std::move(quantized_batch_norm)},
        {"quantized::batch_norm_relu", std::move(batch_norm_relu), quantized_batch_norm_relu},
        ...
        {"quantized::mul_relu", std::move(inplace_mul_inplace_relu), std::move(quantized_mul_relu)},
...
inline std::vector<QuantFusionInfo> dynamic_quantized_linear_pattern_and_replacements()
{
