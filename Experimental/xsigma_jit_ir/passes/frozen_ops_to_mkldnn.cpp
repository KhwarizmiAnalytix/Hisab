...
#include <Quarisma/native/ConvUtils.h>
#include <algorithm>
#include <memory>
...
#include <Quarisma/native/mkldnn/Utils.h>
#include <Quarisma/native/mkldnn/MKLDNNCommon.h>
#include <ideep.hpp>
#endif
...
quarisma::AliasAnalysisKind aliasAnalysisFromSchema()
{
...
void merge_sets(
    std::unordered_map<Value*, ValueSetPtr>& alias_mapping, Value* existing, Value* new_v)
{
...
void InplaceMKLDNNSubgraph(const std::shared_ptr<Graph>& graph)
{
    ...
    for (auto& set : alias_mapping)
    {
        ...
        {
            auto k = v->node()->kind();
            if (k == prim::Constant || k == prim::ConstantMKLDNNTensor || k == prim::Param)
            {
    ...
    auto add_to_inplace_set = [&](Node* node)
    {
        ...
        auto output_liveness_end = set_liveness[alias_mapping[node->outputs().quarisma(0)]];
        merge_sets(alias_mapping, node->inputs().quarisma(0), node->output());
    ...
    for (Node* node : graph->nodes())
    {
        ...
        if (k == aten::relu || k == aten::sigmoid || k == aten::dropout ||
            ...
            k == prim::MKLDNNHardTanh || k == aten::tanh || k == prim::MKLDNNClamp ||
            k == Symbol::prim("MKLDNNScalarMul") || k == Symbol::prim("MKLDNNLayerNorm"))
        {
            if (set_liveness[alias_mapping[node->inputs().quarisma(0)]]->isAfter(node))
            {
                continue;
            }
        ...
        else if (k == aten::mul || k == aten::add)
        {
            ...
            {
                TORCH_INTERNAL_ASSERT(node->inputs().quarisma(i)->type()->cast<TensorType>());
                if (!set_liveness[alias_mapping[node->inputs().quarisma(i)]]->isAfter(node))
                {
            ...
            if (reusable_value_index == 1)
            {
                node->insertInput(0, node->inputs().quarisma(1));
                node->removeInput(2);
            }
            add_to_inplace_set(node);
        }
    ...
    bool                                                                        inplace = false)
{
    return [aten_op, inplace](Stack& stack)
    {
        ...
        auto a_it                   = quarisma::native::itensor_from_mkldnn(a);
        auto mkldnn_raw_data        = a_it.get_data_handle();
        auto a_options_with_strided = a.options().layout(quarisma::kStrided);
        ...
        auto nelem =
            static_cast<int64_t>(a_it.get_desc().get_size() / elementSize(a.scalar_type()));
        // we also wrap `a` storage into an aten tensor
        auto in_aten = quarisma::from_blob(mkldnn_raw_data, {nelem}, a_options_with_strided);
        ...
        if (!inplace)
        {
            ...
            out = quarisma::native::new_with_itensor_mkldnn(
                std::move(it_empty),
                quarisma::optTypeMetaToScalarType(a.options().dtype_opt()),
                a.options().device_opt());

            out_raw_data = quarisma::native::itensor_from_mkldnn(out).get_data_handle();
        }

        TORCH_INTERNAL_ASSERT(a_it.get_desc().get_size() % elementSize(a.scalar_type()) == 0);

        auto out_aten = quarisma::from_blob(out_raw_data, nelem, a_options_with_strided);
...
void MKLDNNLayerNormOp(Stack& stack, bool inplace)
{
    ...
    auto [dst, mean, rstd] = quarisma::native::mkldnn_layer_norm_last_index_weight_bias_f32(
...
Operation BroadOp(const Node* node)
{
    return [](Stack& stack)
    {
        ...
        {
            auto    out_size  = quarisma::infer_size(a_size, b_size);
            ...
            if (stacked < 2)
            {
                ...
                quarisma::impl::ExcludeDispatchKeyGuard edkg(quarisma::autograd_dispatch_keyset);
                auto a_it = quarisma::native::itensor_from_mkldnn(exp_a);
                auto b_it = quarisma::native::itensor_from_mkldnn(exp_b);
                ...
                auto a_out     = quarisma::native::new_with_itensor_mkldnn(
                    std::move(a_it),
                    quarisma::optTypeMetaToScalarType(a_options.dtype_opt()),
                    a_options.device_opt());
                push(stack, a_out);
                auto b_options = exp_b.options();
                auto b_out     = quarisma::native::new_with_itensor_mkldnn(
                    std::move(b_it),
                    quarisma::optTypeMetaToScalarType(b_options.dtype_opt()),
...
static std::function<void(quarisma::Tensor output, quarisma::Tensor input)> hardtanh_helper(
    const Node* n)
{
    auto min_val = n->f(attr::min_val);
    auto max_val = n->f(attr::max_val);
    return [min_val, max_val](quarisma::Tensor output, const quarisma::Tensor& input)
    ...
    const Node* n)
{
    ...
    auto max_val = n->f(attr::max_val);
    return [min_val, max_val](quarisma::Tensor output, const quarisma::Tensor& input)
    { quarisma::cpu::clamp_out(output, input, min_val, max_val); };
}
...
const RegisterOperators MKLDNNHardSwishOpReg({
    torch::jit::Operator(
        ...
        createUnaryOp(
            [](quarisma::Tensor output, const quarisma::Tensor& input)
            { quarisma::cpu::hardswish_out(output, input); },
            true),
    ...
    torch::jit::Operator(
        ...
        createUnaryOp(
            [](quarisma::Tensor output, const quarisma::Tensor& input)
            { quarisma::cpu::hardsigmoid_out(output, input); },
            true),
        AliasAnalysisKind::FROM_SCHEMA),
    torch::jit::Operator(
        "prim::MKLDNNHardTanh_(Tensor(a!) self) -> Tensor(a!)",
        [](const Node* n) -> Operation { return createUnaryOp(hardtanh_helper(n), true); },
        AliasAnalysisKind::FROM_SCHEMA),
    torch::jit::Operator(
        "prim::MKLDNNClamp_(Tensor(a!) self) -> Tensor(a!)",
        [](const Node* n) -> Operation { return createUnaryOp(clamp_helper(n), true); },
        AliasAnalysisKind::FROM_SCHEMA),
    torch::jit::Operator(
    ...
    torch::jit::Operator(
        ...
        [](const Node* n) -> Operation { return createUnaryOp(hardtanh_helper(n), false); },
        AliasAnalysisKind::FROM_SCHEMA),
    torch::jit::Operator(
        "prim::MKLDNNClamp(Tensor self) -> Tensor",
        [](const Node* n) -> Operation { return createUnaryOp(clamp_helper(n), false); },
        AliasAnalysisKind::FROM_SCHEMA),
});

const RegisterOperators BroadOpReg({
    torch::jit::Operator(
        prim::BroadcastMKLDNNTensors, BroadOp, AliasAnalysisKind::INTERNAL_SPECIAL_CASE),
});

const RegisterOperators MKLDNNLayerNormOpReg({
    torch::jit::Operator(
        "prim::MKLDNNLayerNorm(Tensor input, int[] normalized_shape, Tensor? weight=None, Tensor? "
        "bias=None, float eps=1e-05, bool cudnn_enable=True) -> Tensor",
        [](Stack& stack) { MKLDNNLayerNormOp(stack, false); },
        AliasAnalysisKind::FROM_SCHEMA),
    torch::jit::Operator(
        "prim::MKLDNNLayerNorm_(Tensor(a!) input, int[] normalized_shape, Tensor? weight=None, "
        "Tensor? bias=None, float eps=1e-05, bool cudnn_enable=True) -> Tensor(a!)",
...
{
    ideep::tensor& x = quarisma::native::itensor_from_mkldnn(tensor);
    ideep::tensor& z = quarisma::native::itensor_from_mkldnn(out);
...
jit::RegisterOperators reg_fut_ops({
    jit::Operator(
        ...
        "prim::mkldnn_convolution(Tensor input, Tensor weight, Tensor? bias, int[] stride, int[] "
        ...
        [](jit::Stack& stack)
        {
            ...
            if (input.size(0) == 0)
            {
                std::vector<int64_t> o = quarisma::native::conv_output_size(
                    input.sizes(), weight.sizes(), padding, stride, dilation);
                push(
                    stack,
                    quarisma::native::empty_mkldnn(
                        o,
                        quarisma::optTypeMetaToScalarType(input.options().dtype_opt()),
            ...
            push(
                stack,
                quarisma::native::mkldnn_convolution(
...
{
    return weight.device().is_cpu() && weight.dtype() == quarisma::ScalarType::Float &&
           weight.ndimension() != 0;
}

void replaceInputWithMKLDNNTensor(Node* n, size_t index)
{
    Value* input         = n->inputs().quarisma(index);
    auto   mkldnn_tensor = constant_as<Tensor>(input)->to_mkldnn();
    auto   mkldnn_tensor_value =
        createConstantMKLDNNTensorOp(n->owningGraph(), mkldnn_tensor)->insertBefore(n)->output();
...
void replaceInputWithMKLDNNTensor(
    Node* n, const std::string& name, const quarisma::Tensor& mkldnn_tensor)
{
...
{
    auto conv_w_mkldnn = constant_as<Tensor>(conv->namedInput("weight")).value().to_mkldnn();
    std::vector<int64_t> padding  = toIValue(conv->namedInput("padding"))->toIntVector();
    std::vector<int64_t> stride   = toIValue(conv->namedInput("stride"))->toIntVector();
    std::vector<int64_t> dilation = toIValue(conv->namedInput("dilation"))->toIntVector();
    auto                 groups   = constant_as<int64_t>(conv->namedInput("groups")).value();

    if (conv->kind() == aten::conv2d)
    {
        conv_w_mkldnn =
            mkldnn_reorder_conv2d_weight(conv_w_mkldnn, padding, stride, dilation, groups);
    }
    else if (conv->kind() == aten::conv3d)
    {
        conv_w_mkldnn =
            mkldnn_reorder_conv3d_weight(conv_w_mkldnn, padding, stride, dilation, groups);
    }
    else
    {
...
void ComputeSubgraphInMKLDNN(Node* subgraph_node)
{
    ...
    {
        Value* v = subgraph_node->inputs().quarisma(i);
        if (!v->type()->cast<TensorType>())
        {
            continue;
        }
        auto to_mkldnn = graph->create(quarisma::Symbol::fromQualString("aten::to_mkldnn"), 1)
    ...
    {
        Value* v = subgraph_node->outputs().quarisma(i);
        if (!v->type()->cast<TensorType>())
        {
            continue;
        }
        auto from_mkldnn =
            graph
                ->create(
                    quarisma::Symbol::fromQualString("aten::to_dense"), {v, none_value, none_value})
    ...
    for (auto it = subgraph->block()->nodes().begin(); it != subgraph->block()->nodes().end();)
    {
        Node* body_node = *it;
        it++;
        ...
        if (body_node->kind() == aten::add ||
            (body_node->kind() == aten::mul && body_node->input(1)->type()->cast<TensorType>()))
        {
            auto node = body_node->owningGraph()->create(
                Symbol::prim("BroadcastMKLDNNTensors"),
                {body_node->inputs().quarisma(0), body_node->inputs().quarisma(1)},
                2);
            node->insertBefore(body_node);
            body_node->replaceInput(0, node->outputs().quarisma(0));
            body_node->replaceInput(1, node->outputs().quarisma(1));
        }
        ...
        if (body_node->matches(
                "aten::layer_norm(Tensor input, int[] normalized_shape, Tensor? weight=None, "
        ...
        {
            auto min_val = constant_as<double>(body_node->namedInput("min_val")).value();
            auto max_val = constant_as<double>(body_node->namedInput("max_val")).value();
        ...
        if (body_node->kind() == aten::conv2d || body_node->kind() == aten::conv3d)
        {
...
bool nonConstantParameters(Node* n)
{
    ...
    {
        if (n->inputs().quarisma(i)->node()->kind() != prim::Constant)
        {
            return true;
        }
    }
...
// up just by converting the constant weights to MKLDNN AOT, and then quarisma runtime
...
{
public:
    MKLDNNSubgraphSlicer(Block* block, std::shared_ptr<Graph> graph, AliasDb& aliasDb)
...
private:
    ...
    bool computableInMKLDNN(Node* n)
    {
        ...
        {
            if (v->type()->cast<TensorType>() && !(tensorInputIsMKLDNNSupported(v, n)))
            {
                return false;
            }
        }
        ...
        {
            auto norm_shape = constant_as<std::vector<int64_t>>(n->namedInput("normalized_shape"));
        ...
        if ((n->kind() == aten::hardtanh || n->kind() == aten::clamp) && !nonConstantParameters(n))
        {
        ...
        if (n->kind() == aten::add)
        {
            ...
            for (const auto i : quarisma::irange(2))
            {
                if (!n->inputs().quarisma(i)->type()->cast<TensorType>())
                {
                    return false;
                }
            }
    ...
    bool shouldConsiderForMerge(Node* node)
    {
        ...
        return frozenMkldnnCompatibleLinearNode(node) || frozenMkldnnCompatibleConvNode(node) ||
               computableInMKLDNN(node);
    }
    ...
    std::pair<graph_node_list::iterator, bool> scanNode(Node* producer)
    {
        if (MKLDNNGroupStart(producer))
        {
            ...
            std::sort(
                output_nodes.begin(),
                output_nodes.end(),
                [&](Node* a, Node* b) { return a->isBefore(b); });
    ...
    std::optional<Node*> tryMerge(Node* producer, Node* consumer)
    {
        ...
        SubgraphUtils::mergeNodeIntoSubgraphAndUpdateAliasing(consumer, producer, aliasDb_);
...
void ConvertFrozenOpsToMKLDNN(std::shared_ptr<Graph>& graph)
{
    ...
    if (containsMKLDNNGroup(graph->block()))
    {
        ...
        RemoveTensorMutation(
            graph,
            [](Node* node_to_functionalize)
            {
