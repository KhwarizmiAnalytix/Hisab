...
std::vector<std::optional<const Use>> gatherLastUses(quarisma::ArrayRef<Value*> values)
{
    return fmap(
        values,
        [&](Value* v) -> std::optional<const Use>
...
struct ValueMapper
{
    ...
    ValueMapper(Node* to_merge, AliasDb& db, std::optional<Node*> existing_subgraph)
    {
        ...
        {
            Value* existing = to_merge->outputs().quarisma(i);
            Value* new_value =
                placeholder_node_->insertOutput(i)->copyMetadata(to_merge->outputs().quarisma(i));
            db.replaceWithNewValue(existing, new_value);
        }
    }

    bool usesEqual(const Use& a, const Use& b) { return a.user == b.user && a.offset == b.offset; }
    ...
    void copyAliasing(Node* merged_node, AliasDb& db)
    {
        ...
        for (Value* v : new_outputs)
        {
            ...
            for (size_t i = 0; i < existing_last_uses_.size() && !is_existing_value; ++i)
            {
            ...
            size_t i = 0;
            while (i < last_uses_.size() && last_uses_.quarisma(i).has_value() &&
                   !usesEqual(*last_uses_.quarisma(i), last_use))
            {
                ++i;
            }
            TORCH_INTERNAL_ASSERT(i != last_uses_.size());
            db.replaceWithNewValue(placeholder_node_->outputs().quarisma(i), v);
        }
...
{
    bool operator()(Node* a, Node* b) const { return a->isBefore(b); }
};
...
std::vector<std::set<Value*, topo_cmp_value>> buildAliasedSets(std::shared_ptr<Graph> subgraph)
{
    ...
    for (auto o : outputs)
    {
        ...
        for (auto& s : res)
        {
            ...
            GRAPH_DEBUG(
                "comparing %", o->debugName(), " with %", os->debugName(), " result ", aliased);
            if (aliased)
            {
                s.insert(o);
...
void unmergeSubgraph(Node* subgraphNode)
{
    ...
    const auto      subgraphOutputs =
        insertGraph(*outerGraph, *getSubgraph(subgraphNode), subgraphNode->inputs());
    ...
    Node*                               input_node)
{
    ...
    if (input_node->kind() == prim::If)
    {
        for (Block* block : input_node->blocks())
        {
            ...
            {
                collectNestedUses(closed_over_values, new_values, externalValuesMap, node);
            }
    ...
    else if (input_node->kind() == prim::Loop)
    {
        ...
        Block* block = input_node->blocks().quarisma(0);
...
static std::unordered_set<Value*> closedOverValues(
    Node* toMerge, std::unordered_map<Value*, Value*>& externalValuesMap)
{
    std::unordered_set<Value*> closed_over_values;
    std::unordered_set<Value*> new_values;
...
void mergeNodeIntoSubgraph(Node* toMerge, Node* subgraphNode, bool destroyNode)
{
    ...
    for (size_t i = 0; i < subgraphNode->outputs().size(); ++i)
    {
        externalValuesMap[subgraphNode->outputs().quarisma(i)] = subgraph->outputs().quarisma(i);
    }
    ...
    Node* guard_node =
        merging_node_after_subgraph ? *subgraph->nodes().end() : *subgraph->nodes().begin();
    WithInsertPoint guard(guard_node);

    std::unordered_set<Value*> closedValues = closedOverValues(toMerge, externalValuesMap);
    ...
    auto mergedNode = subgraph->insertNode(
        subgraph->createClone(toMerge, [&](Value* v) { return externalValuesMap[v]; }));
    ...
    for (const auto i : quarisma::irange(toMerge->outputs().size()))
    {
    ...
    {
        if (!hasUsesOutsideSubgraph(subgraphNode->outputs().quarisma(i)))
        {
...
void mergeNodeIntoSubgraphAndUpdateAliasing(Node* to_merge, Node* subgraphNode, AliasDb& db)
{
...
Node* createSingletonSubgraphAndUpdateAliasing(Node* to_merge, Symbol subgraphKind, AliasDb& db)
{
...
bool unmergeAliasedOutputs(Node* subgraphNode)
{
    ...
    for (auto i : quarisma::irange(sets.size()))
    {
        if (sets[i].size() <= 1)
        {
            GRAPH_DEBUG(
        ...
        // we have quarisma least two aliased outputs
...
void unmergeNode(Node* n, Node* subgraphNode)
{
    ...
    auto                               env = [&](Value* v)
    {
        ...
        TORCH_INTERNAL_ASSERT(
            false, "all inputs should've been mapped. Couldn't map %", v->debugName());
        return v;
    };

    for (auto i : quarisma::irange(subgraph->outputs().size()))
    {
        if (node_outputs.count(subgraph->outputs().quarisma(i)) != 0)
        {
            output_indices.insert(i);
        }

        if (node_inputs.count(subgraph->outputs().quarisma(i)) != 0)
        {
            GRAPH_DEBUG(
                "output %",
                subgraph->outputs().quarisma(i)->debugName(),
                " is already subgraph's output");
            GRAPH_DEBUG(
                "Mapping %",
                subgraph->outputs().quarisma(i)->debugName(),
                " to %",
                subgraphNode->outputs().quarisma(i)->debugName());
            local_map[subgraph->outputs().quarisma(i)] = subgraphNode->outputs().quarisma(i);
            node_inputs.erase(subgraph->outputs().quarisma(i));
        }
    }
    ...
    {
        auto replace_val = local_map[subgraph->outputs().quarisma(*it)];
        subgraphNode->outputs().quarisma(*it)->replaceAllUsesWith(replace_val);
...
static std::string truncateStrWithHash(const std::string& s, size_t maxlen)
{
    ...
    std::string hash_str = std::to_string(quarisma::hash<std::string>{}(s));
    ...
    // into maxlen. If that's not possible, quarisma least truncate the original string
...
std::string generateNameForGraph(
    const std::shared_ptr<Graph>& graph, size_t maxlen, const std::string& prefix)
{
