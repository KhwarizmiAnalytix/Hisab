...
Element* makeFreshValueImpl(
    const Value* v, std::vector<std::unique_ptr<Element>>& indexToElementMap_)
{
    if (v == nullptr)
    {
        // Create a wildcard element, with no corresponding value
        indexToElementMap_.emplace_back(std::make_unique<Element>(indexToElementMap_.size()));
        return indexToElementMap_.back().get();
    }
    indexToElementMap_.emplace_back(std::make_unique<Element>(v, indexToElementMap_.size()));
    return indexToElementMap_.back().get();
}
}  // namespace

Element::Element(const Value* value_, unsigned index_) : index(index_), values({value_}) {}
...
{
    return getAllContainedMemoryLocations(a).intersects(getAllContainedMemoryLocations(b));
}

const MemoryLocations& MemoryDAG::getAllContainedMemoryLocations(const Element* elem) const
{
    if QUARISMA_UNLIKELY (!elem->cachedAllContainedMemoryLocations_.has_value())
    {
        MemoryLocations cache;
        elem->cachedAllContainedMemoryLocations_ = MemoryLocations();
        collectAllContainedMemoryLocationsImpl(elem, *elem->cachedAllContainedMemoryLocations_);
    }
    return *elem->cachedAllContainedMemoryLocations_;
}

void MemoryDAG::collectAllContainedMemoryLocations(const Element* elem, MemoryLocations& cont) const
{
...
{
    unsigned compIdx = elem->index;
    QUARISMA_CHECK_DEBUG(!cont.test(compIdx));
    cont.set(compIdx);
...
bool MemoryDAG::mayContainAlias(const Element* a, const quarisma::ArrayRef<Element*> b) const
{
    if (b.empty())
    {
        return false;
    ...
    return std::any_of(
        b.begin(),
        b.end(),
        [this, &a_contained](Element* b_elem)
...
// `e` (`MemoryLocations` is just a typedef'd quarisma::SparseBitVector).
    ...
    const std::function<Element*(const Value*)>&           getWildcardElement)
{
    ...
    for (const Value* v : wildcards)
    {
        ...
        const MemoryLocations& pointeeSet = getMemoryLocations(elementMap.quarisma(v));
