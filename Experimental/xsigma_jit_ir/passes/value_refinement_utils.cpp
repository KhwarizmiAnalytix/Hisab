...
ListRefinement intersectRefinements(const ListRefinement& ref1, const ListRefinement& ref2)
{
    ListRefinement out;
...
ListRefinement unionRefinements(const ListRefinement& ref1, const ListRefinement& ref2)
{
    ...
    std::unordered_map<Value*, BooleanRefinementMapping>& boolean_value_refinements)
{
    ...
    if (true_block_throws || false_block_throws)
    {
        ...
        Block* non_throwing_block =
            true_block_throws ? if_node->blocks().quarisma(1) : if_node->blocks().quarisma(0);
        for (const auto i : quarisma::irange(if_n.outputs().size()))
        {
            if (boolean_value_refinements.count(non_throwing_block->outputs().quarisma(i)))
            {
                boolean_value_refinements[if_node->outputs().quarisma(i)] =
                    boolean_value_refinements[non_throwing_block->outputs().quarisma(i)];
            }
        }
        return;
    }
    ...
    for (const auto i : quarisma::irange(if_n.outputs().size()))
    {
        if (!(if_n.outputs().quarisma(i)->type() == BoolType::get()))
        {
            return;
        }
        Value* true_v  = if_n.thenOutputs().quarisma(i);
        Value* false_v = if_n.elseOutputs().quarisma(i);
        ...
        if (auto maybe_bool = constant_as<bool>(true_v))
        {
            if (*maybe_bool)
            {
                out = BooleanRefinementMapping::FalseRefinements(unionRefinements(
                    boolean_value_refinements[false_v].false_refine(), false_block_refinements));
            }
            else
            {
                out = BooleanRefinementMapping::TrueRefinements(unionRefinements(
                    boolean_value_refinements[false_v].true_refine(), false_block_refinements));
            }
        }
        else if (auto maybe_bool = constant_as<bool>(false_v))
        {
            ...
            {
                out = BooleanRefinementMapping::FalseRefinements(unionRefinements(
                    boolean_value_refinements[true_v].false_refine(), true_block_refinements));
            }
            else
            {
                out = BooleanRefinementMapping::TrueRefinements(unionRefinements(
                    boolean_value_refinements[true_v].true_refine(), true_block_refinements));
            }
        }
        else if (
            boolean_value_refinements.count(true_v) && boolean_value_refinements.count(false_v))
        {
            out = boolean_value_refinements[true_v].intersectBooleanRefinementMapping(
                boolean_value_refinements[false_v]);
        }
        boolean_value_refinements[if_n.outputs().quarisma(i)] = out;
    }
}
    ...
    std::unordered_map<Value*, BooleanRefinementMapping>& info)
{
    ...
    if (n->kind() == aten::__not__ && n->inputs().quarisma(0)->type()->cast<BoolType>())
    {
        ...
        if (info.count(n->input()))
        {
            ...
            info[n->output()] =
                BooleanRefinementMapping(input_ref.false_refine(), input_ref.true_refine());
        }
        return true;
    }
        ...
        (n->matches("aten::ne(bool a, bool b) -> bool")))
    {
        for (size_t const_index : {0, 1})
        {
            ...
            if ((!const_input && n->kind() == aten::eq) || (const_input && n->kind() == aten::ne))
            {
                info[n->output()] =
                    BooleanRefinementMapping(input_ref.false_refine(), input_ref.true_refine());
            }
            ...
            {
                info[n->output()] =
                    BooleanRefinementMapping(input_ref.true_refine(), input_ref.false_refine());
            }
        }
        return true;
    }
