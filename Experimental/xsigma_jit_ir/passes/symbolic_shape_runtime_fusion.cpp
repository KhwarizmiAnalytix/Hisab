...
static std::map<int64_t, Value*> InsertSymbolicShapesCompute(
    const ShapeComputeGraphMapping& shape_mapping, Node* tensorexpr_graph)
{
    ...
    for (Value* shape_graph_input : shape_mapping.partial_eval_shape_graph->inputs())
    {
        ...
        TORCH_INTERNAL_ASSERT(
            enclosing_graph_input != shape_graph_input_to_enclosing_graph_value.end());
    ...
    auto sym_shape_values = insertGraph(
        *enclosing_graph, *shape_mapping.partial_eval_shape_graph, shape_compute_graph_inputs);
    ...
    size_t                          stride_inputs_offset)
{
    ...
    else if (dim + 1 < sizes.size() && strides[dim] == strides[dim + 1] * sizes[dim + 1])
    {
        return StrideInput::S_CONT;
...
static std::vector<StrideInput> summarizeInputStrides(const TensorType& tt)
{
    ...
    {
        stride_inputs.push_back(summarizeStrideDim(sizes, strides, dim, stride_inputs, 0));
    }
...
TryGeneralizeInputDimensionsToSymbolicShapes(const std::shared_ptr<Graph>& tensorexpr_graph)
{
...
static void moveConstantTensorsOutOfSubgraph(
    Node* tensorexpr_graph_node, const std::shared_ptr<Graph>& tensorexpr_graph)
{
...
bool GenerateGuard(Node* tensorexpr_graph_node, bool add_composed_op)
{
    ...
    // Generalize Inputs
    auto input_striding = TryGeneralizeInputDimensionsToSymbolicShapes(tensorexpr_graph);
    if (!input_striding)
    {
        return false;
    }
    ...
    auto maybe_shape_compute_mapping = PropagateShapesAndBuildLargeShapeComputeGraph(
        tensorexpr_graph, *tensorexpr_graph->nodes().begin(), *tensorexpr_graph->nodes().end());
...
static void inlineFallbackGraphAndAddSRCopyOutOp(std::shared_ptr<Graph> graph)
{
    ...
    std::vector<Value*> false_block_outputs(if_v.elseOutputs().begin(), if_v.elseOutputs().end());
    ...
    auto copy_node =
        graph->create(prim::StaticRuntimeCopyOuts, false_block_outputs, false_block_outputs.size());
    ...
    std::vector<StrideInput>&              output_strides)
{
    ...
    Node* typecheck_node = guarded_node->owningGraph()
    ...
    // Insert if
    auto versioning_if = guarded_node->owningGraph()
    ...
    const auto      subgraph_outputs =
        insertGraph(*guarded_node->owningGraph(), *subgraph, guarded_node->inputs());
    ...
    guarded_node->is_(attr::symbolic_shape_inputs, std::move(symbolic_shape_inputs));

    std::vector<std::vector<std::string>> input_striding;
    for (auto& vec : input_info)
    {
        auto string_info = fmap(vec, [&](StrideInput inp) { return toString(inp); });
    ...
    if (add_composed_op)
    {
        ...
        auto te_dyn_group =
            SubgraphUtils::createSingletonSubgraph(typecheck_node, prim::TensorExprDynamicGroup);
        SubgraphUtils::mergeNodeIntoSubgraph(versioning_if, te_dyn_group);
        inlineFallbackGraphAndAddSRCopyOutOp(SubgraphUtils::getSubgraph(te_dyn_group));
    }
}
...
static RegisterOperators SRCopyOuts({
    torch::jit::Operator(
        prim::StaticRuntimeCopyOuts, StaticRuntimeCopyOuts, AliasAnalysisKind::CONSERVATIVE),
});
