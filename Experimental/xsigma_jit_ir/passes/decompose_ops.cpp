...
{
    return quarisma::AliasAnalysisKind::FROM_SCHEMA;
}
}  // namespace
...
static bool isDecomposableNorm(Node* normalize_op)
{
    static const OperatorSet decomposable_normalization_ops = {
        ...
        "Tensor",
        "aten::layer_norm(Tensor input, int[] normalized_shape, Tensor? weight, Tensor? bias, "
        "float eps, bool cudnn_enable) -> Tensor",
    };
    ...
    {Operator(
     ...
     Operator(
         ...
         [](Stack& stack)
         {
             ...
             {
                 sizes.quarisma(i) = input_shape.get(i);
             }
             push(stack, self.reshape(sizes));
         },
         aliasAnalysisFromSchema())});
...
static bool DecomposeOps(Block* block, CompilationUnit& decompose_funcs)
{
    ...
    for (auto it = block->nodes().begin(), end = block->nodes().end(); it != end; ++it)
    {
                ...
                /*const_inputs=*/{attr::beta, attr::alpha}))
        {
            ...
            // (batchmm)
            if (it->get<quarisma::Scalar>(attr::alpha)->toComplexDouble() != 1.0 ||
                it->get<quarisma::Scalar>(attr::beta)->toComplexDouble() != 1.0)
            {
                continue;
            }
            ...
            Value* new_output = insertGraph(*it->owningGraph(), *d_graph, it->inputs()).quarisma(0);
                     ...
                     "bool cudnn_enabled) -> Tensor"))
        {
            ...
            Value* new_output = insertGraph(*graph, *d_graph, inputs).quarisma(0);
            ...
            {
                Value* expanded_weight = graph->insert(aten::_ncf_unsqueeze, {weight, input_dim});
                new_output             = graph->insert(aten::mul, {new_output, expanded_weight});
            }
            if (isDefined(bias).value())
            {
                Value* expanded_bias = graph->insert(aten::_ncf_unsqueeze, {bias, input_dim});
