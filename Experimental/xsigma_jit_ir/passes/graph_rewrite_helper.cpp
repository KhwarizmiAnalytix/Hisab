...
void replaceConvolutionWithAtenConv(std::shared_ptr<Graph>& graph)
{
    ...
    auto filter_conv1d = [](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
    {
    ...
    auto filter_conv2d = [](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
    {
        auto calc_value_map = getConvParams(match, vmap);
    ...
    auto filter_conv3d = [](const Match& match, const std::unordered_map<std::string, Value*>& vmap)
    {
        auto calc_value_map = getConvParams(match, vmap);
    ...
    rewriter_conv1d.RegisterRewritePattern(convolution_deprecated, conv1d_for_deprecated_conv);
    ...
    rewriter_conv2d.RegisterRewritePattern(convolution_deprecated, conv2d_for_deprecated_conv);
    ...
    rewriter_conv_transpose1d.RegisterRewritePattern(convolution, conv_transpose1d);
    ...
    rewriter_conv_transpose2d.RegisterRewritePattern(convolution, conv_transpose2d);
    ...
    rewriter_conv_transpose3d.RegisterRewritePattern(convolution, conv_transpose3d);
...
bool isClampFusable(const Match& match, const std::unordered_map<std::string, Value*>& vmap)
{
    ...
    if (vmap.find("output_min") != vmap.end())
    {
        ...
        auto output_min = graph_rewrite_helper::getIValue("output_min", match_vmap, vmap);
        auto output_max = graph_rewrite_helper::getIValue("output_max", match_vmap, vmap);
