...
{
    explicit BailOutGraphBuilderForNode(std::shared_ptr<Graph> graph, std::shared_ptr<Graph> target)
    ...
    Value* addNewInputForValue(Value* old_value)
    {
        ...
        GRAPH_DEBUG(
            "Adding a new value %", new_value->debugName(), " for %", old_value->debugName());
    ...
    void buildBailOutBlockFrom(Node* n)
    {
        ...
        if (outer_node)
        {
            ...
            {
                QUARISMA_CHECK(false, "Unexpected outer node");
            }
        }
    }
    ...
    void buildBailOutLoop(Node* outer_node)
    {
        ...
        updated_max_trip_count      = copy_graph_->insert(aten::sub, {updated_max_trip_count, one});
        ...
        // (i.e. `block_outputs`)
        auto new_loop = copy_graph_->insertNode(copy_graph_->create(prim::Loop, {}, 0))
    ...
    void buildBailOutIf(const quarisma::ArrayRef<Value*> block_outputs, Node* outer_node)
    {
...
struct BailOutInserter
{
    ...
    void addUnoptimizedFuncToBailouts()
    {
        ...
        auto unopt_func = graph_->create(prim::BailoutTemplate)->insertAfter(graph_->param_node());
    ...
    // quarisma that particular execution point.
...
static void removeBailouts(Block* b)
{
    for (auto it = b->nodes().begin(); it != b->nodes().end(); it++)
    {
        if (it->kind() == prim::BailOut || it->kind() == prim::Guard)
        {
            ...
            it->inputs().quarisma(0)->setType(TensorType::get());
            it->output()->replaceAllUsesWith(it->inputs().quarisma(0));
            it.destroyCurrent();
        }
        else
        {
...
TORCH_API std::shared_ptr<Graph> BuildBailOutGraphFrom(
    int64_t bailout_index, const std::shared_ptr<Graph>& orig, const std::shared_ptr<Graph>& target)
{
    auto orig_bailout_node = locateBailOutNodeInUnoptimizedGraph(orig->block(), bailout_index);

    GRAPH_DEBUG("bailout triggered for ", *orig_bailout_node);
    GRAPH_DUMP("original bailout graph ", orig);
    TORCH_INTERNAL_ASSERT(
        orig_bailout_node->inputs().quarisma(0)->type()->cast<FunctionType>() == nullptr);
    TORCH_INTERNAL_ASSERT(
        orig_bailout_node &&
