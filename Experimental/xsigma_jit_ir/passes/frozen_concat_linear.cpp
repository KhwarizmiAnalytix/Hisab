...
public:
        ...
        std::vector<Value*>&                            ordered_tensor_inputs)
    {
        ...
        for (Node* n : b->nodes())
        {
            ...
            Value* linear_input = n->inputs().quarisma(0);
    ...
    void mergeLinearLayers(std::vector<Node*>& compatible_layers)
    {
        ...
        for (Node* orig_node : compatible_layers)
        {
            ...
            Node* slice = graph_->create(
                aten::slice, {linear_node->output(), neg1, slice_start_val, slice_end_val, one});
    ...
    void collectAndMergeLinearLayers(std::vector<Node*>& linear_layer_group)
    {
        ...
        for (size_t i = 0; i < linear_layer_group.size(); i++)
        {
            ...
            auto base_weight = constant_as<Tensor>(base_node->namedInput("weight")).value();
            auto base_bias   = constant_as<Tensor>(base_node->namedInput("bias")).value();
            ...
            for (size_t j = i + 1; j < linear_layer_group.size(); j++)
            {
                ...
                if (base_weight.dtype() != weight.dtype() ||
                    base_weight.device() != weight.device() || base_bias.dtype() != bias.dtype() ||
                    base_bias.device() != bias.device())
                {
                    continue;
                }

                if (!isNonZeroDimEqual(base_weight, weight) || !isNonZeroDimEqual(base_bias, bias))
                {
                    continue;
                }
                ...
                {
                    can_move_before_all &= getAliasDb()->couldMoveBeforeTopologically(node, n);
                }
    ...
    void handleBlockAndSubblocks(Block* block)
    {
        ...
        std::unordered_map<Value*, std::vector<Node*>> grouped_linear_layers;
        std::vector<Value*>                            ordered_tensor_inputs;
        collectConstantLinearLayers(block, grouped_linear_layers, ordered_tensor_inputs);
        ...
        for (auto tensor_it = ordered_tensor_inputs.rbegin();
             ...
             ++tensor_it)
        {
            collectAndMergeLinearLayers(grouped_linear_layers.quarisma(*tensor_it));
        }
    }
