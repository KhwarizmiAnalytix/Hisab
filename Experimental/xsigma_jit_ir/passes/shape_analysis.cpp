...
bool mergeTypes(ArrayRef<Value*> lhs, ArrayRef<Value*> rhs, ArrayRef<Value*> outputs)
{
    ...
    bool changed = false;
    for (const auto i : quarisma::irange(lhs.size()))
    {
...
{
    auto then_block = node->blocks().quarisma(0);
    auto else_block = node->blocks().quarisma(1);
...
void PropertyPropBase::processLoop(Node* node)
{
    ...
    do
    {
        ...
        // note: inserting expands is unsafe quarisma this point, we don't know
...
bool isValidArgumentForRunning(Value* v)
{
    ...
    if (TensorTypePtr tt = v->type()->cast<TensorType>())
    {
        ...
        return !quarisma::isIntegralType(*tt->scalarType(), /*includeBool=*/false);
    }
...
{
    return v->type()->isSubtypeOf(*TensorType::get()) || v->type()->isSubtypeOf(*NumberType::get());
}
...
bool containsTensorType(const TypePtr& t)
{
    ...
    if (n_contained == 1)
    {
        return t->containedTypes().quarisma(0)->isSubtypeOf(*TensorType::get());
    }
    else if (n_contained > 1)
    {
        return std::any_of(
            t->containedTypes().begin(), t->containedTypes().end(), containsTensorType);
    }
    return false;
}
...
std::optional<std::vector<TensorTypePtr>> gatherTensorTypes(Node* node, bool complete = false)
{
    ...
    for (const auto i : quarisma::irange(args.size()))
    {
...
int64_t wrapDim(int64_t dim, quarisma::IntArrayRef sizes)
{
    if (dim < 0)
    {
...
quarisma::ScalarType unionScalarTypes(quarisma::ScalarType original, quarisma::ScalarType next)
{
    if (original == quarisma::ScalarType::Undefined)
    {
        return next;
    }
    else
    {
        return quarisma::promoteTypes(original, next);
    }
}
...
std::optional<quarisma::ScalarType> getPromotedTypeForArithmeticOp(Node* node)
{
    quarisma::ScalarType dimmed  = quarisma::ScalarType::Undefined;
    quarisma::ScalarType zerodim = quarisma::ScalarType::Undefined;
    // binary arithmetic ops, more than 2 args is alpha.
    for (const auto i : quarisma::irange(2))
    {
    ...
    if (quarisma::ScalarType::Bool == dimmed && quarisma::ScalarType::Undefined != zerodim)
    {
        return zerodim;
    }
    ...
    if (quarisma::ScalarType::Undefined != dimmed)
    {
        return dimmed;
    }
...
private:
    ...
    bool resizesInput(Node* n)
    {
        ...
        {
            auto arg = n->schema().arguments().quarisma(*out_arg_index);
    ...
    IValue representativeValue(Value* v)
    {
        ...
        if (TensorTypePtr type = type_->cast<TensorType>())
        {
            if (type->isComplete())
            {
                quarisma::DeviceGuard device_guard(*type->device());
                return quarisma::empty_strided(
                           *type->sizes().concrete_sizes(),
                           *type->strides().concrete_sizes(),
                           quarisma::TensorOptions(*type->device()).dtype(type->scalarType()))
                    .zero_();
            }
            // fallthrough
        }
    ...
    void broadcastBinary(Node* node, std::vector<TensorTypePtr>& types, size_t idx1, size_t idx2)
    {
        ...
        auto broadcast = [&](size_t input_idx)
        {
            ...
            Node*           expand = graph
                               ->create(
                                   aten::expand,
                                   {node->inputs().quarisma(input_idx),
        ...
        broadcast(idx1);
        broadcast(idx2);
        types[0] = node->inputs().quarisma(idx1)->type()->expect<TensorType>();
        types[1] = node->inputs().quarisma(idx2)->type()->expect<TensorType>();
    }
    ...
    bool canPropagateShapeByRunningIt(Node* node)
    {
        ...
        bool valid_args =
            std::all_of(node->inputs().begin(), node->inputs().end(), isValidArgumentForRunning);
        if (!valid_args)
            return false;

        bool valid_returns =
            std::all_of(node->outputs().begin(), node->outputs().end(), isValidReturnForRunning);
        if (!valid_returns)
            return false;
    ...
    bool PropagateShapeOnNodeByRunningIt(Node* node, Operation op = nullptr)
    {
        ...
        for (const auto i : quarisma::irange(stack.size()))
        {
            ...
            if (stack[i].isTensor() && tensor_type)
            {
                ...
                node->outputs()[i]->setType(
                    TensorType::create(stack[i].toTensor())->withRequiresGrad(tensor_grad));
            }
        }
        return true;
    }
    ...
    void PropagateCatShape(Node* cat_node)
    {
                                                  ...
                                                  quarisma::ArrayRef<Value*> tensors) -> bool
        {
            ...
            if (!std::all_of(
                    input_types.begin(),
                    input_types.end(),
                    [](const TensorTypePtr& tp) { return tp != nullptr && tp->isComplete(); }))
            {
                return false;
            }
            ...
            for (auto& tp : input_types)
            {
                ...
                if (sizes.size() != tp_sizes.size())
                    return false;
                for (const auto i : quarisma::irange(ndim))
                {
        ...
        static const auto propagate = [](Node* node, quarisma::ArrayRef<Value*> tensors) -> bool
        {
            for (Value* v : tensors)
            {
        ...
        auto list_node =
            ((cat_node->kind() == prim::FusedConcat) ? cat_node
                                                     : cat_node->namedInput(attr::tensors)->node());
        if (list_node->kind() == prim::ListConstruct || cat_node->kind() == prim::FusedConcat)
        {
    ...
    {
        auto input_type = node->inputs().quarisma(0)->type();
        ...
        quarisma::Device default_device = quarisma::kCPU;
        if (auto device_index = node->schema().argumentIndexWithName("device"))
        {
            auto inp = toIValue(node->inputs().quarisma(*device_index));
            if (inp == std::nullopt)
            {
                return;
            }
        ...
        node->output()->setType(
            TensorType::create(default_type, default_device, dims, /*requires_grad=*/std::nullopt));
    }

    // returns whether any such values were found
    bool setUnshapedTypeIfAliasResizedSet(quarisma::ArrayRef<Value*> vs)
    {
    ...
    void propagateNode(Node* node, bool insert_expands = true) override
    {
        ...
        case prim::NumToTensor:
        {
            ...
            if (typ->isSubtypeOf(*IntType::get()) || typ->isSubtypeOf(*BoolType::get()))
            {
                node->output()->setType(
                    TensorType::create(
                        quarisma::kLong, quarisma::kCPU, 0, /*requires_grad=*/std::nullopt));
            }
            else if (node->input()->type()->isSubtypeOf(*FloatType::get()))
            {
                node->output()->setType(
                    TensorType::create(
                        quarisma::kDouble, quarisma::kCPU, 0, /*requires_grad=*/std::nullopt));
            }
            return;
        }
        case aten::tensor:
        case aten::as_tensor:
        {
            ...
            if (node->inputs().quarisma(0)->type()->isSubtypeOf(*TensorType::get()))
            {
                break;
            }
        ...
        case prim::TupleConstruct:
        {
            ...
            auto new_types = fmap(node->inputs(), [](Value* v) { return v->type(); });
            node->output()->setType(orig_type->createWithContained(std::move(new_types)));
            return;
        }
        case prim::TupleUnpack:
        {
            auto tuple_type = node->input()->type()->cast<TupleType>();
            AT_ASSERT(tuple_type && tuple_type->elements().size() == node->outputs().size());
        ...
        if (auto maybe_complete_types = gatherTensorTypes(node, /*complete=*/true))
        {
    ...
    static std::optional<size_t> determineListSize(Value* list)
    {
        ...
        if (auto shape = constant_as<quarisma::List<int64_t>>(list))
        {
            return shape->size();
        }
    ...
    bool PropagateTensorShapeOnNode(Node* node, bool insert_expands)
    {
        ...
        static const register_formula_for simple_unary_ops{
            {
                ...
                "aten::contiguous(Tensor(a) self, *, MemoryFormat memory_format=contiguous_format) "
                "-> Tensor(a)",
                ...
                "aten::rrelu(Tensor self, Scalar lower, Scalar upper, bool training, Generator? "
                "generator) -> Tensor",
                ...
                "aten::slice(Tensor self, int dim, int? start=None, int? end=None, int step=1) -> "
                "Tensor",
        ...
        static const register_formula_for simple_unary_ops_complex_to_float{
            ...
            [](Node* node) -> type_vec_t
            {
                ...
                if (input_type->scalarType())
                {
                    ...
                    {
                        const auto out_type = quarisma::toRealValueType(scalar_type);
        ...
        static const register_formula_for broadcasting_ops{
            ...
            [](Node* node) -> type_vec_t
            {
                if (auto maybe_tensor_types = gatherTensorTypes(node))
                {
                    ...
                    if (quarisma::promoteTypes(*first_scalar_type, *second_scalar_type) !=
                        first_scalar_type)
                    {
                        arg_for_type = 1;
                    }
        ...
        static const register_formula_for fused_accum_binary_ops{
            {
                // Non-binary ops
                "aten::addcdiv(Tensor self, Tensor tensor1, Tensor tensor2, *, Scalar value) -> "
                "Tensor",
                "aten::addcmul(Tensor self, Tensor tensor1, Tensor tensor2, *, Scalar value) -> "
                "Tensor",
            },
            [](Node* node) -> type_vec_t
            {
        ...
        static const register_formula_for broadcasting_tensor_scalar_ops_arithmetic{
            ...
            [](Node* node) -> type_vec_t
            {
                if (auto maybe_tensor_types = gatherTensorTypes(node))
                {
                    ...
                    {
                        auto default_dtype =
                            quarisma::typeMetaToScalarType(caffe2::get_default_dtype());
                        return {broadcast(*maybe_tensor_types, default_dtype)};
                    }
                    if (quarisma::ScalarType::Bool == *first_scalar_type &&
                        quarisma::ScalarType::Bool != *second_scalar_type)
                    {
                        auto result_type =
                            quarisma::promoteTypes(*first_scalar_type, *second_scalar_type);
        ...
        static const register_formula_for comparison_ops{
            ...
            [](Node* node) -> type_vec_t
            {
                if (auto maybe_tensor_types = gatherTensorTypes(node))
                {
                    return {broadcast(*maybe_tensor_types, quarisma::kBool)};
                }
                return {};
            }};

        static const register_formula_for nn_ops_first_input_formula{
            *nn_ops_first_input_preserving(),
            [](Node* node) -> type_vec_t
            {
        ...
        static const register_formula_for all_reduce_ops_with_integer_upcast_and_dtype{
            ...
            [](Node* node) -> type_vec_t
            {
                if (auto type = node->input(0)->type()->cast<TensorType>())
                {
                    ...
                    {
                        return {
                            quarisma::isFloatingType(*type->scalarType())
                                ? std::move(type)
                                : type->withScalarType(quarisma::kLong)};
                    }
                    else
                    {
                        return {std::move(type)};
                                                     ...
                                                     std::nullopt) -> type_vec_t
        {
            if (auto type = node->input(0)->type()->cast<TensorType>())
            {
                ...
                else if (upcast_integer && !quarisma::isFloatingType(*type->scalarType()))
                {
                    type = type->withScalarType(quarisma::kLong);
                }
        ...
        static const auto multidim_reduce_with_keepdim =
            [](Node* node, int64_t num_reduced_dim, bool upcast_integer) -> type_vec_t
        {
        ...
        static const register_formula_for argminmax{
            ...
            [](Node* node) -> type_vec_t
            {
                if (auto type = node->input(0)->type()->cast<TensorType>())
                {
                    if (node->input(1)->type()->kind() == quarisma::TypeKind::NoneType)
                    {
                        return {type->withDim(0)};
                    }
                    else
        ...
        static const register_formula_for dim_reduce_ops{
            ...
            [](Node* node) -> type_vec_t
            {
                ...
                {
                    output_types.push_back(output_types.back()->withScalarType(quarisma::kLong));
                }
                return output_types;
            }};
        ...
        static const auto factory_with_ndim =
            [](Node* node, int dim, quarisma::ScalarType default_dtype) -> type_vec_t
        {
            ...
            if (!maybe_device_option)
                return {};
            auto device =
                (maybe_device_option->isNone() ? quarisma::kCPU : maybe_device_option->toDevice());

            std::optional<IValue> maybe_dtype_option = node->get(attr::dtype);
            if (!maybe_dtype_option)
                return {};
            auto dtype =
                (maybe_dtype_option->isNone() ? default_dtype : maybe_dtype_option->toScalarType());

            return {TensorType::create(dtype, device, dim, /*requires_grad=*/std::nullopt)};
        };

        static const auto factory_like_with_ndim = [](Node* node, int dim) -> type_vec_t
        {
            ...
            return {TensorType::create(in_type, in_dev, dim, /*requires_grad=*/std::nullopt)};
        };
        ...
        static const register_formula_for like_factories_with_options{
            {
                "aten::empty_like(Tensor self, *, int? dtype=None, int? layout=None, Device? "
                "device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor",
                "aten::full_like(Tensor self, Scalar fill_value, *, int? dtype=None, int? "
                "layout=None, Device? device=None, bool? pin_memory=None, MemoryFormat? "
                "memory_format=None) -> Tensor",
                "aten::ones_like(Tensor self, *, int? dtype=None, int? layout=None, Device? "
                "device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor",
                "aten::rand_like(Tensor self, *, int? dtype=None, int? layout=None, Device? "
                "device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor",
                "aten::randint_like(Tensor self, int high, *, int? dtype=None, int? layout=None, "
                "Device? device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> "
                "Tensor",
                "aten::randint_like(Tensor self, int low, int high, *, int? dtype=None, int? "
                "layout=None, Device? device=None, bool? pin_memory=None, MemoryFormat? "
                "memory_format=None) -> Tensor",
                "aten::randn_like(Tensor self, *, int? dtype=None, int? layout=None, Device? "
                "device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor",
                "aten::zeros_like(Tensor self, *, int? dtype=None, int? layout=None, Device? "
                "device=None, bool? pin_memory=None, MemoryFormat? memory_format=None) -> Tensor",
            },
            [](Node* node) -> type_vec_t
            {
        ...
        static const register_formula_for size_factories_with_options{
            {
                "aten::empty(int[] size, *, int? dtype, int? layout, Device? device, bool? "
                "pin_memory, MemoryFormat? memory_format=contiguous_format) -> Tensor",
                "aten::full(int[] size, Scalar fill_value, *, int? dtype, int? layout, Device? "
                ...
                "pin_memory) -> Tensor",
                "aten::randn(int[] size, *, int? dtype, int? layout, Device? device, bool? "
                "pin_memory) -> Tensor",
            ...
            [](Node* node) -> type_vec_t
            {
                if (auto maybe_size = node->get<quarisma::List<int64_t>>(attr::size))
                {
                    return factory_with_ndim(node, (int)maybe_size->size(), quarisma::kDouble);
                }
                return {};
            }};
        ...
        static const register_formula_for randint{
            {
                "aten::randint(int high, int[] size, *, int? dtype, int? layout, Device? device, "
                "bool? pin_memory) -> Tensor",
                "aten::randint(int low, int high, int[] size, *, int? dtype, int? layout, Device? "
                "device, bool? pin_memory) -> Tensor",
            },
            [](Node* node) -> type_vec_t
            {
                if (auto maybe_size = node->get<quarisma::List<int64_t>>(attr::size))
                {
                    return factory_with_ndim(node, (int)maybe_size->size(), quarisma::kLong);
                }
                return {};
            }};

        static const auto get_cast_scalar_type = [](Node* node) -> quarisma::ScalarType
        {
            switch (node->kind())
            {
        ...
        for (auto& entry : shape_formulas)
        {
            if (node->isMemberOf(entry.first))
            {
                ...
                else
                {
                    ...
                    for (const auto i : quarisma::irange(types.size()))
                    {
                     ...
                     "aten::batch_norm_stats(Tensor input, float eps) -> (Tensor, Tensor)"))
        {
            if (auto type = input_type(0))
            {
                if (type->scalarType() == quarisma::kHalf)
                {
                    type = type->withScalarType(quarisma::kFloat);
                }
                type = type->withDim(1);
        ...
        else if (node->matches("aten::dot(Tensor self, Tensor tensor) -> Tensor"))
        {
            ...
            node->matches(
                "aten::addmv(Tensor self, Tensor mat, Tensor vec, *, Scalar beta, Scalar alpha) -> "
                "Tensor"))
        {
            if (auto type = any_tensor_type(node))
            {
            ...
            node->matches(
                "aten::addmm(Tensor self, Tensor mat1, Tensor mat2, *, Scalar beta, Scalar alpha) "
                "-> Tensor") ||
            node->matches(
                "aten::addbmm(Tensor self, Tensor batch1, Tensor batch2, *, Scalar beta, Scalar "
                "alpha) -> Tensor") ||
            node->matches(
                "aten::addr(Tensor self, Tensor vec1, Tensor vec2, *, Scalar beta, Scalar alpha) "
                "-> Tensor"))
        {
            if (auto type = any_tensor_type(node))
            {
                ...
                return true;
                     ...
                     "aten::baddbmm(Tensor self, Tensor batch1, Tensor batch2, *, Scalar beta, "
                     "Scalar alpha) -> Tensor"))
        {
            if (auto type = any_tensor_type(node))
            {
               ...
               const std::vector<TensorTypePtr>& tensor_types) -> TensorTypePtr
        {
            ...
            {
                return tensor_types.quarisma(0)->withDim(list_size);
            }
            return nullptr;
        };
        const auto getSingleOutputType = [&]() -> TypePtr
        {
            ...
            {
                return tensor_types.quarisma(0)->withDim(tensor_types.quarisma(1)->dim());
            }
            else if (
                ...
                node->matches(
                    "aten::as_strided(Tensor self, int[] size, int[] stride, int? storage_offset) "
                    "-> Tensor"))
            {
                return reshape_prop(node, attr::size, tensor_types);
                         ...
                         "Tensor"))
            {
                TypePtr input_type = node->inputs().quarisma(0)->type();
                if (auto type = input_type->cast<TensorType>())
                {
                    if (type->scalarType() && type->device())
                    {
                        quarisma::ScalarType default_type   = *type->scalarType();
                        quarisma::Device     default_device = *type->device();
                        if (auto dtype_index = node->schema().argumentIndexWithName("dtype"))
                        {
                            auto inp = toIValue(node->inputs().quarisma(*dtype_index));
                            if (inp == std::nullopt)
                            {
                                return nullptr;
                            }
            ...
            else if (node->matches("aten::reshape(Tensor(a) self, int[] shape) -> Tensor(a)"))
            {
            ...
            {
                auto& t = tensor_types.quarisma(0);
                if (!t->dim())
                {
                    return t;
                }
                ...
                node->matches("aten::select(Tensor self, int dim, int index) -> Tensor") ||
                node->matches(
            ...
            {
                if (!tensor_types.quarisma(0)->dim() || !tensor_types.quarisma(1)->dim())
                {
                    return nullptr;
                }
                auto dim1 = *tensor_types.quarisma(0)->dim();
                auto dim2 = *tensor_types.quarisma(1)->dim();
                if (dim1 == 1 && dim2 == 1)
                {
                    // Dot product
                    return tensor_types.quarisma(0)->withDim(0);
                    // NOLINTNEXTLINE(bugprone-branch-clone)
                }
                else if (dim1 == 2 && dim2 == 2)
                {
                    // Matrix multiply
                    return tensor_types.quarisma(0);
                }
                ...
                else if (dim1 == 2 && dim2 == 1)
                {
                    ...
                    return tensor_types.quarisma(1);
                }
                else
                {
            ...
            {
                return tensor_types.quarisma(0)->dimensionedOnly()->withScalarType(quarisma::kLong);
            }
            else if (node->matches("aten::take(Tensor self, Tensor index) -> Tensor"))
            {
                return tensor_types.quarisma(1)->dimensionedOnly()->withScalarType(
                    tensor_types.quarisma(0)->scalarType());
            }
            else if (node->matches("aten::diagflat(Tensor self, int offset) -> Tensor"))
            {
                return tensor_types.quarisma(0)->withDim(2);
            }
    ...
    bool PropagateCompleteShapeOnNode(
        Node* node, bool insert_expands, std::vector<TensorTypePtr> tensor_types)
    {
            ...
            node->matches("aten::mul(Tensor self, Scalar other) -> Tensor"))
        {
            ...
            {
                auto default_dtype = quarisma::typeMetaToScalarType(caffe2::get_default_dtype());
            ...
            if (quarisma::ScalarType::Bool == *first_scalar_type &&
                quarisma::ScalarType::Bool != *second_scalar_type)
            {
                auto result_type = quarisma::promoteTypes(*first_scalar_type, *second_scalar_type);
            ...
            insert_expands && (node->matches("aten::pow(Tensor self, Tensor exponent) -> Tensor") ||
                               ...
                               node->matches("aten::ne(Tensor self, Tensor other) -> Tensor")))
        {
            ...
            node->matches("aten::logit(Tensor self, float? eps = None) -> Tensor") ||
        ...
        {
            node->output()->setType(tensor_types.quarisma(0)->contiguous());
            return true;
        }
        else if (node->matches("aten::mm(Tensor self, Tensor mat2) -> Tensor"))
        {
            auto lhs_type  = tensor_types.quarisma(0);
            auto rhs_type  = tensor_types.quarisma(1);
            ...
            node->output()->setType(
                TensorType::createContiguous(
                    *lhs_type->scalarType(),
                    *lhs_type->device(),
                    quarisma::IntArrayRef{lhs_sizes[0], rhs_sizes[1]}));
            return true;
        }
        else if (node->matches("aten::t(Tensor self) -> Tensor"))
        {
            auto tp      = tensor_types.quarisma(0);
            auto sizes   = tp->sizes().concrete_sizes().value();
            auto strides = tp->strides().concrete_sizes().value();
            SHAPE_ASSERT(sizes.size() == 2);
            std::swap(sizes.quarisma(0), sizes.quarisma(1));
            std::swap(strides.quarisma(0), strides.quarisma(1));
        ...
        {
            node->output()->setType(tensor_types.quarisma(0)->withSizes({}));
            return true;
        }
        else if (node->matches(
                     ...
                     /*const_inputs=*/{attr::dim, attr::keepdim}))
        {
            auto& tp      = tensor_types.quarisma(0);
            auto  sizes   = tp->sizes().concrete_sizes().value();
            auto  dims    = node->get<quarisma::List<int64_t>>(attr::dim).value();
                     ...
                     /*const_inputs=*/attr::dim))
        {
            ...
            int64_t new_stride = dim >= static_cast<int64_t>(sizes.size())
                                     ? 1
                                     : sizes.quarisma(dim) * strides.quarisma(dim);
        ...
        {
            auto    sizes        = node->get<quarisma::List<int64_t>>(attr::size).value();
            bool    inferred     = false;
            ...
            if (inferred)
            {
                ...
                int64_t numel          = 1;
                auto    concrete_sizes = tensor_types.quarisma(0)->sizes().concrete_sizes().value();
                for (int64_t s : concrete_sizes)
                    numel *= s;
                int64_t inferred_size = numel / size_product;
                sizes[inferred_idx]   = inferred_size;
            }
            node->output()->setType(tensor_types.quarisma(0)->withSizes(sizes.vec()));
            return true;
        }
        else if (node->matches("aten::type_as(Tensor self, Tensor other) -> Tensor"))
        {
            if (tensor_types.quarisma(0)->scalarType() == tensor_types.quarisma(1)->scalarType())
            {
            ...
            else
            {
                ...
                node->output()->setType(tensor_types.quarisma(1)->withSizes(
                    tensor_types.quarisma(0)->sizes().concrete_sizes().value()));
            }
            return true;
        }
        else if (node->matches(
        ...
        {
            auto tp              = tensor_types.quarisma(0);
            auto sizesAndStrides = quarisma::inferExpandGeometry_dimvector(
                tp->sizes().concrete_sizes().value(),
                tp->strides().concrete_sizes().value(),
                node->get<quarisma::List<int64_t>>(attr::size).value().vec());
            node->output()->setType(
            ...
            return true;
                     ...
                     /*const_inputs=*/attr::dim))
        {
            auto    ten   = tensor_types.quarisma(0);
            auto    index = tensor_types.quarisma(1);
            int64_t dim   = node->get<int64_t>(attr::dim).value();
        ...
        {
            auto    input_type = tensor_types.quarisma(0);
        ...
        else if (node->kind() == ::quarisma::onnx::Reshape)
        {
            setUnshapedType(node);
            return true;
        }
...
TypePtr getOrCreateUnshapedType(const TypePtr& type, TypeCache& unshaped_type_cache);
...
TypePtr unshapedTypeImpl(TypePtr type, TypeCache& unshaped_type_cache)
{
    ...
    quarisma::ArrayRef<TypePtr> contained = type->containedTypes();
    if (contained.empty())
    {
        return type;
    }
...
void EraseShapeInformation(quarisma::ArrayRef<Value*> vals, TypeCache& unshaped_type_cache)
{
    for (Value* v : vals)
    {
