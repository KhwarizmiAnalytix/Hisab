...
template <typename T>
static void genericAddOptionalInput(Node* n, const char* name, const std::optional<T>& value)
{
    if (value)
    {
    ...
    if (var.isTensorList())
    {
        return graph
            ->insertNode(graph->createList(
                TensorType::get(),
                fmap(var.toTensorVector(), [&](const IValue& val) { return getValue(val); })))
            ->output();
    }
    else if (var.isTuple())
    {
...
Value* TracingState::getOutput(const IValue& iv, size_t i)
{
    ...
    else if (iv.isTensorList())
    {
        ...
        return graph
            ->insertNode(graph->createList(
                TensorType::get(),
                fmap(iv.toTensorVector(), [&](const IValue& ival) { return getOutput(ival, i); })))
            ->output();
    }
    else if (iv.isTuple())
    {
        ...
        auto        tuple_node =
            graph->createTuple(fmap(tuple, [&](const IValue& ival) { return getOutput(ival, i); }));
    ...
    else if (iv.isGenericDict())
    {
        ...
        bool key_type_valid =
            key_type->isSubtypeOf(*StringType::get()) || key_type->isSubtypeOf(*TensorType::get());
    ...
    Value*                               value)
{
    ...
    else if (auto dict_type = type->cast<DictType>())
    {
        ...
        for (const auto& entry : dict)
        {
            ...
            dict.insert_or_assign(
                entry.key(),
                addInput(state, entry.value(), dict_type->getValueType(), static_value));
        }
    ...
    {
        size_t num_elems =
            input.isList() ? input.toListRef().size() : input.toTensorVector().size();
        auto list_unpack =
            state->graph->insertNode(state->graph->createListUnpack(value, num_elems));
    ...
    const std::string&                   prefix)
{
    ...
    for (const NameValue& s : self.named_attributes(/*recurse=*/false))
    {
        ...
        if (attr_type->is_module() && attr_type->kind() != TypeKind::InterfaceType)
        {
            gatherParametersAndBuffers(state, trace_get_attr, Module(s.value.toObject()), qualname);
        }
    }
}
    ...
    const std::vector<std::string>&             argument_names)
{
    try
    {
        ...
        {
            Value* self_value =
                state->graph->insertInput(0, "self")->setType(self->_ivalue()->type());
        ...
        if (argument_names.size() >= inputs.size())
        {
            for (size_t i = 0, e = inputs.size(); i < e; ++i)
            {
                ...
                input =
                    addInput(state, input, input.type(), state->graph->addInput(argument_names[i]));
            }
        }
        else
        {
...
void addInputs(Node* n, const char* name, const std::optional<std::string_view>& value)
{
    detail::genericAddOptionalInput(n, name, value);
}
...
void addInputs(Node* n, const char* name, quarisma::ITensorListRef value, bool allow_undefined)
{
    ...
    if (allow_undefined)
    {
        ...
        list_node =
            g->insertNode(g->createList(OptionalType::ofTensor(), fmap(value, getValueTrace)));
    }
    else
    {
        list_node = g->insertNode(g->createList(TensorType::get(), fmap(value, getValueTrace)));
    }
    ...
    Node* n, const char* name, const List<std::optional<quarisma::Tensor>>& value)
{
    ...
    Node*  list_node = nullptr;
    list_node =
        g->insertNode(g->createList(OptionalType::ofTensor(), fmap(value, getOptTensorValueTrace)));
    n->addInput(list_node->output());
}
void addInputs(
    Node*                                                       n,
...
void addInputs(Node* n, const char* name, std::optional<quarisma::SymInt> value)
{
    addInputs(
        n,
        name,
        value.has_value() ? std::make_optional(value->guard_int(__FILE__, __LINE__))
                          : std::nullopt);
}
...
void ensureUniqueIfOutOfPlaced(const char* name, const quarisma::Tensor& tensor)
{
    ...
    if (isTracing() && aliases > 1)
    {
        ...
        ss << "There are " << aliases
           ...
           << "On the other hand, if all other views use the same memory chunk, but are disjoint "
              "(e.g. "
    ...
    const std::string& arg_name, size_t size, size_t idx, const Variable& var)
{
    ...
    if (!isTracing())
        return;
    IntArrayRefTrace& list_trace = stash.intlists.emplace(arg_name, size).first->second;
...
void ArgumentStash::stashValue(
    const std::string& arg_name, size_t idx, const Variable& var, const TypePtr& type)
{
    if (!isTracing())
        return;
...
static std::atomic<decltype(&defaultRecordSourceLocation)> record_source_location(
...
static std::atomic<decltype(&defaultPythonCallstack)> python_callstack_fn(defaultPythonCallstack);
