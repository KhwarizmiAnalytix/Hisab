    ...
    -> variable_list
{
    ...
    THPObjectPtr fwdInputMetadatas(PyTuple_New(static_cast<Py_ssize_t>(is_variable_input.size())));
    ...
    THPObjectPtr saved_tensors(
        unpack_saved_variables(py_fn, [](const Variable& var) { return THPVariable_Wrap(var); }));
...
void PyNode::compiled_args(CompiledNodeArgs& args) const
{
    ...
    auto prior = args.set_default_dyn_type(torch::dynamo::autograd::SizeInput::DYNAMIC);
...
variable_list PyNode::apply_with_saved(const variable_list& inputs, SwapSavedVariables& saved)
{
    auto f = (THPFunction*)obj;
    ...
    const variable_list& inputs, quarisma::OptionalDeviceGuard* device_guard)
{
    ...
    for (const auto i : quarisma::irange(num_inputs))
    {
        ...
        if (inputs[i].defined() || !py_fn->materialize_grads ||
            (input_metadata(i).was_default_constructed() && !py_fn->materialize_non_diff_grads))
        {
            input = THPVariable_Wrap(inputs[i]);
        }
        else
        {
            input = THPVariable_Wrap(zeros_without_gil(output_info[i], *device_guard));
        }
        if (!input)
            throw_python_error();
...
variable_list PyNode::to_variable_list(
    const PyObject* outputs, const std::vector<bool>& is_variable_input)
{
...
static PyObject* THPFunction_new(PyTypeObject* type, PyObject* args, PyObject* kwargs)
{
        ...
        else
        {
            if (is_executable)
            {
                ...
                bool is_differentiable =
                    (non_differentiable.count(wrapped_output->unsafeGetTensorImpl()) == 0 &&
    ...
    int64_t                                             num_outputs)
{
    ...
    for (const auto& opt_tensor : tensors_to_save)
    {
        ...
        {
            bool is_output = output_impls.count(opt_tensor.value().unsafeGetTensorImpl()) > 0;
...
std::pair<UnpackedInput, InputFlags> unpack_input(PyObject* args)
{
    ...
    for (const auto i : quarisma::irange(num_args))
    {
        ...
        if (!is_variable)
        {
            ...
            {
                THPUtils_setError("expected a Tensor argument, but got ", THPUtils_typename(arg));
                throw python_error();
            }
            Py_INCREF(Py_False);
    ...
    flags.next_edges =
        (flags.is_executable ? collect_next_edges(unpacked.input_vars) : edge_list());
    ...
    bool                            unpack_output)
{
    ...
    node->g_(
        torch::jit::attr::Subgraph, std::make_shared<torch::jit::Graph>(graph->current_scope()));
    ...
    for (it++; it != owning_block->nodes().end(); ++it)
    {
        ...
        for (size_t i = 0; i < node->outputs().size(); ++i)
        {
            ...
            auto trace_it =
                std::find(trace_outputs.begin(), trace_outputs.end(), node->outputs()[i]);
...
torch::jit::Node* _trace_pre_record(
    PyObject* op_obj, PyObject* input_objects, const variable_list& input_vars)
{
    ...
    bool                 unpack_output)
{
    ...
    if (PyObject* module_name =
            PyDict_GetItemString(((PyTypeObject*)op_obj)->tp_dict, "__module__"))
    {
    ...
    if (py::cast<bool>(is_in_onnx_export) && py::cast<bool>(is_autograd_inlining_enabled))
    {
    ...
    bool              overridden_setup_context)
{
    ...
    if (is_executable)
    {
        _save_variables(tensors_to_save, cdata, grad_fn, outputs.get(), num_outputs);
    }
    else
    {
...
PyObject* THPFunction_maybe_clear_saved_tensors(PyObject* self, PyObject* noargs)
{
    HANDLE_TH_ERRORS;
    auto cdata = ((THPFunction*)self)->cdata.lock();
...
THPObjectPtr make_ctx_input_tuple(
    THPFunction* ctx, const UnpackedInput& unpacked_input, int64_t num_args)
{
...
THPObjectPtr make_ctx_input_output_tuple(
    THPFunction* ctx, UnpackedInput& unpacked_input, PyObject* output)
{
...
int THPFunction_set_pure_view(THPFunction* self, PyObject* value, void* unused)
{
    HANDLE_TH_ERRORS
    if (!PyBool_Check(value))
    {
...
PyObject* THPFunction_get_materialize_non_diff_grads(THPFunction* self, void* _unused)
{
    HANDLE_TH_ERRORS
...
int THPFunction_set_materialize_non_diff_grads(THPFunction* self, PyObject* value, void* unused)
{
    HANDLE_TH_ERRORS
    if (!PyBool_Check(value))
    {
        THPUtils_invalidArguments(value, nullptr, "set_materialize_non_diff_grads", 1, "(bool)");
        return -1;
    }
...
PyObject* THPFunction_saved_variables(THPFunction* self, void* _unused)
{
    HANDLE_TH_ERRORS
    auto r = PyErr_WarnEx(
        PyExc_DeprecationWarning, "'saved_variables' is deprecated; use 'saved_tensors'", 0);
    if (r != 0)
        throw python_error();
    return unpack_saved_variables(self, [](const Variable& var) { return THPVariable_Wrap(var); });
    END_HANDLE_TH_ERRORS
}

PyObject* THPFunction_get_compiled_autograd_symints(PyObject* _self, PyObject* _unused)
{
    HANDLE_TH_ERRORS
    auto      self   = (THPFunction*)_self;
...
PyObject* THPFunction_get_compiled_autograd_backward_state(PyObject* _self, void* _unused)
{
    HANDLE_TH_ERRORS
    auto      self     = (THPFunction*)_self;
...
int THPFunction_set_compiled_autograd_backward_state(
    PyObject* _self, PyObject* bw_state, void* _unused)
{
    HANDLE_TH_ERRORS
    auto self = (THPFunction*)_self;
...
static struct PyGetSetDef THPFunction_properties[] = {
    ...
    {"_input_metadata", (getter)THPFunction_input_metadata, nullptr, nullptr, nullptr},
    {"materialize_grads", nullptr, (setter)THPFunction_set_materialize_grads, nullptr, nullptr},
    {"_is_pure_view", nullptr, (setter)THPFunction_set_pure_view, nullptr, nullptr},
