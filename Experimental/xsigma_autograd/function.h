...
public:
    ...
    uint32_t num_inputs() const noexcept { return input_metadata_.size(); }

    const InputMetadata& input_metadata(size_t index) const { return input_metadata_[index]; }

    // Danger: not thread safe, caller must protect with lock
    InputMetadata& mutable_input_metadata(size_t index) { return input_metadata_[index]; }
   ...
   */
    ...
    const Edge& next_edge(size_t index) const noexcept { return next_edges_[index]; }

    const edge_list& next_edges() const noexcept { return next_edges_; }

    edge_list& next_edges() noexcept { return next_edges_; }

    uint32_t num_outputs() const noexcept { return next_edges_.size(); }
    ...
    void set_sequence_nr(uint64_t sequence_nr) { sequence_nr_ = sequence_nr; }
    ...
    uint64_t thread_id() const noexcept { return thread_id_; }
    ...
    std::vector<std::unique_ptr<FunctionPostHook>>& post_hooks() noexcept { return post_hooks_; }
    ...
    void add_retains_grad_hook(std::unique_ptr<FunctionPreHook>&& pre_hook, size_t output_idx)
    {
    ...
    std::vector<std::unique_ptr<FunctionPreHook>>& pre_hooks() noexcept { return pre_hooks_; }

    virtual std::vector<std::unique_ptr<FunctionPreHook>>& tensor_pre_hooks() noexcept
    {
        return tensor_pre_hooks_;
    }

    virtual std::unique_ptr<PostAccumulateGradHook>& tensor_post_acc_grad_hooks() const noexcept
    {
        static std::unique_ptr<PostAccumulateGradHook> empty = nullptr;
        return empty;
    }

    std::unordered_map<size_t, std::unique_ptr<FunctionPreHook>>& retains_grad_hooks() noexcept
    {
    ...
    virtual bool is_traceable() { return false; }
    ...
    virtual bool passes_state_transparently() { return false; }
    ...
    {
        TORCH_CHECK_NOT_IMPLEMENTED(false, std::string("compiled_args not implemented: ") + name());
    }
    ...
    virtual variable_list apply_with_saved(const variable_list& inputs, SwapSavedVariables& saved)
    {
    ...
    virtual bool is_aot_backward() const { return false; }
...
{
    using Node::Node;
    bool is_traceable() final { return true; }
};
...
struct MakeNextFunctionList : IterArgs<MakeNextFunctionList>
{
    ...
    void operator()(const Variable* variable) { operator()(*variable); }
...
inline void create_gradient_edge(Variable& variable, std::shared_ptr<Node> function)
{
    // Copy before move.
...
inline bool any_variable_requires_grad(const variable_list& variables)
{
    return std::any_of(
        variables.begin(),
        variables.end(),
        [](const Variable& variable) { return variable.defined() && variable.requires_grad(); });
}
