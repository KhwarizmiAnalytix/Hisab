...
struct GraphTask : std::enable_shared_from_this<GraphTask>
{
    ...
    struct ExecInfo
    {
        struct Capture
        {
            ...
            void DO_NOT_USE_DEPRECATED_register_capture_hook(std::unique_ptr<GradCaptureHook> hook)
            {
    ...
    bool can_checkpoint() const { return exec_info_.empty(); }
...
TORCH_API const std::unordered_map<Node*, GraphTask::ExecInfo>* get_current_graph_task_exec_info();
TORCH_API const std::unordered_set<Node*>* get_current_graph_task_nodes_in_graph();
