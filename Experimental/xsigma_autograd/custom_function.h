...
struct CppNode : public Node
{
    ...
    void compiled_args(CompiledNodeArgs& args) const override
    {
        ...
        args.collect(ctx_.saved_variables_, true);  // always unpacked as output in eager
    ...
    variable_list apply_with_saved(const variable_list& inputs, SwapSavedVariables& saved) override
    {
        ...
        static_assert(std::is_same_v<std::remove_cv_t<decltype(T::is_traceable)>, bool>);
        ...
        auto results = pyinterface->call_function(
            saved.get_py_compiler(), "apply_functional", fn_name, inputs, args, output_metadata);
