...
{
    auto* metadata =
        static_cast<PyAnomalyMetadata*>(reinterpret_cast<THPCppFunction*>(self)->cdata->metadata())
            ->dict();
...
static PyObject* THPCppFunction_set_sequence_nr(PyObject* self, PyObject* sequence_nr)
{
    HANDLE_TH_ERRORS
    auto& fn = *((THPCppFunction*)self)->cdata;
...
static struct PyMethodDef default_methods[] = {THP_FUNCTION_DEFAULT_METHODS, {nullptr}};

// NOLINTNEXTLINE(cppcoreguidelines-avoid-c-arrays,cppcoreguidelines-avoid-non-const-global-variables,modernize-avoid-c-arrays)
static struct PyGetSetDef default_properties[] = {THP_FUNCTION_DEFAULT_PROPERTIES, {nullptr}};
...
void registerCppFunction(const std::type_info& type, PyTypeObject* pytype)
{
    ...
    cpp_function_types_map[std::type_index(type)] = THPObjectPtr((PyObject*)pytype);
...
bool THPCppFunction_Check(PyObject* obj)
{
    ...
    if (cpp_function_types_set.find((PyTypeObject*)type.get()) == cpp_function_types_set.end())
    {
        return false;
    }
    else
