...
static void recordSliceTrace(PyObject* obj)
{
    ...
    {
        torch::jit::tracer::ArgumentStash::stashValue(
            std::string("end"), 1, THPVariable_Unpack(sliceobj->stop), torch::jit::IntType::get());
    }
    if (THPVariable_Check(sliceobj->step))
    {
        torch::jit::tracer::ArgumentStash::stashValue(
            std::string("step"), 1, THPVariable_Unpack(sliceobj->step), torch::jit::IntType::get());
    }
}
...
int THPVariable_setitem(PyObject* self, PyObject* index, PyObject* py_value)
{
    ...
    if ((check_has_torch_function(self)) || (check_has_torch_function(py_value)))
    {
    ...
    const auto& self_ = THPVariable_Unpack(self);
    if (self_.layout() == kSparse || self_.layout() == kSparseCsr || self_.layout() == kSparseCsc ||
        self_.layout() == kSparseBsr || self_.layout() == kSparseBsc)
    {
