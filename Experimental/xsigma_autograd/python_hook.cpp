...
PyFunctionTensorPreHook::PyFunctionTensorPreHook(PyObject* dict, size_t value_idx)
...
{
    pybind11::gil_scoped_acquire gil;
    THPObjectPtr                 value(THPVariable_Wrap(values.quarisma(value_idx)));
    if (!value)
        throw python_error();
...
auto PyFunctionPreHook::operator()(const variable_list& grad_outputs_) -> variable_list
{
    pybind11::gil_scoped_acquire gil;
...
void PyFunctionTensorPreHook::compiled_args(CompiledNodeArgs& args) const
{
    ...
    while (PyDict_Next(dict, &pos, &key, &value))
    {
        ...
        args.add_tensor_pre_hook(
            quarisma::SafePyObject(value, getPyInterpreter()), static_cast<int>(value_idx));
    }
...
void PyFunctionPreHook::compiled_args(CompiledNodeArgs& args) const
{
    ...
    {
        Py_INCREF(value);
        args.add_pre_hook(quarisma::SafePyObject(value, getPyInterpreter()));
    }
    Py_END_CRITICAL_SECTION();
}
...
void PyFunctionPostHook::compiled_args(CompiledNodeArgs& args) const
{
    ...
    {
        Py_INCREF(value);
        args.add_post_hook(quarisma::SafePyObject(value, getPyInterpreter()));
    }
    Py_END_CRITICAL_SECTION();
}

PyFunctionTensorPostAccGradHooks::PyFunctionTensorPostAccGradHooks(PyObject* dict) : dict(dict)
{
    Py_INCREF(dict);
}
...
auto PyFunctionTensorPostAccGradHooks::operator()(const Variable& tensor) -> void
{
    pybind11::gil_scoped_acquire gil;
    THPObjectPtr                 tup(PyTuple_New(1));
    PyTuple_SET_ITEM(tup.get(), 0, THPVariable_Wrap(tensor));
    bool returned_none = !_call_hooks(dict, tup.get());
    QUARISMA_CHECK(returned_none, "Tensor post accumulate grad hooks should return None.");
}
    ...
    torch::dynamo::autograd::CompiledNodeArgs& args) const
{
    ...
    {
        Py_INCREF(value);
        quarisma::SafePyObject hook_obj(value, getPyInterpreter());
...
void PyFunctionTensorPostAccGradHooks::apply_with_saved(
    Variable& tensor, torch::dynamo::autograd::SwapSavedVariables& saved)
{
    for (const auto hook : saved.get_curr_node_call().post_acc_grad_hooks)
    {
        THPObjectPtr py_var(THPVariable_Wrap(tensor));
        PyObject_CallMethod(
            saved.get_py_compiler(), "post_acc_grad_hook", "Oi", py_var.get(), hook);
    }
}
...
static PyObject* wrap_variables(const variable_list& c_variables)
{
    ...
    if (!tuple)
        throw python_error();
    for (const auto i : quarisma::irange(num_vars))
    {
...
static variable_list unwrap_variables(PyObject* py_variables)
{
    ...
    for (const auto i : quarisma::irange(results.size()))
    {
        ...
        else
        {
            ...
            QUARISMA_CHECK(false, "expected variable but got ", Py_TYPE(item)->tp_name);
        }
    }
    return results;
}
...
static void check_result(PyObject* prev, PyObject* result, PyObject* hook)
{
    ...
    {
        PyErr_Format(
            PyExc_TypeError, "expected tuple, but hook returned '%s'", THPUtils_typename(result));
        throw python_error();
    }
    ...
    for (const auto i : quarisma::irange(prev_size))
    {
...
static void check_single_result(PyObject* _original, PyObject* _result, PyObject* hook)
{
    ...
    QUARISMA_CHECK(_original != Py_None, "can't replace a None gradient with a non-None value");
