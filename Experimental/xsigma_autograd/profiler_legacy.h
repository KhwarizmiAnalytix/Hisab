...
struct TORCH_API LegacyEvent
{
    ...
    // serialization.
    quarisma::IValue toIValue() const;

    // Reconstructs an event from IValues given by toIValue.
    static LegacyEvent fromIValue(const quarisma::IValue& eventIValue);
    ...
    std::string kindStr() const
    {
        ...
        QUARISMA_CHECK(false, "unknown event kind");
    }
    ...
    uint64_t threadId() const { return thread_id_; }
    ...
    void setCpuUs(int64_t cpu_us) { cpu_ns_ = cpu_us * 1000; }

    double cpuUs() const { return static_cast<double>(cpu_ns_) / (1000.0); }

    double cudaElapsedUs(const LegacyEvent& e) const;

    bool hasCuda() const { return cuda_event != nullptr || (isRemote() && device_ != -1); }

    quarisma::DeviceIndex device() const { return device_; }

    void updateMemoryStats(int64_t alloc_size, quarisma::Device device)
    {
        if (device.is_cuda() || device.type() == quarisma::DeviceType::HIP)
        {
            cuda_memory_usage_ = alloc_size;
        }
        else if (
            device.is_cpu() || device.type() == quarisma::DeviceType::MKLDNN ||
            device.type() == quarisma::DeviceType::IDEEP)
        {
    ...
    int64_t cpuMemoryUsage() const { return cpu_memory_usage_; }

    int64_t cudaMemoryUsage() const { return cuda_memory_usage_; }

    quarisma::RecordFunctionHandle handle() const { return handle_; }
    ...
    void setName(quarisma::StringView newName_) { name_ = std::move(newName_); }

    bool isRemote() const { return is_remote_; }

    void setCudaUs(int64_t cuda_us) { cuda_us_ = cuda_us; }

    void setSequenceNr(int64_t sequence_nr) { sequence_nr_ = sequence_nr; }

    int64_t sequenceNr() const { return sequence_nr_; }

    void setCorrelationId(uint64_t correlation_id) { correlation_id_ = correlation_id; }

    uint64_t correlationId() const { return correlation_id_; }

    const std::vector<std::string>& stack() const { return stack_; }

    void setStack(const std::vector<std::string>& stack) { stack_ = stack; }

    uint64_t fwdThreadId() const { return fwd_thread_id_; }

    void setFwdThreadId(uint64_t fwd_thread_id) { fwd_thread_id_ = fwd_thread_id; }

    uint8_t scope() const { return scope_; }
    ...
    uint64_t flops() { return flops_; }

    bool isAsync() { return is_async_; }

    void setFlops(uint64_t flops) { flops_ = flops; }
...
{
    RangeEventList() { events_.reserve(kReservedCapacity); }
...
struct TORCH_API ProfilerDisableOptions
{
    ...
    ProfilerDisableOptions(bool shouldCleanupTLSState, bool shouldConsolidate)
        : cleanupTLSState(shouldCleanupTLSState), consolidate(shouldConsolidate)
    {
    }
...
TORCH_API void writeProfilerEventsToStream(
    std::ostream& out, const std::vector<LegacyEvent*>& events);
