...
void _foreach_tensor(F fn, torch::jit::Stack* stack, size_t stack_start, size_t size)
{
    ...
    for (const auto idx_arg : quarisma::irange(size))
    {
        ...
        if (ivalue.isTensor())
        {  // true for optional tensor that has value
...
{
    WarnNotImplemented(std::string op_name, size_t num_outputs, edge_list&& next_edges)
    ...
    torch::jit::Stack*              stack)
{
    ...
    if (any_input_requires_grad)
    {
        ...
        grad_fn = std::shared_ptr<WarnNotImplemented>(
            new WarnNotImplemented(op_name, all_tensors_on_stack.size()), deleteNode);
        ...
        if (schema.is_aliasing({quarisma::SchemaArgType::output, i}))
        {
            ...
            else
            {
                QUARISMA_CHECK(
                    ...
                    "Non-composite functions where multiple outputs are aliased with inputs aren't "
                    "supported."
    ...
    for (const auto i : quarisma::irange(num_arguments))
    {
            ...
            !schema.is_mutable({quarisma::SchemaArgType::input, i}))
        {
            QUARISMA_CHECK(
                ...
                "Non-composite functions where multiple inputs are aliased with outputs aren't "
                "supported. "
                "Please rewrite your function as a composite function.");
    ...
    torch::jit::Stack*              stack)
{
    ...
    if (is_view)
    {
        ...
        else
        {
            ...
            auto view_func =
                std::make_unique<GenericViewFunc>(non_tensor_stack, aliased_input_idx_val, op);
