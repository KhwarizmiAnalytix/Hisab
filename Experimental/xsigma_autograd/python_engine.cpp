...
quarisma::intrusive_ptr<quarisma::ivalue::Future> PythonEngine::execute_with_graph_task(
    const std::shared_ptr<GraphTask>& graph_task,
...
static PyObject* THPEngine_run_backward(PyObject* self, PyObject* args, PyObject* kwargs)
{
    HANDLE_TH_ERRORS
    ...
    QUARISMA_CHECK(
        !backward_api_called || quarisma::impl::VmapMode::current_vmap_level() == 0,
    ...
    for (const auto i : quarisma::irange(num_tensors))
    {
        PyObject* _tensor = PyTuple_GET_ITEM(tensors, i);
        Edge      gradient_edge;  // Temporary variable to hold the gradient edge
        std::optional<quarisma::Tensor> mb_output;
    ...
    if (inputs != nullptr)
    {
        QUARISMA_CHECK(PyTuple_CheckExact(inputs), "inputs to run_backward must be a tuple");
    ...
    variable_list outputs;
    {
        ...
        outputs =
            engine.execute(roots, grads, keep_graph, create_graph, accumulate_grad, output_edges);
    }
...
static PyObject* THPEngine_queue_callback(PyObject* self, PyObject* _callback)
{
    HANDLE_TH_ERRORS
    auto&                     engine = python::PythonEngine::get_python_engine();
    std::shared_ptr<PyObject> callback(
        _callback,
        [](PyObject* obj)
        {
            pybind11::gil_scoped_acquire gil;
...
static PyObject* THPEngine_is_checkpoint_valid(PyObject* self, PyObject* noargs)
{
    HANDLE_TH_ERRORS
    auto& engine = python::PythonEngine::get_python_engine();
...
static PyObject* THPEngine_new(PyTypeObject* type, PyObject* args, PyObject* kwargs)
{
...
static struct PyMethodDef THPEngine_methods[] = {
    ...
    {"is_checkpoint_valid", THPEngine_is_checkpoint_valid, METH_NOARGS, nullptr},
    {nullptr}};
