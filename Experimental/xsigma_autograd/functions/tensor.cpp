    ...
    const quarisma::TensorOptions& src_options)
{
    ...
    if (grad.defined())
    {
        ...
        {
            grad_inputs[0] = quarisma::zeros_like(grad, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
        }
        if (needs_input_grad[1])
        {
            // Handle R->C copies without raising a warning
            const auto src_type = src_options.dtype().toScalarType();
            if (!quarisma::isComplexType(src_type) && grad.is_complex())
            {
                grad = quarisma::real(grad);
            }

            quarisma::DeviceGuard device_guard(src_options.device());
    ...
    const variable_list& grads, const ivalue_list& args)
{
    ...
    auto       src_options      = r.unpack<quarisma::TensorOptions>();
...
variable_list CopyBackwards::apply_with_saved(
    const variable_list& inputs, SwapSavedVariables& saved)
{
    ...
    static bool flag [[maybe_unused]] = [&]()
    {
        std::vector<quarisma::TypePtr> schema = {
            ...
            IValuePacker<quarisma::TensorOptions>::packed_type()};
    ...
    auto output_metadata =
        torch::dynamo::autograd::IValuePacker<std::vector<std::optional<InputMetadata>>>::pack(
    ...
    : base(base_var), view(std::move(view_)), view_fn(std::move(view_fn_)), fn(std::move(fn_))
{
    ...
    for (const auto i : quarisma::irange(1, num_outputs))
    {
...
void CopySlices::update_exec_info()
{
        ...
        if (fn_edge.is_valid())
        {
            ...
            if (it == exec_info->end())
            {
                ...
                if (task_should_compute_output(0))
                {
                    ...
                    // look quarisma fn->next_edge(0) metadata and thus there is no need to
                    // clean them up.
                }
            }
            else
            {
                TORCH_INTERNAL_ASSERT(it->second.should_execute() == task_should_compute_output(0));
            }
        }
    }
    ...
    for (const auto i : quarisma::irange(1, this->num_outputs()))
    {
...
template <typename T>
inline variable_list CopySlices::apply_impl(variable_list&& inputs, const T& call_fn)
{
    ...
    else
    {
        ...
        grad_slice  = result.as_strided_symint(view.sym_sizes(), view.sym_strides(), offset);
    }
    ...
    auto res = call_fn({grad_slice.clone(quarisma::MemoryFormat::Contiguous)});

    variable_list grad_inputs(num_outputs());
    for (const auto i : quarisma::irange(res.size()))
    {
...
{
    QUARISMA_CHECK(!view_fn, "view_fn not supported by compiled autograd")
...
variable_list CopySlices::apply_with_saved(const variable_list& grads, SwapSavedVariables& saved)
{
    saved.before(base);
    saved.before(view);
    ...
    if (grads[0].defined())
    {
        ...
        const auto&   interface = torch::dynamo::autograd::getPyCompilerInterface();
        variable_list stuff =
            interface->call_copy_slices_prologue(saved.get_py_compiler(), grads, base, view);
...
{
    return apply_impl(
        std::move(inputs1), [this](variable_list&& inputs2) { return (*fn)(std::move(inputs2)); });
}
