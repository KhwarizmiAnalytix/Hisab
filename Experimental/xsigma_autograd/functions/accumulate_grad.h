...
struct TORCH_API AccumulateGrad : public Node
{
    ...
    std::vector<std::unique_ptr<FunctionPreHook>>& tensor_pre_hooks() noexcept override
    {
    ...
    std::unique_ptr<PostAccumulateGradHook>& tensor_post_acc_grad_hooks() const noexcept override
    {
        // NB: Since the AccumulateGrad Node is only a weak ref from the Tensor,
        //     it can be destroyed even though the Tensor is still alive (contrary
        //     to all other Nodes). So we must lazily read the Tensor hooks here.
        ...
        const T&                update_grad)
    {
        if (!variable_grad.defined())
        {
                ...
                (new_grad.is_mkldnn() || utils::obeys_layout_contract(new_grad, variable)))
            {
                ...
                new_grad._indices().is_contiguous() && new_grad._values().is_contiguous() &&
                ...
                new_grad._indices().use_count() <= 1 && new_grad._values().use_count() <= 1 &&
