    ...
    const variable_list& grads, const ivalue_list& args)
{
    ...
    // called
    if (variable.grad_fn() && std::dynamic_pointer_cast<Error>(variable.grad_fn()) == nullptr)
    {
...
variable_list AccumulateGrad::apply_with_saved(
    const variable_list& grads, SwapSavedVariables& saved)
{
    if (!(variable.defined() && variable.requires_grad()) || !grads[0].defined())
    {
        return variable_list();
    }
    ...
    static bool flag [[maybe_unused]] = [&]()
    {
        ...
        interface->bind_function(
            saved.get_py_compiler(), name, AccumulateGrad_apply_functional_no_hooks_ivalue, schema);
        return true;
    }();
