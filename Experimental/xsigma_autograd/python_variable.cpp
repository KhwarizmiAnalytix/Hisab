    ...
    const quarisma::OperatorHandle& op, const std::vector<quarisma::IValue>& arguments)
{
    ...
    {
        const auto& arg   = schema.arguments()[idx];
        auto        match = [&](quarisma::TypeKind kind)
        {
            const auto& t = arg.real_type();
            if (t->kind() == kind)
                return true;
            if (auto opt_t = t->cast<quarisma::OptionalType>())
            {
        ...
        else if (match(quarisma::ScalarTypeType::Kind))
        {
            auto* obj = getTHPDtype(static_cast<quarisma::ScalarType>(arguments[idx].toInt()));
            return py::reinterpret_borrow<py::object>(reinterpret_cast<PyObject*>(obj));
        }
        else if (match(quarisma::LayoutType::Kind))
        {
            auto* obj = getTHPLayout(static_cast<quarisma::Layout>(arguments[idx].toInt()));
            return py::reinterpret_borrow<py::object>(reinterpret_cast<PyObject*>(obj));
        }
        else if (match(quarisma::MemoryFormatType::Kind))
        {
            return py::cast(static_cast<quarisma::MemoryFormat>(arguments[idx].toInt()));
        }
        else
        {
    ...
    for (const auto idx : quarisma::irange(positional_default_start))
    {
        PyTuple_SET_ITEM(args.ptr(), idx, schemaAwareToPyObject(idx).release().ptr());
    }

    // Populate keyword arguments
    for (const auto idx : quarisma::irange(kwarg_only_start, arguments.size()))
    {
...
{
    QUARISMA_CHECK(PyGILState_Check(), "GIL must be held before you call pushPyOutToStack");
    ...
    {
        torch::jit::push(stack, torch::jit::toIValue(out.ptr(), schema_returns[0].real_type()));
    }
    else
    {
        auto outs = py::cast<py::sequence>(out);
        for (const auto idx : quarisma::irange(outs.size()))
        {
            torch::jit::push(
                stack, torch::jit::toIValue(outs[idx].ptr(), schema_returns[idx].real_type()));
        }
    }
}
...
quarisma::TensorImpl::SizesStridesPolicy parseSizesStridesPolicyArgument(std::string_view arg)
{
    if (arg == "strides")
    {
        return quarisma::TensorImpl::SizesStridesPolicy::CustomStrides;
    }

    if (arg == "sizes")
    {
        return quarisma::TensorImpl::SizesStridesPolicy::CustomSizes;
    }

    TORCH_CHECK_VALUE(
        false, "Unknown sizes_strides_policy: ", arg, "; expected 'strides' or 'sizes'");
}
}  // anonymous namespace
...
static PyObject*
    device_to_py_class_[static_cast<size_t>(quarisma::DeviceType::COMPILE_TIME_MAX_DEVICE_TYPES)];

void registerPythonTensorClass(const std::string& device, PyObject* python_tensor_class)
{
    quarisma::Device dev(device);

    QUARISMA_CHECK(dev.type() == kXLA, "Only the python class for XLA can be overridden");
...
static PyObject* getPythonTensorClass(quarisma::Device d)
{
...
{
    quarisma::impl::GPUTrace::set_trace(getPyInterpreter());
}

PyObject* THPVariable_Wrap(const quarisma::TensorBase& var)
{
    if (!var.defined())
    {
        Py_RETURN_NONE;
    ...
    if (quarisma::impl::HermeticPyObjectTLS::get_state())
    {
        return THPVariable_NewWithVar((PyTypeObject*)THPVariableClass, var);
    }

    std::optional<PyObject*> mb_obj = var.unsafeGetTensorImpl()->pyobj_slot()->check_pyobj(
    ...
    if QUARISMA_LIKELY (var.device().type() != quarisma::kXLA)
    {
        return THPVariable_NewWithVar((PyTypeObject*)THPVariableClass, var);
    }
...
static bool THPVariable_tryResurrect(THPVariable* self)
{
    ...
    TORCH_INTERNAL_ASSERT(!quarisma::impl::HermeticPyObjectTLS::get_state());
...
{
    TORCH_INTERNAL_ASSERT(false, "TensorBase tp_traverse function was not overridden properly");
    return 0;
}

static int THPFake_clear(THPVariable* self)
{
    TORCH_INTERNAL_ASSERT(false, "TensorBase tp_clear function was not overridden properly");
    return 0;
}
...
template <typename T>
static std::vector<T> map_py_func(const py::function& func, const std::vector<T>& items)
{
...
template <>
std::vector<quarisma::Tensor> map_py_func(
    const py::function& func, const std::vector<quarisma::Tensor>& items)
{
    std::vector<quarisma::Tensor> new_items;
    new_items.reserve(items.size());
    for (auto& item : items)
    {
        ...
        {
            new_items.emplace_back(py::cast<quarisma::Tensor>(output));
        }
    }
    return new_items;
}
...
static PyObject* view_func_impl(
    PyObject* _self, PyObject* args, PyObject* kwargs, bool check_has_same_meta)
{
    HANDLE_TH_ERRORS
    const auto& self = THPVariable_Unpack(_self);

    static PythonArgParser parser({
        "_view_func(Tensor new_base, PyObject* symint_visitor_fn=None, PyObject* "
        "tensor_visitor_fn=None)",
    });
    ...
    if (diff_view_meta && diff_view_meta->has_bw_view())
    {
            ...
            torch::autograd::utils::has_same_meta(new_base, view_info.base_))
        {
            ...
            if (view_info.has_view_fn())
            {
                ...
                std::optional<std::vector<quarisma::SymInt>> new_symints = std::nullopt;
                if (symint_visitor_fn != Py_None)
                {
                    new_symints = map_py_func(
                        py::cast<py::function>(symint_visitor_fn), view_func.get_symints());
                }

                std::optional<std::vector<quarisma::Tensor>> new_tensors = std::nullopt;
                if (tensor_visitor_fn != Py_None)
                {
                    new_tensors = map_py_func(
                        py::cast<py::function>(tensor_visitor_fn), view_func.get_tensors());
                }
            ...
            {
                out = new_base.as_strided(self.sizes(), self.strides(), self.storage_offset());
            }
        }
    }
...
static PyObject* THPVariable_view_func(PyObject* self_, PyObject* args, PyObject* kwargs)
{
    return view_func_impl(self_, args, kwargs, /*check_has_same_meta=*/true);
}

static PyObject* THPVariable_view_func_unsafe(PyObject* self_, PyObject* args, PyObject* kwargs)
{
...
static PyObject* rev_view_func_impl(PyObject* self_, PyObject* arg)
{
    ...
    QUARISMA_CHECK(
        THPVariable_Check(arg), "_rev_view_func expect a single argument that is a Tensor");
    ...
    if (diff_view_meta && diff_view_meta->has_bw_view())
    {
        const auto& view_info = diff_view_meta->get_backward_view();
        // Do the actual view replay
        QUARISMA_CHECK(view_info.has_view_fn(), "No _rev_view_func() found");
...
static PyObject* THPVariable_rev_view_func_unsafe(PyObject* self_, PyObject* arg)
{
...
static PyObject* THPVariable_make_subclass(PyObject* _ignored, PyObject* args, PyObject* kwargs)
{
    HANDLE_TH_ERRORS
    static PythonArgParser parser({
        "_make_subclass(PyObject* cls, Tensor data, bool require_grad=False, *, std::string_view? "
        "dispatch_sizes_strides_policy=None, bool dispatch_device=False, bool "
        "dispatch_layout=False, Device? device_for_backend_keys=None)",
    });
    ...
    std::optional<DispatchKeySet>          extra_dispatch_keys)
{
    ...
    {
        size_bytes = quarisma::detail::computeStorageNbytes(
            sym_sizes, sym_strides.value(), dtype_itemsize, sym_storage_offset.value_or(0));
    }
    else
    {
        size_bytes = quarisma::detail::computeStorageNbytesContiguous(
    ...
    Storage storage{
        ...
        size_bytes,
        quarisma::DataPtr{nullptr, options.device()},
        /*allocator=*/quarisma::GetAllocator(quarisma::kMeta),
        /*resizable=*/true};

    auto keys = quarisma::DispatchKeySet({options.computeDispatchKey()});
    if (extra_dispatch_keys.has_value())
    {
        keys = keys | *extra_dispatch_keys;
    }
    Tensor tensor =
        quarisma::detail::make_tensor<TensorImpl>(std::move(storage), keys, options.dtype());

    TensorImpl* tensor_impl = tensor.unsafeGetTensorImpl();

    if (sym_strides.has_value())
    {
        tensor_impl->set_sizes_and_strides(sym_sizes, sym_strides.value(), sym_storage_offset);
    }
    else
    {
        QUARISMA_CHECK(
            !sym_storage_offset.has_value(), "setting storage offset without stride not supported");
    ...
    PyObject* /*unused*/, PyObject* args, PyObject* kwargs)
{
    ...
    static PythonArgParser parser({
        ...
        "Layout layout=torch.strided, Device device=None, bool pin_memory=False, bool "
        "requires_grad=False, "
        "std::string_view? dispatch_sizes_strides_policy=None, bool dispatch_device=False, bool "
    ...
    TORCH_CHECK_TYPE(
        attr.ptr() != nullptr && attr.ptr() != torch::disabled_torch_dispatch_impl(),
...
static quarisma::SymDimVector tuple_to_symintlist(PyObject* obj)
{
    ...
    for (const auto idx : quarisma::irange(size))
    {
        ...
        {
            res.push_back(py::handle(item).cast<quarisma::SymInt>());
        }
        else
        {
...
// overhead.
static PyObject* THPVariable_dtensor_new(PyObject* /*unused*/, PyObject* args, PyObject* kwargs)
{
    HANDLE_TH_ERRORS
    static PythonArgParser parser({
    ...
    if (tensor_keys.has(quarisma::DispatchKey::Conjugate))
    {
        extra_dispatch_keys = extra_dispatch_keys.add(quarisma::DispatchKey::Conjugate);
    }
    if (tensor_keys.has(quarisma::DispatchKey::Negative))
    {
        extra_dispatch_keys = extra_dispatch_keys.add(quarisma::DispatchKey::Negative);
    }

    py::handle spec        = py::handle(r.pyobject(2));
    const auto tensor_meta = spec.attr(dtensor_interned_strings.tensor_meta);
    QUARISMA_CHECK(!tensor_meta.is_none());
    const auto sizes = tensor_meta.attr(dtensor_interned_strings.shape);
    QUARISMA_CHECK(PyTuple_Check(sizes.ptr()), "spec.tensor_meta.shape must be a tuple");
    const auto stride = tensor_meta.attr(dtensor_interned_strings.stride);
    QUARISMA_CHECK(PyTuple_Check(stride.ptr()), "spec.tensor_meta.stride must be a tuple");
...
static bool DTensor_OpSchema_recompute_comparison_key_impl(
    PyObject* self, const py::tuple& args_schema)
{
    py::object       static_kwargkey;
    size_t           static_argnum = 0;
    const py::handle self_handle   = py::handle(self);
    const py::handle schema_info   = self_handle.attr(dtensor_interned_strings.schema_info);
    ...
    {
        static_argnum = py::cast<size_t>(schema_info.attr(dtensor_interned_strings.static_argnum));
    ...
    for (const auto& e : args_schema)
    {
        if (idx >= static_argnum || arg_type_tensor_or_tensor_list_like(e))
        {
            ...
            {
                args_to_hash.push_back(py::reinterpret_steal<py::object>(PyList_AsTuple(e.ptr())));
            }
            else
            {
    ...
    for (const auto idx : quarisma::irange(args_to_hash.size()))
    {
    ...
    if (!static_kwargkey.is_none())
    {
        ...
        {
            PyErr_SetString(PyExc_TypeError, "self.schema_info.static_kwargkey must be a list!");
            return false;
        }
        py::list static_kwargkey_list = py::reinterpret_borrow<py::list>(static_kwargkey);
        auto     raw_kwargs_schema    = self_handle.attr(dtensor_interned_strings.kwargs_schema);
...
static PyObject* DTensor_OpSchema_recompute_comparison_key(PyObject* mod, PyObject* self)
{
    HANDLE_TH_ERRORS
    const py::handle self_handle     = py::handle(self);
    const py::handle raw_args_schema = self_handle.attr(dtensor_interned_strings.args_schema);
...
static PyObject* DTensor_OpSchema_post_init(PyObject* mod, PyObject* self)
{
    ...
    {
        PyErr_SetString(
            PyExc_TypeError, "DTensor_OpSchema_post_init requires self.args_schema to be a tuple!");
        return nullptr;
    }
    py::tuple args_schema = py::reinterpret_borrow<py::tuple>(raw_args_schema);
    if (!DTensor_OpSchema_recompute_comparison_key_impl(self, args_schema))
...
static PyObject* THPVariable_get_python_dispatch(THPVariable* self, void* unused)
{
    HANDLE_TH_ERRORS
    const auto& var = THPVariable_Unpack(self);
...
//   - This function calls the relevant Quarisma on the tensor
template <typename T>
...
struct PropertyReal : GetterBase<PropertyReal>
{
    ...
    static Tensor                fn(const Tensor& t) { return quarisma::real(t); }
};

struct PropertyImag : GetterBase<PropertyImag>
{
    static constexpr const char* name = "imag";
    static Tensor                fn(const Tensor& t) { return quarisma::imag(t); }
};
...
static int THPVariable_set_grad_fn(THPVariable* self, PyObject* obj, void* unused)
{
    ...
    QUARISMA_CHECK(obj, "Deletion of _grad_fn not allowed. Detach tensor instead!");
    QUARISMA_CHECK(obj == Py_None, "_grad_fn can be only set to None");
...
static int THPVariable_set_data(THPVariable* self, PyObject* data, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
    ...
    QUARISMA_CHECK(data, "Deleting tensor data is not allowed. Delete tensor instead!");
    TORCH_CHECK_TYPE(
...
static int THPVariable_set_grad(THPVariable* self, PyObject* py_grad, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
    ...
    QUARISMA_CHECK(self != (THPVariable*)py_grad, "can't assign Variable as its own grad");
    ...
    if (grad.layout() != kSparse)
    {
        auto expected_options = var.options().dtype(
            var.grad_dtype().has_value() ? var.grad_dtype().value() : grad.scalar_type());
        QUARISMA_CHECK(
...
static int THPVariable_set_volatile(THPVariable* self, PyObject* obj, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
...
static PyObject* THPVariable_get_requires_grad(THPVariable* self, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
...
static PyObject* THPVariable_get_names(PyObject* self, void* unused)
{
    ...
    for (const auto i : quarisma::irange(size))
    {
        ...
        if (dimnames[i].type() == quarisma::NameType::WILDCARD)
        {
...
static int THPVariable_set_names(PyObject* self, PyObject* names, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function(self))
    {
    ...
    if (names == Py_None)
    {
        quarisma::internal_set_names_inplace(var, std::nullopt);
    }
    else
    {
        QUARISMA_CHECK(
            THPUtils_checkDimnameList(names), "names must either be None or a tuple of dim names");
        quarisma::internal_set_names_inplace(var, torch::parseDimnameList(names));
    }
    return 0;
    END_HANDLE_TH_ERRORS_RET(-1)
}

static int THPVariable_set_requires_grad(THPVariable* self, PyObject* obj, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
    ...
    QUARISMA_CHECK(obj && PyBool_Check(obj), "requires_grad must be a bool");
    const auto& var           = THPVariable_Unpack(self);
    ...
    if (requires_grad && !isDifferentiableType(quarisma::typeMetaToScalarType((var.dtype()))))
    {
...
static PyObject* THPVariable_get_backwards_hooks(THPVariable* self, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
...
static int THPVariable_set_backwards_hooks(THPVariable* self, PyObject* obj, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
    ...
    QUARISMA_CHECK(obj, "Deletion of _backwards_hooks not allowed!");
    if (obj == Py_None)
    {
        obj = nullptr;
    }
    ...
    if (obj)
    {
        torch::autograd::impl::add_hook(tensor, std::make_unique<PyFunctionTensorPreHook>(obj, 0));
    }
    return 0;
    END_HANDLE_TH_ERRORS_RET(-1)
}

static PyObject* THPVariable_get_post_accumulate_grad_hooks(THPVariable* self, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
...
static int THPVariable_set_post_accumulate_grad_hooks(
    THPVariable* self, PyObject* obj, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
        return handle_torch_function_setter(self, "_post_accumulate_grad_hooks", obj);
    }
    QUARISMA_CHECK(obj, "Deletion of _post_accumulate_grad_hooks not allowed!");
    if (obj == Py_None)
    {
        obj = nullptr;
    }
...
static PyObject* THPVariable_has_symbolic_sizes_strides(THPVariable* self, void* unused)
{
    HANDLE_TH_ERRORS
    auto& self_ = THPVariable_Unpack(self);
...
static PyObject* THPVariable_get_grad_dtype(THPVariable* self, void* unused)
{
    ...
    const auto& var = THPVariable_Unpack(self);
    QUARISMA_CHECK(!var.grad_fn(), "grad_dtype can only be accessed on leaf tensors.");
    if (!var.grad_dtype().has_value())
    {
        Py_RETURN_NONE;
    }
...
static int THPVariable_set_grad_dtype(THPVariable* self, PyObject* obj, void* unused)
{
    HANDLE_TH_ERRORS
    if (check_has_torch_function((PyObject*)self))
    {
    ...
    std::optional<quarisma::ScalarType> new_dtype;
    if (obj != Py_None)
    {
...
static int THPVariable_set_real(PyObject* self, PyObject* real, void* unused)
{
    HANDLE_TH_ERRORS
    auto& self_     = THPVariable_Unpack(self);
    auto  self_real = quarisma::real(self_);
...
static int THPVariable_set_imag(PyObject* self, PyObject* imag, void* unused)
{
    HANDLE_TH_ERRORS
    auto& self_     = THPVariable_Unpack(self);
    auto  self_imag = quarisma::imag(self_);
...
static struct PyGetSetDef THPVariable_properties[] = {
    ...
    {"data", (getter)PropertyData::getter, (setter)THPVariable_set_data, nullptr, nullptr},
    {"_grad",
     (getter)PropertyGrad::getter,
     (setter)THPVariable_set_grad,
     nullptr,
     nullptr},  // Allows the python class to override .grad
    {"grad", (getter)PropertyGrad::getter, (setter)THPVariable_set_grad, nullptr, nullptr},
    ...
    {"real", (getter)PropertyReal::getter, (setter)THPVariable_set_real, nullptr, nullptr},
    {"imag", (getter)PropertyImag::getter, (setter)THPVariable_set_imag, nullptr, nullptr},
    {"grad_dtype",
...
static PyMethodDef extra_methods[] = {
    ...
    {"_view_func_unsafe",
     ...
     METH_VARARGS | METH_KEYWORDS,
     nullptr},
    {"_rev_view_func_unsafe", THPVariable_rev_view_func_unsafe, METH_O, nullptr},
    {"_use_count", THPVariable__use_count, METH_NOARGS, nullptr},
    {nullptr}};

// NOLINTNEXTLINE(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays,cppcoreguidelines-avoid-non-const-global-variables)
static PyMethodDef extra_functions[] = {
    {"_DTensor_OpSchema_post_init", DTensor_OpSchema_post_init, METH_O, nullptr},
...
static int THPVariableMetaType_init(PyObject* cls, PyObject* args, PyObject* kwargs);

static PyTypeObject THPVariableMetaType = {
    PyVarObject_HEAD_INIT(DEFERRED_ADDRESS(&PyType_Type), 0) "torch._C._TensorMeta", /* tp_name */
...
static PyTypeObject THPVariableType = {
    PyVarObject_HEAD_INIT(&THPVariableMetaType, 0) "torch._C.TensorBase", /* tp_name */
...
PyObject* THPVariable_pynew(PyTypeObject* type, PyObject* args, PyObject* kwargs)
{
    ...
    // NB: base_tensor_ctor can call into dispatched Quarisma functions (e.g.,
...
static int THPVariable_subclass_clear(THPVariable* self)
{
    ...
    if (tensor.defined())
    {
                ...
                /*ignore_hermetic_tls=*/false) == (PyObject*)self)
        {
            ...
            //  ASSERT FAILED quarisma "../torch/csrc/autograd/python_variable.cpp":171,
            //  please report a bug to PyTorch. Exception raised from
            //  THPVariable_subclass_clear quarisma
            //  ../torch/csrc/autograd/python_variable.cpp:171 (most recent call
            //  first): frame #0: quarisma::Error::Error(quarisma::SourceLocation,
            //  std::__1::basic_string<char, std::__1::char_traits<char>,
            //  std::__1::allocator<char> >) + 98 (0x1158a0442 in libc10.dylib) frame
            //  #1: quarisma::detail::torchCheckFail(char const*, char const*, unsigned
            //  int, char const*) + 205 (0x11589ed3d in libc10.dylib) frame #2:
            //  quarisma::detail::torchInternalAssertFail(char const*, char const*,
            //  unsigned int, char const*, quarisma::detail::CompileTimeEmptyString) + 9
            ...
            //  namespace)::concrete_decref_fn(quarisma::impl::PyInterpreter const*,
            //  _object*) + 53 (0x1148a5ea5 in libtorch_python.dylib) frame #6:
            //  quarisma::TensorImpl::release_resources() + 182 (0x11588c4a6 in
            //  libc10.dylib) frame #7:
            //  quarisma::MaybeOwned<quarisma::Tensor>::operator=(quarisma::MaybeOwned<quarisma::Tensor>&&)
    ...
    // the base class
    if QUARISMA_LIKELY (type->tp_dictoffset)
    {
    ...
    std::optional<bool>         has_torch_dispatch_if_known)
{
    ...
    QUARISMA_CHECK(
        ...
        "that does not inherit from Tensor is not possible. Make sure your class inherits from "
        "Tensor.");
    ...
    if (obj)
    {
        ...
        else
        {
            ...
            if (has_torch_dispatch_if_known.has_value() ? *has_torch_dispatch_if_known
                                                        : check_has_torch_dispatch(obj))
            {
...
int THPVariableMetaType_init(PyObject* cls, PyObject* args, PyObject* kwargs)
{
    ...
    // torch_function.
    py::object torch_dispatch_impl =
        py::reinterpret_steal<py::object>(PyObject_GetAttrString(cls, "__torch_dispatch__"));
    ...
    {
        py::object torch_function_impl =
            py::reinterpret_steal<py::object>(PyObject_GetAttrString(cls, "__torch_function__"));
...
static void initTensorImplConversion(PyObject* module)
{
    ...
    m.def(
        ...
        [](void* ptr)
        {
            auto p = quarisma::intrusive_ptr<quarisma::TensorImpl, quarisma::UndefinedTensorImpl>::
                unsafe_reclaim_from_nonowning(static_cast<quarisma::TensorImpl*>(ptr));
            QUARISMA_CHECK(p.defined(), "Can't wrap undefined tensor");
            auto tensor = quarisma::Tensor::wrap_tensor_impl(std::move(p));
            return py::cast(std::move(tensor));
        });
    // set on the module level to avoid mixing pybind and plain CPython extensions
    m.def(
        "_tensor_impl_raw_handle",
        [](torch::autograd::Variable* t) -> void*
        {
