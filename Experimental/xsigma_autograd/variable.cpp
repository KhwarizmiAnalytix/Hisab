      ...
      creation_meta_(creation_meta)
{
    ...
    if (shared_view_info_)
    {
        TORCH_INTERNAL_ASSERT(
            backward_info_.has_value(), "Shared view info require a backward view info.");
        TORCH_INTERNAL_ASSERT(
            !forward_info_.has_value(), "Shared view info require forward view info to be empty")
    }
}
    ...
    std::function<Variable(const Variable&)> rev_view_func) const
{
    ...
    if (view_func)
    {
        ...
        {
            view_func =
                std::make_unique<ChainedViewFunc>(view_fn_->clone_and_set(), std::move(view_func));
...
void create_cpp_hook(const quarisma::TensorBase& self, bool is_retains_grad_hook)
{
    ...
    auto hook_ptr = std::make_unique<CppFunctionTensorPreHook>(list, self.output_nr());
...
void set_grad_accumulator(const Variable& self, std::weak_ptr<Node> grad_accumulator)
{
...
std::shared_ptr<Node> grad_accumulator(const Variable& self)
{
    ...
    {
        throw std::logic_error("grad_accumulator() should be only called on leaf Variables");
    }
...
std::unique_ptr<PostAccumulateGradHook>& post_acc_grad_hooks(const Variable& self)
{
    TORCH_INTERNAL_ASSERT(get_autograd_meta(self));
    ...
    std::function<quarisma::TensorBase(const quarisma::TensorBase&)> hook) const
{
    ...
    if (!list)
    {
        torch::autograd::impl::create_cpp_hook(self, /*is_retains_grad_hooks=*/false);
    }
    ...
    if (creation_meta != CreationMeta::DEFAULT)
    {
        ...
        if (creation_meta == CreationMeta::MULTI_OUTPUT_NODE)
        {
            msg = quarisma::str(
                ...
                "functions do not"
                " allow the output views to be modified inplace. You should replace the inplace "
                "operation by an"
                " out-of-place one.");
        }
        else if (creation_meta == CreationMeta::NO_GRAD_MODE)
        {
            msg = quarisma::str(
                ...
                " inside the no_grad block (if you don't want the inplace to be tracked) or both "
                "outside (if you want"
        ...
        else if (creation_meta == CreationMeta::INFERENCE_MODE)
        {
            msg = quarisma::str(
                ...
                " Given that this use case is ambiguous and error-prone, it is forbidden."
                " You can clarify your code by moving both the view and the inplace either both"
                " inside the inference_mode block (if you don't want the inplace to be tracked) or "
                "both outside (if you want"
                " the inplace to be tracked).");
        }
        else if (creation_meta == CreationMeta::IN_CUSTOM_FUNCTION)
        {
            msg = quarisma::str(
                ...
                "as-is) and the"
                " autograd logic to handle view+inplace would override the custom backward "
                "associated with the custom"
                " Function, leading to incorrect gradients. This behavior is forbidden. You can "
                "fix this by"
