...
inline void rebase_history(const std::vector<Variable>& vars, const std::shared_ptr<Node>& grad_fn)
{
    if (grad_fn)
    {
        for (auto& var : vars)
    ...
    bool                                                     allow_tensor_metadata_change = true)
{
    ...
    if ((!diff_view_meta || diff_view_meta->shared_view_info()) && is_bw_differentiable &&
        is_fw_differentiable)
    {
        throw_error_if_base_and_tensor_are_same(base, tensor);
        if (diff_view_meta)
        {
            creation_meta =
                propagate_creation_meta(diff_view_meta->get_creation_meta(), creation_meta);
    ...
    if (is_bw_differentiable)
    {
        ...
        if (diff_view_meta && diff_view_meta->has_bw_view())
        {
            ...
            new_bw_info = base_bw_info.chain(base, tensor, std::move(bw_view_func), rev_view_func);
        }
        else
        {
    ...
    if (is_fw_differentiable)
    {
        ...
        if (diff_view_meta && diff_view_meta->has_fw_view())
        {
            ...
            new_fw_info =
                base_fw_info.chain(base, tensor, std::move(view_func), std::move(rev_view_func));
        }
        else
        {
            new_fw_info = ViewInfo(base, std::move(view_func), std::move(rev_view_func));
        }
    }
