...
const char* kCudnnDoubleBackwardMsg =
    ...
    "double backwards, please disable the CuDNN backend temporarily while running the forward pass "
    "of your RNN. For example: \nwith torch.backends.cudnn.flags(enabled=False):\n    output = "
    "model(inputs)";
...
Tensor norm_backward(
    const Tensor& grad, const Tensor& self, const std::optional<Scalar>& p_, const Tensor& norm)
{
    ...
    bool                         keepdim)
{
    ...
    else if (std::isinf(p))
    {
        ...
        return (quarisma::real(self_p.sgn() * self_t.conj()) * is_eq_max / nb_max)
            .sum(dim, keepdim);
    }
    else if (p < 1.0)
    {
...
Tensor norm_jvp(
    const Tensor& self_p, const Tensor& self_t, const std::optional<Scalar>& p_, Tensor norm)
{
    return norm_jvp(self_p, self_t, p_, std::move(norm), {}, true);
}

Tensor _nested_from_padded_backward(const Tensor& grad, const Tensor& input, bool do_transform_0213)
{
    if (do_transform_0213)
    {
        auto new_sizes            = {input.size(0), input.size(2), (input.size(1) * input.size(3))};
        auto out                  = grad.to_padded_tensor(0, new_sizes);
        auto expand_last_dim_size = {input.size(0), input.size(2), input.size(1), input.size(3)};
...
std::tuple<Tensor, Tensor, Tensor> linear_double_backward(
    const variable_list& grads, const Tensor& self, const Tensor& grad_output, const Tensor& weight)
{
    ...
    {
        grad_self =
            (grad_output.dim() == 1 ? grad_output.unsqueeze(0) : grad_output).matmul(grads[1]);
    ...
    if (grads[0].defined())
    {
        grad_weight = (grad_output.dim() == 1 ? grad_output.unsqueeze(1) : grad_output.mT())
    ...
    if (grads[0].defined())
    {
        grad_grad_output =
            grad_grad_output +
            (grads[0].dim() == 1 ? grads[0].unsqueeze(0) : grads[0]).matmul(weight.mT());
    }
    if (grads[1].defined())
    {
        grad_grad_output =
            grad_grad_output + (self.dim() == 1 ? self.unsqueeze(0) : self).matmul(grads[1].mT());
    }
    if (grads[2].defined())
    {
    ...
    return std::make_tuple(
        std::move(grad_self), std::move(grad_grad_output), std::move(grad_weight));
}
    ...
    bool                                 keepdim)
{
    ...
    // function
    auto dim = opt_dim.value_or(IntArrayRef({}));
    return norm_backward(std::move(grad), self, scalar_ord, std::move(norm), dim, keepdim);
}
...
Tensor pow_backward(Tensor grad, const Tensor& self, const Scalar& exponent)
{
    ...
    else
    {
        ...
        Tensor out         = (exponent.isComplex()) ? grad_lambda(exponent.toComplexDouble())
...
Tensor pow_backward_exponent(
    const Tensor& grad, const Tensor& self, const Tensor& exponent, const Tensor& result)
{
    Tensor cond;
...
Tensor pow_backward_exponent(
    const Tensor& grad, const Scalar& base, const Tensor& exponent, const Tensor& result)
{
    auto grad_lambda = [](const Tensor& a, const Scalar& b) { return (a * b.log()).conj(); };
    auto base_       = exponent.is_complex() && !base.isComplex() ? base.toComplexDouble() : base;
    if (base.equal(0.0))
    {
...
template Tensor div_tensor_self_backward(
    const Tensor&, Tensor, ScalarType, const std::optional<std::string_view>&);
template Tensor div_tensor_self_backward(
    const Tensor&, Scalar, ScalarType, const std::optional<std::string_view>&);
    ...
    const Tensor& grad, quarisma::SymIntArrayRef sizes, quarisma::IntArrayRef dims, bool keepdim)
{
    if (!keepdim && !sizes.empty() && !dims.empty())
    {
        ...
        TORCH_CHECK_NOT_IMPLEMENTED(
            false, "Only the keepdim=true path is implemented to support symints in autograd");
    }
    else
    {
        return grad.expand_symint(sizes);
    ...
    bool                     keepdim)
{
    ...
    auto n             = is_all_reduce ? std::move(numel) : _safe_size(shape, opt_dim.value());
...
Tensor prod_safe_zeros_backward(const Tensor& grad, const Tensor& inp, int64_t dim)
{
    ...
    Tensor exclusive_reverse_nocp =
        quarisma::cat({std::move(ones), std::move(narrow_reverse)}, dim);
...
Tensor logsumexp_backward(
    Tensor grad, const Tensor& self, Tensor result, IntArrayRef dim, bool keepdim)
{
    if (!keepdim && self.dim() != 0)
    {
...
Tensor logcumsumexp_jvp(const Tensor& self_p, const Tensor& self_t, int64_t dim)
{
...
std::vector<Tensor> stack_tensors_backward(
    const Tensor& grad, int64_t dim, const std::vector<ScalarType>& dtypes)
{
    ...
    const std::vector<ScalarType>&           dtypes)
{
    ...
    for (const auto i : quarisma::irange(sizes.size()))
    {
        ...
        auto slice =
            grad_val.slice(0, cur_dim0, cur_dim0 + dim0).slice(1, cur_dim1, cur_dim1 + dim1);
        if (shape.size() == 1)
        {
...
Tensor convolution_backward_jvp_grad_bias(const Tensor& grad_out_t, const Tensor& grad_bias)
{
    ...
    else
    {
        TORCH_INTERNAL_ASSERT(
            false,
            "convolution_backward_jvp_grad_bias expected dim of grad_out_t to be 3, 4, or 5, but "
            "got: ",
            grad_out_t.dim());
    }
}
    ...
    const Scalar&            alpha)
{
    ...
    {
        auto grad_inp =
            (quarisma::_grouped_mm(mat2, grad.transpose(-2, -1), offs)).transpose(-2, -1);
...
static Tensor sparse_mask_like_grad(const Tensor& x, const Tensor& gx, bool accumulate_matches)
{
...
Tensor sparse_sparse_matmul_backward(
    const Tensor& grad, const Tensor& a, const Tensor& b, int64_t grad_order)
{
    /*
...
Tensor infinitely_differentiable_native_dropout_backward(
    const Tensor& grad, const Tensor& mask, double scale)
{
    return grad * (mask.type_as(grad) * scale);
}

Tensor native_dropout_double_backward(
    const Tensor& ggI, const Tensor& grad, const Tensor& mask, double scale)
{
    ...
    bool                                   keepdim)
{
    ...
    if (gmean.defined())
    {
        auto aux = mean_backward(
            gmean, self.sym_sizes(), dim_opt.value_or(IntArrayRef({})), self.sym_numel(), keepdim);
...
Tensor pinv_backward(const Tensor& grad, const Tensor& pinvA, const Tensor& A)
{
    ...
    if (m <= n)
    {
        ...
        return -(pinvA.matmul(K)).mH() + KpinvAh - (A.matmul(pinvA)).matmul(KpinvAh) +
    ...
    else
    {
        ...
        return -(K.matmul(pinvA)).mH() + (gradh - A.matmul(K)).matmul(pinvA).matmul(pinvAh) +
...
Tensor glu_double_backward(
    const Tensor& grad, const Tensor& grad_output, const Tensor& input, int64_t dim)
{
    auto& gO                      = grad_output;
...
Tensor infinitely_differentiable_silu_backward(const Tensor& grad_output, const Tensor& input)
{
    const Tensor sigmoid = input.sigmoid();
    return grad_output * sigmoid * (1.0 + input * (1.0 - sigmoid));
}

Tensor infinitely_differentiable_mish_backward(const Tensor& grad_output, const Tensor& input)
{
    const Tensor sigmoid       = input.sigmoid();
...
Tensor softmax_double_backward(
    const Tensor& grad, const Tensor& grad_output, int dim, const Tensor& output)
{
...
Tensor smooth_l1_loss_double_backward(
    const Tensor& grad, const Tensor& input, const Tensor& target, int64_t reduction, double beta)
{
...
Tensor huber_loss_double_backward(
    const Tensor& grad, const Tensor& input, const Tensor& target, int64_t reduction, double delta)
{
    auto d          = (input - target).abs();
...
Tensor soft_margin_loss_double_backward(
    const Tensor& grad, const Tensor& input, const Tensor& target, int64_t reduction)
{
...
Tensor softplus_double_backward(
    const Tensor& grad, const Tensor& input, const Scalar& beta, const Scalar& threshold)
{
    auto x = (input * beta);
...
static bool _maybe_overlapping_memory(
    quarisma::SymIntArrayRef sizes, quarisma::SymIntArrayRef strides)
{
    if (!sizes.empty())
    {
...
static quarisma::SymInt _min_storage_size(
    ...
    quarisma::SymInt         storage_offset)
{
    ...
    const std::optional<quarisma::SymInt>& sym_storage_offset_)
{
    ...
    if (out_maybe_overlap)
    {
        // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
        auto out_indices =
            flatten_full_indices->as_strided_symint(out_sizes_, out_strides_, out_effective_offset);
        storage.index_add_(0, out_indices.reshape(-1), grad.reshape(-1));
    }
    else
    {
        // assume that new tensors have 0 storage offset
        storage.as_strided_symint(out_sizes_, out_strides_, out_effective_offset).copy_(grad);
    }
    ...
    if (inp_maybe_overlap)
    {
        ...
        auto inp_indices =
            ...
            flatten_full_indices->as_strided_symint(inp_sizes_, inp_strides_, inp_effective_offset)
                .reshape(-1);
        count.index_add_(
            0, inp_indices, quarisma::ones({1}, grad.options()).expand_as(inp_indices));
    ...
    return storage.as_strided_symint(inp_sizes, inp_strides, inp_effective_offset);
}
    ...
    std::optional<quarisma::SymInt> storage_offset)
{
    ...
    auto result =
        result_buffer.as_strided_symint(input_geometry.sym_sizes(), input_geometry.sym_strides());
    auto result_slice = result_buffer.as_strided_symint(sizes, strides, std::move(storage_offset));
    ...
    quarisma::SymInt                step)
{
    ...
    return slice_backward_symint(
        grad, input_sizes, dim, std::move(start_val), std::move(end_val), std::move(step));
}
    ...
    const bool    symeig_eigenvectors)
{
    ...
    if (V.is_complex() && !quarisma::isTensorSubclassLike(diag_VhgV))
    {
        ...
        QUARISMA_CHECK(
            ...
            "by e^{i phi}. The specified loss function depends on this quantity, so it is "
            "ill-defined.");
    }
    ...
    const Tensor& dA, const Tensor& L, const Tensor& V, const bool is_hermitian)
{
    ...
    auto       dV = [&dP, &V, &L, is_hermitian]
    {
        ...
        {
            return dX - V * quarisma::real(quarisma::matmul(V.mH(), dX).diagonal(0, -2, -1))
                                .unsqueeze(-2);
        }
    }();
...
)
{
    ...
    if (areAnyTensorSubclassLike({A, grad}))
    {
        meta_grad = quarisma::cat(
            {quarisma::cat({A, grad}, -1),
             quarisma::cat({quarisma::zeros_like(A), std::move(A)}, -1)},
            -2);
    }
    else
    {
    ...
    return matrix_function(meta_grad).narrow_symint(-2, 0, n).narrow_symint(-1, n, n);
}
...
static Tensor masked_fmap(
    const Tensor& mask, const F1& f1, const F2& f2, const Tensor& t, const Ts&... ts)
{
...
Tensor linalg_det_jvp(
    const Tensor& dA, const Tensor& det, const Tensor& LU, const Tensor& pivots, const bool use_A_T)
{
    ...
    const Tensor& grad, const Tensor& det, const Tensor& A, const Tensor& LU, const Tensor& pivots)
{
    ...
    if (!quarisma::GradMode::is_enabled())
    {
        ...
        auto LU_ =
            LU + quarisma::diag_embed(quarisma::where(LU.diagonal(0, -2, -1) == 0., eps, 0.));
    ...
    else
    {
        ...
        {
            return masked_fmap(det.abs() < 100. * eps, singular, non_singular, A, d, grad);
        }
    }
}
    ...
    const bool    use_A_T)
{
    ...
    auto trAinvE = quarisma::linalg_lu_solve(LU, pivots, dA, /*left*/ true, use_A_T)
                       .diagonal(0, -2, -1)
                       .sum(-1);
    if (LU.is_complex())
    {
    ...
    {
        return std::make_tuple(
            quarisma::_efficientzerotensor(sign.sizes(), sign.options()), trAinvE);
    }
}
    ...
    std::array<bool, 2> output_mask)
{
    ...
    if (grad_x.defined() || grad_m.defined())
    {
        ...
        {
            grad_b =
                std::get<0>(grad_x.triangular_solve(a.conj(), upper, !transpose, unitriangular));
            if (output_mask[1])
            {
...
Tensor fft_c2r_backward(const Tensor& grad, IntArrayRef dim, int64_t normalization)
{
    ...
    if (double_length > 0)
    {  // also covers case when signal size is zero
    ...
    const quarisma::SymInt& last_dim_size)
{
    ...
    auto       complex_full_grad = zero_length > 0 ? grad.new_zeros_symint(new_grad_shape) : grad;
    if (zero_length > 0)
    {
        complex_full_grad.slice_symint(last_dim, 0, half_sizes[last_dim]).copy_(grad);
    }
    return quarisma::real(
        quarisma::_fft_c2c(complex_full_grad, dim, normalization, /*forward=*/false));
}
    ...
    std::array<bool, 3>          output_mask)
{
    ...
    auto mu = unsqueeze_dim1(
        training ? toNonOptTensor(save_mean).to(input.scalar_type()) : toNonOptTensor(running_mean),
        input);
    ...
    auto first_back_grad_input = [&](const Tensor& gO, const Tensor& gamma) -> Tensor
    {
        ...
        auto h1 =
            (M * gO)
                ...
                .sub_(input_sub_mu.mul(sigma2_eps_neg_1) * sum_exclude_dim1(gO * input_sub_mu));
        return h0 * h1;
    };
    ...
    {
        TORCH_INTERNAL_ASSERT(affine, "gamma should always be defined when it requires grad");
    }
    ...
    std::array<bool, 3>          output_mask)
{
    ...
    const int64_t M =
        quarisma::multiply_integers(input_shape.cbegin(), input_shape.cbegin() + axis);
    ...
    if (affine && ggG.defined())
    {
        ...
        auto t2 =
            (input_mu_sigma2_neg_3_2 * (gO * ggG_expanded * input_sub_mu).sum(1, true)).div_(-N);
    ...
    auto first_bwd_fn_grad_input = [&](const Tensor& gO_local, const Tensor& gamma_local) -> Tensor
    {
        ...
        auto h1 =
            (N * gO_local)
                ...
                .sub_(input_sub_mu.mul(sigma2_eps_neg_1) * (gO_local * input_sub_mu).sum(1, true));
        return h0 * h1;
    };
    ...
    {
        gG =
            first_bwd_fn_grad_input(ggI_expanded, quarisma::ones({}, sigma2_eps_neg_1_2.options()));
        gG = (gO * gG).sum(0);
        // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
    ...
    std::array<bool, 2>          grad_input_mask)
{
    ...
    for (int i = 0; i < std::max(static_cast<int>(normalized_ndim - rstd.dim()), 0); ++i)
    {
    ...
    if (grad_input_mask[0])
    {
        ...
        if (dY.defined() && grad_x_hat.defined())
        {
            ...
            dX_from_dY_path =
                rstd_broadcast * grad_x_hat -
                (input * rstd_pow3 / static_cast<double>(N_rms)) * sum_input_times_grad_x_hat;
        }
    ...
    std::array<bool, 3>          grad_input_mask)
{
    ...
    if (grad_input_mask[0])
    {
        ...
        const Tensor var       = ((rstd_tensor * rstd_tensor).reciprocal_() - eps).clamp_min(0);
        ...
        if (dY.defined())
        {
            const Tensor a = isDefined(gamma) ? rstd_tensor * gamma_tensor : rstd_tensor;
            Tensor b = (isDefined(gamma) ? (ds * gamma_tensor).sum(2) : ds.sum(2)).unsqueeze_(-2);
            Tensor c = (isDefined(gamma) ? (db * gamma_tensor).sum(2) : db.sum(2)).unsqueeze_(-2);
...
Tensor log1p_backward(const Tensor& grad, const Tensor& self)
{
        ...
        self.layout() == quarisma::kSparseBsc)
    {
        ...
        // happens memory wise
        TORCH_WARN(
            "log1p_backward: received self with sparse layout, but backward requires "
            "materialization of a dense tensor with this shape");
...
Tensor sinc_backward(const Tensor& grad, const Tensor& self)
{
    ...
    return quarisma::where(
        self_squared_pi == 0.0, quarisma::scalar_tensor(0.0, grad.options()), out);
}
...
Tensor index_backward(
    Tensor zeros_like_self, const torch::List<std::optional<Tensor>>& indices, const Tensor& grad)
{
...
Tensor _cudnn_ctc_loss_backward(
    const Tensor& grad_out, const Tensor& loss, const Tensor& raw_grad, bool zero_infinity)
{
    if (zero_infinity)
    {
        return quarisma::where(
    ...
    bool                    left              = true)
{
    ...
    if (left)
    {
        if (modify_K_in_place)
        {
            ...
            auto u = v_full.narrow_symint(-2, k, m - k).mH().matmul(K.narrow_symint(-2, k, m - k));
    ...
    else
    {
        ...
        {
            auto v = u_full.narrow_symint(-2, k, m - k);
            auto u = K.narrow_symint(-1, k, m - k)
    ...
    const bool    flip_order)
{
    ...
    auto input_first_k_cols_norm_squared = (input_first_k_cols * input_first_k_cols.conj()).sum(-2);
    ...
    const auto next_i     = [flip_order](int64_t i) -> int64_t { return !flip_order ? ++i : --i; };
    ...
    if (areAnyTensorSubclassLike({input, tau, K}))
    {
        ...
        for (const auto i_idx : quarisma::irange(k))
        {
            ...
            if (i != flip_i(k - 1))
            {
                ...
                K = apply_householder_reflector(i_next, v_i_next, s_i_next, K, /*left=*/apply_left);
...
Tensor householder_product_jvp(
    const Tensor& dV_, const Tensor& dtau, const Tensor& prod, const Tensor& V_, const Tensor& tau)
{
    ...
    auto V_first_k_cols_norm_squared = (V_first_k_cols * V_first_k_cols.conj()).sum(-2);
    auto sigma                       = tau / (tau * V_first_k_cols_norm_squared - 1.0);

    auto apply_householder_reflector =
        [m](const Tensor& v_full, const Tensor& t, Tensor& K, bool left = true) -> Tensor
    {
        return apply_simple_transformation(
    ...
    // computes (-t u v^H) K
    auto apply_simple_product =
        [m](const Tensor& u_full, const Tensor& v_full, const Tensor& t, Tensor& K) -> Tensor
    {
        return apply_simple_transformation(
    ...
    auto        H_minus =
        quarisma::diag_embed(quarisma::ones({1}, V.options()).expand(batch_vector_shape));
...
Tensor i1_backward(const Tensor& grad, const Tensor& self, const Tensor& result)
{
    return AT_DISPATCH_FLOATING_TYPES(
        ...
        [&]()
        {
            ...
            auto safe_self = quarisma::where(
                self_is_not_tiny, self, quarisma::scalar_tensor(eps, self.options()));
    ...
    const bool    B_requires_grad)
{
    ...
    else
    {
        ...
        gB_ =
            quarisma::linalg_lu_solve(LU, pivots, vector_to_matrix(gX), left, /*adjoint*/ !use_A_T);
    }
...
Tensor solve_jvp(const Tensor& X, const Tensor& A, const Tensor& dA, const Tensor& dB)
{
    return generic_solve_jvp(
        [](const Tensor& A, const Tensor& dB, const Tensor& dA_contrib)
    ...
    const quarisma::SymInt& n)
{
    ...
    const auto get_L2 = [m, k](const Tensor& L) { return L.narrow_symint(-2, k, m - k); };
    const auto get_U1 = [n, k](const Tensor& U)
    { return n == k ? U.triu() : U.narrow_symint(-1, 0, k).triu(); };
    const auto get_U2 = [n, k](const Tensor& U) { return U.narrow_symint(-1, k, n - k); };
    ...
    double                       eps)
{
    ...
    else
    {
        ...
        invstd_p =
            (1 / quarisma::sqrt(running_var.value() + quarisma::Scalar(eps))).view(view_size);
        result_t = input_t * invstd_p;
    }

    std::optional<Tensor> result_p =
        weight_p.defined() ? std::optional<Tensor>((input_p - mean_p) * invstd_p) : std::nullopt;
    return _affine_jvp(
        result_p,
        result_t,
    ...
    IntArrayRef   normalized_shape)
{
    ...
    Tensor rstd_t;
    if (areAnyTensorSubclassLike({input_t, input_p, rstd_p}) || input_t._is_zerotensor())
    {
    ...
    int64_t       groups)
{
    ...
    if (weight_p.defined())
    {
        ...
        result_p = ((input_p_reshaped - saved_mean.view(view_size)) * saved_invstd.view(view_size))
                       .view(input_shape);
    }
    ...
    const Tensor&    result)
{
    ...
    else if (reduce == "prod")
    {
        ...
        Tensor src_num_zeros =
            zeros_like(self).scatter_add(dim, index, src_zero.to(self.dtype())).gather(dim, index);
        ...
        if (GradMode::is_enabled() && (src_num_zeros > 1).any().item<bool>())
        {
            auto node = std::make_shared<DelayedError>(
                "scatter_reduce(): Double backward is unsupported for src when >1 zeros in src are "
                "scattered to the same position in self",
                /* num inputs */ 1);
    ...
    const Tensor&    result)
{
    ...
    if (reduce == "prod")
    {
        ...
        {
            auto node = std::make_shared<DelayedError>(
                "index_reduce(): Double backward is unsupported for source when >1 zeros in source "
    ...
    const quarisma::Tensor&      workspace)
{
    ...
    {
        bias_ih =
            quarisma::zeros({4 /* num_bias_gates of LSTM */ * hidden_size}, weight0.options());
        bias_hh =
            quarisma::zeros({4 /* num_bias_gates of LSTM */ * hidden_size}, weight0.options());
    }
...
Tensor values_backward(const Tensor& grad, const Tensor& self)
{
    ...
    if (grad.defined())
    {
        ...
        {
            TORCH_CHECK_NOT_IMPLEMENTED(
                false, "values backward with respect to self with layout ", self.layout());
        }
    }
    return grad_self;
}
