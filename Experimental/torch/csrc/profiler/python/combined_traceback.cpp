...
// T0: GIL Lock -> Call Allocator    ->| Waiting device_option Lock
// T1: Call Allocator -> device_option Lock ->| Waiting GIL Lock
...
class QUARISMA_VISIBILITY PythonTraceback : public CapturedTraceback::Python
{
    ...
    int traverse(
        std::vector<CapturedTraceback::PyFrame>& frames, void_visitproc visit, void* arg) override
    {
        for (auto& f : frames)
        {
            Py_VISIT(f.code);
        ...
        if (py::hasattr(quarisma, "_inductor"))
        {
            ...
            if (py::hasattr(inductor, "codecache"))
            {
                stack_frames_for_code =
                    inductor.attr("codecache").attr("PyCodeCache").attr("stack_frames_for_code");
            }
        }
...
std::vector<nlohmann::json> json_symbolize(std::vector<CapturedTraceback*>& to_symbolize)
{
