...
{
public:
    explicit Dtype(int8_t type) : scalar_type_(static_cast<ScalarType>(type)), lanes_(1) {}
    explicit Dtype(ScalarType type) : scalar_type_(type), lanes_(1) {}
    Dtype(int8_t type, int64_t lanes) : scalar_type_(static_cast<ScalarType>(type)), lanes_(lanes)
    {
    }
    Dtype(ScalarType type, int64_t lanes) : scalar_type_(type), lanes_(lanes) {}
    Dtype(Dtype type, int64_t lanes) : scalar_type_(type.scalar_type_), lanes_(lanes)
    {
        if (type.lanes() != 1)
        {
    ...
    bool is_integral() const { return quarisma::isIntegralType(scalar_type_, true); }
    bool is_floating_point() const { return quarisma::isFloatingType(scalar_type_); }
    bool is_signed() const { return quarisma::isSignedType(scalar_type_); }
...
NNC_DTYPE_DECLARATION(quarisma::quint8, QUInt8)
NNC_DTYPE_DECLARATION(quarisma::qint8, QInt8)
...
NNC_TODTYPE_DECLARATION(quarisma::quint8, QUInt8)
NNC_TODTYPE_DECLARATION(quarisma::qint8, QInt8)
...
inline Dtype promoteTypes(Dtype a, Dtype b)
{
    ...
    return Dtype(
        static_cast<ScalarType>(quarisma::promoteTypes(
            static_cast<quarisma::ScalarType>(a.scalar_type()),
            static_cast<quarisma::ScalarType>(b.scalar_type()))),
        a.lanes());
}

inline Dtype BinaryOpDtype(
    Dtype op1_dtype, Dtype op2_dtype, ScalarType ret_type = ScalarType::Undefined)
{
