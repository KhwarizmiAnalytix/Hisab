...
public:
    CudaAnalysis()
    {
        ...
        gpu_thread_extents_ = {alloc<IntImm>(1), alloc<IntImm>(1), alloc<IntImm>(1)};
    }
    bool is_buf_store_target(const BufPtr& buf) const { return store_targets_.count(buf) > 0; }

    const std::unordered_set<VarPtr>& thread_local_bufs() const { return thread_local_bufs_; }

    const std::unordered_set<VarPtr>& cross_block_bufs() const { return cross_block_bufs_; }

    const std::vector<ExprPtr>& gpu_block_extents() const { return gpu_block_extents_; }

    const std::vector<ExprPtr>& gpu_thread_extents() const { return gpu_thread_extents_; }

private:
    void visit(const StorePtr& v) override { store_targets_.insert(v->buf()); }
...
{
public:
    explicit GPUMetaVarRewriter(const CudaAnalysis* cuda_analysis) : cuda_analysis_(cuda_analysis)
    {
        gpu_block_vars_ = {
        ...
        current_block_reach_  = {alloc<IntImm>(1), alloc<IntImm>(1), alloc<IntImm>(1)};
        current_thread_reach_ = {alloc<IntImm>(1), alloc<IntImm>(1), alloc<IntImm>(1)};
    }
    ...
    const std::vector<VarPtr>& gpu_thread_vars() const { return gpu_thread_vars_; }
...
{
public:
    explicit CudaPrinter(std::ostream* os, const CudaAnalysis* cuda_analysis, bool has_random)
...
public:
    ...
    std::string getCodeText(const std::string& attr = "") override { return oss_.str(); }
