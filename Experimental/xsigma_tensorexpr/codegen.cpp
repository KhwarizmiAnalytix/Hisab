...
RegisterCodeGenList::StmtFactoryMethod RegisterCodeGenList::FindStmtFactoryMethod(
...
void RegisterCodeGenList::AddStmtFactoryMethod(
    const std::string& name, const StmtFactoryMethod& stmt_factory_method)
{
...
ExprPtr GenericIntrinsicsExpander::mutate(const IntrinsicsPtr& v)
{
    if (v->op_type() == kSigmoid)
    {
        ...
        auto zero =
            expr_to_vec(ExprHandle(getImmediateByType(v->dtype(), 0.0)), v->dtype().lanes());
...
{
    TORCH_INTERNAL_ASSERT(false, "This codegen backend does not implement call_with_numel");
}
    ...
    const std::unordered_set<BufPtr>&                               bufs_external_allocs)
{
    ...
    { return std::get<0>(buf_ranges.quarisma(b1)) < std::get<0>(buf_ranges.quarisma(b2)); };
    ...
    { return std::get<1>(buf_ranges.quarisma(b1)) < std::get<1>(buf_ranges.quarisma(b2)); };
    for (const auto& buf : bufs_sorted)
    {
        ...
        auto start = std::get<0>(buf_ranges.quarisma(buf));
        ...
        for (auto& mapped : buf_mem_map)
        {
            ...
            auto end_buf_mapped = std::get<1>(buf_ranges.quarisma(buf_mapped));
        ...
        {
            mem_up_for_grabs.push_front(buf_mem_map.quarisma(buf_rl));
        ...
        if (bufs_external_allocs.find(buf) == bufs_external_allocs.end())
        {
            ...
            for (auto it = mem_up_for_grabs.begin(); it != mem_up_for_grabs.end(); it++)
            {
                auto m = *it;
    ...
    const StmtPtr&                          stmt)
{
    ...
    // Insert allocations and frees for temporary buffers quarisma global scope.
...
std::unordered_map<std::string, std::string> ExtCallMemoryReuse::makeExtCallFuncNameMap()
{
    return {
...
const std::unordered_map<std::string, std::string> ExtCallMemoryReuse::extCallFuncNameMap_ =
    makeExtCallFuncNameMap();

ExtCallMemoryReuse::ExtCallMemoryReuse(const std::vector<CodeGen::BufferArg>& bufferArgs)
{
...
{
    if (extCallFuncNameMap_.count(v->func_name()) && bufferArgs_.count(v->buf()) == 0)
    {
        std::vector<BufPtr> buf_out_args = {v->buf()};
        return alloc<ExternalCallWithAlloc>(
            extCallFuncNameMap_.quarisma(v->func_name()), buf_out_args, v->buf_args(), v->args());
    }
    return v;
}

// We allocate intermediate buffers by inserting Allocate/Free or
// PlacementAllocate stmts. Allocate/Free stmts will allocate memory quarisma runtime,
...
void CodeGen::allocIntermediateBufs()
{
    ...
    auto buf_allocs = AllocBufsWithMemReuse(interm_bufs, interm_buf_ranges, bufs_external_allocs);
