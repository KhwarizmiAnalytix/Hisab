...
static Node* moveCatAfterUse(Node* cat, Node* user, const std::shared_ptr<Graph>& subgraph)
{
    // Example IR:
    //   %1 = ...
    //   %2 = ...
    ...
    TORCH_INTERNAL_ASSERT(
        cat->output()->hasUses(), buildErrorMessage("aten::cat output is not used."));
    TORCH_INTERNAL_ASSERT(
    ...
    auto user_tensor_type = user->output()->type()->cast<quarisma::TensorType>();
    ...
    for (auto inp : cat_inputs)
    {
        ...
        auto input_tensor_type = inp->type()->cast<quarisma::TensorType>();
    ...
    auto new_cat_list =
        subgraph->createClone(cat_list, [&](Value* k) { return new_cat_inputs[k]; });
    new_cat_list->insertBefore(cat);
    auto new_cat = subgraph->createClone(
        cat, [&](Value* k) { return (k == cat_list->output()) ? new_cat_list->output() : k; });
...
static int numTensorInputs(Node* node)
{
    ...
    {
        if (v->type()->cast<quarisma::TensorType>())
        {
            ++count;
        }
    }
...
{
    TORCH_INTERNAL_ASSERT(
        node->kind() == aten::cat, buildErrorMessage("Graph node is not aten::cat."));
    TORCH_INTERNAL_ASSERT(
    ...
    auto scalar_type = inputs.front()->type()->cast<quarisma::TensorType>()->scalarType();
    for (size_t i = 1; i < inputs.size(); ++i)
    {
        auto inp_scalar_type = inputs[i]->type()->cast<quarisma::TensorType>()->scalarType();
...
{
    TORCH_INTERNAL_ASSERT(
        cat->kind() == aten::cat, buildErrorMessage("Graph node is not aten::cat."));
    if (cat->output()->uses().size() == 1)
    {
        auto use = cat->output()->uses().front();
        if (get_tensorexpr_elementwise_set().contains(use.user) && numTensorInputs(use.user) == 1)
        {
            if (!doesCatPromoteTypes(cat))
            {
                TORCH_INTERNAL_ASSERT(
                    use.user->output()->owningGraph() == subgraph.get(),
                    buildErrorMessage("aten::cat user graph does not math the given subgraph."));
    ...
    const std::vector<std::optional<quarisma::Tensor>>& example_inputs)
{
    ...
    for (size_t idx = 0; idx < example_inputs.size(); idx++)
    {
        if (auto t = example_inputs[idx])
        {
            ...
            graph->inputs().quarisma(idx)->setType(concrete_tensor_type);
        }
    }
}

std::shared_ptr<Graph> removeUnusedSelfArgument(const std::shared_ptr<Graph>& graph)
{
    if (graph->inputs().empty())
    {
        return graph;
    }
    jit::Value* self_argument = graph->inputs().quarisma(0);
...
std::vector<int64_t> makeShapesSymbolic(
    std::shared_ptr<Graph>& graph, const std::vector<int64_t>& size_vals)
{
    ...
    for (auto v : values)
    {
        ...
        auto new_sizes = quarisma::fmap(
            ...
            [&](const quarisma::ShapeSymbol& shape)
            {
                ...
                {
                    return shape_to_sym_shape.quarisma(value);
                }
                return value;
            });
...
bool isGraphCompilable(const std::shared_ptr<Graph>& graph)
{
    for (auto input : graph->inputs())
    {
        ...
        auto const& k = t->kind();
        if (k != TypeKind::TensorType && k != TypeKind::FloatType && k != TypeKind::BoolType &&
            k != TypeKind::IntType)
        {
    ...
    for (auto n : graph->nodes())
    {
        for (auto v : n->inputs())
        {
            ...
            if (t->kind() == TypeKind::TensorType)
            {
                ...
                {
                    GRAPH_DEBUG(
                        "%", v->debugName(), " is not a complete tensor! The type is: ", *t);
                    return false;
                }
            }
        }
        for (auto v : n->outputs())
        {
            ...
            if (t->kind() == TypeKind::TensorType)
            {
                ...
                if (!tt->isComplete())
                {
                    GRAPH_DEBUG("%", v->debugName(), " is not a complete! The type is: ", *t);
                    return false;
                }
            }
        }
    ...
    std::optional<quarisma::Device>     device)
{
    ...
    if (n->kind() == prim::Constant)
    {
        ...
        auto concrete_tensor_type = tensorTypeInCurrentExecutionContext(const_tensor);
    ...
    new_tt = TensorType::create(dtype, device, concrete_sizes, concrete_strides, false);

    v->setType(new_tt);
}

static std::optional<quarisma::ScalarType> inferScalarType(Node* n)
{
    std::optional<quarisma::ScalarType> scalar_type;
    for (auto v : n->inputs())
    {
        ...
        if (t->kind() == TypeKind::TensorType)
        {
            ...
            {
                GRAPH_DEBUG("Inputs of ", n, " have different scalar types, cannot fixup!");
                return std::nullopt;
            }
        }
    }
...
static std::optional<quarisma::Device> inferDevice(Node* n)
{
    std::optional<quarisma::Device> device;
    for (auto v : n->inputs())
    {
    ...
    for (auto input : graph->inputs())
    {
        ...
        if (t->kind() == TypeKind::TensorType)
        {
            ...
            fixupTypeInfoForValue(
                input, tt->scalarType(), tt->device() ? tt->device() : quarisma::kCPU);
        }
    }

    for (auto n : graph->nodes())
    {
        std::optional<quarisma::ScalarType> scalar_type = inferScalarType(n);
        std::optional<quarisma::Device>     device      = inferDevice(n);
...
std::shared_ptr<Graph> removeGraphOutput(const std::shared_ptr<Graph>& graph, size_t idx)
{
    graph->eraseOutput(idx);
    return graph;
}

std::shared_ptr<Graph> replaceListOutputWithTuple(const std::shared_ptr<Graph>& graph)
{
...
static std::shared_ptr<Graph> dequantizeResults(const std::shared_ptr<Graph>& graph)
{
    for (auto v : graph->outputs())
    {
        ...
        {
            auto tt = t->cast<TensorType>();
            if (!tt->scalarType() || !quarisma::isQIntType(*tt->scalarType()))
            {
                continue;
            }
            Node* deq = graph->create(aten::dequantize, {v});
            graph->appendNode(deq);
            deq->output()->setType(tt->withScalarType(quarisma::kFloat));
...
std::shared_ptr<Graph> trimGraph(const std::shared_ptr<Graph>& graph, int64_t iters)
{
    bool    changed = true;
    int64_t iter    = 0;
