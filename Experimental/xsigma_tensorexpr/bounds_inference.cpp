    ...
    bool                                      distinctAccessKinds)
{
    ...
    {
        if (access->type() == AccessType::Input || access->type() == AccessType::Output)
        {
            continue;
        }
...
BoundsInfo inferBounds(const StmtPtr& s, bool distinctAccessKinds)
{
    ...
    return mergeTensorAccesses(checker.getHistory(), varToBuf, distinctAccessKinds);
}

BoundsInfo getInferredBounds(
    MemDependencyChecker& analyzer, const StmtPtr& s, bool distinctAccessKinds)
{
    return mergeTensorAccesses(analyzer.accessesWithin(s), getAllBufs(s), distinctAccessKinds);
}

BoundsInfo getInferredBounds(
    MemDependencyChecker& analyzer, const ExprPtr& e, bool distinctAccessKinds)
{
    return mergeTensorAccesses(analyzer.accessesWithin(e), getAllBufs(e), distinctAccessKinds);
}
...
std::vector<ExprPtr> getBoundExtents(const std::vector<TensorAccessBoundsInfo>& infos)
{
    ...
    for (const TensorAccessBoundsInfo& p : infos)
    {
        for (const auto i : quarisma::irange(p.start.size()))
        {
            ...
            {
                starts[i] = IRSimplifier::simplify(alloc<Min>(starts[i], p.start[i], true));
            }
            ...
            {
                stops[i] = IRSimplifier::simplify(alloc<Max>(stops[i], p.stop[i], true));
            }
        }
    }
...
static BoundSet convertBounds(
    const std::vector<TensorAccessBoundsInfo>& bounds, TensorAccessKind filter = kMutate)
{
    BoundSet ret;
...
static BoundSet convertBounds(
    BoundsInfo& bounds, const BufPtr& buf, TensorAccessKind filter = kMutate)
{
...
HazardKind getPotentialHazards(MemDependencyChecker& analyzer, const StmtPtr& A, const StmtPtr& B)
{
...
{
    TORCH_INTERNAL_ASSERT(tabi.start.size() == tabi.stop.size(), buildErrorMessage());
...
static std::vector<IndexBounds> getIndexBounds(
    const std::vector<TensorAccessBoundsInfo>& vTABI, TensorAccessKind filter = kMutate)
{
...
bool isOverlapping(analysis::MemDependencyChecker& analyzer, const StorePtr& S1, const StorePtr& S2)
{
...
bool isOverlapping(analysis::MemDependencyChecker& analyzer, const StorePtr& S, const LoadPtr& L)
{
