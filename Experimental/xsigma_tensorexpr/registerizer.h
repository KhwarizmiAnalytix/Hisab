...
{
  A[0] = 0;
  for(const auto x : quarisma::irange(10)) {
    A[0] = (A[0]) + x;
  }
}
...
public:
    AccessInfo() = default;
    AccessInfo(SimplifierHashType h, BufPtr b, std::vector<ExprPtr> i, size_t accessOrder)
        : hash_(h),
          buf_(std::move(b)),
          indices_(std::move(i)),
    ...
    // scope.
    void addLoad(const LoadPtr& load, const std::shared_ptr<Scope>& scope, const StmtPtr& usage);
    ...
    static std::shared_ptr<AccessInfo> cloneWithHiddenInfo(const std::shared_ptr<AccessInfo>& orig);

    // print for debugging.
    void print() const;

    SimplifierHashType hash() const { return hash_; }

    BufPtr buf() const { return buf_; }

    const std::vector<ExprPtr>& indices() const { return indices_; }

    BlockPtr block() const { return block_; }

    void setEnclosingBlock(BlockPtr b) { block_ = std::move(b); }

    StmtPtr first_usage() const { return first_usage_; }
    StmtPtr last_usage() const { return last_usage_; }
    ...
    bool firstUsageOverlapped() const { return firstUsageOverlapped_; }

    ExprPtr store_cost() const { return store_cost_; }

    ExprPtr load_cost() const { return load_cost_; }

    const std::vector<StorePtr>& stores() const { return stores_; }

    const std::vector<LoadPtr>& loads() const { return loads_; }
    ...
    size_t conditionId() const { return conditionId_; }

    void setConditionId(size_t c) { conditionId_ = c; }

    size_t accessOrder() const { return accessOrder_; }

    std::shared_ptr<AccessInfo> hiddenAccess() const { return hiddenAccess_; }
    ...
    ScalarReplacement& replacement() { return replacement_; }
...
using AccessHashMap = std::unordered_map<SimplifierHashType, std::shared_ptr<AccessInfo>>;
...
public:
    Scope(BlockPtr b, std::shared_ptr<Scope> parent, size_t conditionId = 0)
        : block_(std::move(b)), parent_(std::move(parent)), conditionId_(conditionId)
    {
    }

    AccessHashMap& getAccessMapByBuf(const BufPtr& b);

    std::unordered_map<BufPtr, AccessHashMap>& openAccesses() { return openAccesses_; }

    std::vector<std::shared_ptr<AccessInfo>>& closedAccesses() { return closedAccesses_; }
    ...
    const std::unordered_set<VarPtr>& localVars() const { return localVars_; }
    void                              addLocalVar(VarPtr v) { localVars_.insert(std::move(v)); }
 ...
 * there is already a reference to that same access quarisma the higher scope, since
 * we don't know if the condition was guarding an array access not safe quarisma the
...
{
public:
    RegisterizerAnalysis() : currentScope_(std::make_shared<Scope>(nullptr, nullptr, 0)) {}
...
private:
    ...
    void closeAccessIntoScope(
        const std::shared_ptr<AccessInfo>& info, const std::shared_ptr<Scope>& scope);
...
{
public:
    RegisterizerReplacer(std::vector<std::shared_ptr<AccessInfo>>& vec) : infoSet_(vec)
    {
        buildReplacements();
    }
...
private:
    ...
    {
        std::unordered_map<StmtPtr, std::deque<std::shared_ptr<AccessInfo>>> initializerPoints_;
        std::unordered_map<StmtPtr, std::deque<std::shared_ptr<AccessInfo>>> finalizePoints_;
    };
    ...
    unsigned int getBufferAccessCount(const BufPtr& b) { return ++bufferAccessCounts_[b]; }
};
}  // namespace registerizer
