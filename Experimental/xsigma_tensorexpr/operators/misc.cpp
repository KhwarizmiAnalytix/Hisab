...
int64_t normalizeAndCheckIndex(int64_t idx, int64_t list_size)
{
    ...
    {
        QUARISMA_CHECK(false, "Invalid index ", idx, " for list_size", list_size);
    }
    return idx;
}
...
#undef TYPE_CASE
    case ScalarType::QUInt8:
        e = cast<quarisma::quint8>(e);
        break;
    case ScalarType::QInt8:
        e = cast<quarisma::qint8>(e);
        break;
    default:
...
static bool checkTypes(const ScalarType highType, const int typeConstraints)
{
    ...
    if (quarisma::isIntegralType(highType, false))
    {
        return (typeConstraints & kIntegralTypes) != 0;
    }
    else if (quarisma::isFloatingType(highType))
    {
    ...
    TORCH_INTERNAL_ASSERT(
        ...
        buildErrorMessage("Qint and Complex types are not supported in the fuser."));
    return false;
}
...
{
    auto scalarType = static_cast<quarisma::ScalarType>(e.dtype().scalar_type());
    auto floatType  = static_cast<quarisma::ScalarType>(tensorexpr::ScalarType::Float);
    if (quarisma::isFloatingType(scalarType) &&
        (quarisma::elementSize(scalarType) < quarisma::elementSize(floatType)))
    {
...
void promoteInputs(std::vector<ExprHandle>& inputs, const int typeConstraints)
{
    ...
    for (const auto& input : inputs)
    {
        ...
        if (isScalar(input))
        {
            ...
            {
                highType = quarisma::get_default_dtype_as_scalartype();
            }
            else if (highType == quarisma::kBool)
            {
                highType = inputType;
            }
        }
...
{
    auto scalarType = static_cast<quarisma::ScalarType>(e.dtype().scalar_type());
    if (!quarisma::isIntegralType(scalarType, /*includeBool*/ true))
    {
        return e;
    }

    auto defaultType = quarisma::typeMetaToScalarType(quarisma::get_default_dtype());

    // We intend to promote Integers to floating-point types
    TORCH_INTERNAL_ASSERT(!quarisma::isIntegralType(defaultType, /*includeBool*/ true));

    return Cast::make(
        Dtype(static_cast<tensorexpr::ScalarType>(defaultType), e.dtype().lanes()), e);
}

ExprHandle demoteOutput(const ExprHandle& e, const std::optional<ScalarType> type)
{
    if (!type.has_value())
    {
        return e;
...
std::optional<TensorInfo> getTensorInfo(const BufHandle& b)
{
    ...
    return TensorInfo{dims, static_cast<quarisma::ScalarType>(b.dtype().scalar_type())};
}
    ...
    const std::vector<ExprHandle>& a, const std::vector<ExprHandle>& b)
{
    ...
    while (quarisma != a.rend() || bt != b.rend())
    {
        ...
        {
            hasBroadcast = true;
            ret.push_back(*quarisma++);
            continue;
        }
        // TODO: if neither *quarisma nor *bt is 1, ensure they are identical
        // expressions.  Nb: `==` doesn't work since that simply produces a new
        // ExprHandle.
        ExprHandle dim = *quarisma;
        if (isOne(*quarisma))
        {
            if (!isOne(*bt))
            {
...
std::vector<ExprHandle> broadcastShapes(std::vector<std::vector<ExprHandle>> shapes)
{
...
ExprHandle tensorOrConstant(const ArgValue& v, const std::vector<ExprHandle>& axes)
{
...
std::vector<ExprHandle> computeIndicesToBroadcast(
    const std::vector<ExprHandle>& outputAxes, const std::vector<ExprHandle>& inputSizes)
{
    ...
    quarisma::Device                 device)
{
    ...
    if (A.ndim() <= 1)
    {
        return Compute(
            ...
            {
                TORCH_INTERNAL_ASSERT(
                    axes.size() <= 1, buildErrorMessage("Invalid axes size in transpose"));
                return A.load(axes);
            });
    }
    // Usual case where transpose actually swaps dimensions
    auto start_dim = quarisma::maybe_wrap_dim(std::get<int64_t>(inputs[1]), A.ndim());
    auto to_dim    = quarisma::maybe_wrap_dim(std::get<int64_t>(inputs[2]), A.ndim());
    return Compute(
        "aten_transpose",
        outputShape,
    ...
    quarisma::Device                 device)
{
    ...
    return computeReshape(reshapeInputs, outputShape, outputStrides, outputType, device);
}
    ...
    const std::vector<BufHandle>& bufList)
{
    ...
    for (auto buf : bufList)
    {
        ...
        TORCH_INTERNAL_ASSERT(!buf.node()->dims().empty(), buildErrorMessage("Invalid buf rank"));
    ...
    const std::vector<ExprHandle>& outputStrides)
{
    ...
    auto output_buf =
        alloc<Buf>("aten_cat", output_sizes_expr, ToDtype(high_type), nullptr, output_strides_expr);
    if (non_empty_inputs.empty())
    {
        return Tensor(output_buf, alloc<tensorexpr::Block>(std::vector<StmtPtr>({})));
    }
                                  ...
                                  const std::vector<ExprHandle>& dims)
    {
        ...
        for (int64_t i = 0; i < static_cast<int64_t>(dims.size()); ++i)
        {
            for_vars[i] = alloc<Var>(
                "i" + std::to_string(inp_pos) + "_" + std::to_string(i), dims[i].dtype());
        ...
        {
            st = alloc<For>(
                for_vars[dim_index], immLike(dims[dim_index], 0), dims[dim_index].node(), st);
        }
    ...
    {
        auto input_dims = ExprVectorToExprHandleVector(non_empty_inputs[i].node()->dims());
        if (concat_dim_size == nullptr)
        {
            concat_dim_size = immLike(input_dims[norm_concat_dim], 0);
        }
        block->append_stmt(gen_code_for_input(non_empty_inputs[i], i, concat_dim_size, input_dims));
        concat_dim_size = alloc<Add>(concat_dim_size, input_dims[norm_concat_dim].node());
    }
    ...
    quarisma::Device                 device)
{
    ...
    return Compute(
        ...
        [&](const std::vector<VarHandle>& axes)
        {
            ...
            // and we know that there is quarisma least one such an input. With all
    ...
    quarisma::Device                 device)
{
    ...
    StmtPtr s = ExternalCall::make(ResultBuf, "nnc_aten_embedding", {w, indices}, {});
