    ...
    const std::optional<std::vector<ExprHandle>>& outputStrides)
{
    return Compute(
        ...
        [&](ParameterList& axes)
        {
            ...
            std::vector<ExprHandle> inputs = {tensorOrConstant(inputValues[0], indices)};
            auto                    inp    = inputs[0];
    ...
    const int                                           checkParamTypes)
{
    return Compute(
        ...
        outputShape,
        outputStrides,
        [inputValues, outputType, innerExpr, checkParamTypes](const std::vector<VarHandle>& axes)
        {
            std::vector<ExprHandle> indices(axes.begin(), axes.end());
            std::vector<ExprHandle> inputs = {tensorOrConstant(inputValues[0], indices)};
...
Tensor computeTwoOperand(
    ...
    const std::vector<ExprHandle>&                                         outputStrides,
    const std::optional<ScalarType>&                                       outputType,
    const std::function<ExprHandle(const ExprHandle&, const ExprHandle&)>& innerExpr)
{
    return Compute(
        name,
        outputShape,
...
Tensor computeConditionWithTwoOperand(
    ...
    const std::vector<ExprHandle>&   outputStrides,
    const std::optional<ScalarType>& outputType,
    const std::function<ExprHandle(const ExprHandle&, const ExprHandle&, const ExprHandle&)>&
        innerExpr)
{
    return Compute(
        name,
    ...
    bool promote_inputs)
{
    return Compute(
        ...
        outputShape,
        outputStrides,
        [inputValues, outputType, innerExpr, promote_inputs](const std::vector<VarHandle>& axes)
        {
            std::vector<ExprHandle> indices(axes.begin(), axes.end());
            std::vector<ExprHandle> inputs = {
                tensorOrConstant(inputValues[0], indices),
...
Tensor computeFourOperand(
    ...
    const std::function<ExprHandle(
        const ExprHandle&, const ExprHandle&, const ExprHandle&, const ExprHandle&)>& innerExpr)
{
    return Compute(
        name,
        outputShape,
        ...
        [inputValues, outputType, innerExpr](const std::vector<VarHandle>& axes)
        {
            ...
            promoteInputs(inputs);
            ExprHandle compute = innerExpr(inputs[0], inputs[1], inputs[2], inputs[3]);
            return demoteOutput(compute, outputType);
        });
}
    ...
    const std::function<ExprHandle(const ExprHandle&, const ExprHandle&)>& innerExpr)
{
    ...
    promoteInputs(inputs);
    ExprHandle compute  = innerExpr(inputs[0], inputs[1]);
    StmtPtr    let_stmt = Let::make(VarHandle(let_var), demoteOutput(compute, outputType));
