...
static std::vector<VarHandle> squeezeIndices(
    const ParameterList& indices, const std::vector<size_t>& axes)
{
    ...
    quarisma::Device                 device)
{
    ...
    return Reduce(
        ...
        [&](ParameterList& indices)
        {
            ...
            auto indices_squeezed = keepdim ? squeezeIndices(indices, axes) : indices;
            ...
            for (auto axis : axes)
            {
                indices_exprs.insert(
                    indices_exprs.begin() + static_cast<std::ptrdiff_t>(axis), indices_squeezed[i]);
                ++i;
            }
    ...
    quarisma::Device                 device)
{
    ...
    return Tensor(
        ResultBuf.node(), ExternalCall::make(ResultBuf, "nnc_aten_mean", {InputBuf}, extra_args));
}
