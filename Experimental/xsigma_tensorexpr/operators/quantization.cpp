...
{
    TORCH_INTERNAL_ASSERT(qx.node()->qscale(), buildErrorMessage("Expects BufHandle with qscale"));
    return to<DoubleImm>(IRSimplifier::simplify(qx.node()->qscale()))->value();
}

int64_t immQZero(const BufHandle& qx)
{
    TORCH_INTERNAL_ASSERT(qx.node()->qzero(), buildErrorMessage("Expects BufHandle with qzero"));
    ...
    const int64_t                  qzero)
{
    return makeQBufHandleChannelsLast(
        name, dims, dtype, DoubleImm::make(qscale).node(), LongImm::make(qzero).node());
}
...
static bool isChannelsLast(const BufHandle& buf)
{
    ...
    auto stridesLast = to<LongImm>(IRSimplifier::simplify(strides[rank - 1]))->value();

    return ((stridesLast == dimsC) && (stridesC == 1));
}

static ExprHandle quant(const ExprHandle& x, Dtype out_dtype, ExprHandle qscale, ExprHandle qzero)
{
    auto promoted_qscale = promoteToDtype(std::move(qscale), x.dtype().scalar_type());
    auto promoted_qzero  = promoteToDtype(std::move(qzero), x.dtype().scalar_type());
    return promoteToDtype(
        x / promoted_qscale + promoted_qzero + FloatImm::make(0.5f), out_dtype.scalar_type());
}

static ExprHandle dequant(ExprHandle qx, Dtype out_dtype, ExprHandle qscale, ExprHandle qzero)
{
    auto qx_promoted     = promoteToDtype(std::move(qx), out_dtype.scalar_type());
    auto qscale_promoted = promoteToDtype(std::move(qscale), out_dtype.scalar_type());
    auto qzero_promoted  = promoteToDtype(std::move(qzero), out_dtype.scalar_type());
    return promoteToDtype(
        (qx_promoted - qzero_promoted) * qscale_promoted, out_dtype.scalar_type());
}
    ...
    quarisma::Device /*unused*/)
{
    ...
    ExprHandle e = quant(tensorOrConstant(inputs[0], indices), dtype, qscale, qzero);
    ...
    quarisma::Device /*unused*/)
{
    ...
    auto ResultBuf = [&]()
    {
        ...
        return makeQBufHandleContiguous("quantize_per_tensor", outputShape, dtype, qscale, qzero);
    }();
    StmtPtr s =
        ExternalCall::make(ResultBuf, "nnc_aten_quantize_per_tensor", {x}, {qscale, qzero, qdtype});
    ...
    quarisma::Device /*unused*/)
{
    ...
    TORCH_INTERNAL_ASSERT(
        qw.node()->qzero(),
        buildErrorMessage("quantized_conv2d_prepack: Expects quantized weights, qzero is missing"));
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    auto       ResultBuf  = makeQBufHandleChannelsLast(
        "quantized_conv1d", outputShape, Dtype(out_qdtype), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(
        ResultBuf,
        "nnc_aten_quantized_conv1d",
        {qx, prepacked},
        {immQScale(qx), immQZero(qx), (int64_t)immQDType(qx), out_qscale, out_qzero});
    return Tensor(ResultBuf.node(), s);
}
    ...
    quarisma::Device                 device)
{
    ...
    const auto out_qdtype = immQDType(qx);
    auto       ResultBuf  = makeQBufHandleChannelsLast(
        "quantized_conv2d", outputShape, Dtype(out_qdtype), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    const auto out_qdtype = immQDType(qx);
    auto       ResultBuf  = makeQBufHandleChannelsLast(
        "quantized_conv2d_relu", outputShape, Dtype(out_qdtype), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    auto       ResultBuf  = makeQBufHandleContiguous(
        "quantized_linear", outputShape, Dtype(out_qdtype), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    const auto out_qdtype = immQDType(qx);
    auto       ResultBuf  = makeQBufHandleContiguous(
        "quantized_linear_relu", outputShape, Dtype(out_qdtype), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    auto       ResultBuf =
        (isQAChannelsLast || isQBChannelsLast)
    ...
    quarisma::Device                 device)
{
    ...
    auto       ResultBuf  = makeQBufHandleContiguous(
        "quantized_mul_scalar", outputShape, Dtype(out_qdtype), scale1 * scalar, immQZero(qa));
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    auto             ResultBuf =
        isQAChannelsLast
                        ? makeQBufHandleChannelsLast(
                  "quantized_relu", outputShape, Dtype(out_qdtype), immQScale(qa), immQZero(qa))
                        : makeQBufHandleContiguous(
                  "quantized_relu", outputShape, Dtype(out_qdtype), immQScale(qa), immQZero(qa));
    StmtPtr s = ExternalCall::make(
        ResultBuf,
    ...
    quarisma::Device                 device)
{
    ...
    auto ResultBuf = makeQBufHandleContiguous(
        "quantized_cat", outputShape, Dtype(immQDType(inputList[0])), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(ResultBuf, "nnc_aten_quantized_cat", args, extra_args);
    return Tensor(ResultBuf.node(), s);
}
    ...
    quarisma::Device /*unused*/)
{
    ...
    TORCH_INTERNAL_ASSERT(
        qx.node()->qscale(), buildErrorMessage("Missing quantized scale for dequantize"));
    TORCH_INTERNAL_ASSERT(
        qx.node()->qzero(), buildErrorMessage("Missing quantized zero point for dequantize"));
    ...
    BufPtr buf = alloc<Buf>("dequantize", ExprHandleVectorToExprVector(outputShape), dtype);
    ...
    quarisma::Device /*unused*/)
{
    ...
    auto scale_h = promoteToDtype(input_height, ScalarType::Double) / output_height;
    ...
    auto compute_nearest_idx =
        [](const ExprHandle& scale, const ExprHandle& dst_index, const ExprHandle& input_size)
    {
        return Min::make(
            promoteToDtype(floor(dst_index * scale), ScalarType::Long), input_size - 1, true);
    };
    ...
    quarisma::Device /*unused*/)
{
    ...
    BufHandle ResultBuf = [&]()
    {
        ...
        {
            return makeQBufHandleChannelsLast(
                "upsample_nearest2d", outputShape, Dtype(immQDType(x)), qx_qscale, qx_qzero);
        }
    ...
    quarisma::Device /*unused*/)
{
    ...
    auto ResultBuf =
        isChannelsLast(qx)
            ? makeQBufHandleChannelsLast(
                  "quantized_sigmoid", outputShape, Dtype(out_qdtype), out_qscale, out_qzero)
            : makeQBufHandleContiguous(
                  "quantized_sigmoid", outputShape, Dtype(out_qdtype), out_qscale, out_qzero);
    StmtPtr s = ExternalCall::make(
        ResultBuf,
