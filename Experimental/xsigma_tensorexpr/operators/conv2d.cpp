    ...
    int              groups)
{
    ...
    Tensor conv = Reduce(
        ...
        [&](const std::vector<VarHandle>& v)
        {
            ...
            auto in          = ifThenElse(
                cond, 0.f, input.load(n, k, oh * stride - pad + r, ow * stride - pad + s));
...
Tensor conv2d_depthwise(
    BufHandle input, BufHandle weight, BufHandle bias, int stride, int pad, int groups)
{
    assert_dims_constant(bias);
    auto init_func = [&](const std::vector<VarHandle>& v) { return bias.load(v[1]); };
    return conv2d_depthwise_static(input, weight, init_func, stride, pad, groups);
}

Tensor conv2d_depthwise(BufHandle input, BufHandle weight, int stride, int pad, int groups)
{
    ...
    ExprHandle groups)
{
    ...
    return conv2d_depthwise_dynamic(
        input, weight, init_func, N, C, H, W, K, CperG, R, S, stride, pad, groups);
}
    ...
    int64_t                     groups)
{
    ...
    if (input.dims.size() != 4 || weight.dims.size() != 4 || bias.dims.size() != 1)
    {
    ...
    quarisma::Device                 device)
{
    ...
    if (inpInfo && wInfo && bInfo &&
        conv2dIsSupported(*inpInfo, *wInfo, *bInfo, strides, padding, dilation, groups))
    {
    ...
    quarisma::Device                 device)
{
    ...
    StmtPtr s = ExternalCall::make(
        ResultBuf, "nnc_aten_conv1d", {inp, w, b}, {strides[0], padding[0], dilation[0], groups});
    return Tensor(ResultBuf.node(), s);
}
    ...
    quarisma::Device                 device)
{
    ...
    StmtPtr          s =
        ExternalCall::make(ResultBuf, "nnc_prepacked_conv2d_clamp_run", {inp, prepacked}, {});
    return Tensor(ResultBuf.node(), s);
}
    ...
    quarisma::Device                 device)
{
    ...
    const BufHandle& prepacked = std::get<BufHandle>(inputs[1]);
    StmtPtr          s =
        ExternalCall::make(ResultBuf, "nnc_prepacked_linear_clamp_run", {inp, prepacked}, {});
    return Tensor(ResultBuf.node(), s);
}
    ...
    quarisma::Device                 device)
{
    ...
    BufHandle        ResultBuf("mkldnn_prepacked_conv_run", outputShape, outputStrides, dtype);
    const BufHandle& inp       = std::get<BufHandle>(inputs[0]);
    const BufHandle& prepacked = std::get<BufHandle>(inputs[1]);
    StmtPtr          s =
        ExternalCall::make(ResultBuf, "nnc_mkldnn_prepacked_conv_run", {inp, prepacked}, {});
    return Tensor(ResultBuf.node(), s);
}
