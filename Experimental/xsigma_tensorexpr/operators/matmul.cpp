    ...
    quarisma::Device                 device)
{
    ...
    auto total_size = to<LongImm>(
        IRSimplifier::simplify(
            cast<int64_t>(size_a[0]) * cast<int64_t>(size_a[1]) * cast<int64_t>(size_b[1]))
            .node());
    ...
    {
        return Tensor(
            ResultBuf.node(), ExternalCall::make(ResultBuf, "nnc_aten_matmul", {a, b}, {}));
    }
}
    ...
    return Tensor(
        ...
        ExternalCall::make(
            ...
            {std::get<int64_t>(inputs[3]),
             std::get<int64_t>(inputs[4])}));  // TODO: handle other dtypes of alpha and beta
}
