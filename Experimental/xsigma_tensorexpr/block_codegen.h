...
public:
    ...
    const std::unordered_set<BufPtr>& loads() const { return loads_; }

    const std::unordered_set<BufPtr>& stores() const { return store_targets_; }
    ...
    std::string getFlatInputName(const BufPtr& buf) const { return getInputName(buf) + "_flat"; }
