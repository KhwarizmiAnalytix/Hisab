...
template <typename T>
static inline std::enable_if_t<std::is_integral_v<T>, T> mod_value(T lhs, T rhs)
{
    return lhs % rhs;
}

template <typename T>
static inline std::enable_if_t<std::is_floating_point_v<T>, T> mod_value(T lhs, T rhs)
{
...
{
    QUARISMA_CHECK(rhs != 0, "Division by zero");
    return lhs / rhs;
}

template <typename T>
static inline std::enable_if_t<std::is_floating_point_v<T>, T> __ubsan_ignore_float_divide_by_zero__
div_value(T lhs, T rhs)
{
    return lhs / rhs;
}

static inline quarisma::Half div_value(quarisma::Half lhs, quarisma::Half rhs)
{
    return lhs / rhs;
}

static inline quarisma::BFloat16 div_value(quarisma::BFloat16 lhs, quarisma::BFloat16 rhs)
{
    return lhs / rhs;
}
...
public:
    ...
    TORCH_API void visit(const AddPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const SubPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const MulPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const DivPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const ModPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const MaxPtr& v) override { visit_binary_op(v, v->propagate_nans()); }
    TORCH_API void visit(const MinPtr& v) override { visit_binary_op(v, v->propagate_nans()); }

    TORCH_API void visit(const AndPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const OrPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const XorPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const LshiftPtr& v) override { visit_binary_op(v); }
    TORCH_API void visit(const RshiftPtr& v) override { visit_binary_op(v); }
    ...
    template <typename T>
    typename std::enable_if_t<std::is_floating_point_v<T>, T> max_value(T a, T b)
    {
    ...
    template <typename T>
    typename std::enable_if_t<std::is_floating_point_v<T>, T> min_value(T a, T b)
    {
    ...
    template <typename T>
    InterpValue binary_op(const InterpValue& lhs, const InterpValue& rhs, IRNodeType op_type)
    {
        std::vector<T> lhs_v = lhs.as_vec<T>();
        std::vector<T> rhs_v = rhs.as_vec<T>();
        std::vector<T> result_v(lhs_v.size());
        for (const auto i : quarisma::irange(lhs_v.size()))
        {
            switch (op_type)
            {
    ...
    InterpValue bitwise_binary_op(
        const InterpValue& lhs, const InterpValue& rhs, IRNodeType op_type)
    {
        std::vector<T> lhs_v = lhs.as_vec<T>();
        std::vector<T> rhs_v = rhs.as_vec<T>();
        std::vector<T> result_v(lhs_v.size());
    ...
    template <typename T>
    InterpValue shift_binary_op(const InterpValue& lhs, const InterpValue& rhs, IRNodeType op_type)
    {
        std::vector<T> lhs_v = lhs.as_vec<T>();
        std::vector<T> rhs_v = rhs.as_vec<T>();
        std::vector<T> result_v(lhs_v.size());
    ...
    template <
        typename D,
        std::enable_if_t<
            std::is_same_v<decltype(detail::bin_op_deducer(std::declval<D>())), void>>* = nullptr>
...
#undef TYPE_CASE
    ...
    void visit_compare_select_op(const CompareSelectPtr& v, CompareSelectOperation cmp_op)
    {
        v->lhs()->accept(this);
        InterpValue lhs_v = value_;
        v->rhs()->accept(this);
        ...
        if (lhs_v.dtype() != rhs_v.dtype() || ret_val1_v.dtype() != ret_val2_v.dtype())
        {
...
#define TYPE_CASE(Type, Name)                                                                  \
    case ScalarType::Name:                                                                     \
        value_ = compare_select_op_helper<Type>(lhs_v, rhs_v, ret_val1_v, ret_val2_v, cmp_op); \
        break;
            AT_FORALL_SCALAR_TYPES_AND3(Bool, Half, BFloat16, TYPE_CASE)
#undef TYPE_CASE
        default:
...
#define IMM_VISIT(Type, Name) \
    TORCH_API void visit(const Name##ImmPtr& v) override { value_ = InterpValue(v->value()); }
...
#undef IMM_VISIT
    ...
    void doCastFromSrc(const Dtype& src_dtype, const Dtype& dst_dtype, const InterpValue& v)
    {
...
#define DST_TYPE_CASE_QUANT(Type, Name, CppType)                               \
    ...
    break;
            DST_TYPE_CASE_QUANT(quarisma::quint8, QUInt8, uint8_t)
            DST_TYPE_CASE_QUANT(quarisma::qint8, QInt8, int8_t)
...
#define SRC_TYPE_CASE(Type, Name)                          \
    case ScalarType::Name:                                 \
        ...
        break;
                ...
                SRC_TYPE_CASE(quarisma::quint8, QUInt8);
                SRC_TYPE_CASE(quarisma::qint8, QInt8);
#undef SRC_TYPE_CASE
            default:
                throw unsupported_dtype();
            }
    ...
    template <typename SrcType, typename DstType>
    std::vector<DstType> bitcastValues(const Dtype& src_dtype, const InterpValue& v)
    {
        const std::vector<SrcType>& src_values = v.as_vec<SrcType>();
        std::vector<DstType>        dst_values(src_values.size());
        for (int i = 0; i < src_dtype.lanes(); ++i)
    ...
    template <typename SrcType>
    void doBitCastFromSrc(const Dtype& src_dtype, const Dtype& dst_dtype, const InterpValue& v)
    {
        switch (dst_dtype.scalar_type())
        {
#define DST_TYPE_CASE(Type, Name)                                               \
...
#undef SRC_TYPE_CASE
    ...
    TORCH_API void visit(const RampPtr& v) override
    {
        ...
        for (const auto i : quarisma::irange(lanes))
        {
...
#undef TYPE_CASE
    ...
    {
        std::stringstream ss;
        ss << "Index out of bounds in check_bounds. Index: " << idx << "; bounds: [0, " << bound
           << ").";
    ...
    TORCH_API void visit(const LoadPtr& v) override
    {
        ...
        ExprPtr flat_idx = flatten_index(v->buf()->dims(), v->indices(), v->buf()->strides());
...
#undef TYPE_CASE
    ...
    void visit(const ExternalCallPtr& v) override
    {
        ...
        GRAPH_DEBUG("EXTERNAL CALL: func=", v->func_name(), ", buf=", v->buf()->name_hint());
        ...
        for (const ExprPtr& a : v->args())
        {
            ...
            {
                throw malformed_input("extra_args in ExternalCalls must have int64 dtype", v);
            }
            extra_args.push_back(val);
        }

        auto fn_ptr = func_registry.quarisma(v->func_name());
        (*fn_ptr)(
            bufs.size(),
            buf_ptrs.data(),
            buf_ranks.data(),
    ...
    void visit_intrinsics_helper(const IntrinsicsPtr& v)
    {
        ...
        if (values.size() == 1ULL)
        {
            for (const auto i : quarisma::irange(v1.size()))
            {
    ...
    TORCH_API void visit(const IntrinsicsPtr& v) override
    {
        ...
        if (v->op_type() == kIsNan)
        {
            auto inp_dtype = v->params().quarisma(0)->dtype().scalar_type();
...
#undef TYPE_CASE
    ...
    void visit(const AllocatePtr& v) override
    {
        ...
        GRAPH_DEBUG("ALLOCATE: buf=", v->buf()->name_hint(), ", size=", total_byte_size);
    ...
    {
        buffer_mapping_[v->buf()] = buffer_mapping_.quarisma(v->buf_to_reuse());
    }
    ...
    void visit(const FreePtr& v) override
    {
        ...
        {
            throw std::runtime_error(
                "Free a buffer that is not currently bound: " + v->buffer_var()->name_hint());
        }
    ...
    void visit(const FreeExtPtr& v) override
    {
        ...
        for (const auto& buf : bufs)
        {
            ...
            {
                throw std::runtime_error(
                    "Free an external allocated buffer that does not have corresponding pointer "
                    "for freeing: " +
...
private:
    ...
    static TReturn compute_intrinsics(IntrinsicsOp op_type, TInput v)
    {
        ...
        default:
            throw std::runtime_error("Invalid integral op_type: " + std::to_string(op_type));
        }
    }
    ...
    std::unordered_map<BufPtr, std::unique_ptr<std::vector<int>>> internal_buffers_;
