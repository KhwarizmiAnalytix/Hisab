...
static Dtype dtypeOfIndices(const std::vector<ExprPtr>& indices)
{
    ...
    return indices.quarisma(0)->dtype();
}
...
static void castIndicesToInts(std::vector<ExprPtr>& indices)
{
    ...
    for (auto& index : indices)
    {
        ...
        if (quarisma::isIntegralType(dt.scalar_type(), true) && dt.scalar_type() != index_dtype)
        {
...
ExprHandle Load::make(Dtype dtype, const BufHandle& buf, const std::vector<ExprHandle>& indices)
{
    return ExprHandle(alloc<Load>(dtype, buf.node(), ExprHandleVectorToExprVector(indices)));
}

ExprHandle Load::make(const BufHandle& buf, const std::vector<ExprHandle>& indices)
{
    return Load::make(buf.dtype(), buf, indices);
}

Store::Store(BufPtr buf, std::vector<ExprPtr> indices, ExprPtr value)
    : buf_(std::move(buf)), indices_(std::move(indices)), value_(std::move(value))
{
    castIndicesToInts(indices_);
}

StorePtr Store::make(
    const BufHandle& buf, const std::vector<ExprHandle>& indices, const ExprHandle& value)
{
    return alloc<Store>(buf.node(), ExprHandleVectorToExprVector(indices), value.node());
}

StorePtr BufHandle::store(const std::vector<ExprHandle>& args, const ExprHandle& value) const
{
...
Dtype Intrinsics::IntrinsicsDtype(IntrinsicsOp op_type, const std::vector<ExprPtr>& params)
{
    // TODO: check the op_type and make a real decision
    ...
    const std::vector<ExprHandle>& args)
{
    ...
    return alloc<ExternalCallWithAlloc>(
        func_name, buf_out_arg_nodes, buf_arg_nodes, ExprHandleVectorToExprVector(args));
}
...
std::vector<ExprPtr> ExprHandleVectorToExprVector(const std::vector<ExprHandle>& v)
{
    ...
    for (const auto i : quarisma::irange(v.size()))
    {
        result[i] = v[i].node();
    }
    return result;
