...
// we need to look quarisma the operator precedence to make the output simpler.
template <
    typename Op,
    std::enable_if_t<std::is_same_v<decltype(detail::bin_op_deducer(std::declval<Op>())), void>>* =
        nullptr>
static void visitBinaryOp(
    NodePtr<Op> v, const std::string& op_str, IRPrinter* printer, bool parens = true)
{
...
{
    os() << "Alias(" << *v->buf()->base_handle() << "," << *v->buf_to_reuse()->base_handle()
         << ");";
}
...
{
    IRPrinter::PrinterStream* printer_stream = dynamic_cast<IRPrinter::PrinterStream*>(&stream);
...
std::string to_string(const Tensor& t)
{
    ...
    for (const auto i : quarisma::irange(t.buf()->ndim()))
    {
        if (i != 0)
        {
            oss << ", ";
