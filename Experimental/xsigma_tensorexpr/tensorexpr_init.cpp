...
void initTensorExprBindings(PyObject* module)
{
    ...
    auto dtype_class = py::class_<Dtype>(te, "Dtype").def(py::init(&parsePythonDtype));
    py::implicitly_convertible<py::object, Dtype>();

#define DTYPE_SINGLETON_ACCESSOR(ctype, name) \
    dtype_class.def_property_readonly_static(#name, [](const py::object&) { return k##name; });
...
#undef DTYPE_SINGLETON_ACCESSOR
    ...
    auto expr_handle_class =
        py::class_<ExprHandle>(te, "ExprHandle")
            ...
            .def(
                "__pow__",
                [](const ExprHandle& self, const ExprHandle& other) { return pow(self, other); })
            ...
            .def("abs", [](const ExprHandle& self) { return tensorexpr::abs(self); })
            .def("log", [](const ExprHandle& self) { return log(self); })
            .def("fast_tanh", [](const ExprHandle& self) { return fast_tanh(self); })
            .def("fast_sigmoid", [](const ExprHandle& self) { return fast_sigmoid(self); })
            .def("fast_log", [](const ExprHandle& self) { return fast_log(self); })
            ...
            .def("sqrt", [](const ExprHandle& self) { return tensorexpr::sqrt(self); })
            ...
            .def(
                "cast",
                [](const ExprHandle& self, const Dtype& dt) { return Cast::make(dt, self); })
#define EXPRHANDLE_INIT(ctype, name) .def(py::init([](ctype val) { return name##Imm::make(val); }))
                AT_FORALL_SCALAR_TYPES_AND3(Bool, Half, BFloat16, EXPRHANDLE_INIT)
#undef EXPRHANDLE_INIT
        ;

#define EXPRHANDLE_IMPL_CONV(ctype, name) py::implicitly_convertible<ctype, ExprHandle>();
...
#undef EXPRHANDLE_IMPL_CONV
    ...
    te.def("atan2", [](const ExprHandle& v1, const ExprHandle& v2) { return atan2(v1, v2); });
    te.def("pow", [](const ExprHandle& v1, const ExprHandle& v2) { return pow(v1, v2); });
    te.def("fmod", [](const ExprHandle& v1, const ExprHandle& v2) { return fmod(v1, v2); });
    te.def(
        "remainder", [](const ExprHandle& v1, const ExprHandle& v2) { return remainder(v1, v2); });
...
#undef EXPRHANDLE_CTOR
    ...
    py::class_<BufHandle, ExprHandle>(te, "BufHandle")
        .def(py::init<const std::string&, const std::vector<ExprHandle>&, Dtype>())
        .def(py::init<const std::vector<ExprHandle>&, Dtype>())
        .def(py::init<Dtype>())
        .def("__hash__", [](const BufHandle& self) { return std::hash<BufPtr>()(self.node()); })
        .def(
            "__eq__",
        ...
        .def(
            "load",
            [](BufHandle& self, const std::vector<ExprHandle>& v) { return Load::make(self, v); })
        .def("load", [](BufHandle& self, const ExprHandle& v) { return Load::make(self, {v}); })
        .def(
            "store",
            [](BufHandle& self, const std::vector<ExprHandle>& i, const ExprHandle& v)
    ...
    py::class_<Tensor>(te, "Tensor")
        .def(py::init([](const BufHandle& b, const StmtPtr& s) { return Tensor(b.node(), s); }))
        .def("load", [](Tensor& self, const std::vector<ExprHandle>& v) { return self.load(v); })
    ...
    py::class_<Cast, std::shared_ptr<Cast>>(te, "Cast")
        ...
        .def("src_value", [](CastPtr& self) { return ExprHandle(self->src_value()); })
        .def(
            "set_src_value",
            [](CastPtr& self, const ExprHandle& value) { self->set_src_value(value.node()); });
           ...
           const py::function&            func)
        {
            if (dim_args.size() == 1)
            {
                return Compute(
                    func_name,
                    dim_args,
                    [&func](const VarHandle& a) { return py::cast<ExprHandle>(func(a)); });
            }
            ...
            else if (dim_args.size() == 3)
            {
                return Compute(
                    func_name,
                    dim_args,
                    [&func](const VarHandle& a, const VarHandle& b, const VarHandle& c)
            ...
            else if (dim_args.size() == 4)
            {
                return Compute(
                    ...
                    [&func](
                        ...
                        const VarHandle& b,
                        const VarHandle& c,
                        const VarHandle& d) { return py::cast<ExprHandle>(func(a, b, c, d)); });
            }
            else
            {
    ...
    te.def(
           ...
           const py::function&            func)
        {
            return Compute(
                func_name,
                dim_args,
                [&func](const std::vector<VarHandle>& dims)
                { return py::cast<ExprHandle>(func(dims)); });
        },
        py::return_value_policy::reference);

    py::class_<Reducer>(te, "Reducer")
        .def(py::init<ExprHandle, std::function<ExprHandle(ExprHandle, ExprHandle)>>());
    ...
    te.def(
        ...
        [](const std::string&                                              func_name,
           const std::vector<ExprHandle>&                                  dim_args,
           const Reducer&                                                  reducer,
           const std::function<ExprHandle(const std::vector<VarHandle>&)>& init_func,
           const std::function<ExprHandle(const std::vector<VarHandle>&)>& body_func,
           const std::vector<ExprHandle>&                                  reduce_args)
        { return Reduce(func_name, dim_args, reducer, body_func, reduce_args); },
        py::return_value_policy::reference);
    ...
    py::class_<Store, Stmt, std::shared_ptr<Store>>(te, "Store")
        .def_static(
            "make",
            [](const BufHandle& buf, std::vector<ExprHandle>& indices, const ExprHandle& value)
    ...
    py::class_<For, Stmt, std::shared_ptr<For>>(te, "For")
        ...
        .def(
            "set_gpu_block_index",
            [](For& self, int block_index) { self.set_gpu_block_index(block_index); })
        .def(
            "set_gpu_thread_index",
            [](For& self, int thread_index) { self.set_gpu_thread_index(thread_index); })
        .def_static(
            "make",
            [](const VarHandle&  var,
               const ExprHandle& start,
               const ExprHandle& stop,
               const StmtPtr&    body) { return For::make(var, start, stop, body); });

    py::class_<Cond, Stmt, std::shared_ptr<Cond>>(te, "Cond")
        .def_static(
            "make",
            [](const ExprHandle& condition, const StmtPtr& true_stmt, const StmtPtr& false_stmt)
    ...
    py::class_<LoopNest>(te, "LoopNest")
        ...
        .def(
            "get_loop_body_for",
            [](const LoopNest& self, const Tensor& t) { return self.getLoopBodyFor(t); },
            py::return_value_policy::reference)
        .def(
            "get_loop_body_for",
            [](const LoopNest& self, BufHandle& b) { return self.getLoopBodyFor(b.node()); },
            py::return_value_policy::reference)
        .def(
            "get_loops_for",
            [](const LoopNest& self, const Tensor& t) { return self.getLoopStmtsFor(t); },
            py::return_value_policy::reference)
        .def(
        ...
        .def(
            ...
            [](const LoopNest& self, const StmtPtr& s) { return self.getEnclosingLoopNest(s); },
            py::return_value_policy::reference)
        .def(
        ...
        .def(
            "get_loop_at",
            [](const LoopNest& self, ForPtr root, const std::vector<int>& indices)
            { return self.getLoopAt(std::move(root), indices); },
            py::return_value_policy::reference)
        .def(
            "get_parent_loop",
            [](const LoopNest& self, const StmtPtr& s) { return self.getParentLoop(s); },
            py::return_value_policy::reference)
        .def_static(
            "get_loop_stmts_in_loopnest",
            [](const ForPtr& f, size_t num) { return LoopNest::getLoopStmtsInLoopNest(f, num); },
            py::return_value_policy::reference)
        .def(
            "split_with_tail",
        ...
        .def(
            "tile",
            [](LoopNest& self, const ForPtr& x, const ForPtr& y, int x_factor, int y_factor)
        ...
        .def_static(
            ...
            [](const ForPtr& f) { return LoopNest::distributeLoopOverInnerLoops(f); },
            py::return_value_policy::reference)
        .def_static(
            "unsafe_fuse_loops",
        ...
        .def_static(
            "cache_accesses",
            [](const BufHandle& producer, const std::string& name, const StmtPtr& consumer)
            {
        ...
        .def_static(
            "compute_at",
            [](const StmtPtr& s, const ForPtr& quarisma) { LoopNest::computeAt(s, quarisma); })
        .def(
            "compute_inline",
        ...
        .def(
            "compute_inline",
            [](LoopNest& self, const BufHandle& b) { self.computeInline(b.node()); },
            py::return_value_policy::reference)
        .def(
            "rfactor",
        ...
        .def(
            ...
            py::return_value_policy::reference)
        .def("reorder_axis", &LoopNest::reorderAxis, py::return_value_policy::reference)
        ...
        .def("eliminate_dead_stores", [](LoopNest& self) { self.eliminateDeadStores(); })
        .def(
            "__str__",
            [](const LoopNest& self)
            {
        ...
        .def("root_stmt", &LoopNest::root_stmt, py::return_value_policy::reference);
    ...
    te.def(
        ...
        [](const std::string&             op_str,
           ...
           Dtype                          outputType)
        {
            auto                  op = quarisma::Symbol::fromQualString(op_str);
    ...
    py::class_<ArgValue>(te, "ArgValue")
        ...
        .def("as_int", [](const ArgValue& self) { return std::get<int64_t>(self); })
        .def("as_bool", [](const ArgValue& self) { return std::get<bool>(self); })
        .def("as_none", [](const ArgValue& self) { return std::get<ArgNone>(self); })
    ...
    py::class_<quarisma::ScalarType> give_me_a_name(te, "ScalarType");
    ...
    py::class_<TensorExprKernel>(te, "TensorExprKernel")
        ...
        .def(
            py::init(
                [](const TSGraph&                                              g,
                   const std::unordered_map<std::string, NNCLoweringFunction>& custom_lowerings_str,
                   ...
                   bool                 pre_alloc = false)
                {
                    ...
                    {
                        custom_lowerings[quarisma::Symbol::fromQualString(kv.first)] = kv.second;
                    }
        ...
        .def(
            ...
            [](CodeGen& self, const py::sequence& values)
            {
                ...
                for (const auto& value : values)
                {
                    ...
                    {
                        value_ptrs.emplace_back(value.cast<quarisma::Tensor>().data_ptr());
                    }
                }
...
#error Unexpected or undefined __BYTE_ORDER__
        ...
        .def(
            ...
            [](CodeGen& self, const py::sequence& values)
            {
                ...
                for (const auto& value : values)
                {
                    ...
                    value_ptrs.emplace_back(reinterpret_cast<void*>(value.cast<intptr_t>()));
                }
                self.call_raw(value_ptrs);
            })
        .def(
            "get_code_text",
            [](CodeGen& self, const std::string& attr = "") { return self.getCodeText(attr); },
            py::arg("attr") = "");
    ...
    te.def(
        ...
        [](const std::optional<std::string>& val) { tensorexpr::LLVMTargetTriple() = val; });
    te.def(
        "set_llvm_target_cpu",
        [](const std::optional<std::string>& val) { tensorexpr::LLVMTargetCPU() = val; });
    te.def(
        "set_llvm_target_attrs",
        [](const std::optional<std::string>& val) { tensorexpr::LLVMTargetAttrs() = val; });
    te.def("set_llvm_aot_workflow", [](bool val) { tensorexpr::LLVMAOTWorkflow() = val; });
#endif
}
