...
static void getDependencyChain(const std::shared_ptr<AccessInfo>& info, DependencySet& dependencies)
{
...
static void getDependentsChain(const std::shared_ptr<AccessInfo>& info, DependencySet& dependents)
{
...
void AccessInfo::addDependency(const std::shared_ptr<AccessInfo>& write)
{
    ...
    TORCH_INTERNAL_ASSERT(
        res.second, buildErrorMessage("Duplicate entry in mem dep checker in the fuser."));
}
...
{
    if (type_ == AccessType::Input || type_ == AccessType::Output || type_ == AccessType::Alloc)
    {
        os << "n" << id_ << " [\n";
        ...
        if (!bounds_.empty())
        {
            ...
            size_t i = bounds_.size() - 1;
            os << *IRSimplifier::simplify(alloc<Add>(bounds_[i].end, immLike(bounds_[i].end, 1)));
            os << "]\"\n ";
        }
        if (isWrite())
        {
...
MemDependencyChecker::MemDependencyChecker(
    const std::unordered_set<BufPtr>& inputs, const std::unordered_set<BufPtr>& outputs)
{
...
MemDependencyChecker::MemDependencyChecker(
    const std::vector<BufHandle>& inputs, const std::vector<BufHandle>& outputs)
{
    for (auto& s : inputs)
    {
...
const std::vector<std::shared_ptr<AccessInfo>>& MemDependencyChecker::getHistory() const
{
...
DependencySet MemDependencyChecker::getAllWriteDependencies(const DependencySet& products)
{
...
bool MemDependencyChecker::dependsDirectly(
    const std::shared_ptr<AccessInfo>& A, const std::shared_ptr<AccessInfo>& B)
{
    return A->hasDependency(B) && B->isWrite();
}

bool MemDependencyChecker::dependsIndirectly(const ExprPtr& A, const StmtPtr& B)
{
    return dependsIndirectlyHelper(A, B);
}

bool MemDependencyChecker::dependsIndirectly(const StmtPtr& A, const StmtPtr& B)
{
    return dependsIndirectlyHelper(A, B);
}
...
bool MemDependencyChecker::dependsIndirectly(const StmtPtr& A, const BufPtr& I)
{
    auto aReads      = getAllReadsWithin(A);
    auto inputAccess = input(I);
...
bool MemDependencyChecker::dependsIndirectly(const ExprPtr& A, const BufPtr& I)
{
    auto aReads      = getAllReadsWithin(A);
    auto inputAccess = input(I);
...
std::shared_ptr<AccessInfo> MemDependencyChecker::accessFor(const StmtPtr& A) const
{
...
std::shared_ptr<AccessInfo> MemDependencyChecker::accessFor(const ExprPtr& A) const
{
...
std::unordered_set<std::shared_ptr<AccessInfo>> MemDependencyChecker::accessesWithin(
...
std::shared_ptr<AccessInfo> MemDependencyChecker::output(const BufPtr& b) const
{
...
void MemDependencyChecker::visit(const LoadPtr& v)
{
    ...
    // this load.
    auto indicesScope = std::make_shared<Scope>(currentScope_->block, currentScope_);
    ...
    auto   load = std::make_shared<AccessInfo>(
        nextAccess_++, AccessType::Load, v, lastStmt_, var, getIndicesBounds(v->indices()));
    ...
    bool                               parallelized)
{
    ...
    TORCH_INTERNAL_ASSERT(
        ...
        buildErrorMessage("Dimension mismatch for two accesses in mem dep checker in the fuser."));
    for (size_t b = 0; b < info->bounds().size(); ++b)
    {
        ...
        ExprPtr minStride = IRSimplifier::simplify(alloc<Min>(aIndexStride, oIndexStride, true));
        ExprPtr maxStride = IRSimplifier::simplify(alloc<Max>(aIndexStride, oIndexStride, true));
        ...
        ExprPtr startDiff =
            IRSimplifier::simplify(alloc<Sub>(info->bounds()[b].start, other->bounds()[b].start));
        ...
        {
            startDiff = IRSimplifier::simplify(alloc<Sub>(immLike(startDiff, 0), startDiff));
        }
        ...
        {
            ExprPtr check1 =
                IRSimplifier::simplify(alloc<CompareSelect>(startDiff, minStride, kLT));
        ...
        ExprPtr check =
            IRSimplifier::simplify(alloc<CompareSelect>(startDiff, immLike(startDiff, 0), op));
        ...
        // infer safety.
        if (!parallelized && diffNegative == strideNegative && immediateEquals(startDiff, 0))
        {
            return true;
        }
    }
...
void MemDependencyChecker::visit(const ForPtr& v)
{
    ...
    auto extentsScope = std::make_shared<Scope>(currentScope_->block, currentScope_);
    ...
    // the size of quarisma least one dependent index must be >= the size of the
    // loop.
    ...
    for (size_t a = 0; a < currentScope_->accesses_.size(); ++a)
    {
        ...
        for (const auto i : quarisma::irange(indices.size()))
        {
            VarFinder vf;
            ...
            else
            {
                ...
                ExprPtr oneStep =
                    SubstituteInClone(indices[i], {{var, alloc<Add>(var, immLike(var, 1))}});
                loopIndicesStride[i] = IRSimplifier::simplify(alloc<Sub>(oneStep, zeroStep));
    ...
    for (auto& openWritePair : currentScope_->openWrites_)
    {
        for (auto& pair : openWritePair.second)
        {
            ...
            {
                bound.start =
                    IRSimplifier::simplify(SubstituteInClone(bound.start, {{var, v->start()}}));
                bound.end = IRSimplifier::simplify(SubstituteInClone(
                    bound.end, {{var, alloc<Sub>(v->stop(), immLike(v->stop(), 1))}}));

                // If the start < end then swap the order of the bound.
                ExprPtr diff = IRSimplifier::simplify(alloc<Sub>(bound.end, bound.start));
                if (diff->isConstant() && immediateIsNegative(diff))
                {
                    bound.swap();
                }
    ...
    // Store buffers allocated quarisma this scope.
    ...
    for (size_t a = 0; a < currentScope_->accesses_.size(); ++a)
    {
        ...
        for (size_t j = currentScope_->accesses_.size() - 1; j > a; --j)
        {
            ...
            for (auto& b : openBounds)
            {
                ...
                std::move(slices.begin(), slices.end(), std::back_inserter(newBoundSlices));
            }
    ...
    mergedAccesses.reserve(extentsScope->accesses_.size() + currentScope_->accesses_.size());
    std::copy(
...
void MemDependencyChecker::visit(const CondPtr& v)
{
    ...
    auto enclosingScope = std::make_shared<Scope>(currentScope_->block, currentScope_);
...
void MemDependencyChecker::insertBuffers(
    std::unordered_map<BufPtr, std::shared_ptr<AccessInfo>>& bufs, AccessType type)
{
    for (auto& pair : bufs)
    {
        ...
        auto info = std::make_shared<AccessInfo>(nextAccess_++, type, nullptr, var, bounds);
...
void MemDependencyChecker::visit(const AllocatePtr& v)
{
    ...
    flat_size         = IRSimplifier::simplify(alloc<Sub>(flat_size, immLike(flat_size, 1)));
    bounds.emplace_back(immLike(flat_size, 0), flat_size);

    auto info =
        std::make_shared<AccessInfo>(nextAccess_++, AccessType::Alloc, nullptr, var, bounds);
...
void MemDependencyChecker::visit(const FreePtr& v)
{
    ...
    auto info = std::make_shared<AccessInfo>(nextAccess_++, AccessType::Free, nullptr, var, bounds);
    ...
    bool                               insert)
{
    ...
    for (auto it = writeHistory.begin(); it != writeHistory.end();)
    {
        ...
        else
        {
            ...
            auto newBounds = subtractIndicesBounds(indexBounds, info->bounds(), overlap);
...
void MemDependencyChecker::mergeScope(
    const std::shared_ptr<Scope>& child, const std::shared_ptr<Scope>& parent, bool closeOverlapped)
{
    ...
    for (auto& info : child->accesses_)
    {
        ...
        updateWriteHistory(writeHistory, info, latestAccessToClose, closeOverlapped, false);
    }
...
std::vector<Bound> MemDependencyChecker::getIndicesBounds(const std::vector<ExprPtr>& indices)
{
