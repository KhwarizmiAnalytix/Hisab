...
template <
    ...
    std::enable_if_t<std::is_same_v<decltype(detail::bin_op_deducer(std::declval<Op>())), void>>* =
        nullptr>
static ExprPtr mutate_binary_op(NodePtr<Op> v, IRCloner* cloner, bool option = false)
{
...
ExprPtr IRCloner::mutate(const CompareSelectPtr& v)
{
    ...
    return alloc<CompareSelect>(
        lhs_new, rhs_new, retval1_new, retval2_new, v->compare_select_op(), v->bias());
}
...
// initializers, this is the expected usage of clone quarisma this point.
//
...
{
    return alloc<RoundOff>(v->lhs()->accept_mutator(this), v->rhs()->accept_mutator(this));
}

ExprPtr IRCloner::mutate(const MaxTermPtr& v)
{
    ExprPtr scalar_new = v->scalar() ? v->scalar()->accept_mutator(this) : nullptr;
    ...
    return alloc<MaxTerm>(v->hasher(), scalar_new, v->propagate_nans(), variables_new);
}
...
ExprPtr IRCloner::mutate(const MinTermPtr& v)
{
    ...
    return alloc<MinTerm>(v->hasher(), scalar_new, v->propagate_nans(), variables_new);
}
