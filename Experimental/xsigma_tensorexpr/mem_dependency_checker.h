...
{
public:
    AccessInfo(size_t id, AccessType type, StmtPtr stmt, VarPtr var, IndexBounds bounds)
        : id_(id),
          type_(type),
          stmt_(std::move(stmt)),
          expr_(nullptr),
    ...
    AccessInfo(
        size_t id, AccessType type, ExprPtr expr, StmtPtr stmt, VarPtr var, IndexBounds bounds)
        : id_(id),
          type_(type),
          stmt_(std::move(stmt)),
          expr_(std::move(expr)),
    ...
    StmtPtr stmt() const { return stmt_; }
    ...
    // dimension.
    IndexBounds& bounds() { return bounds_; }
...
public:
    ...
    std::unordered_set<std::shared_ptr<AccessInfo>> accessesWithin(const StmtPtr& A) const;
    ...
    // of A.
    std::unordered_set<std::shared_ptr<AccessInfo>> accessesWithin(const ExprPtr& A) const;
...
private:
    ...
    struct Scope
    {
        Scope(BlockPtr b, std::shared_ptr<Scope> p) : block(std::move(b)), parent(std::move(p)) {}
