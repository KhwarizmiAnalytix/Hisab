...
static llvm::orc::JITTargetMachineBuilder makeJTMBFromHost(
    std::optional<std::string> cpu, std::optional<std::string> attrs)
{
...
static llvm::orc::JITTargetMachineBuilder makeJTMBFromTriple(
    const std::string& triple, std::optional<std::string> cpu, std::optional<std::string> attrs)
{
...
{
    auto JTMB = triple ? makeJTMBFromTriple(*triple, cpu, attrs) : makeJTMBFromHost(cpu, attrs);
#if LLVM_VERSION_MAJOR >= 18
    ...
    std::unordered_set<std::string>& intrinsics)
{
    ...
    {
        assertSuccess(JD.define(absoluteSymbols({entry(kv.first.c_str(), kv.second)})));
    }
    assertSuccess(JD.define(absoluteSymbols({entry("DispatchParallel", DispatchParallel)})));
    assertSuccess(JD.define(absoluteSymbols({entry("nnc_aten_free", nnc_aten_free)})));
}
...
public:
    PytorchLLVMJITImpl(
        ...
        std::optional<std::string> cpu,
        std::optional<std::string> attrs)
        : TM(assertSuccess(makeTargetMachineBuilder(triple, cpu, attrs).createTargetMachine())),
          LLJ(assertSuccess(
              LLJITBuilder()
                  .setJITTargetMachineBuilder(makeTargetMachineBuilder(triple, cpu, attrs))
#if LLVM_VERSION_MAJOR >= 17
                  .setObjectLinkingLayerCreator(
                      ...
                      )
                      {
                          return std::make_unique<ObjectLinkingLayer>(
                              ES, assertSuccess(jitlink::InProcessMemoryManager::Create()));
                      })
#endif
                  .create()))
    {
        ...
        // - https://github.com/llvm/llvm-project/issues/65253
        {
            ...
            assertSuccess(LLJ->addIRModule(ThreadSafeModule(std::move(module_), std::move(ctx))));
        }
    }
    ...
    bool hasSymbol(const std::string& Name) { return intrinsics.find(Name) != intrinsics.end(); }

    TargetMachine& getTargetMachine() { return *TM; }

    const DataLayout& getDataLayout() { return LLJ->getDataLayout(); }
};
...
public:
    PytorchLLVMJITImpl(
        ...
        : Resolver(createLegacyLookupResolver(
              ...
              [this](const std::string& Name) -> JITSymbol
              {
                  ...
                  return assertSuccess(lookup({&ES.getMainJITDylib()}, Mangle(Name)));
              },
              [](Error Err) { assertSuccess(std::move(Err), "lookupFlags failed"); })),
          TM(assertSuccess(makeTargetMachineBuilder(triple, cpu, attrs).createTargetMachine())),
    ...
    void addModule(std::unique_ptr<Module> M, std::unique_ptr<LLVMContext> C)
    {
        ...
        assertSuccess(
            CompileLayer.addModule(K, std::move(M)), "Failed to add module to compile layer");
    }
    ...
    void removeModule(VModuleKey K) { assertSuccess(CompileLayer.removeModule(K)); }
...
void PytorchLLVMJIT::addModule(std::unique_ptr<Module> M, std::unique_ptr<LLVMContext> C)
{
