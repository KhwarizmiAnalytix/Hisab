...
public:
    ...
    CodeGen(StmtPtr stmt, Ts... ts) : stmt_(std::move(stmt)), buffer_args_({BufferArg(ts)...})
    {
    }
    ...
    void set_stmt(StmtPtr s) { stmt_ = std::move(s); }

    void apply_mutator(IRMutator* mutator) { stmt_ = stmt_->accept_mutator(mutator); }

    void apply_visitor(IRVisitor* visitor) { stmt_->accept(visitor); }

    std::vector<BufferArg>& buffer_args() { return buffer_args_; }
    ...
    // a string.
    virtual std::string getCodeText(const std::string& attr [[maybe_unused]] = "") { return ""; }
...
public:
    explicit ExtCallMemoryReuse(const std::vector<CodeGen::BufferArg>& bufferArgs);
...
public:
    ...
    VarPtr var() const { return isVar_ ? var_ : buf_->base_handle(); }

    BufPtr buf() const { return buf_; }

    bool isVar() const { return isVar_; }

    Dtype dtype() const { return isVar_ ? var_->dtype() : buf_->dtype(); }
...
public:
    ...
    template <typename T>
    CallArg(const std::vector<T>& buffer) : data_(const_cast<T*>(buffer.data()))
    {
    }
