...
#include <Quarisma/native/mkldnn/Common.h>
...
RegisterNNCLoweringsFunction::RegisterNNCLoweringsFunction(
    const std::vector<std::string>& schemas, const NNCLoweringFunction& fn)
{
...
int nnc_lowerings_lazy_registration()
{
    RegisterNNCLoweringsFunction aten_dropout(
        {"aten::dropout(Tensor input, float p, bool train) -> (Tensor)"}, computeNoop);
    ...
    RegisterNNCLoweringsFunction prepacked_conv2d_clamp_run(
        {"prepacked::conv2d_clamp_run(Tensor X, __torch__.torch.classes.xnnpack.Conv2dOpContext "
         "W_prepack) -> (Tensor Y)"},
        computePrepackedConv2dClampRun);

    // TODO: add a test
    RegisterNNCLoweringsFunction prepacked_linear_clamp_run(
        {"prepacked::linear_clamp_run(Tensor X, __torch__.torch.classes.xnnpack.LinearOpContext "
         "W_prepack) -> (Tensor Y)"},
    ...
    RegisterNNCLoweringsFunction mkldnn_prepacked_conv2d_run(
        {"mkldnn_prepacked::conv2d_run(Tensor X, __torch__.torch.classes.mkldnn.ConvOpContext "
         "W_prepack) -> (Tensor Y)"},
           ...
           quarisma::Device                 device)
        {
            ...
            return (inputs.size() > 2)
                       ? computeTwoOperandWithAlpha(
                             "aten_sub", inputs, outputShape, outputStrides, outputType, sub_lambda)
                       : computeTwoOperand(
                             "aten_sub",
                             inputs,
                             outputShape,
    ...
    RegisterNNCLoweringsFunction aten_div_scalar(
           ...
           quarisma::Device                 device)
        {
            return computeScalar(
                ...
                { return promoteIntegerToDefaultType(a) / promoteIntegerToDefaultType(b); });
        });
...
#define DEFINE_BITWISE_SCALAR_OP_LOWERING(op_name, op)                        \
    ...
    DEFINE_BITWISE_SCALAR_OP_LOWERING(__and__, boolToInteger(a) & boolToInteger(b))
    DEFINE_BITWISE_SCALAR_OP_LOWERING(__or__, boolToInteger(a) | boolToInteger(b))
    DEFINE_BITWISE_SCALAR_OP_LOWERING(__xor__, boolToInteger(a) ^ boolToInteger(b))
...
#undef DEFINE_LOGICAL_SCALAR_OP_LOWERING
    ...
    RegisterNNCLoweringsFunction aten_div(
           ...
           quarisma::Device                 device)
        {
            return computeTwoOperand(
                ...
                outputType,
                [](const ExprHandle& lhs, const ExprHandle& rhs)
                { return promoteIntegerToDefaultType(lhs) / promoteIntegerToDefaultType(rhs); });
        });
    ...
    RegisterNNCLoweringsFunction aten_gt(
           ...
           quarisma::Device                 device)
        {
            return computeTwoOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& lhs, const ExprHandle& rhs) { return cast<bool>(lhs > rhs); });
        });
    ...
    RegisterNNCLoweringsFunction aten_masked_fill(
           ...
           quarisma::Device                 device)
        {
            return computeThreeOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& input, const ExprHandle& mask, const ExprHandle& value)
                {
    ...
    RegisterNNCLoweringsFunction aten_clamp(
           ...
           quarisma::Device                 device)
        {
            ...
            return computeThreeOperand(
                ...
                outputStrides,
                outputType,
                [noMin, noMax](const ExprHandle& in, const ExprHandle& min, const ExprHandle& max)
                {
                    auto cast = [&](const ExprHandle& e) { return Cast::make(in.dtype(), e); };
    ...
    RegisterNNCLoweringsFunction aten_addcmul(
        {"aten::addcmul(Tensor self, Tensor tensor1, Tensor tensor2, *, Scalar value=1) -> "
         "(Tensor)"},
        [](const std::vector<ArgValue>&     inputs,
           const std::vector<ExprHandle>&   outputShape,
           const std::vector<ExprHandle>&   outputStrides,
    ...
    RegisterNNCLoweringsFunction aten_sigmoid(
           ...
           quarisma::Device                 device)
        {
            ...
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return sigmoid(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_relu(
           ...
           quarisma::Device                 device)
        {
            ...
            if (A.node()->qscale())
            {
                return computeQuantizedRelu(inputs, outputShape, outputStrides, outputType, device);
            }
            return computeOneOperand(
                "aten_relu",
                inputs,
    ...
    RegisterNNCLoweringsFunction aten_batch_norm(
        {"aten::batch_norm(Tensor input, Tensor? weight, Tensor? bias, Tensor? running_mean, "
         "Tensor? running_var, bool training, float momentum, float eps, bool cudnn_enabled) -> "
         "(Tensor)"},
        computeBatchNorm);
    ...
    RegisterNNCLoweringsFunction aten_log(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return log(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_log10(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return log10(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_log1p(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return log1p(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_log2(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return log2(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_exp(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return exp(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_expm1(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return expm1(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_erf(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return erf(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_erfc(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return erfc(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_cos(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return cos(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_sin(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return sin(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_tan(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return tan(promoteIntegerToDefaultType(a)); });
        });
           ...
           quarisma::Device                 device)
        {
            return computeTwoOperand(
                ...
                [](const ExprHandle& lhs, const ExprHandle& rhs)
                {
                    ...
                    double val = immediateAs<double>(IRSimplifier::simplify(rhs.node()));
    ...
    RegisterNNCLoweringsFunction aten_lerp(
           ...
           quarisma::Device                 device)
        {
            return computeThreeOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a, const ExprHandle& end, const ExprHandle& weight)
    ...
    RegisterNNCLoweringsFunction aten_remainder(
           ...
           quarisma::Device                 device)
        {
            ...
            {
                auto const& shape = broadcastShapes(valueShape(inputs[0]), valueShape(inputs[1]));
                return Compute(
                    "aten_remainder",
                    shape,
                    [&](const std::vector<VarHandle>& axes)
                    {
                        ...
                        if (allInt)
                        {
                            return demoteOutput(imodImpl(exprInputs[0], exprInputs[1]), outputType);
                        }
                        else
                        {
                            return demoteOutput(fmodImpl(exprInputs[0], exprInputs[1]), outputType);
                        }
                    });
            }
        });
    ...
    RegisterNNCLoweringsFunction aten_acos(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return acos(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_asin(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return asin(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_cosh(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return cosh(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_sinh(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return sinh(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_atan(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return atan(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_tanh(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return tanh(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_hardtanh(
           ...
           quarisma::Device                 device)
        {
            return computeThreeOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a, const ExprHandle& min_val, const ExprHandle& max_val)
                {
    ...
    RegisterNNCLoweringsFunction aten_softplus(
           ...
           quarisma::Device                 device)
        {
            return computeThreeOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a, const ExprHandle& beta, const ExprHandle& threshold)
                {
                    ...
                    return CompareSelect::make(
                        beta_a, threshold_promoted, a, log1p(exp(beta_a)) / beta_promoted, kGT);
                });
        });
    ...
    RegisterNNCLoweringsFunction aten_elu(
        {"aten::elu(Tensor self, Scalar alpha=1, Scalar scale=1, Scalar input_scale=1) -> "
         "(Tensor)"},
        [](const std::vector<ArgValue>&     inputs,
           const std::vector<ExprHandle>&   outputShape,
           const std::vector<ExprHandle>&   outputStrides,
           ...
           quarisma::Device                 device)
        {
            return computeFourOperand(
                   ...
                   const ExprHandle& input_scale)
                {
                    ...
                    return CompareSelect::make(
                        a, zero, a * poscoef, (exp(a * negiptcoef) - one) * negcoef, kGT);
                });
        });
           ...
           quarisma::Device                 device)
        {
            return computeTwoOperand(
                ...
                [](const ExprHandle& a, const ExprHandle& lambd)
                {
                    ...
                    auto neg_clambd = Cast::make(a.dtype(), ExprHandle(-0)) - pos_clambd;
                    auto zero       = Cast::make(a.dtype(), 0);
    ...
    RegisterNNCLoweringsFunction aten_rsqrt(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return rsqrt(promoteIntegerToDefaultType(a)); });
        });
    ...
    RegisterNNCLoweringsFunction aten_to(
        {"aten::to.dtype(Tensor(a) self, int dtype, bool non_blocking=False, bool copy=False, int? "
         "memory_format=None) -> (Tensor(a))",
         "aten::to.dtype_layout(Tensor(a) self, *, int? dtype=None, int? layout=None, Device? "
         "device=None, bool? pin_memory=None, bool non_blocking=False, bool copy=False, int? "
         "memory_format=None) -> (Tensor(a))",
         "aten::to.device(Tensor(a) self, Device device, int dtype, bool non_blocking=False, bool "
         "copy=False, int? memory_format=None) -> (Tensor(a))",
         "aten::to.prim_Device(Tensor(a) self, Device? device, int? dtype=None, bool "
         "non_blocking=False, bool copy=False) -> Tensor(a|b)",
         "aten::to.prim_dtype(Tensor(a) self, int? dtype=None, bool non_blocking=False, bool "
         "copy=False) -> Tensor(a|b)",
         "aten::_autocast_to_reduced_precision(Tensor(a) self, bool cuda_enabled, bool "
         "cpu_enabled, ScalarType cuda_dtype, ScalarType cpu_dtype) -> Tensor(a)",
         "aten::_autocast_to_full_precision(Tensor(a) self, bool cuda_enabled, bool cpu_enabled) "
         "-> Tensor(a)"},
        [](const std::vector<ArgValue>&     inputs,
           const std::vector<ExprHandle>&   outputShape,
           const std::vector<ExprHandle>&   outputStrides,
           ...
           quarisma::Device                 device)
        {
            ...
            return computeOneOperand(
                ...
                {
                    TORCH_INTERNAL_ASSERT(outputType, buildErrorMessage("Output type is null."));
    ...
    RegisterNNCLoweringsFunction aten_threshold(
           ...
           quarisma::Device                 device)
        {
            return computeThreeOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a, const ExprHandle& threshold, const ExprHandle& value)
    ...
    RegisterNNCLoweringsFunction aten_lgamma(
           ...
           quarisma::Device                 device)
        {
            return computeOneOperand(
                ...
                outputStrides,
                outputType,
                [](const ExprHandle& a) { return lgamma(promoteIntegerToDefaultType(a)); });
        });
    ...
    //           [&](const std::vector<VarHandle>& axes) {
    //             int64_t dim =
    //                 quarisma::maybe_wrap_dim(std::get<int64_t>(inputs[1]),
    //                 axes.size());
           ...
           quarisma::Device                 device)
        {
            ...
            auto tensor       = Compute(
                ...
                [&](const std::vector<VarHandle>& axes)
                {
                    ...
                    {
                        auto new_dim      = quarisma::maybe_wrap_dim(permute_dims[i], A.ndim());
    ...
    RegisterNNCLoweringsFunction aten_sum(
        {"aten::sum(Tensor self, *, int? dtype=None) -> (Tensor)",
         "aten::sum.dim_IntList(Tensor self, int[1]? dim, bool keepdim=False, *, int? dtype=None) "
         "-> (Tensor)"},
        computeSum);
    ...
    RegisterNNCLoweringsFunction aten_conv1d(
        {"aten::conv1d(Tensor input, Tensor weight, Tensor? bias=None, int[1] stride=1, int[1] "
         "padding=0, int[1] dilation=1, int groups=1) -> (Tensor)"},
        computeConv1d);
    RegisterNNCLoweringsFunction aten_conv2d(
        {"aten::conv2d(Tensor input, Tensor weight, Tensor? bias=None, int[2] stride=[1, 1], "
         "int[2] padding=[0, 0], int[2] dilation=[1, 1], int groups=1) -> (Tensor)"},
        computeConv2d);

    RegisterNNCLoweringsFunction aten_addmm(
        {"aten::addmm(Tensor self, Tensor mat1, Tensor mat2, *, Scalar beta=1, Scalar alpha=1) -> "
         "(Tensor)"},
        computeAddMM);

    RegisterNNCLoweringsFunction aten_mean(
        {"aten::mean(Tensor self, *, int? dtype=None) -> (Tensor)",
         "aten::mean.dim(Tensor self, int[1]? dim, bool keepdim=False, *, int? dtype=None) -> "
         "(Tensor)"},
        computeMean);
    RegisterNNCLoweringsFunction aten_max_reduction(
        {"aten::max.dim(Tensor self, int dim, bool keepdim=False) -> (Tensor values, Tensor "
         "indices)"},
        computeMax);
    ...
    RegisterNNCLoweringsFunction aten_add(
           ...
           quarisma::Device                 device)
        {
            ...
            return (inputs.size() > 2)
                       ? computeTwoOperandWithAlpha(
                             "aten_add", inputs, outputShape, outputStrides, outputType, add_lambda)
                       : computeTwoOperand(
                             "aten_add",
                             inputs,
                             outputShape,
    ...
    RegisterNNCLoweringsFunction aten_embedding(
        {"aten::embedding(Tensor weight, Tensor indices, int padding_idx=-1, bool "
         "scale_grad_by_freq=False, bool sparse=False) -> Tensor"},
        computeEmbedding);
...
#define NNC_QUANTIZATION_EXPR_DEQUANT 1
    ...
    RegisterNNCLoweringsFunction aten_quantize_per_tensor(
        {"aten::quantize_per_tensor(Tensor self, float scale, int zero_point, int dtype) -> "
         "(Tensor)",
         "aten::quantize_per_tensor.tensor_qparams(Tensor self, Tensor scale, Tensor zero_point, "
         "int dtype) -> (Tensor)",
         "aten::quantize_per_tensor.tensors(Tensor[] tensors, Tensor scales, Tensor zero_points, "
         "int dtype) -> (Tensor[])"},
    ...
    RegisterNNCLoweringsFunction quantized_conv1d(
        {"quantized::conv1d(Tensor qx, __torch__.torch.classes.quantized.Conv2dPackedParamsBase "
         "packed_weight, float output_scale, int output_zero_point) -> (Tensor)"},
        computeQuantizedConv1d);

    RegisterNNCLoweringsFunction quantized_conv2d(
        {"quantized::conv2d.new(Tensor qx, "
         "__torch__.torch.classes.quantized.Conv2dPackedParamsBase packed_weight, float "
    ...
    RegisterNNCLoweringsFunction quantized_linear(
        {"quantized::linear(Tensor X, __torch__.torch.classes.quantized.LinearPackedParamsBase "
         "W_prepack, float Y_scale_i, int Y_zero_point_i) -> (Tensor Y)"},
        computeQuantizedLinear);

    RegisterNNCLoweringsFunction quantized_linear_relu(
        {"quantized::linear_relu(Tensor X, "
         "__torch__.torch.classes.quantized.LinearPackedParamsBase W_prepack, float Y_scale_i, int "
    ...
    RegisterNNCLoweringsFunction quantized_mul_scalar(
        {"quantized::mul.Scalar(Tensor qa, Scalar b) -> (Tensor qc)"}, computeQuantizedMulScalar);

    RegisterNNCLoweringsFunction quantized_conv2d_prepack(
        {"quantized::conv2d_prepack(Tensor weight, Tensor? bias, int[] stride, int[] padding, "
    ...
    RegisterNNCLoweringsFunction aten_upsample_nearest2d(
        {"aten::upsample_nearest2d.vec(Tensor input, int[]? output_size, float[]? scale_factors) "
         "-> (Tensor)"},
