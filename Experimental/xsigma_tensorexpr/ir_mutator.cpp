...
template <
    ...
    std::enable_if_t<std::is_same_v<decltype(detail::bin_op_deducer(std::declval<Op>())), void>>* =
        nullptr>
static ExprPtr mutate_binary_op(NodePtr<Op> v, IRMutator* mutator, bool option = false)
{
    ExprPtr lhs     = v->lhs();
    ExprPtr rhs     = v->rhs();
    ...
    Dtype dtype_new = BinaryOpDtype(lhs_new->dtype(), rhs_new->dtype(), ScalarType::Undefined);
...
StmtPtr IRMutator::mutate(const ExternalCallPtr& v)
{
    BufPtr buf     = v->buf();
    BufPtr buf_new = to<Buf>(buf->accept_mutator(this));
    TORCH_INTERNAL_ASSERT(buf_new, buildErrorMessage("IRMutator produced null for Buf."));
    ...
    for (const BufPtr& buf_arg : v->buf_args())
    {
        ...
        TORCH_INTERNAL_ASSERT(buf_arg_new, buildErrorMessage("IRMutator produced null for Buf."));
...
StmtPtr IRMutator::mutate(const PlacementAllocatePtr& v)
{
    ...
    TORCH_INTERNAL_ASSERT(buf_to_reuse_new, buildErrorMessage("IRMutator produced null for Buf."));
