...
QUARISMA_DIAGNOSTIC_PUSH_AND_IGNORED_IF_DEFINED("-Wmismatched-new-delete")
    ...
    namespace torch::jit::tensorexpr
{
    ...
    llvm::CmpInst::Predicate llvm_comparison_predicate(
        CompareSelectOperation compare_op, const ScalarType& type)
    {
        switch (compare_op)
        {
    ...
    llvm::CmpInst::Predicate llvm_fp_comparison_predicate(CompareSelectOperation compare_op)
    {
        switch (compare_op)
        {
        case CompareSelectOperation::kEQ:
...
#error Only LLVM versions 8 and above are supported.
    ...
    struct FunctionCallee
    {
        ...
        llvm::FunctionType* getFunctionType() { return ft_; }

        llvm::Value* getCallee() { return v_; }
...
#error Only LLVM versions 8 and above are supported.
    ...
    {
    public:
        LLVMCodeGenCallee(std::unique_ptr<llvm::orc::PytorchLLVMJIT> jit, void* kernelAddress)
            : jit_(std::move(jit)), kernelAddress_(kernelAddress)
        {
        }

        llvm::orc::PytorchLLVMJIT* getJIT() { return jit_.get(); }

        void* getKernelAddress() { return kernelAddress_; }

        void setKernelAddress(void* kernelAddress) { kernelAddress_ = kernelAddress; }
...
#undef LLVM_TYPE_DECLARE
    ...
    private:
        ...
        SimdCallee getSimdFunction(
            const std::string& name, llvm::Type* type, Arity arity, int lanes);
...
#undef IMM_VISIT_DECLARE
        ...
        llvm::Value* emitMaskedLoad(
            llvm::Type* ty, llvm::Value* addr, llvm::Value* idx, llvm::Value* mask);
        void emitUnmaskedStore(
            llvm::Type* ty, llvm::Value* base, llvm::Value* idx, llvm::Value* val);
...
LLVMCodeGen::LLVMCodeGen(StmtPtr stmt) : LLVMCodeGen(stmt, std::vector<CodeGen::BufferArg>()) {}
    ...
    : CodeGen(stmt, args, device, kernel_func_name)
{
    impl_ = std::make_unique<LLVMCodeGenImpl>(
        this->stmt(), args, device, dtype, this->kernel_func_name(), triple, cpu, attrs);
    callee_ =
        std::make_unique<LLVMCodeGenCallee>(impl_->releaseJIT(), (void*)impl_->getKernelAddress());
}
...
quarisma::Tensor LLVMCodeGen::empty_strided(
...
void LLVMCodeGenImpl::emitWrapper(const std::vector<llvm::Type*>& params)
{
...

    {
        ...
        // Otherwise, if the function was placed quarisma the beginning of a page, reading
    ...
    for (const auto i : quarisma::irange(params.size()))
    {
#if LLVM_VERSION_MAJOR >= 15
        auto argp = irb_.CreateGEP(
            OpqPtrTy_, wrapper->arg_begin(), llvm::ConstantInt::getSigned(IntTy_, i));
        if (params[i]->isPointerTy())
        {
            auto arg = irb_.CreatePointerCast(irb_.CreateLoad(OpqPtrTy_, argp), params[i]);
            wrappedArgs.push_back(arg);
        }
        else
        {
            auto p   = irb_.CreatePointerCast(irb_.CreateLoad(OpqPtrTy_, argp), OpqPtrTy_);
            auto arg = irb_.CreateLoad(params[i], p);
            wrappedArgs.push_back(arg);
        }
#else
        auto argp = irb_.CreateGEP(
            voidPtrTy, wrapper->arg_begin(), llvm::ConstantInt::getSigned(IntTy_, i));
        if (params[i]->isPointerTy())
        {
            auto arg = irb_.CreatePointerCast(
                irb_.CreateLoad(argp->getType()->getPointerElementType(), argp), params[i]);
            wrappedArgs.push_back(arg);
        }
        else
        {
...
private:
    ExprPtr mutate(const IntrinsicsPtr& v) override
    {
        ...
        else if (v->op_type() == kSigmoid)
        {
            ...
            if (stype == ScalarType::Float)
            {
                return fast_sigmoid(ExprHandle(v->param(0)->accept_mutator(this))).node();
            }
        }
        // TODO: fast exp
        // TODO: fast erf
...
void LLVMCodeGenImpl::emitKernel(StmtPtr stmt, const std::vector<llvm::Type*>& params)
{
...
void LLVMCodeGenImpl::visit(const MaxPtr& v)
{
    ...
    {
        auto icmp =
            v->dtype().is_signed() ? irb_.CreateICmpSGT(lhs, rhs) : irb_.CreateICmpUGT(lhs, rhs);
        value_ = irb_.CreateSelect(icmp, lhs, rhs);
        return;
    }

    value_ = irb_.CreateSelect(
        irb_.CreateFCmp(llvm::FCmpInst::FCMP_UNO, lhs, llvm::ConstantFP::get(lhs->getType(), 0.0)),
        lhs,
        irb_.CreateSelect(irb_.CreateFCmp(llvm::FCmpInst::FCMP_OGT, lhs, rhs), lhs, rhs));
}
...
void LLVMCodeGenImpl::visit(const MinPtr& v)
{
    ...
    {
        auto icmp =
            v->dtype().is_signed() ? irb_.CreateICmpSLT(lhs, rhs) : irb_.CreateICmpULT(lhs, rhs);
        value_ = irb_.CreateSelect(icmp, lhs, rhs);
        return;
    }
    ...
    value_ = irb_.CreateSelect(
        irb_.CreateFCmp(llvm::FCmpInst::FCMP_UNO, lhs, llvm::ConstantFP::get(lhs->getType(), 0.0)),
        lhs,
        irb_.CreateSelect(irb_.CreateFCmp(llvm::FCmpInst::FCMP_OLT, lhs, rhs), lhs, rhs));
}
...
void LLVMCodeGenImpl::visit(const CompareSelectPtr& v)
{
    auto genUnbiased = [this, v]() -> llvm::Value*
    {
        ...
        if (quarisma::isIntegralType(type_used, true))
        {
            cmp_ = irb_.CreateICmp(llvm_comparison_predicate(cmp_op_, type_used), lhs, rhs);
        }
        else if (quarisma::isFloatingType(type_used))
        {
    ...
    auto genBiased = [this, v]() -> llvm::Value*
    {
        ...
        if (quarisma::isIntegralType(cmp_type, true))
        {
            cmp = irb_.CreateICmp(llvm_comparison_predicate(cmp_op, cmp_type), lhs, rhs);
        }
        else if (quarisma::isFloatingType(cmp_type))
        {
...
template <typename T>
std::enable_if_t<std::is_integral_v<T>, llvm::Value*> getFromType(llvm::Type* type, T value)
{
    return llvm::ConstantInt::get(type, value, std::is_signed_v<T>);
}

template <typename T>
std::enable_if_t<std::is_floating_point_v<T>, llvm::Value*> getFromType(llvm::Type* type, T value)
{
...
void LLVMCodeGenImpl::visit(const CastPtr& v)
{
    ...
    bool is_to_bf16    = (dst_type == quarisma::kBFloat16);
    bool is_to_float   = (dst_type == quarisma::kFloat);
    bool is_from_bf16  = (src_type == quarisma::kBFloat16);
    bool is_from_float = (src_type == quarisma::kFloat);
    ...
    llvm::Type* dstType = llvmTypeToVec(dtypeToLLVM(v->dtype()), v->dtype().lanes());
    ...
    bool srcUnsigned = v->src_value()->dtype().scalar_type() == ScalarType::Byte ||
    ...
    if (is_to_bf16)
    {
        ...
        // STEP1: U32_VAL = BITCAST(F32_VAL)
        auto fp32_i32_value = irb_.CreateBitOrPointerCast(value_, llvmTypeToVec(IntTy_, lans));
        // STEP2: U32_VAL_TMP = (U32_VAL >> 16)
    ...
    if (srcType->isFPOrFPVectorTy())
    {
        if (dstType->isFPOrFPVectorTy())
        {
            ...
            {
                value_ = irb_.CreateFPCast(value_, llvmTypeToVec(FloatTy_, v->dtype().lanes()));
            }
        ...
        else if (dstType->isIntOrIntVectorTy())
        {
            ...
            {
                llvm::Value* zero = toVec(llvm::ConstantFP::get(srcType, 0.), v->dtype().lanes());
                value_            = irb_.CreateFCmp(llvm::FCmpInst::FCMP_UNE, value_, zero);
                value_ = irb_.CreateICmpEQ(value_, llvm::ConstantInt::get(value_->getType(), 1));
...
llvm::Value* LLVMCodeGenImpl::varToValue(VarPtr v)
{
    ...
    {
        return varToVal_.quarisma(v);
    }
    else if (varToArg_.count(v))
    {
        auto idx = varToArg_.quarisma(v);
...
void LLVMCodeGenImpl::replaceVarMapping(
    const std::vector<VarPtr>& vars, const std::vector<llvm::Value*>& vals)
{
    QUARISMA_CHECK(vars.size() == vals.size());
    for (const auto i : quarisma::irange(vars.size()))
    {
        VarPtr       var = vars[i];
...
llvm::Value* LLVMCodeGenImpl::emitUnmaskedLoad(llvm::Type* ty, llvm::Value* base, llvm::Value* idx)
{
#if LLVM_VERSION_MAJOR >= 15
...
#else
    auto addr =
        irb_.CreateGEP(base->getType()->getScalarType()->getPointerElementType(), base, idx);
    return irb_.CreateLoad(addr->getType()->getPointerElementType(), addr);
#endif
}

llvm::Value* LLVMCodeGenImpl::emitMaskedLoad(
    llvm::Type* ty, llvm::Value* base, llvm::Value* idx, llvm::Value* mask)
{
    ...
    if (idx_ramp)
    {
        ...
        if (stride_imm && *stride_imm == 1)
        {
            ...
            auto addr = irb_.CreateGEP(
                base->getType()->getScalarType()->getPointerElementType(), base, first_idx);
#endif
...
TypedPointer LLVMCodeGenImpl::packFuncArgs(const std::vector<llvm::Value*>& func_args)
{
    if (func_args.empty())
    {
    ...
    for (const auto i : quarisma::irange(func_args.size()))
    {
...
std::vector<llvm::Value*> LLVMCodeGenImpl::unpackFuncArgs(TypedPointer packed, int arg_count)
{
    // TODO: extract arg_count from packed.
    std::vector<llvm::Value*> func_args(arg_count);
    llvm::Value*              zero = llvm::ConstantInt::get(IntTy_, 0);
    for (const auto i : quarisma::irange(arg_count))
    {
...
#else
// Pack the arguments into an aggregate struct for forwarding.
llvm::Value* LLVMCodeGenImpl::packFuncArgs(const std::vector<llvm::Value*>& func_args)
{
    if (func_args.empty())
    {
...
// Unpack the aggregate struct into individual arguments.
std::vector<llvm::Value*> LLVMCodeGenImpl::unpackFuncArgs(llvm::Value* packed, int arg_count)
{
    // TODO: extract arg_count from packed.
    std::vector<llvm::Value*> func_args(arg_count);
    llvm::Value*              zero = llvm::ConstantInt::get(IntTy_, 0);
    for (const auto i : quarisma::irange(arg_count))
    {
        ...
        llvm::Value* dst_ptr =
            irb_.CreateInBoundsGEP(packed_type, packed, {zero, llvm::ConstantInt::get(IntTy_, i)});
        func_args[i] = irb_.CreateLoad(dst_ptr->getType()->getPointerElementType(), dst_ptr);
    }
    return func_args;
}
#endif
...
void LLVMCodeGenImpl::processParallelFor(ForPtr v)
{
...
#if LLVM_VERSION_MAJOR >= 15
    auto func_type = llvm::FunctionType::get(VoidTy_, {LongTy_, OpqPtrTy_}, false);
#else
    auto func_type = llvm::FunctionType::get(VoidTy_, {LongTy_, Int8PtrTy_}, false);
#endif

    llvm::Function* func =
        llvm::Function::Create(func_type, llvm::Function::PrivateLinkage, "func", module_.get());
...
#if LLVM_VERSION_MAJOR >= 15
    llvm::Value* packed_caller_args_ptr = irb_.CreatePointerCast(packed_caller_args, OpqPtrTy_);
    llvm::Value* func_value             = irb_.CreatePointerCast(func, OpqPtrTy_);
    llvm::FunctionType* dispatcher_fntype =
        llvm::FunctionType::get(VoidTy_, {OpqPtrTy_, LongTy_, LongTy_, OpqPtrTy_}, false);
#else
    llvm::Value* packed_caller_args_ptr = irb_.CreatePointerCast(packed_caller_args, Int8PtrTy_);
    llvm::Value* func_value             = irb_.CreatePointerCast(func, Int8PtrTy_);
    llvm::FunctionType* dispatcher_fntype =
        llvm::FunctionType::get(VoidTy_, {Int8PtrTy_, LongTy_, LongTy_, Int8PtrTy_}, false);
#endif

    FunctionCallee dispatcher_callee =
        module_->getOrInsertFunction("DispatchParallel", dispatcher_fntype);
    llvm::Function* dispatcher = llvm::cast<llvm::Function>(dispatcher_callee.getCallee());
    ...
    irb_.CreateCall(dispatcher, {func_value, start, stop, packed_caller_args_ptr});
    value_ = llvm::ConstantInt::get(IntTy_, 0);
}
...
void LLVMCodeGenImpl::emitMaskedStore(
    llvm::Type* ty, llvm::Value* base, llvm::Value* idx, llvm::Value* mask, llvm::Value* val)
{
...
void LLVMCodeGenImpl::visit(const StorePtr& v)
{
    ...
    if (idx_ramp)
    {
        ...
        if (stride_imm && *stride_imm == 1)
        {
...
#if LLVM_VERSION_MAJOR >= 15
            auto addr = irb_.CreateGEP(dtypeToLLVM(v->value()->dtype()), base, first_idx);
#else
            auto addr = irb_.CreateGEP(
                base->getType()->getScalarType()->getPointerElementType(), base, first_idx);
#endif
            ...
            auto vaddr =
                irb_.CreateBitOrPointerCast(addr, llvm::PointerType::get(val->getType(), 0));
...
void LLVMCodeGenImpl::visit(const IfThenElsePtr& v)
{
    ...
    llvm::Value* c = irb_.CreateICmpNE(condition, llvm::ConstantInt::get(condition->getType(), 0));
...
void LLVMCodeGenImpl::emitIsNan(IntrinsicsPtr v)
{
    ...
    else
    {
        ...
        auto is_nan = irb_.CreateFCmpUNO(value_, llvm::ConstantFP::get(value_->getType(), 0.));
        if (v->dtype().lanes() > 1)
        {
            dstType = llvm::VectorType::get(dstType, ElementCount(v->dtype().lanes()));
        }
...
LLVMCodeGenImpl::SimdCallee LLVMCodeGenImpl::getSimdFunction(
    const std::string& basename, llvm::Type* basetype, Arity arity, int lanes)
{
    std::string name;
    llvm::Type* type;
    bool        useSimd;
...
#undef SIMD_BINARY_MATH_CASE
        ...
        case kRemainder:
        {
            FunctionCallee callee = module_->getOrInsertFunction(
                "remainderf", llvm::FunctionType::get(FloatTy_, {FloatTy_, FloatTy_}, false), {});
    ...
    else
    {
        TORCH_INTERNAL_ASSERT(
            ...
            buildErrorMessage(
                ...
                std::to_string(v->dtype().scalar_dtype()) + " in LLVM codegen of the fuser."));
    }
...
{
    llvm::Value* ptr = varToVal_.quarisma(buf_to_reuse->base_handle());
...
void LLVMCodeGenImpl::visit(const ExternalCallPtr& v)
{
...
#if LLVM_VERSION_MAJOR >= 15
    llvm::Value* buf_ptrs =
        irb_.CreateAlloca(OpqPtrTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_num));
#else
    llvm::Value* buf_ptrs =
        irb_.CreateAlloca(Int8PtrTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_num));
#endif
    llvm::Value* buf_ranks =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_num));
    llvm::Value* buf_dims =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, dims_num));
    llvm::Value* buf_strides =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, dims_num));
    llvm::Value* buf_dtypes =
        irb_.CreateAlloca(ByteTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_num));
    llvm::Value* extra_args =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, args_num));
...
#if LLVM_VERSION_MAJOR >= 15
        auto gep =
            irb_.CreateInBoundsGEP(OpqPtrTy_, buf_ptrs, llvm::ConstantInt::getSigned(IntTy_, i));
        auto buf_void_ptr = irb_.CreatePointerCast(buf_ptr, OpqPtrTy_);
#else
        auto gep =
            irb_.CreateInBoundsGEP(Int8PtrTy_, buf_ptrs, llvm::ConstantInt::getSigned(IntTy_, i));
        auto buf_void_ptr = irb_.CreatePointerCast(buf_ptr, Int8PtrTy_);
#endif
        irb_.CreateStore(buf_void_ptr, gep);

        // Store dtype of the buf
        gep = irb_.CreateInBoundsGEP(ByteTy_, buf_dtypes, llvm::ConstantInt::getSigned(IntTy_, i));
        irb_.CreateStore(
            llvm::ConstantInt::getSigned(ByteTy_, (int8_t)b->dtype().scalar_type()), gep);

        // Store rank of the buf
        gep = irb_.CreateInBoundsGEP(LongTy_, buf_ranks, llvm::ConstantInt::getSigned(IntTy_, i));
        irb_.CreateStore(llvm::ConstantInt::getSigned(LongTy_, b->dims().size()), gep);

        // Store dims of the buf
        for (const auto dim : quarisma::irange(b->dims().size()))
        {
    ...
    {
        auto gep =
            irb_.CreateInBoundsGEP(LongTy_, extra_args, llvm::ConstantInt::getSigned(IntTy_, i));
        arg->accept(this);
...
void LLVMCodeGenImpl::visit(const ExternalCallWithAllocPtr& v)
{
    ...
    llvm::Value* buf_ptrs = irb_.CreateAlloca(
        OpqPtrTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_in_size + 2 * bufs_out_size));
#else
    llvm::Value* buf_ptrs = irb_.CreateAlloca(
        Int8PtrTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_in_size + 2 * bufs_out_size));
#endif
    // @lint-ignore CLANGTIDY
    llvm::Value* buf_ranks =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_in_size));
    llvm::Value* buf_dims =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, dims_num));
    llvm::Value* buf_strides =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, dims_num));
    llvm::Value* buf_dtypes =
        irb_.CreateAlloca(ByteTy_, llvm::ConstantInt::getSigned(IntTy_, bufs_in_size));
    // @lint-ignore CLANGTIDY
    llvm::Value* extra_args =
        irb_.CreateAlloca(LongTy_, llvm::ConstantInt::getSigned(IntTy_, args_num));
    ...
    // @lint-ignore CLANGTIDY
    for (const auto i : quarisma::irange(bufs_out_size))
    {
        ...
        for (auto it = bufsExtAllocReuse_.find(buf_out); it != bufsExtAllocReuse_.end(); it++)
        {
            auto buf = it->second;
...
{
    llvm::Value* size = llvm::ConstantInt::getSigned(LongTy_, v->dtype().byte_size());
    ...
    llvm::Instruction* I =
        irb_.CreateMalloc(LongTy_, dtypeToLLVM(v->dtype()), size, nullptr, nullptr, "");
    varToVal_[v->buffer_var()] = I;
#else
    llvm::Instruction* I = llvm::CallInst::CreateMalloc(
        irb_.GetInsertBlock(), LongTy_, dtypeToLLVM(v->dtype()), size, nullptr, nullptr);
...
void LLVMCodeGenImpl::visit(const FreePtr& v)
{
    ...
    llvm::Value* ptr = bufsExtToFreeVal_.count(v->buffer_var())
                           ? bufsExtToFreeVal_.quarisma(v->buffer_var())
                           : varToVal_.quarisma(v->buffer_var());
...
void LLVMCodeGenImpl::visit(const FreeExtPtr& v)
{
...
#if LLVM_VERSION_MAJOR >= 15
        llvm::Value* gep =
            irb_.CreateInBoundsGEP(OpqPtrTy_, ptrs, llvm::ConstantInt::getSigned(IntTy_, i));
#else
        llvm::Value* gep =
            irb_.CreateInBoundsGEP(Int8PtrTy_, ptrs, llvm::ConstantInt::getSigned(IntTy_, i));
#endif
    ...
    irb_.CreateCall(call_ty, call_fn, {llvm::ConstantInt::getSigned(LongTy_, bufs_num), ptrs});
...
void LLVMCodeGenImpl::visit(const CondPtr& v)
{
    ...
    llvm::Value* condition = value_;
    llvm::Value* c = irb_.CreateICmpNE(condition, llvm::ConstantInt::get(condition->getType(), 0));
    llvm::BasicBlock* then_block = llvm::BasicBlock::Create(getContext(), "then", fn_);
    llvm::BasicBlock* else_block = nullptr;
    if (v->false_stmt())
    {
        else_block = llvm::BasicBlock::Create(getContext(), "else", fn_);
    }
    llvm::BasicBlock* end_block = llvm::BasicBlock::Create(getContext(), "end", fn_);
...
void LLVMCodeGenImpl::optimize(llvm::Module& M)
{
    ...
    // Take a look quarisma the PassBuilder constructor parameters for more
