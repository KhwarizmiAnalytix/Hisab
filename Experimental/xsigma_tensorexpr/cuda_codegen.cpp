...
void CudaAnalysis::visit(const ForPtr& v)
{
    ...
    if (loop_options.is_gpu_block_index())
    {
        ...
        if (!is_zero(v->start()))
        {
            throw std::runtime_error(
                "start must be zero for gpu_block_index: " + std::to_string(v->start()));
        }
    ...
    else if (loop_options.is_gpu_thread_index())
    {
        ...
        {
            throw std::runtime_error(
                "start must be zero for gpu_thread_index: " + std::to_string(v->start()));
        }
...
{
    std::string castFn = v->dtype().scalar_type() == ScalarType::Half       ? "__float2half"
...
public:
    AtomicAddFuser(
        const std::unordered_set<VarPtr>& thread_local_bufs, const GPUMetaVarRewriter& metavars)
...
void CudaPrinter::visit(const AtomicAddPtr& v)
{
    ...
    if (cuda_analysis_->thread_local_bufs().count(v->base_handle()) > 0)
    {
        ...
        os() << *v->base_handle() << "[" << *v->flat_index() << "] += " << *v->value() << ";";
    }
    else
    {
...
private:
    ...
    void PushList() { load_stack_.emplace_back(); }
    ...
    for (auto& segment : innerSegments)
    {
        ...
        for (size_t i = 0; i < gpu_thread_vars_.size(); ++i)
        {
            if (!exprEquals(current_thread_reach_[i], thread_extents[i]))
            {
                ...
                inner = alloc<Cond>(
                    alloc<CompareSelect>(
                        gpu_thread_vars_[i], current_thread_reach_[i], CompareSelectOperation::kLT),
                    inner,
                    nullptr);
            }
        }
        ...
        for (size_t i = 0; i < gpu_block_vars_.size(); ++i)
        {
            if (!exprEquals(current_block_reach_[i], block_extents[i]))
            {
                ...
                inner = alloc<Cond>(
                    alloc<CompareSelect>(
                        gpu_block_vars_[i], current_block_reach_[i], CompareSelectOperation::kLT),
                    inner,
                    nullptr);
            }
        }
...
static std::ostream& operator<<(std::ostream& out, const std::vector<ExprPtr>& exprs)
{
    size_t i = 0;
...
void CudaCodeGen::Initialize()
{
    ...
    printer_          = std::make_unique<CudaPrinter>(&oss_, cuda_analysis_.get(), has_random_);
    metavar_rewriter_ = std::make_unique<GPUMetaVarRewriter>(cuda_analysis_.get());
    ...
    for (size_t i = 0; i < buffer_args.size(); i++)
    {
        ...
        os() << printer_->dtypeToCppString(dtype) << (buffer_arg.isVar() ? " " : "* ")
    ...
    if (has_random_)
    {
        ...
        os() << "Philox " << *rand_func << "(" << *rand_seed << ", " << *idx << ", " << *rand_offset
             << ");" << '\n';
        os() << '\n';
    }
    ...
    AtomicAddFuser atomic_add_fuser(cuda_analysis_->thread_local_bufs(), *metavar_rewriter_);
    ...
    const std::vector<ExprPtr>& gpu_block_extents = metavar_rewriter_->gpu_block_extents();
    ...
    bool canCallWithNumel = !has_random_ && !block_extents.empty() && !thread_extents.empty();
...
void CudaCodeGen::call_with_numel(void** args, int64_t numel)
{
    ...
    size_t      gpu_block_extents  = (numel + thread_block_size_ - 1) / thread_block_size_;
...
void CudaCodeGen::call_raw(const std::vector<void*>& raw_args)
{
    ...
    const std::vector<ExprPtr>& gpu_block_extents  = metavar_rewriter_->gpu_block_extents();
    const std::vector<ExprPtr>& gpu_thread_extents = metavar_rewriter_->gpu_thread_extents();
    if (gpu_block_extents.size() > 3 || gpu_thread_extents.size() > 3)
    {
        throw malformed_input("cuda_codegen: block or thread extent greater than 3D");
    }
    ...
    for (size_t i = 0; i < buffer_args.size(); i++)
    {
        ptr_to_args[i] = buffer_args[i].isVar() ? raw_args[i] : const_cast<void**>(&raw_args[i]);
    }
...
void CudaCodeGen::CompileToNVRTC(const std::string& code, const std::string& func_name)
{
    ...
    AT_CUDA_NVRTC_CHECK(
        nvrtc().nvrtcCreateProgram(&program, code.c_str(), nullptr, 0, nullptr, nullptr));
    ...
    const std::vector<const char*> args = {"--std=c++17", compute.c_str(), "-default-device"};
#endif
    ...
    ResourceGuard holdProgram([&] { AT_CUDA_NVRTC_CHECK(nvrtc().nvrtcDestroyProgram(&program)); });
    ...
    AT_CUDA_DRIVER_CHECK(nvrtc().cuModuleGetFunction(&function_, module, func_name.c_str()));
