...
{
    static const auto enable_opt = quarisma::utils::get_env("PYTORCH_TENSOREXPR_FALLBACK");
...
{
    const auto enable_opt = quarisma::utils::get_env("PYTORCH_TENSOREXPR_RANDOM_TRANSFORM_SEED");
    if (!enable_opt.has_value())
    {
        return 0;
    }
...
{
    static const auto enable_opt = quarisma::utils::get_env("PYTORCH_TENSOREXPR_DONT_USE_LLVM");
    if (!enable_opt)
    {
        return false;
    }
...
std::optional<quarisma::Device> pickDeviceType(const quarisma::ArrayRef<torch::jit::Value*>& inputs)
{
...
static std::optional<quarisma::Device> pickDeviceType(const std::shared_ptr<Graph>& graph)
{
    std::optional<quarisma::Device> device = std::nullopt;
            ...
            if (auto tt = input->type()->cast<TensorType>())
            {
                if (auto inputDevice = tt->device())
                {
                    TORCH_INTERNAL_ASSERT(
                        ...
                        buildErrorMessage("Different devices specified for inputs to the fuser."));
                    device = inputDevice;
                }
            }
        }
...
static std::optional<TensorInfo> getTensorInfoJit(torch::jit::Value* v)
{
    ...
    quarisma::ScalarType dtype = quarisma::ScalarType::Float;
...
bool isContiguous(const torch::jit::Value* v, quarisma::MemoryFormat memory_format)
{
    ...
    if ((memory_format == quarisma::MemoryFormat::ChannelsLast && ndims != 4) ||
        (memory_format == quarisma::MemoryFormat::ChannelsLast3d && ndims != 5))
    {
        return false;
    }
...
bool mkldnnPrepackedConvIsSupportedJit(const torch::jit::Node* node)
{
    ...
    {
        GRAPH_DEBUG("mkldnnPrepackedConvIsSupported: weight or bias is not Constant");
        return false;
    }

    // Input and weight should be NHWC contiguous.
    if (!(isContiguous(node->input(0), quarisma::MemoryFormat::ChannelsLast) &&
          isContiguous(node->input(1), quarisma::MemoryFormat::ChannelsLast)))
    {
        GRAPH_DEBUG(
...
bool isConv2d(const Node* node)
{
    ...
    if (stride.value().toIntList().size() != 2 || pad.value().toIntList().size() != 2 ||
        dilation.value().toIntList().size() != 2 || output_padding.value().toIntList().size() != 2)
    {
...
static quarisma::ScalarType tensorType(const BufPtr& b)
{
    return static_cast<quarisma::ScalarType>(b->dtype().scalar_type());
}
...
ExprHandle TensorExprKernel::getVarForShape(const quarisma::ShapeSymbol& ss)
{
    if (ss.is_static())
    {
...
#if QUARISMA_RETURN_MOVE_IF_OLD_COMPILER
        return std::move(var);
#else
        return var;
#endif
    ...
    const quarisma::SymbolicShape& shape)
{
    ...
    for (const auto i : quarisma::irange(rank))
    {
...
std::vector<ExprHandle> TensorExprKernel::sizesForValue(const torch::jit::Value* v)
{
    if (known_sizes_.count(v))
    {
        return known_sizes_.quarisma(v);
    }
    ...
    if (v->type()->isSubtypeOf(*FloatType::get()) || v->type()->isSubtypeOf(*BoolType::get()) ||
    ...
    std::string msg =
        std::string("Unhandled node kind (in sizesForValue): ") + v->node()->kind().toQualString();
...
static bool constZeroDimTensorAsScalarArg(const Value* v, std::vector<ArgValue>& args)
{
    ...
    quarisma::ScalarType dtype = quarisma::typeMetaToScalarType(t.dtype());
    switch (dtype)
    {
...
Tensor TensorExprKernel::computeValue(const torch::jit::Value* v)
{
    ...
    {
        outputStrides = quarisma::fmap<ExprHandle>(make_channels_last_strides(outputShape));
    }
    else
    {
        // Default
        outputStrides = quarisma::fmap<ExprHandle>(make_contiguous_strides(outputShape));
    }
    ...
    else if (op == aten::conv2d)
    {
        ...
        if (std::get_if<ArgNone>(&argInputs[2]))
        {
            ...
            auto bias_tensor = quarisma::zeros({outputShape[1].AsNode<LongImm>()->value()});
    ...
    {
        return custom_lowering(argInputs, outputShape, outputStrides, outputType, device_);
    }
    if (v->node()->maybeSchema())
    {
        if (NNCLoweringFunction lowering =
                getStandardLoweringFor(quarisma::toString(v->node()->schema())))
        {
    ...
    if (v->node()->maybeSchema())
    {
        msg += std::string("\nSchema: ") + quarisma::toString(v->node()->schema());
    }
    throw malformed_input(msg);
}
...
// quarisma any level containing non-loops or non-matching bounds.  The restriction
...
{
    auto tc =
        IRSimplifier::simplify(cast<int64_t>(ExprHandle(loop->stop()) - ExprHandle(loop->start())));
...
{
    int64_t trips   = 1;
    auto    threads = quarisma::get_num_threads();
    auto    it      = loops.begin();
    for (; it != loops.end(); it++)
    {
        if (trips >= threads)
...
StmtPtr TensorExprKernel::transformLoops(BackendType backendType, StmtPtr st)
{
    ...
    if (random_tr_seed)
    {
        ...
        GRAPH_DEBUG("After random transform:\n", std::to_string(l.root_stmt()), "\n");
    }
    ...
    if (backendType == kCudaCodeGen)
    {
        for (const auto& buf : bufOutputs_)
        {
            ...
            {
                throw std::runtime_error("Invalid loop-level: " + std::to_string(loopLevels));
            }
        }
    }
    ...
    if (backendType == kBlockCodeGen)
    {
        for (const auto& buf : bufOutputs_)
        {
            ...
            TORCH_INTERNAL_ASSERT(
                !loops.empty(),
                buildErrorMessage(
                    "No loops found for the buffer " + buf->name_hint() + " in the fuser."));
            ForPtr flattened = nullptr;
            LoopNest::flatten(loops, &flattened);
            assert(flattened);
...
std::string TensorExprKernel::getCodeGenName(BackendType backendType)
{
    ...
    default:
        throw std::runtime_error(
            "invalid backend type: " + std::to_string(static_cast<int>(backendType)));
    }
}
...
TensorExprKernel::BackendType TensorExprKernel::inferBackendTypeFromDevice(quarisma::Device device)
{
    BackendType backendType = BackendType::kUninitialized;
    if (device.type() == quarisma::kCUDA)
    {
        backendType = kCudaCodeGen;
    }
    else if (device.type() == quarisma::kCPU && getTEGenerateBlockCode())
    {
        backendType = kBlockCodeGen;
    }
    else if (device.type() == quarisma::kCPU)
    {
#ifdef TORCH_ENABLE_LLVM
...
ExprHandle TensorExprKernel::getStrideArg(size_t tensor_input_index, size_t stride_index)
{
    auto it = strideArgToVar_.find(std::pair<size_t, size_t>(tensor_input_index, stride_index));
    ...
    const torch::jit::Value* input, const std::vector<ExprHandle>& inputTensorDims)
{
    ...
    {
        auto const strides = input->type()->expect<TensorType>()->strides().concrete_sizes();
        std::vector<ExprHandle> inputTensorStrides;
    ...
    if (stride_input.size() == 1 && (stride_input[0] == StrideInput::TENSOR_CONT_CHANNELS_LAST ||
                                     stride_input[0] == StrideInput::TENSOR_CONT))
    {
        ...
        return fmap(strides, [&](ExprPtr stride) { return ExprHandle(std::move(stride)); });
    }
    ...
    while (generated_strides != rank)
    {
        ...
        {
            if (stride_input[i] == torch::jit::StrideInput::S_CONT && stride_set[i + 1])
            {
                inputTensorStrides[i] = inputTensorStrides[i + 1] * inputTensorDims[i + 1];
        ...
        {
            if (stride_input[i] == torch::jit::StrideInput::S_TRAN_CONT && stride_set[i - 1])
            {
                inputTensorStrides[i] = inputTensorStrides[i - 1] * inputTensorDims[i - 1];
                stride_set[i]         = true;
                generated_strides++;
...
Tensor TensorExprKernel::bindInput(const torch::jit::Value* input)
{
    ...
    auto is_concrete_cont =
        [](const torch::jit::Value* input, const MemoryLayoutPolicy& mem_layout_policy)
    {
        if (input->isCompleteTensor())
        {
    ...
    auto is_symbolic_cont =
        [](std::vector<torch::jit::StrideInput> desc, const MemoryLayoutPolicy& mem_layout_policy)
    {
        if (desc.size() == 1)
        {
            auto mem_layout = (mem_layout_policy == MemoryLayoutPolicy::kContiguous)
    ...
    {
        auto tt                         = input->type()->cast<TensorType>();
        bool contiguous_concrete_tensor = is_concrete_cont(input, memory_layout_policy_);
        bool contiguous_symbolic_tensor = false;
        if (has_symbolic_shapes_)
        {
            auto desc                  = getSymbolicStrideDesc(input);
            contiguous_symbolic_tensor = is_symbolic_cont(desc, memory_layout_policy_);
        }
        ...
        //  2) it is contiguous
        bool contiguous = contiguous_concrete_tensor || contiguous_symbolic_tensor;
        if (!outputs_set.count(input) && contiguous)
        {
            ...
            QUARISMA_CHECK_DEBUG(
                ...
                inBuffer.node()->is_contiguous(quarisma::MemoryFormat::ChannelsLast) ||
                inBuffer.node()->is_contiguous(quarisma::MemoryFormat::ChannelsLast3d));
...
NNCLoweringFunction TensorExprKernel::getCustomLoweringFor(quarisma::Symbol op) const
{
    if (custom_lowerings_.count(op))
        return custom_lowerings_.quarisma(op);
    return nullptr;
}
...
static bool denseAndNonOverlapping(
    quarisma::ArrayRef<int64_t> sizes, quarisma::ArrayRef<int64_t> strides)
{
    return (strides == quarisma::infer_dense_strides(sizes, strides));
}
    ...
    BufPtr&                        buf)
{
    ...
    // in indexing in to the output tensor quarisma [i, j] = val
    ...
    return Compute(
        "output_1",
        sizes,
        [&](const std::vector<VarHandle>& axes_input)
        {
...
Tensor TensorExprKernel::convertSymbolicOutputToCorrectStrides(torch::jit::Value* v)
{
    ...
    TORCH_INTERNAL_ASSERT(stride_desc[0] == torch::jit::StrideInput::TENSOR_CONT_CHANNELS_LAST);
    ...
    return convertSymbolicOutputToCorrectStrides(sizes, sorted_stride_indices, strides, buf);
}
...
Tensor TensorExprKernel::convertStaticShapeOutputToCorrectStrides(torch::jit::Value* v)
{
    ...
    {
        std::string msg = std::string("Shapes for output '%") + v->debugName() + "' are unknown";
        throw malformed_input(msg);
    }
    ...
    TORCH_INTERNAL_ASSERT(
        tt->strides().concrete_sizes(), buildErrorMessage("Output strides are unknown."));
        ...
        [&](const std::vector<VarHandle>& axes_input)
        {
            ...
            for (size_t stride_index : sorted_stride_indices)
            {
                ...
                if (size != 1)
                {
                    ...
                    index = absolute_position / ExprHandle(immLike(absolute_position, stride));
                    absolute_position =
                        absolute_position % ExprHandle(immLike(absolute_position, stride));
                }
                new_axes[stride_index] = index;
            }
            return BufHandle(buf).load(new_axes);
...
void TensorExprKernel::bindConstant(const torch::jit::Value* v)
{
    ...
    auto scalar_type  = quarisma::typeMetaToScalarType(const_tensor.options().dtype());
    ...
    {
        const_tensor = const_tensor.clone(quarisma::MemoryFormat::Contiguous);
...
BlockPtr TensorExprKernel::bindAllInputs()
{
    ...
    auto symbolic_shape_inputs_start_pos = nInputs_ - symbolic_shape_inputs_.size();
    if (has_symbolic_shapes_)
    {
        // The graph is supposed to have input params that represent the symbolic
        // dims quarisma the end of the list of inputs. The number of such symbolic input
        ...
        for (size_t i = symbolic_shape_inputs_start_pos; i < static_cast<size_t>(nInputs_); ++i)
        {
            ...
            {
                throw std::runtime_error("Expected integer type input to graph for symbolic dims.");
            }
            VarHandle v("v" + input_name_map_[input], kLong);
    ...
    for (const auto i : quarisma::irange(symbolic_shape_inputs_start_pos))
    {
        auto   input = graph_->inputs()[i];
    ...
    bufferArgs_.insert(bufferArgs_.end(), symbolic_shape_args.begin(), symbolic_shape_args.end());

    // Now, add all the variables corresponding to symbolic stride inputs
    bufferArgs_.insert(bufferArgs_.end(), symbolic_stride_args.begin(), symbolic_stride_args.end());
...
void TensorExprKernel::deduceMemoryLayoutPolicy()
{
    ...
    auto _prefer_symbolic_mem = [](const torch::jit::Value*                    val,
                                   const std::vector<torch::jit::StrideInput>& stride_desc_vec)
    {
        ...
        return (cur_stride_desc == torch::jit::StrideInput::TENSOR_CONT_CHANNELS_LAST)
    ...
    auto _prefer_static_mem = [](const torch::jit::Value* val)
    {
        ...
        return (quarisma::is_channels_last_strides_2d(sizes, strides))
                   ? MemoryLayoutPolicy::kChannelsLastNdContiguous
                   : MemoryLayoutPolicy::kContiguous;
    };
    ...
    for (auto el : graph_io_tensors)
    {
        ...
        auto preferred_mem_layout =
            is_complete ? _prefer_static_mem(el)
    ...
    memory_layout_policy_ = prefer_channels_last ? MemoryLayoutPolicy::kChannelsLastNdContiguous
                                                 : MemoryLayoutPolicy::kContiguous;
}
...
void TensorExprKernel::optimizeOwningGraph()
{
    ...
    for (auto i : quarisma::irange(graph_outputs.size()))
    {
        auto el_orig = _orignal_graph_outputs.quarisma(i);
        auto el_new  = graph_outputs.quarisma(i);
...
void TensorExprKernel::compile()
{
    ...
    for (auto const& n : graph_->nodes())
    {
        ...
        {
            throw std::runtime_error("Cannot support broadcast and random within one kernel");
        }
    }

    // Move output operands from `bufs_` to `bufOutputs_`
    for (auto i : quarisma::irange(graph_->outputs().size()))
    {
        auto& output = graph_->outputs().quarisma(i);
        ...
        if (!output->type()->cast<TensorType>())
        {
            ...
            bufOutputs_.insert(bufs_.quarisma(output));
            bufsToBeParallelized_.insert(bufs_.quarisma(output));
            bufferArgs_.emplace_back(BufHandle(bufs_.quarisma(output)));
            tensorOutputTensorOptions_.emplace_back(
                quarisma::TensorOptions(tensorType(bufs_.quarisma(output))).device(device_));
        ...
        else
        {
            ...
            // strides quarisma the end of the kernel (if already contiguous it's a no-op)
    ...
    // Generate code.
    codegen_ =
        CreateCodeGen(getCodeGenName(backendType), stmt_, bufferArgs_, device_, kernel_func_name_);
}

void TensorExprKernel::recompile()
{
    codegen_ = CreateCodeGen("llvm_codegen", stmt_, bufferArgs_, device_, kernel_func_name_);
}
    ...
    std::vector<std::vector<int64_t>>* strides) const
{
    ...
    // been computed quarisma compile time. That has to be done here by using the
    ...
    *strides             = tensorOutputStrides_;
    ...
    for (size_t i = 0, e = bufOutputs_.size(); i < e; ++i)
    {
        ...
        for (auto t : tensorOutputSymbolicSizes_[i])
        {
            ...
            {
                auto input_pos = shapeSymbolInputPos_.quarisma(t.node());
        ...
        {
            static_strides[i] = quarisma::get_channels_last_strides_2d(static_sizes[i]);
        }
        else
        {
            std::string output_desc = toString(tensorOutputStrideDesc_[i]);
            TORCH_INTERNAL_ASSERT(false, "Expected contiguous or channels last, got ", output_desc);
        }
    }
}
    ...
    const quarisma::ArrayRef<IValue>& inputs, std::vector<quarisma::Tensor>& outputs) const
{
    ...
    runArgs.reserve(inputs.size() + input_stride_args_.size() + bufOutputs_.size());
    ...
    if (has_symbolic_shapes_)
    {
        ...
        for (const auto& input_stride_arg : input_stride_args_)
        {
            runArgs.emplace_back(
                inputs[input_stride_arg.first].toTensor().strides().quarisma(
...
void TensorExprKernel::runFast(
    const std::vector<void*>& inputs, const std::vector<void*>& outputs) const
{
...
{
    TORCH_INTERNAL_ASSERT(
        device_ == quarisma::kCPU, "Pre-allocated output tensors are supported only on CPUs.");
    ...
    for (auto i : quarisma::irange(nInputs_))
    {
        ...
        {
            TORCH_INTERNAL_ASSERT(false, "Unhandled input type while calling TensorExprKernel");
        }
    }
    ...
    if (has_symbolic_shapes_)
    {
        ...
        // add stride args
        for (auto idx : quarisma::irange(input_stride_args_.size()))
        {
            const auto& input_stride_arg = input_stride_args_[idx];
            stride_values[idx] = stack_inputs[input_stride_arg.first].toTensor().strides().quarisma(
                input_stride_arg.second);
    ...
    {
        for (auto i : quarisma::irange(nOutputs_))
        {
