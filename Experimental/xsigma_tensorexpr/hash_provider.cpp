...
void HashProvider::visit(const RampPtr& v)
{
    ...
    putHash(v, hash_combine("ramp", hashOf(v->base()), hashOf(v->stride()), v->lanes()));
}
...
void HashProvider::visit(const StorePtr& v)
{
    ...
    v->value()->accept(this);
    putHash(v, hash_combine("store", hashOf(v->base_handle()), indices_hash, hashOf(v->value())));
}
...
void HashProvider::visit(const ForPtr& v)
{
    ...
    SimplifierHashType hash =
        hash_combine("for", hashOf(v->var()), hashOf(v->start()), hashOf(v->stop()));
...
void HashProvider::visit(const AllocatePtr& v)
{
    ...
    SimplifierHashType hash = hash_combine("allocate", hashOf(buffer_var), v->dtype());
