...
public:
    ...
    void visit(const BufPtr& v) override { v->base_handle()->accept(this); }
};
...
static inline std::enable_if_t<!std::is_floating_point_v<T>, void> visit_mod(
    std::ostream& os, const ExprPtr& lhs, const ExprPtr& rhs)
{
    os << *lhs << " % " << *rhs;
}
...
template <typename T>
static inline std::enable_if_t<std::is_floating_point_v<T> || std::is_integral_v<T>, void>
...
static void visit_binary_op(
    std::ostream& os, const ExprPtr& lhs, const ExprPtr& rhs, IRNodeType op_type)
{
    switch (op_type)
    {
...
{
    os() << "((" << *v->lhs() << " " << IRPrinter::to_string(v->compare_select_op()) << " "
         << *v->rhs() << ") ? " << *v->ret_val1() << " : " << *v->ret_val2() << ")";
}
...
void CppPrinter::visit(const AllocatePtr& v)
{
    ...
    emitIndent();
    os() << v->dtype().ToCppString() << "* " << (*v->buffer_var()) << " = static_cast<"
         << v->dtype().ToCppString() << "*>(malloc(" << size << "));" << '\n';
}
...
{
    auto flat_idx = flatten_index(v->buf()->dims(), v->indices(), v->buf()->strides());
...
void CppPrinter::visit(const ExternalCallPtr& v)
{
    ...
    for_buf([&](const BufPtr& buf) { os() << static_cast<int>(buf->dtype().scalar_type()); });
    os() << "};" << '\n';
