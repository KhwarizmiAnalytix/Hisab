    ...
    for (auto& x : v)
    {
        ...
        else
        {
            throw std::runtime_error(
                "vector type not homogeneous - found " + getArgValueName(x) + ", expected " +
                getArgValueName(v[0]));
        }
    }
    return res;
...
public:
    ...
    void runFast(const std::vector<void*>& inputs, const std::vector<void*>& outputs) const;
    ...
    void fallback(Stack& stack) const { InterpreterState(code_).run(stack); }
    void recompile();

    StmtPtr getCodeGenStmt();

    std::string getCodeText(const std::string& attr = "") { return codegen_->getCodeText(attr); }

    const std::shared_ptr<Graph> graph() { return graph_; }

    const std::vector<ConstantDescr>& getConstantDescriptors() const { return constants_; }

    const std::vector<CodeGen::BufferArg>& getBufferArgs() const { return bufferArgs_; }
    ...
    const std::vector<int64_t>& getSymbolicShapeInputs() const { return symbolic_shape_inputs_; }
...
private:
    ...
    std::vector<ExprHandle> broadcastShapesMut(std::vector<std::vector<ExprHandle>> shapes);
    ...
    std::unordered_map<const torch::jit::Value*, std::vector<ExprHandle>> known_sizes_;
    ...
    std::unordered_map<std::pair<size_t, size_t>, VarHandle, SmallSizeTPairHash> strideArgToVar_;
    std::unordered_map<const torch::jit::Value*, std::vector<torch::jit::StrideInput>>
        symbolic_strides_;
