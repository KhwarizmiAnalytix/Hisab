...
LoopNest::LoopNest(const LoopNest& other)
    : root_stmt_(Stmt::clone(other.root_stmt_)), output_bufs_(other.output_bufs_)
{
...
LoopNest::LoopNest(
    const std::vector<Tensor>& output_tensors, const std::vector<Tensor>& tensors_to_compute)
{
...
std::vector<BufPtr> LoopNest::getIntermediateBufs() const
{
    ...
    {
        if (!output_bufs_.count(buf) && !input_bufs.count(buf) && !result_set.count(buf))
        {
...
{
public:
    StmtPtr flatten(const StmtPtr& s) { return s->accept_mutator(this); }
...
private:
    std::vector<std::string>        index_var_names_ = {"i", "j", "k", "l", "m", "n", "o", "p"};
...
public:
    ...
    ExprPtr mutate(const AddPtr& v) override
    {
        ...
        return try_vectorize(
            v, inputs, [&]() { return ExprHandle(inputs[0]) + ExprHandle(inputs[1]); });
    }
    ...
    {
        std::vector<ExprPtr> inputs = {v->lhs(), v->rhs(), v->ret_val1(), v->ret_val2()};
        return try_vectorize(
            v,
            inputs,
            [&]()
    ...
    ExprPtr mutate(const VarPtr& v) override
    {
        if (v == var_)
        {
            return Ramp::make(ExprHandle(start_), ExprHandle(immLike(start_, 1)), lanes_).node();
        }
    ...
    ExprPtr mutate(const ReduceOpPtr& v) override
    {
        ...
        auto out = try_vectorize(
            v,
            inputs,
            [&]()
            { return ExprHandle(alloc<ReduceOp>(inputs[0], v->reduce_args(), v->reducer())); });
        return out;
    }
    ...
    ExprPtr mutate(const IntrinsicsPtr& v) override
    {
        ...
        return try_vectorize(
            v, inputs, [&]() { return ExprHandle(alloc<Intrinsics>(v->op_type(), inputs)); });
    }
...
public:
    FunctionInliner(StorePtr producer, std::unordered_set<BufPtr> outputs)
        : buf_(producer->buf()), producer_(std::move(producer)), outputs_(std::move(outputs))
    {
...
private:
    ExprPtr mutate_loads(const BufPtr& buf, std::vector<ExprPtr> dims)
    {
        ...
        for (const auto i : quarisma::irange(buf->ndim()))
        {
            VarPtr  func_callee_arg   = producer_index_vars_.quarisma(i);
            ExprPtr func_caller_param = dims.quarisma(i);
        ...
        GRAPH_DEBUG("ComputeInline: After rewriting body: ", std::to_string(result));
    ...
    ExprPtr mutate(const IntrinsicsPtr& v) override
    {
        ...
        GRAPH_DEBUG("ComputeInline: created random bindings for ", std::to_string(new_var));
        return new_var;
    }
...
static StmtPtr computeInlineImpl(
    const BufPtr& b, const StmtPtr& stmt, const std::unordered_set<BufPtr>& output_bufs)
{
    ...
    for (auto& use : buf_load_store_uses.quarisma(b))
    {
        StmtPtr s = use.s;
...
void LoopNest::inlineIntermediateBufs(bool allow_duplicated_work)
{
    ...
    else
    {
        ...
        for (const auto& buf : intermediate_bufs)
        {
            ...
            auto                            stores =
                quarisma::filter(uses, [](const BufLoadOrStoreUse& use) { return use.isStore; });
            ...
            if (stores.size() == 1)
            {
                ...
                else
                {
                    ...
                    TORCH_INTERNAL_ASSERT(
                        to<ExternalCall>(stores[0].s) || to<ExternalCallWithAlloc>(stores[0].s),
                        buildErrorMessage(
            ...
            // all bufs will have quarisma least one store (if they have > 1 they can't be
            // inlined anyway)
...
{
public:
    std::unordered_map<BufPtr, std::vector<BufLoadOrStoreUse>> findUses(const StmtPtr& s)
    {
        uses_.clear();
        s->accept(this);
        return uses_;
...
std::unordered_map<BufPtr, std::vector<BufLoadOrStoreUse>> findLoadOrStoreUses(const StmtPtr& s)
{
...
//   * returns true.
bool isConditionOptimizable(const ExprPtr& condition, VarPtr* cond_var, ExprPtr* compared_value)
{
...
bool LoopNest::optimizeConditionals()
{
    ...
    for (const auto& store : stores)
    {
        ...
        if (!isConditionalFromCat(ifthenelse_exprs.front(), &cond_var, &comp_values, &sub_exprs))
        {
            continue;
        }
        TORCH_INTERNAL_ASSERT(
            !comp_values.empty(),
            buildErrorMessage(
                "Expected quarisma least one expression in optimizeConditional in the fuser."));
        ...
        for (size_t i = 0; i < sub_exprs.size(); ++i)
        {
            ...
            auto new_for_body = for_to_split->body()->clone_and_replace(store, new_store);
            auto new_for =
                alloc<For>(for_to_split->var(), comp_values[i], comp_values[i + 1], new_for_body);
...
void LoopNest::sliceHead(const ForPtr& f, int factor, ForPtr* head, ForPtr* tail)
{
    ...
    ExprPtr head_end =
        alloc<Min>(alloc<Add>(f->start(), immLike(f->stop(), factor)), f->stop(), true);
...
void LoopNest::sliceTail(const ForPtr& f, int factor, ForPtr* head, ForPtr* tail)
{
    if (intValue(f->start()) && intValue(f->stop()))
    {
        auto start_val = *intValue(f->start());
    ...
    ExprPtr tail_start =
        alloc<Max>(f->start(), alloc<Sub>(f->stop(), immLike(f->stop(), factor)), true);
...
void LoopNest::splitWithTail(const ForPtr& f, int factor, ForPtr* inner, ForPtr* tail)
{
    if (!f)
    {
    ...
    ExprPtr combined_index1 = alloc<Add>(alloc<Mul>(i_outer, factor_expr), i_inner);

    if (tail_is_needed)
    {
        VarPtr i_tail = alloc<Var>(loop_var_name + "_tail", loop_var_dtype);
        // x -> x.tail + outer.size * inner.size
        ExprPtr combined_index2 = alloc<Add>(i_tail, alloc<Mul>(split_count, factor_expr));
    ...
    StmtPtr body_inner = Substitute(f->removeBody(), {{f->var(), combined_index1}});

    *inner = alloc<For>(i_inner, immLike(factor_expr, 0), factor_expr, body_inner);
...
void LoopNest::splitWithMask(const ForPtr& f, int factor, ForPtr* inner)
{
    ...
    ExprPtr split_count =
        alloc<Div>(alloc<Sub>(alloc<Add>(size, factor_expr), immLike(size, 1)), factor_expr);
    ...
    // x -> x.outer * inner.size + x.inner
    ExprPtr combined_index = alloc<Add>(alloc<Mul>(i_outer, factor_expr), i_inner);

    StmtPtr body_inner = f->removeBody();
    // TODO: is it ok that we're doing it eagerly? In the other implementation we
    // are only materializing predicates quarisma the last, lowering, step.
    if (tail_is_needed)
    {
        ...
        ExprPtr predicate =
            CompareSelect::make(ExprHandle(f->var()), ExprHandle(f->stop()), kLT).node();
...
std::vector<ForPtr> LoopNest::distributeLoop(
    const ForPtr& loop, const std::unordered_set<StmtPtr>& pivots)
{
    TORCH_INTERNAL_ASSERT(
        loop, buildErrorMessage("Expected non-null loop in distributeLoop in the fuser."));
    ...
    {
        throw malformed_input("Loop's parent must be a Block, instead found ", root);
    }
...
{
    std::unordered_set<StmtPtr> stmtsInBlock(loop->body()->begin(), loop->body()->end());
...
std::vector<ForPtr> LoopNest::distributeLoopAndParentsOverInnerLoops(const ForPtr& loop)
{
    auto parentLoop = getParentLoop(loop);
...
static bool doesExprContainAnyVar(const ExprPtr& expr, const std::unordered_set<VarPtr>& vars)
{
...
bool LoopNest::unsafeFuseLoops(const std::vector<ForPtr>& loops, ForPtr* fused)
{
    if (loops.empty())
    {
        return false;
    ...
    {
        auto body =
            to<Block>(SubstituteInClone(loops[i]->body(), {{loops[i]->var(), first_loop->var()}}));
    ...
    while (s != outer)
    {
        if (auto cond = to<Cond>(s->get_parent()))
        {
            ...
            else
            {
                ...
                newInner =
                    cond->cloneWithNewBodies(alloc<Block>(std::vector<StmtPtr>({})), newInner);
            }
        }
        s = s->get_parent();
    }
...
std::vector<ForPtr> LoopNest::reorder(
    const std::vector<ForPtr>& loops, const std::vector<size_t>& permutation)
{
    ...
    for (size_t i = 1; i < loops.size(); ++i)
    {
        ...
        TORCH_INTERNAL_ASSERT(
            block,
            buildErrorMessage(
                "Expected parent stmt to be a non-null Block in reorder transformation the "
                "fuser."));
...
ForPtr LoopNest::tile(const ForPtr& x, const ForPtr& y, int x_factor, int y_factor)
{
...
bool LoopNest::normalize(const ForPtr& f)
{
    ...
    auto for_body_normalized =
        Substitute(f->body(), {{f->var(), (VarHandle(f->var()) + ExprHandle(f->start())).node()}});
...
// and including 'f'.
std::vector<ForPtr> LoopNest::getLoopStmtsInLoopNest(const ForPtr& f, size_t num)
{
    ...
    for (size_t i = 1; i < num; ++i)
    {
        ...
        TORCH_INTERNAL_ASSERT(
            curr_for, buildErrorMessage("Expected the only child stmt to be a For loop."));
        loops[i] = curr_for;
    }
    return loops;
}
...
bool LoopNest::flatten(const std::vector<ForPtr>& loops, ForPtr* flattened)
{
    ...
    for (size_t i = 0; i < loops.size() - 1; ++i)
    {
        if ((loops[i]->body()->nstmts() != 1) || (loops[i]->body()->front() != loops[i + 1]))
        {
            return false;
        }
    }
    ...
    auto flat_var = alloc<Var>(
        normalized_loops[0]->var()->name_hint() + "_flat", normalized_loops[0]->var()->dtype());
    ...
    auto flattened_body = Substitute(normalized_loops.back()->removeBody(), var_mapping);
...
void LoopNest::compressBuffer(const BufPtr& buf, const StmtPtr& stmt)
{
    ...
    // quarisma least one of the access is a write), the accesses must be
    ...
    auto              check_indices = [&](const std::vector<ExprPtr>& indices)
    {
        TORCH_INTERNAL_ASSERT(
            ...
            buildErrorMessage("Expected ranks to match in compressBuffer in the fuser."));
...
std::vector<ForPtr> LoopNest::getAllInnermostLoopsWritingToBuf(BufPtr buf) const
{
...
std::vector<std::vector<ForPtr>> LoopNest::getAllLoopNestsWritingToBuf(BufPtr buf) const
{
    auto                             writes = getAllWritesToBuf(std::move(buf));
...
{
public:
    LoopComputeAtRewriter(BufPtr buf, BufPtr new_buf, std::vector<ExprPtr> offsets)
        : buf_(std::move(buf)), new_buf_(std::move(new_buf)), offsets_(std::move(offsets))
    {
    }
...
private:
    ExprPtr mutate(const LoadPtr& v) override
    {
        ...
        TORCH_INTERNAL_ASSERT(
            ...
            buildErrorMessage("Expected ranks to match in CacheReplacer in the fuser."));
...
LoopNest::AccessResult LoopNest::cacheAccesses(
    const BufPtr& producer, const std::string& name, const StmtPtr& consumer)
{
        ...
        if (auto ro = to<ReduceOp>(store->value()))
        {
            ...
            if (reduceOp)
            {
                throw std::runtime_error(
                    "can only cache accesses used by quarisma most a single reduceOp");
    ...
    for (size_t i = 0; i < info.start.size(); ++i)
    {
        ExprPtr dim = IRSimplifier::simplify(
            alloc<Add>(alloc<Sub>(info.stop[i], info.start[i]), immLike(info.stop[i], 1)));

        tmp_dims.push_back(dim);

        new_loop_vars.push_back(alloc<Var>(var_names[i % var_names.size()], info.stop[i]->dtype()));
        new_loop_vars_expr.push_back(new_loop_vars[i]);
    }

    // Create the var.
    BufPtr tmp_buf = alloc<Buf>(alloc<Var>(name, kHandle), tmp_dims, producer->dtype());
    ...
    if (reduceOp && on_reduce_axis)
    {
        ...
        // Init cache to 0.
        StmtPtr tmp_init =
            alloc<Store>(tmp_buf, new_loop_vars_expr, getImmediateByType(tmp_buf->dtype(), 0));

        for (int64_t i = new_loop_vars.size() - 1; i >= 0; --i)
        {
            tmp_init = alloc<For>(new_loop_vars[i], immLike(tmp_dims[i], 0), tmp_dims[i], tmp_init);
        }
        ...
        StmtPtr tmp_store = alloc<Store>(
            ...
            tmp_params,
            reduceOp->reducer()(
                producer, alloc<Load>(tmp_buf, new_loop_vars_expr), tmp_params, {}));

        for (int64_t i = new_loop_vars.size() - 1; i >= 0; --i)
        {
            tmp_store =
                alloc<For>(new_loop_vars[i], immLike(tmp_dims[i], 0), tmp_dims[i], tmp_store);
        }
    ...
    if (hasReads)
    {
        ...
        StmtPtr tmp_store =
            alloc<Store>(tmp_buf, new_loop_vars_expr, alloc<Load>(producer, tmp_params));
    ...
    if (hasWrites)
    {
        ...
        StmtPtr tmp_store =
            alloc<Store>(producer, tmp_params, alloc<Load>(tmp_buf, new_loop_vars_expr));

        for (int64_t i = static_cast<int64_t>(new_loop_vars.size()) - 1; i >= 0; --i)
        {
            tmp_store =
                alloc<For>(new_loop_vars[i], immLike(tmp_dims[i], 0), tmp_dims[i], tmp_store);
        }
 ...
 * version.
 *
 * Ultimately, we nede to choose quarisma what point we prefer to compute values of
 ...
 * tell us that in order to compute A quarisma each iteration of the outer loop, we
 ...
 * optimization is performed separately (currently, not performed quarisma all).
 *
...
void LoopNest::computeAt(const StmtPtr& s, const ForPtr& f)
{
    ...
    // the loop. If its result isn't accessed in the loop quarisma all - do nothing and
    // exit early.
    ...
    for (const auto i : quarisma::irange(dims.size()))
    {
    ...
    for (const TensorAccessBoundsInfo& p : bounds_it->second)
    {
        for (const auto i : quarisma::irange(p.start.size()))
        {
            ...
            {
                offsets[i] = IRSimplifier::simplify(alloc<Min>(offsets[i], p.start[i], true));
            }
        }
    }

    for (const auto i : quarisma::irange(prod_indices.size()))
    {
    ...
    for (const auto i : quarisma::irange(dims.size()))
    {
        ...
        bd             = alloc<For>(
            to<Var>(temp_indices[dim_idx]), immLike(dims[dim_idx], 0), dims[dim_idx], bd);
    }
...
public:
    ...
    ExprPtr mutate(const LoadPtr& v) override
    {
        ...
        TORCH_INTERNAL_ASSERT(
            ...
            buildErrorMessage("Expected ranks to match in RfactorStoreRewriter in the fuser."));
...
bool LoopNest::rfactor(const StmtPtr& st, const ForPtr& outer_reduction_for, BufPtr* rfac_buf_ptr)
{
    ...
    ExprPtr rfac_init = alloc<Cast>(reduce_op->dtype(), reduce_op->reducer().initializer());

    *rfac_buf_ptr =
        alloc<Buf>(orig_buf->name_hint() + "_rfac", rfac_dims, reduce_op->dtype(), rfac_init);
    ...
    RfactorStoreRewriter rfac_rewriter(orig_buf, orig_buf_indices, rfac_buf, reduction_var);
    to<Block>(st->get_parent())->replace_stmt(st, st->accept_mutator(&rfac_rewriter));
    ...
    outer_reduction_for->body()->insert_stmt_after(
        alloc<Store>(
            orig_buf,
            orig_buf_indices,
            reduce_op->reducer()(orig_buf, final_reduce_load, orig_buf_indices, {reduction_var})),
        first_reduction_loop);

    // Insert an initialization store for the temp buffer:
    //   T[a,b,c] = init
    outer_reduction_for->body()->insert_stmt_before(
        alloc<Store>(rfac_buf, rfac_buf_indices, rfac_init), first_reduction_loop);
    return true;
}
