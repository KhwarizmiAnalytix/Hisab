...
{
public:
    InterpValue() : dtype_(kInt) { Intvalues.push_back(0); }
...
#define VALUE_CTOR(Type, Name) \
    InterpValue(Type v) : dtype_(k##Name) { Name##values.push_back(v); }
...
public:
    ...
    void operator()(const std::vector<CallArg>& call_args) { call(call_args); }

    void bindVar(VarPtr v, ExprPtr e) { codegen_->bindVar(v, e); }

    void bindVar(const VarHandle& v, const ExprHandle& e) { codegen_->bindVar(v.node(), e.node()); }
...
inline ExprPtr SubstituteInClone(const ExprPtr& expr, const VarMapping& var_mapping)
{
    VarSubMutator var_sub(var_mapping);
...
// NOTE: This works because cloning reuses variables and does not create new
// ones, and `VarMapping` input has variables as the key.
inline StmtPtr SubstituteInClone(const StmtPtr& stmt, const VarMapping& var_mapping)
{
    VarSubMutator var_sub(var_mapping);
