...
// ============================================================================
class QUARISMA_VISIBILITY CodeLocation
{
...
PyCodeObject* getCode<CallType::PyModuleCall>()
{
    static auto module_call_code = []()
    {
        ...
        auto                         res =
            py::module::import("quarisma.nn").attr("Module").attr("__call__").attr("__code__").ptr();
...
template <>
class QUARISMA_VISIBILITY std ::hash<quarisma::profiler_impl::impl::CodeLocation>
{
...
private:
    static_assert(
        CallType::PyCall == 0, "CallTypeHelper uses integer math which depends on a zero start.");
...
class QUARISMA_VISIBILITY ExtendedPyCallConfig
{
    using key_t       = Key;
    using cls_t       = Cls;
...
struct Config<CallType::PyModuleCall>
    : ExtendedPyCallConfig<PyModuleSelf, PyModuleCls, NNModuleInfo::ParameterInfo>
{
};
...
// ============================================================================
template <CallType C>
class QUARISMA_VISIBILITY Callsite
{
public:
    ...
    static_assert(
        std::is_trivially_copyable_v<key_t>, "Key should be trivial, as it is passed by value.");
...
class QUARISMA_VISIBILITY ValueCache
{
public:
    ValueCache()                              = default;
...
TensorMetadata toTensorMetadata(PyObject* self)
{
    ...
    return TensorMetadata{
        m, t.sizes().vec(), m.layout_ == quarisma::kStrided ? t.strides().vec() : std::vector<int64_t>()};
}
    ...
    const PyModuleCallKey& key) const
{
    ...
    return {/*frame_state_=*/std::get<CallType::PyCall>(state_).at(*cache.location_),
    ...
    const PyOptimizerCallKey& key) const
{
    ...
    return {/*frame_state_=*/std::get<CallType::PyCall>(state_).at(
...
template <CallType C>
class QUARISMA_VISIBILITY TraceKeyCacheState
{
    struct Hash
    {
...
class QUARISMA_VISIBILITY TraceContext
{
...
class QUARISMA_VISIBILITY gil_and_restore_thread
{
public:
...
// ============================================================================
class PythonTracer;
class QUARISMA_VISIBILITY ThreadLocalResults
{
    ThreadLocalResults(
        PyThreadState* thread_state, ValueCache* value_cache, PythonTracer* active_tracer)
...
static PyObject* c_call_callback(
    _PyEventHandler* self, PyObject* const* args, size_t nargsf, PyObject* kwnames);
#endif
...
private:
    ...
    void recordCCall(
        ThreadLocalResults& tls, PyFrameObject* frame, PyObject* arg, bool start_frame = false);
...
class QUARISMA_VISIBILITY _PyEventHandler
{
    ...
    _PyEventHandler* self, PyObject* const* args, size_t nargsf, PyObject* kwnames)
{
    ...
    if (Py_TYPE(callable) == &PyMethod_Type)
    {
        ...
        if (PyCFunction_Check(func))
        {
            ...
            Py_INCREF(frame);
            auto& local_results =
                *reinterpret_cast<TraceContext*>(tstate->c_profileobj)->thread_local_results_;
...
static void registerMonitoringCallback()
{
    ...
    if (!sys_module)
    {
        QUARISMA_WARN("Failed to import sys module.");
        PyErr_Clear();
        return;
    }
    auto monitoring = THPObjectPtr(PyObject_GetAttrString(sys_module, "monitoring"));
    if (!monitoring)
    {
        QUARISMA_WARN("Failed to get monitoring from sys module.");
        PyErr_Clear();
        return;
    }
    auto result = THPObjectPtr(
        PyObject_CallMethod(monitoring, "use_tool_id", "is", PROFILER_ID, "PyTorch Profiler"));
    if (!result)
    {
        QUARISMA_WARN("Failed to call sys.monitoring.use_tool_id");
        PyErr_Clear();
        return;
    }
    ...
    if (!handler)
    {
        QUARISMA_WARN("Failed to create _PyEventHandler object.");
        PyErr_Clear();
        return;
    }
    reinterpret_cast<_PyEventHandler*>(handler.get())->vectorcall = (vectorcallfunc)c_call_callback;
    ...
    if (!result)
    {
        QUARISMA_WARN("Failed to call sys.monitoring.register_callback.");
        PyErr_Clear();
        return;
    }
    result = THPObjectPtr(PyObject_CallMethod(
        monitoring, "set_events", "ii", PROFILER_ID, 1 << PY_MONITORING_EVENT_CALL));
    if (!result)
    {
        QUARISMA_WARN("Failed to call sys.monitoring.set_events.");
        PyErr_Clear();
        return;
    }
}
...
static void unregisterMonitoringCallback()
{
    ...
    auto tool_name = THPObjectPtr(PyObject_CallMethod(monitoring, "get_tool", "i", PROFILER_ID));
    if (!tool_name)
    {
        QUARISMA_WARN("Failed to call sys.monitoring.use_tool_id");
        PyErr_Clear();
    ...
    result = THPObjectPtr(PyObject_CallMethod(monitoring, "set_events", "ii", PROFILER_ID, 0));
    if (!result)
    {
        QUARISMA_WARN("Failed to call sys.monitoring.set_events.");
        PyErr_Clear();
    ...
    result = THPObjectPtr(PyObject_CallMethod(monitoring, "free_tool_id", "i", PROFILER_ID));
    if (!result)
    {
        QUARISMA_WARN("Failed to call sys.monitoring.free_tool_id.");
        PyErr_Clear();
        return;
    }
}
      ...
      optimizer_hook_(getCode<CallType::PyOptimizerCall>())
{
    ...
    {
        QUARISMA_WARN("PyThreadState_Get returned NULL");
        return;
    }
...
void PythonTracer::register_gc_callback()
{
    ...
    static PyMethodDef method_def = {
        "gc_event_callback", (PyCFunction)gc_event_callback, METH_VARARGS, nullptr};
...
PythonTracer::~PythonTracer()
{
    if (active_)
    {
        QUARISMA_WARN("`PythonTracer::stop()` was not called.");
        stop();
    }
}

void PythonTracer::recordPyCall(
    ThreadLocalResults& tls, PyFrameObject* frame, bool is_startup_frame)
{
...
void PythonTracer::recordCCall(
    ThreadLocalResults& tls, PyFrameObject* frame, PyObject* arg, bool start_frame)
{
...
// == Post processing =========================================================
// ============================================================================
class QUARISMA_VISIBILITY Exit
{
...
class QUARISMA_VISIBILITY PostProcess
{
public:
    PostProcess(
        std::function<quarisma::time_t(quarisma::approx_time_t)> time_converter,
    ...
    template <CallType C>
    void operator()(
        const TraceKeyCacheState<C>& trace_cache, const ValueCache& value_cache, size_t python_tid)
    {
    ...
    std::vector<std::shared_ptr<Result>> run(std::vector<python_tracer::CompressedEvent>& enters)
    {
        std::stable_sort(
            enters.begin(),
            enters.end(),
...
private:
        ...
        std::vector<std::shared_ptr<Result>>&        out)
    {
        ...
        auto       pop          = [](stack_t& stack, quarisma::time_t t)
        {
            ...
            {
                QUARISMA_WARN_ONCE(
                    "Python replay stack is empty during pop operation! May result in incorrect "
                    "stack tracing.");
            }
        };
...
class QUARISMA_VISIBILITY PythonIDVisitor
{
    ...
    quarisma::time_t                                       end_time_ns)
{
    ...
    PostProcess post_process(
        std::move(time_converter), thread_local_results_, value_cache_, end_time_ns);
...
{
    pybind11::gil_scoped_acquire gil;
    THPObjectPtr torch_cuda_memory_module(PyImport_ImportModule("quarisma.cuda.memory"));
    if (!torch_cuda_memory_module)
    {
        return;
    }
    THPObjectPtr snapshot_func(
        PyObject_GetAttrString(torch_cuda_memory_module.get(), "_record_memory_history_impl"));
    if (!snapshot_func)
    {
        return;
    }
...
void init()
{
    ...
    quarisma::profiler_impl::impl::python_tracer::registerTracer(&quarisma::profiler_impl::impl::getTracer);
