...
class QUARISMA_VISIBILITY NVTXThreadLocalState : ProfilerStateBase
{
    ...
    static NVTXThreadLocalState* getTLS()
    {
        ...
        QUARISMA_CHECK_DEBUG(tls == nullptr || tls->profilerType() == ActiveProfilerType::NVTX);
        return static_cast<NVTXThreadLocalState*>(tls);
    }
    std::pair<quarisma::RecordFunctionHandle, int> getOpIdFromInput(const quarisma::Tensor& tensor);

    void setProducerTensorMap(quarisma::TensorImpl* tensor, quarisma::RecordFunctionHandle op_id, int output_nr)
    {
...
{
    std::pair<quarisma::RecordFunctionHandle, int>            undefined_op_pair(0, -1);
