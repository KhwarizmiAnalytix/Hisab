...
#include "unwind/communicate.h"
#include "unwind/dwarf_enums.h"
#include "unwind/eh_frame_hdr.h"
#include "unwind/fast_symbolizer.h"
#include "unwind/fde.h"
                ...
                for (auto i : quarisma::irange(info->dlpi_phnum))
                {
                    ...
                    if (segments[i].p_type == PT_GNU_EH_FRAME)
                    {
                        ...
                        self->all_libraries_.emplace_back(
                            std::move(library_name), info->dlpi_addr, last_addr, eh_frame_hdr);
                        return 0;
                    }
                }
    ...
    Entry& getOrCreate(const std::string& name)
    {
        ...
        if (it == entries_.end())
        {
            ...
            it = entries_
                     .insert_or_assign(
                         name,
                         Entry{std::make_unique<Communicate>(addr2line_binary_.c_str(), args), {}})
                     .first;
        }
        return it->second;
    }
...
static std::vector<Frame> symbolize_addr2line(const std::vector<void*>& frames)
{
