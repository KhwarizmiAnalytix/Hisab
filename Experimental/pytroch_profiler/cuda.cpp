...
class QUARISMA_VISIBILITY CUDAMethods : public ProfilerStubs
{
    void record(
        quarisma::device_enum* device, ProfilerVoidEventStub* event, int64_t* cpu_ns) const override
    {
        if (device)
        {
        ...
        *event = std::shared_ptr<CUevent_st>(
            cuda_event_ptr, [](CUevent_st* ptr) { QUARISMA_CUDA_CHECK(cudaEventDestroy(ptr)); });
    ...
    {
        QUARISMA_WARN_ONCE("Warning: roctracer isn't available on Windows");
    }
    ...
    void synchronize() const override { QUARISMA_CUDA_CHECK(cudaDeviceSynchronize()); }

    bool enabled() const override { return true; }
};

class QUARISMA_VISIBILITY RegisterCUDAMethods
{
    RegisterCUDAMethods()
    {
