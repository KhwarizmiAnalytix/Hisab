...
#include "execution_trace_observer.h"
...
{  // NOLINT
    ...
    std::string get_string_for_tensor_range()
    {
        ...
        for (auto const& [key, value] : tensor_index_min_max_map)
        {
            ...
            {
                result +=
                    json_str_escape(fmt::format("\"{}\":[{},{}]", key, value.first, value.second));
            }
            else
            {
                result +=
...
static bool initExecutionTraceStart(ExecutionTraceObserver& ob)
{
    ...
    ob.recordTime           = timeString(std::chrono::system_clock::to_time_t(current_time));
                ...
                tensor.numel() != 0)
            {
                    ...
                    !ob.resourceDir.empty())
                {
                    std::string tensor_dump_file_name = ob.resourceDir + "/nid_" +
                                                        std::to_string(opId) + "_tid_" +
                                                        std::to_string(tensorIndex) + ".dat";
...
{
    auto tuple = convertIValue(
        ob, functionName, opId, tensorIndex, tensor_index_min_max_map, isInput, val, true);
...
static void handleKernelBackendInfo(FunctionCallContext& fc, const RecordFunction& fn)
{
    ...
    if (kwinputs.find("kernel_backend") != kwinputs.end())
    {
        ...
        if (fc.kernelBackend == "triton")
        {
            ...
            {
                fc.kernelFile = fc.kernelFile.substr(fc.kernelFile.find_last_of('/') + 1);
            }
...
{  // NOLINT
    ...
    auto addAttr = [&](const char* commsMetaName, const char* etMetaName, const char* type)
    {
...
static void recordOperatorStart(
    ExecutionTraceObserver& ob, FunctionCallContext& fc, const RecordFunction& fn)
{
...
static void onFunctionExit(const RecordFunction& fn, ObserverContext* ctx_ptr)
{
    ...
    if (ob->getState() == RunState::enabled)
    {
        ...
        {
            LOG(WARNING) << "Exception in execution trace observer: [" << fc.name << " (" << fc.opId
                         << ")] " << e.what();
        }
    }
}
...
bool addExecutionTraceObserver(const std::string& output_file_path)
{
    ...
    if (ObserverManager::get() == nullptr)
    {
        ...
        VLOG(1) << "PyTorch Execution Trace: added observer, output=" << output_file_path;
    }
