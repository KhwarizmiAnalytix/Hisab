...
DeviceType sparseTensorSetToDeviceType(DispatchKeySet key_set)
{
    ...
    TORCH_CHECK(
        c10::toFunctionalityKey(k) == DispatchKey::Sparse,
        "cannot create sparse tensor with non sparse dispatch key ",
        k);
...
void SparseTensorImpl::set_indices_and_values_unsafe(const Tensor& indices, const Tensor& values)
{
    ...
    TORCH_CHECK(
        indices.dim() == 2, "indices must be sparse_dim x nnz, but got: ", indices.sym_sizes());
    TORCH_CHECK(
        indices.sym_size(1) == values.sym_size(0),
        "indices and values must have same nnz, but got nnz from indices: ",
        indices.sym_size(1),
        ", nnz from values: ",
        values.sym_size(0));
    TORCH_CHECK(
        indices.sym_size(0) == sparse_dim_,
    ...
    expected_values_size_vec.insert(
        expected_values_size_vec.end(), dense_size_original.begin(), dense_size_original.end());
    SymIntArrayRef expected_values_size(expected_values_size_vec);
    auto           new_values_size = values.sym_sizes();
    TORCH_CHECK(
        std::equal(
            expected_values_size.begin(), expected_values_size.end(), new_values_size.begin()),
