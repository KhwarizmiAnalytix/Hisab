...
SparseCsrTensorImpl::SparseCsrTensorImpl(
    ...
    : SparseCsrTensorImpl(
          ...
          at::empty(
              {0},
              at::initialTensorOptions().device(device).dtype(ScalarType::Int))  // crow_indices
          ,
          at::empty(
              {0},
              at::initialTensorOptions().device(device).dtype(ScalarType::Int))  // col_indices
          ,
          at::empty(
              {0},
              at::initialTensorOptions().device(device).dtype(data_type))  // values
          ,
          layout)
{
}
      ...
      layout_(layout)
{
    ...
    TORCH_INTERNAL_ASSERT(
        ((key_set.has(DispatchKey::SparseCsrCPU) && device().type() == kCPU) ||
    ...
    TORCH_CHECK(
        ...
        "Values and ",
        at::sparse_csr::compressedIndicesName(layout_),
        " need to be on the same device.");
    TORCH_CHECK(
        values_.device() == col_indices_.device(),
        "Values and ",
        at::sparse_csr::plainIndicesName(layout_),
        " need to be on the same device.");
    TORCH_INTERNAL_ASSERT(
        values_.device() == device(),
...
{
    TORCH_CHECK(!has_symbolic_sizes_strides_, "resize_ called on tensor with symbolic shape")
    ...
    {
        crow_indices_.narrow(-1, old_crow_indices_size, rows + 1 - old_crow_indices_size)
            .fill_(nnz);
    }
    else
    {
...
{
    TORCH_CHECK(
        !has_symbolic_sizes_strides_, "resize_and_clear_ called on tensor with symbolic shape");
    TORCH_CHECK(
        sparse_dim == 2, "resize_and_clear_ sparse dimensionality must be 2, got ", sparse_dim);
    TORCH_CHECK(
        static_cast<int64_t>(size.size()) >= sparse_dim + dense_dim,
        "resize_and_clear_ size length must be at least sparse dimensionality (=",
        sparse_dim,
        ") plus dense dimensionality (=",
        dense_dim,
        "), got ",
        size.size());
    ...
    auto n_compressed_indices = AT_DISPATCH_ROW_SPARSE_COMPRESSED_LAYOUTS(
        layout_,
        "resize_and_clear_",
    ...
    TORCH_CHECK(
        n_compressed_indices % block_factor == 0,
        "The size of the compressed dimension (=",
        n_compressed_indices,
        ") must be divisible with the corresponding block size (=",
        block_factor,
        ")");
...
void SparseCsrTensorImpl::resize_as_sparse_compressed_tensor_(const Tensor& src)
{
    TORCH_CHECK(
    ...
    TORCH_CHECK(
        src.layout() == layout_,
        "resize_as_sparse_compressed_tensor_: self and src must have the same layout, but got: "
        "self (",
        layout_,
        ") and source (",
        src.layout(),
    ...
    auto [compressed_indices, plain_indices] = sparse_csr::getCompressedPlainIndices(src);
...
{
    TORCH_CHECK(
        !has_symbolic_sizes_strides_, "set_member_tensors called on tensor with symbolic shape");
...
void SparseCsrTensorImpl::set_member_tensors(
    const Tensor& crow_indices, const Tensor& col_indices, const Tensor& values, IntArrayRef size)
{
...
c10::SymBool SparseCsrTensorImpl::sym_is_contiguous_custom(MemoryFormat /*memory_format*/) const
{
    TORCH_CHECK(
        ...
        "Sparse ",
        at::sparse_csr::layoutToString(layout_, /*upper=*/true),
        " tensors do not have is_contiguous");
}
}  // namespace at
