...
inline void validate_nested_tensor_metadata(
    const at::Tensor& nested_sizes, const at::Tensor& nested_strides, const at::Tensor& offsets)
{
 ...
 */
inline c10::DispatchKeySet generate_nested_key_set_from_buffer(const at::Tensor& buffer)
{
    ...
    nested_key_set =
        nested_key_set - c10::DispatchKeySet{c10::DispatchKey::Dense, c10::DispatchKey::Autograd};

    // Add nested tensor specific keys
    nested_key_set = nested_key_set | c10::DispatchKeySet{c10::DispatchKey::NestedTensor};
    nested_key_set = has_autograd ? nested_key_set | c10::autograd_nested : nested_key_set;
...
{
    return base.is_nested() ? base.key_set() : generate_nested_key_set_from_buffer(base);
}
...
at::Tensor construct_offsets(const at::Tensor& sizes)
{
    ...
    {
        const int64_t row_product =
            std::accumulate(sizes_ptr, sizes_ptr + orig_dim, 1, std::multiplies());
      ...
      opt_sizes_(std::nullopt)
{
    ...
    TORCH_INTERNAL_ASSERT(
        storage_device.is_cpu() || storage_device.is_cuda() || storage_device.is_xpu() ||
            storage_device.is_hpu() || storage_device.is_privateuseone(),
        "NestedTensorImpl storage must be either CUDA, CPU, XPU or ",
...
NestedTensorImpl::NestedTensorImpl(const at::Tensor& buffer, const at::Tensor& nested_sizes)
    : NestedTensorImpl(
          buffer,
          nested_sizes,
...
NestedTensorImpl::NestedTensorImpl(
    ...
    : TensorImpl(
          impl_type,
          Storage(base_tensor.storage()),
          get_view_key_set(base_tensor),
          base_tensor.dtype()),
      nested_sizes_(std::move(nested_sizes)),
      nested_strides_(std::move(nested_strides)),
      storage_offsets_(std::move(storage_offsets)),
...
{
    TORCH_CHECK(
        false, "Internal error: NestedTensorImpl doesn't support sizes. Please file an issue.");
}
...
{
    TORCH_CHECK(
        false, "Internal error: NestedTensorImpl doesn't support strides. Please file an issue.");
}
    ...
    VariableVersion&& version_counter, bool allow_tensor_metadata_change) const
{
    ...
    auto impl = c10::make_intrusive<NestedTensorImpl>(
        storage_, key_set_, data_type_, nested_sizes_, nested_strides_, storage_offsets_);
...
{
    return shallow_copy_and_detach_core(version_counter, allow_tensor_metadata_change);
}

c10::intrusive_ptr<TensorImpl> NestedTensorImpl::shallow_copy_and_detach(
    c10::VariableVersion&& version_counter, bool allow_tensor_metadata_change) const
{
    return shallow_copy_and_detach_core(std::move(version_counter), allow_tensor_metadata_change);
}
