...
void checkSize_symint(
    CheckedFrom c, const TensorGeometryArg& t, int64_t dim, const c10::SymInt& size)
{
    TORCH_CHECK(
        t->sym_size(dim) == size,
        "Expected tensor to have size ",
...
void checkScalarTypes(CheckedFrom c, const TensorArg& t, at::ArrayRef<ScalarType> l)
{
    ...
    {
        std::ostringstream oss;
        oss << "Expected tensor for " << t << " to have one of the following " << "scalar types: ";
        size_t i = 0;
        for (auto ty : l)
        {
            if (i != 0)
        ...
        oss << "; but got " << t->toString() << " instead (while checking arguments for " << c
            << ")";
        TORCH_CHECK(false, oss.str());
    }
}
    ...
    ResultVec          toResult(const NewShapeVec&))
{
    ...
    for (int64_t tensor_d = oldshape.size() - 1; tensor_d >= 0; tensor_d--)
    {
        ...
        if ((tensor_d == 0) || (TORCH_GUARD_OR_TRUE(sym_ne(oldshape[tensor_d - 1], 1)) &&
                                TORCH_GUARD_OR_TRUE(sym_ne(
                                    oldstride[tensor_d - 1], tensor_numel * chunk_base_stride))))
        {
            ...
            // and tensor_numel = u1].
            while (view_d >= 0 && (TORCH_GUARD_OR_TRUE(sym_lt(view_numel, tensor_numel)) ||
                                   TORCH_GUARD_OR_FALSE(sym_eq(newshape[view_d], 1))))
            {
...
std::optional<std::vector<int64_t>> computeStride(
    IntArrayRef oldshape, IntArrayRef oldstride, IntArrayRef newshape)
{
    auto toResult = [](const IntArrayRef& a) { return a.vec(); };
    return computeStride_impl<std::vector<int64_t>, IntArrayRef, int64_t>(
        oldshape, oldstride, newshape, toResult);
}

std::optional<SymDimVector> computeStride(
    c10::SymIntArrayRef oldshape, c10::SymIntArrayRef oldstride, c10::SymIntArrayRef newshape)
{
    auto toResult = [](const SymIntArrayRef& a) { return SymDimVector(a); };
    return computeStride_impl<SymDimVector, c10::SymIntArrayRef, c10::SymInt>(
        oldshape, oldstride, newshape, toResult);
}
    ...
    IntArrayRef oldshape, IntArrayRef oldstride, const DimVector& newshape)
{
    ...
    return computeStride_impl<DimVector, IntArrayRef, int64_t>(
        oldshape, oldstride, newshape, toResult);
}
