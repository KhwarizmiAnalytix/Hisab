...
#include <Quarisma/native/IndexKernel.h>
#include <Quarisma/native/IndexingUtils.h>
#include <Quarisma/native/Resize.h>
#include <Quarisma/native/ScatterGatherChecks.h>
#include <Quarisma/native/TensorAdvancedIndexing.h>
#include <Quarisma/native/TensorAdvancedIndexingUtils.h>

#ifndef AT_PER_OPERATOR_HEADERS
#include <Quarisma/Functions.h>
#include <Quarisma/NativeFunctions.h>
#else
#include <Quarisma/ops/_gather_sparse_backward.h>
#include <Quarisma/ops/_gather_sparse_backward_native.h>
#include <Quarisma/ops/_index_put_impl.h>
#include <Quarisma/ops/_index_put_impl_native.h>
#include <Quarisma/ops/_sparse_coo_tensor_unsafe.h>
#include <Quarisma/ops/_unsafe_index_native.h>
#include <Quarisma/ops/_unsafe_index_put_native.h>
#include <Quarisma/ops/arange.h>
#include <Quarisma/ops/argwhere_native.h>
#include <Quarisma/ops/as_strided.h>
#include <Quarisma/ops/broadcast_to.h>
#include <Quarisma/ops/count_nonzero.h>
#include <Quarisma/ops/count_nonzero_native.h>
#include <Quarisma/ops/empty.h>
#include <Quarisma/ops/empty_quantized.h>
#include <Quarisma/ops/gather.h>
#include <Quarisma/ops/gather_backward_native.h>
#include <Quarisma/ops/gather_meta.h>
#include <Quarisma/ops/gather_native.h>
#include <Quarisma/ops/index.h>
#include <Quarisma/ops/index_add_meta.h>
#include <Quarisma/ops/index_add_native.h>
#include <Quarisma/ops/index_copy_meta.h>
#include <Quarisma/ops/index_copy_native.h>
#include <Quarisma/ops/index_fill_native.h>
#include <Quarisma/ops/index_meta.h>
#include <Quarisma/ops/index_native.h>
#include <Quarisma/ops/index_put_native.h>
#include <Quarisma/ops/index_reduce_meta.h>
#include <Quarisma/ops/index_reduce_native.h>
#include <Quarisma/ops/index_select_backward_native.h>
#include <Quarisma/ops/index_select_native.h>
#include <Quarisma/ops/masked_fill_native.h>
#include <Quarisma/ops/masked_scatter_native.h>
#include <Quarisma/ops/masked_select_backward_native.h>
#include <Quarisma/ops/masked_select_native.h>
#include <Quarisma/ops/nested_to_padded_tensor_native.h>
#include <Quarisma/ops/nonzero_native.h>
#include <Quarisma/ops/nonzero_numpy_native.h>
#include <Quarisma/ops/nonzero_static_native.h>
#include <Quarisma/ops/ones_like.h>
#include <Quarisma/ops/put_native.h>
#include <Quarisma/ops/quantize_per_tensor.h>
#include <Quarisma/ops/scatter_add_meta.h>
#include <Quarisma/ops/scatter_add_native.h>
#include <Quarisma/ops/scatter_meta.h>
#include <Quarisma/ops/scatter_native.h>
#include <Quarisma/ops/scatter_reduce_meta.h>
#include <Quarisma/ops/scatter_reduce_native.h>
#include <Quarisma/ops/take_along_dim_native.h>
#include <Quarisma/ops/take_native.h>
 ...
 const std::string_view reduce)
{
    TORCH_WARN_ONCE(
        ...
        "in a future PyTorch release. Use torch.scatter_reduce instead for more reduction "
        "options.");
...
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& source)
{
    ...
    else if ((source.dim() != self.dim()) && (source.dim() != 0 && self.dim() != 0))
    {
        TORCH_CHECK_INDEX(
            false,
            "index_copy_(): When source and destination are not scalars, their dimensionality must "
            "match. Source dimensionality (",
            source.dim(),
    ...
    if (selfSlicedSizes.size() != sourceSlicedSizes.size() ||
        !std::equal(selfSlicedSizes.begin(), selfSlicedSizes.end(), sourceSlicedSizes.begin()))
    {
        std::stringstream ss;
    ...
    std::string_view func)
{
    ...
    TORCH_CHECK(
        ...
        "source tensor shape must match self tensor shape, excluding the specified dimension. Got "
        "self.shape = ",
        self.sizes(),
        " source.shape = ",
        source.sizes());
 ...
 bool                   include_self)
{
    (void)include_self;
    TORCH_CHECK(
        reduce == "prod" || reduce == "mean" || reduce == "amax" || reduce == "amin",
...
static void build_index_op(
    TensorIteratorBase& iter, const at::native::AdvancedIndex& info, const Tensor& result)
{
...
static void check_indices_on_cpu_or_selfdevice(
    const Tensor& self, const at::MaterializedIOptTensorListRef& indices)
{
    auto dev                   = self.device();
    bool indices_on_cpu_or_dev = std::all_of(
        indices.begin(),
        indices.end(),
        [=](const at::OptionalTensorRef& opt)
...
static Tensor restride_src(
    const Tensor& src, int64_t dims_before, int64_t dims_indexed, IntArrayRef replacement_shape)
{
...
AdvancedIndex::AdvancedIndex(const Tensor& src, TensorList indices_list)
{
    ...
    if (indices.size() >= 2 &&
        (this->src.device().type() == kCUDA || this->src.device().type() == kMPS ||
...
Tensor quantized_index(const Tensor& self, const torch::List<std::optional<Tensor>>& indices)
{
    TORCH_INTERNAL_ASSERT(
        self.qscheme() == c10::kPerTensorAffine || self.qscheme() == c10::kPerTensorSymmetric,
    ...
    return at::quantize_per_tensor(result, self.q_scale(), self.q_zero_point(), self.scalar_type());
}
    ...
    const Scalar&                             fill)
{
    ...
    std::transform(
        indices.begin(), indices.end(), self.sizes().begin(), clamped_indices.begin(), clamp);
    ...
    const bool                                unsafe)
{
    ...
    if ((self.device().type() == DeviceType::CUDA || self.device().type() == DeviceType::XPU) &&
        (accumulate || (globalContext().deterministicAlgorithms() && value_.numel() > 1)))
    {
        TORCH_CHECK(
...
Tensor& take_out(const Tensor& self, const Tensor& index, Tensor& out)
{
    ...
    // index checks
    TORCH_CHECK_INDEX(
        !(self.numel() == 0 && index.numel() != 0), "take(): tried to take from an empty tensor");
...
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& source, const Tensor& result)
{
    ...
    index_copy_stub(iter.device_type(), iter, dim, result_dim_size, result_dim_stride);
}
 ...
 const Tensor& result)
{
    ...
    if (result.dim() > 1)
    {
            ...
            result.scalar_type() != ScalarType::ComplexHalf)
        {
            ...
            auto check_sizes = [&ep_sizes, &ep_strides, &numel](
        ...
        AT_DISPATCH_INDEX_TYPES(
            ...
            [&]()
            {
                ...
                for (const auto i : c10::irange(numel))
                {
                    ...
                    TORCH_CHECK_INDEX(
                        (self_i >= 0) && (self_i < self_dim_size), "index out of range in self");
                    auto self_data =
                        static_cast<char*>(selfSlice.data_ptr()) + self_i * self_stride_bytes;
                    auto source_data = static_cast<const char*>(sourceSlice.const_data_ptr()) +
                     ...
                     &counts_stride]
                    {
                        ...
                        for (const auto i : c10::irange(numel))
                        {
                            ...
                            case ReductionType::MIN:
                                ...
                                *self_ip =
                                    at::_isnan<scalar_t>(val) ? val : std::min(*self_ip, val);
                                break;
                            case ReductionType::MAX:
                                val = *(source_ptr + i * source_stride);
                                *self_ip =
                                    at::_isnan<scalar_t>(val) ? val : std::max(*self_ip, val);
                                break;
                            default:
                                break;
                            }
...
static void check_indexarray_range(const IndexType* indices, int64_t n, IndexType indexing_axis_dim)
{
                 ...
                 &result_stride_bytes]()
                {
                    ...
                    for (const auto i : c10::irange(start, end))
                    {
                        ...
                        TORCH_CHECK_INDEX(
                            ...
                            "index out of range in self");
                        auto self_data =
                            static_cast<const char*>(selfSlice_data) + self_i * self_stride_bytes;
                        auto result_data =
                            static_cast<char*>(resultSlice_data) + i * result_stride_bytes;
...
Tensor index_select_backward_symint(
    const Tensor& grad, c10::SymIntArrayRef self_sizes, int64_t dim, const Tensor& index)
{
...
Tensor& index_fill_(Tensor& self, int64_t dim, const Tensor& index, const Scalar& source)
{
    ...
    index_fill_stub(iter.device_type(), iter, dim, self_dim_size, self_dim_stride, source);
...
static bool can_use_expanded_index_path(
    const Tensor& self, int64_t dim, const Tensor& index, const Tensor& src, bool is_scatter_like)
{
#ifdef USE_FBGEMM
...
static void scatter_reduce_exclude_self_helper(
    const Tensor& self, int64_t dim, const Tensor& index, const ReductionType& op)
{
...
(const Tensor& self, int64_t dim, const Tensor& index, const Tensor& src, const Tensor& out)
{
    scatter_impl(self, dim, index, src, out, scatter_reduce_stub, scatter_stub);
}

TORCH_IMPL_FUNC(scatter_value_out)
(const Tensor& self, int64_t dim, const Tensor& index, const Scalar& value, const Tensor& out)
{
    scatter_impl(self, dim, index, value, out, scatter_scalar_reduce_stub, scatter_fill_stub);
}
...
(const Tensor&          self,
 ...
 const Tensor&          out)
{
    scatter_impl(self, dim, index, src, out, scatter_reduce_stub, scatter_stub, reduce);
}
...
{
    scatter_impl(
        self, dim, index, value, out, scatter_scalar_reduce_stub, scatter_fill_stub, reduce);
}
 ...
 const Tensor&          out)
{
    ...
    scatter_impl</*use_new_options=*/true>(
        self, dim, index, src, out, scatter_reduce_two_stub, scatter_stub, reduce, include_self);
...
Tensor masked_scatter_backward_symint(
    const Tensor& grad, const Tensor& mask, c10::SymIntArrayRef sizes)
{
    c10::SymInt numel = 1;
    ...
    if (diff_nelem > 0)
    {
        ...
        // tensor2's size.
        auto zeros_fillin = at::zeros_symint({std::move(diff_nelem)}, grad.options());
...
inline std::tuple<Tensor, Tensor, int64_t> _take_along_dim_helper(
    const Tensor& self, const Tensor& indices, int64_t dim)
{
    TORCH_CHECK(
    ...
    return std::make_tuple(
        std::move(self_broadcasted), std::move(indices_broadcasted), std::move(dim));
}
...
inline void checkDevice(CheckedFrom c, at::ArrayRef<Tensor> tensors, Device device)
{
    for (auto& t : tensors)
    {
...
Tensor& take_along_dim_out(
    const Tensor& self, const Tensor& indices, std::optional<int64_t> opt_dim, Tensor& result)
{
    ...
    const Tensor& self, int64_t dim, const Tensor& index, const Tensor& grad)
{
    ...
    if (self.ndimension() == 0)
        return at::_sparse_coo_tensor_unsafe_symint(
            at::empty_symint({0, grad.sym_numel()}, index.options()), grad, self.sym_sizes());
    if (grad.ndimension() == 0)
        return at::_sparse_coo_tensor_unsafe_symint(index.view({1, 1}), grad, self.sym_sizes());
    Tensor sparse_ind =
        at::empty_symint({self.ndimension(), grad.sym_numel()}, self.options().dtype(at::kLong));
    ...
    if (grad_numel > 0)
    {
        ...
        for (const auto i : c10::irange(self.ndimension()))
        {
            ...
            {
                sparse_ind[i] = at::arange(grad.sym_size(i), self.options().dtype(at::kLong))
                                    .unsqueeze(1)
...
Tensor& nonzero_out_cpu(const Tensor& self, Tensor& result)
{
    ...
    auto iter = TensorIteratorConfig().add_const_input(self).enforce_linear_iteration().build();
        ...
        [&]
        {
            at::parallel_for(
                ...
                [&](int64_t begin, int64_t end)
                {
                    ...
                    TORCH_INTERNAL_ASSERT(
                        out_ptr == out_accessor[thread_count_nonzero[tid + 1]].data());
                });
        });
    return result;
}
...
Tensor& nonzero_static_out_cpu(const Tensor& self, int64_t size, int64_t fill_value, Tensor& result)
{
...
Tensor nonzero_static_cpu(const Tensor& self, int64_t size, int64_t fill_value)
{
    // Check if `size` is not negative
    TORCH_CHECK(size >= 0, "nonzero_static: 'size' must be an non-negative integer");
    ...
    int64_t ndim = self.dim();
    auto    result =
        at::empty({size, ndim}, at::TensorOptions().dtype(at::ScalarType::Long).device(at::kCPU));
...
Tensor& masked_scatter__cpu(Tensor& self, const Tensor& mask, const Tensor& source)
{
    ...
    {
        TORCH_WARN(
            "masked_scatter_ received a mask with dtype torch.uint8, this behavior is now "
            "deprecated,"
