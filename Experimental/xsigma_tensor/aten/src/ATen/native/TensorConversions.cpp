...
#else
#include <Quarisma/ops/_autocast_to_full_precision_native.h>
#include <Quarisma/ops/_autocast_to_reduced_precision_native.h>
#include <Quarisma/ops/_convert_indices_from_coo_to_csr.h>
#include <Quarisma/ops/_convert_indices_from_coo_to_csr_native.h>
#include <Quarisma/ops/_convert_indices_from_csr_to_coo.h>
#include <Quarisma/ops/_convert_indices_from_csr_to_coo_native.h>
#include <Quarisma/ops/_sparse_bsc_tensor_unsafe_native.h>
#include <Quarisma/ops/_sparse_bsr_tensor_unsafe_native.h>
#include <Quarisma/ops/_sparse_compressed_tensor_unsafe_native.h>
#include <Quarisma/ops/_sparse_coo_tensor_unsafe_native.h>
#include <Quarisma/ops/_sparse_coo_tensor_with_dims_native.h>
#include <Quarisma/ops/_sparse_csc_tensor_unsafe_native.h>
#include <Quarisma/ops/_sparse_csr_tensor_unsafe_native.h>
#include <Quarisma/ops/_to_copy.h>
#include <Quarisma/ops/_to_copy_native.h>
#include <Quarisma/ops/_to_cpu_native.h>
#include <Quarisma/ops/_to_dense_native.h>
#include <Quarisma/ops/_to_sparse_bsc_native.h>
#include <Quarisma/ops/_to_sparse_bsr_native.h>
#include <Quarisma/ops/_to_sparse_csc_native.h>
#include <Quarisma/ops/_to_sparse_csr_native.h>
#include <Quarisma/ops/_to_sparse_native.h>
#include <Quarisma/ops/arange_native.h>
...
#include <Quarisma/ops/to_dense_backward_native.h>
#include <Quarisma/ops/to_dense_native.h>
#include <Quarisma/ops/to_mkldnn_backward_native.h>
#include <Quarisma/ops/to_native.h>
#include <Quarisma/ops/to_sparse_bsc_native.h>
#include <Quarisma/ops/to_sparse_bsr_native.h>
#include <Quarisma/ops/to_sparse_csc_native.h>
#include <Quarisma/ops/to_sparse_csr_native.h>
#include <Quarisma/ops/to_sparse_native.h>
#include <Quarisma/ops/view_native.h>
#include <Quarisma/ops/zeros.h>
#endif

#include <Quarisma/SparseCsrTensorUtils.h>
#include <Quarisma/core/ATen_fwd.h>
#include <Quarisma/native/IndexingUtils.h>
#include <Quarisma/native/NonSymbolicBC.h>
#include <Quarisma/native/SparseTensorUtils.h>
#include <Quarisma/native/TensorConversions.h>
...
void dense_to_sparse_compressed_prepare_check_mask_values_batched(
    const Layout& target_layout, Tensor& values, Tensor& mask, const int64_t& n_batch_dim)
{
    if (n_batch_dim > 1)
    {
...
Tensor compressed_to_batched_compressed_indices(
    const Tensor& compressed_in, const int64_t& n_batch, bool out_int32)
{
    auto       n_compressed_per_batch = (compressed_in.size(0) - 1) / n_batch;
    ScalarType out_type               = out_int32 ? ScalarType::Int : ScalarType::Long;
    auto       batched_out =
        at::zeros({n_batch, n_compressed_per_batch + 1}, compressed_in.options().dtype(out_type));
    ...
    if (n_compressed_per_batch > 0)
    {
        ...
        // to n-batch rows
        auto trailing_slice = compressed_in.slice(0, 1, std::nullopt, 1).reshape({n_batch, -1});
        ...
        // reshape
        auto offsets = compressed_in.slice(0, 0, -1, n_compressed_per_batch).reshape({n_batch, -1});
        ...
        // the output buffer.
        batched_out.narrow(-1, 1, n_compressed_per_batch).copy_(trailing_slice - offsets);
    }
    return batched_out;
}
    ...
    Tensor&           values)
{
    ...
    auto n_batch =
        std::accumulate(batch_shape.begin(), batch_shape.end(), 1, std::multiplies<int64_t>());
    ...
    compressed_indices =
        compressed_to_batched_compressed_indices(compressed_indices, n_batch, /*out_int32*/ false);
    ...
    std::optional<c10::MemoryFormat> optional_memory_format)
{
    ...
    else if (at::sparse_csr::is_sparse_compressed(self))
    {
        ...
        return at::_sparse_compressed_tensor_unsafe(
            new_compressed_indices, new_plain_indices, new_values, self.sizes(), options);
    }

    bool pin_out =
        (non_blocking && at::accelerator::isAcceleratorExcluded(self.device().type(), at::kMPS) &&
    ...
    ScalarType    cpu_dtype)
{
    if (self.dtype() == at::ScalarType::Float &&
        ((self.device().is_cuda() && cuda_enabled) || (self.device().is_cpu() && cpu_enabled)))
    {
...
Tensor to_dense(
    const Tensor& tensor, std::optional<c10::ScalarType> dtype, std::optional<bool> masked_grad)
{
    ...
    if (tensor.layout() == c10::kSparseCsr || tensor.layout() == c10::kSparseCsc ||
        tensor.layout() == c10::kSparseBsr || tensor.layout() == c10::kSparseBsc)
    {
        return tensor._to_dense(dtype, masked_grad);
    }
    ...
    TORCH_CHECK(
        tensor.layout() == c10::kStrided, "to_dense does not support layout ", tensor.layout());
    if (dtype)
    {
...
Tensor sparse_to_dense(
    const Tensor& self, std::optional<ScalarType> dtype, std::optional<bool> masked)
{
...
Tensor sparse_compressed_to_dense(
    const Tensor& self, std::optional<ScalarType> dtype, std::optional<bool> masked_grad)
{
    TORCH_CHECK(!dtype.has_value(), "dtype argument is not supported by sparse_csr_to_dense");
    ...
    auto [compressed_indices, plain_indices] = sparse_csr::getCompressedPlainIndices(self);
    ...
    Tensor indices = at::_convert_indices_from_csr_to_coo(
        compressed_indices_over_all_batches, plain_indices.flatten(), false, !compressed_rows);
    ...
    {
        return dense.unflatten(0, {-1, nrows, ncols}).transpose(2, 3).reshape(self.sizes());
    }
}
...
static inline SymDimVector compute_strides_for_view_dtype_downsize(
    SymIntArrayRef old_strides, int64_t size_ratio, ScalarType old_dtype, ScalarType new_dtype)
{
...
Tensor view_dtype(const Tensor& self, ScalarType dtype)
{
    ...
    TORCH_CHECK(
        !self.is_conj(),
        "torch.Tensor.view is not supported for conjugate view tensors when converting to a "
        "different dtype.");
    TORCH_CHECK(
        !self.is_neg(),
        "torch.Tensor.view is not supported for tensors with negative bit set when converting to a "
        "different dtype.");
    ...
    auto    new_tensor =
        detail::make_tensor<TensorImpl>(std::move(storage), self.key_set(), type_meta);
...
static Tensor _mask_to_indices(const Tensor& mask)
{
    ...
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(mask.dim() == 1, "_mask_to_indices only supports 1-d masks.");
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
        mask.dtype() == at::kBool, "Expected mask to be of dtype bool.");
    return at::native::flatten(at::nonzero(mask));
}

static std::pair<Tensor, Tensor> _not_zero_mask_to_col_row_indices(Tensor not_zero_mask)
{
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
        not_zero_mask.dim() == 2, "_not_zero_mask_to_col_row_indices only supports 2-d masks.");
    TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
...
static inline void _to_sparse_check_arguments(
    const std::string& funcname, const Tensor& self, const int64_t sparse_dim)
{
    ...
    std::optional<int64_t>     dense_dim_opt)
{
    ...
    if (blocksize.has_value())
    {
        ...
        {
            TORCH_CHECK(
                false, funcname, ": blocksize needs to be positive, but got ", blocksize_to);
        }
        ...
        if (layout_to == kSparseBsr || layout_to == kSparseBsc)
        {
            ...
            {
                auto dense_dim =
                    (layout_from == kStrided) ? dense_dim_opt.value_or(0) : self.dense_dim();
    ...
    if (dense_dim_opt.has_value())
    {
        ...
        if (layout_to == kSparse)
        {
            ...
            {
                TORCH_CHECK(
                    false, funcname, ": dense_dim argument must be !=self.dim() when self.dim()>0");
            }
...
{
    static_assert(
        target_layout == Layout::SparseCsr || target_layout == Layout::SparseCsc ||
    ...
    auto not_zero_mask =
        blocked_layout ? _batch_tile_tensor(self_mask, blocksize, dense_dim) : self_mask;
    ...
    std::optional<int64_t>     dense_dim_opt)
{
    auto layout_to = layout.value_or(kSparse);
    TORCH_INTERNAL_ASSERT(
        self.layout() != layout_to, "dense_to_sparse: unexpected same input and output layout");
    TORCH_INTERNAL_ASSERT(
...
Tensor dense_to_sparse_bsr(
    const Tensor& self, IntArrayRef blocksize, std::optional<int64_t> dense_dim_opt)
{
    ...
    std::optional<int64_t>     dense_dim_opt)
{
    ...
    TORCH_CHECK(
        false, "dense_to_sparse: ", self.layout(), " to ", layout_to, " conversion not supported");
    return Tensor{};
}
...
static Tensor sparse_compressed_to_flipped(
    const Tensor& self, std::optional<IntArrayRef> blocksize, const std::string& name)
{
    ...
    const auto compressed_indices_2d = [&]() -> Tensor
    {
        ...
        const auto batch_offsets = batch_nnz_offset.unsqueeze(-1).expand_as(compressed_offsets);
...
Tensor coo_to_sparse_csr(const Tensor& self, std::optional<int64_t> dense_dim_opt)
{
    ...
    auto crow_indices = at::_convert_indices_from_coo_to_csr(row_indices, self.size(0), out_int32);
...
void convert_indices_from_coo_to_csr_cpu(
    const Tensor& result, const Tensor& input, const int64_t size)
{
...
(const Tensor& input, const int64_t size, const bool out_int32, const Tensor& result)
{
    if (out_int32)
    {
        AT_DISPATCH_INTEGRAL_TYPES(
            input.scalar_type(),
            "convert_indices_from_coo_to_csr_cpu",
            [&] { convert_indices_from_coo_to_csr_cpu<scalar_t, int32_t>(result, input, size); });
    }
    else
    {
        AT_DISPATCH_INTEGRAL_TYPES(
 ...
 const Tensor& result)
{
    if (out_int32)
    {
        AT_DISPATCH_INTEGRAL_TYPES(
            ...
            "convert_indices_from_csr_to_coo_cpu",
            [&]
            {
...
template <Layout target_layout>
static Tensor _compressed_to_block_compressed_cpu(const Tensor& self, IntArrayRef blocksize)
{
    static_assert(
        target_layout == Layout::SparseBsr || target_layout == Layout::SparseBsc,
    ...
    auto n_dense =
        std::accumulate(dense_shape.begin(), dense_shape.end(), 1, std::multiplies<int64_t>());
    AT_DISPATCH_INDEX_TYPES(
        input_compressed_indices.scalar_type(),
        "_compressed_to_block_compressed_cpu",
        [&]
    ...
    const Tensor& self, IntArrayRef blocksize, std::optional<int64_t> dense_dim_opt)
{
    ...
    _to_sparse_check_arguments(
        "sparse_compressed_to_sparse_bsr", self, layout_to, blocksize, dense_dim_opt);
    ...
    if (self.layout() == kSparseCsr)
    {
        ...
        {
            TORCH_WARN(
                "sparse_compressed_to_sparse_bsr executing on the CPU device, the performance may "
                "be sub-optimal");
        }
        return _compressed_to_block_compressed_cpu<kSparseBsr>(self.cpu(), blocksize)
            .to(self.device());
    }
    if (self.layout() == kSparseCsc)
    {
    ...
    TORCH_CHECK(
        false,
        "sparse_compressed_to_sparse_bsr: expected SparseCsr, SparseCsc, SparseBsr or SparseBsc "
        "layout but got ",
        self.layout());
    return Tensor{};
}
    ...
    const Tensor& self, IntArrayRef blocksize, std::optional<int64_t> dense_dim_opt)
{
    ...
    _to_sparse_check_arguments(
        "sparse_compressed_to_sparse_bsc", self, layout_to, blocksize, dense_dim_opt);
    ...
    if (self.layout() == kSparseCsc)
    {
        ...
        {
            TORCH_WARN(
                "sparse_compressed_to_sparse_bsc executing on the CPU device, the performance may "
                "be sub-optimal");
        }
        return _compressed_to_block_compressed_cpu<kSparseBsc>(self.cpu(), blocksize)
            .to(self.device());
    }
    if (self.layout() == kSparseCsr)
    {
    ...
    TORCH_CHECK(
        false,
        "sparse_compressed_to_sparse_bsc: expected SparseCsr, SparseCsc, SparseBsr or SparseBsc "
        "layout but got ",
        self.layout());
    return Tensor{};
}
...
Tensor sparse_compressed_to_sparse(const Tensor& self, const int64_t sparse_dim)
{
    ...
    Tensor values;
    Tensor indices = at::_convert_indices_from_csr_to_coo(
        compressed_indices, plain_indices, false, (layout == kSparseCsc || layout == kSparseBsc));
    ...
    AT_DISPATCH_PLAIN_SPARSE_COMPRESSED_LAYOUTS(
        ...
        [&]
        {
            ...
            const auto block_coo_indices =
                at::zeros({batch_ndim + 2, blocksize[0] * blocksize[1]}, indices.options());
