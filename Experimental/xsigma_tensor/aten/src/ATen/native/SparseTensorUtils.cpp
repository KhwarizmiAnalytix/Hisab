...
Tensor flatten_indices(const Tensor& indices, IntArrayRef full_size, bool force_clone /*= false*/)
{
    ...
    else
    {
        ...
        return at::native::flatten_indices_stub(
            indices.device().type(), indices, full_size.slice(0, sparse_dim));
    }
}
...
Tensor flatten_indices_by_dims(
    const Tensor& indices, const IntArrayRef& sizes, const IntArrayRef& dims_to_flatten)
{
