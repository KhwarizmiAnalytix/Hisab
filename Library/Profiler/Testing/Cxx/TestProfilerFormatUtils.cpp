...
QUARISMATEST(Profiler, format_one_digit_rounds_to_single_decimal)
{
    EXPECT_EQ(one_digit(3.14), "3.1");
    EXPECT_EQ(one_digit(-7.95), "-8.0");
}

QUARISMATEST(Profiler, format_two_digits_rounds_half_up)
{
    EXPECT_EQ(two_digits(2.345), "2.35");
    EXPECT_EQ(two_digits(-2.344), "-2.34");
}

QUARISMATEST(Profiler, format_three_digits_preserves_precision)
{
    EXPECT_EQ(three_digits(1.23456), "1.235");
    EXPECT_EQ(three_digits(-9.87654), "-9.877");
}

QUARISMATEST(Profiler, format_max_precision_round_trips_double_value)
{
...
QUARISMATEST(Profiler, format_handles_small_and_large_magnitudes)
{
    EXPECT_EQ(two_digits(0.000123), "0.00");
    EXPECT_EQ(one_digit(9876.543), "9876.5");
}

QUARISMATEST(Profiler, format_handles_zero)
{
