...
constexpr size_t NumRecordScopes = static_cast<size_t>(RecordScope::NUM_SCOPES);

RecordFunctionCallbacks::iterator findCallback(
    RecordFunctionCallbacks& entries, CallbackHandle handle)
{
...
void CacheEntry::rebuildActiveCallbacks()
{
    ...
    for (const auto& i : callbacks_)
    {
        if (i.tries_left_ < 0)
        {
            ...
            active_callbacks_.callbacks_.push_back({i.callback_.start(), i.callback_.end()});
        }
...
StepCallbacks LocalCallbackManager::getActiveCallbacks(const RecordScope scope)
{
...
CallbackHandle LocalCallbackManager::addCallback(RecordFunctionCallback callback)
{
    ...
    rebuild_callback_scopes(GlobalCallbackManager::get().getSnapshot(), callbacks.back().callback_);
    return handle;
}

bool LocalCallbackManager::setCallbackEnabled(CallbackHandle handle, bool enabled)
{
    ...
    {
        it->enabled_ = enabled;
        rebuild_callback_scopes(GlobalCallbackManager::get().getSnapshot(), it->callback_);
    }
    return found;
}
...
bool LocalCallbackManager::removeCallback(CallbackHandle handle)
{
    ...
    {
        rebuild_callback_scopes(GlobalCallbackManager::get().getSnapshot(), *callback);
    }
    ...
    const GlobalCallbackManager::snapshot_t& global_snapshot, const RecordScope scope)
{
    ...
    {
        auto populate_callbacks = [&](const RecordFunctionCallbacks& raw_callbacks)
        {
            for (const auto& i : raw_callbacks)
            {
                if (i.enabled_ && i.callback_.checkScope(scope) && i.callback_.samplingProb() > 0)
                {
...
RecordFunction::RecordFunction(RecordScope scope) : RecordFunction(getStepCallbacks(scope)) {}
...
{
    return std::any_of(
        callbacks.begin(), callbacks.end(), [](const auto& cb) { return cb.enabled_; });
}
}  // namespace
