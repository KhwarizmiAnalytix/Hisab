...
struct FDE
{
    ...
    void advance_raw(int64_t amount)
    {
        ...
        if (LOG)
        {
            (*out_) << (void*)(previous_pc - load_bias_) << "-" << (void*)(current_pc_ - load_bias_)
                    << ": " << state() << "\n";
        }
    }
    ...
    void offset(int64_t reg, int64_t offset)
    {
        ...
        state().registers.at(reg) = Action{A_LOAD_CFA_OFFSET, -1, offset * data_alignment_factor_};
    }
    ...
    void register_(int64_t reg, int64_t rhs_reg)
    {
        ...
        state().registers.at(reg) = Action::regPlusData(static_cast<int32_t>(reg), 0);
    }
    ...
    TableState readUpTo(uint64_t addr)
    {
        ...
        {
            // NOLINTNEXTLINE(performance-no-int-to-ptr)
            (*out_) << "readUpTo " << (void*)addr << " for " << library_name_ << " at "
                    << (void*)load_bias_ << "\n";
        }
        ...
        case 0x0:
        {
            ...
            case DW_CFA_expression:
            {
                ...
                {  // DW_bregX
                    ...
                    if (L.loc() == end)
                    {
                        state().registers.at(reg) = Action::regPlusDataDeref(rhs_reg, addend);
                        return;
                    }
                }
    ...
    TableState initial_state_;  // state after the initial instructions, used by restore
