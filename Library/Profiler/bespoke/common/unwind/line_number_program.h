...
struct LineNumberProgram
{
    ...
    void     parse()
    {
        ...
        UNWIND_CHECK(version == 5 || version == 4, "expected version 4 or 5 but found {}", version);
        if (version == 5)
        {
        ...
        if (version == 5)
        {
            ...
            for (size_t i = 0; i < directories_count; i++)
            {
                for (auto& member : directory_members)
                {
                    ...
                    case DW_LNCT_path:
                    {
                        include_directories_.emplace_back(s_.readString(L, member.form, is_64bit_));
                    }
                    break;
                    default:
                    {
            ...
            for (size_t i = 0; i < files_count; i++)
            {
                for (auto& member : file_members)
                {
                    ...
                    case DW_LNCT_path:
                    {
                        file_names_.emplace_back(s_.readString(L, member.form, is_64bit_));
                    }
                    break;
