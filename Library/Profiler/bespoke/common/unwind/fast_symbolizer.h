...
#include "bespoke/common/unwind/line_number_program.h"
...
struct FastSymbolizer
{
    ...
    Frame symbolize(const std::string& library, uint64_t offset)
    {
        ...
        {
            UNWIND_WARN(warnings_, "failed to find subprogram name for {} 0x{:x}", library, offset);
        }
        ...
        {
            UNWIND_WARN(warnings_, "failed to find file/line for {} 0x{:x}", library, offset);
        }
        return frame;
    }
...
private:
    ...
    Sections* getOrCreateSections(const std::string& library)
    {
        ...
        if (it == libraries_.end())
        {
            ...
            {
                UNWIND_WARN(warnings_, "failed to parse library {}: {}", library, err.what());
            }
        }
        return it->second.get();
    }
    std::optional<std::pair<std::string, int64_t>> findLine(Sections* s, uint64_t offset)
    {
