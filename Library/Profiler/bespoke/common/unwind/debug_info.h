...
#include "bespoke/common/unwind/dwarf_symbolize_enums.h"
...
struct DebugInfo
{
    ...
    std::optional<uint64_t> lineNumberProgramOffset() { return line_number_program_offset_; }
    ...
    std::vector<std::pair<uint64_t, uint64_t>> ranges()
    {
        if (range_ptr_)
        {
            ...
            if (range_ptr_->second == DW_FORM_rnglistx)
            {
                ...
                CheckedLexer L =
                    s_.debug_rnglists.lexer(*rnglists_base_ + offset * sec_offset_size_);
...
private:
    CheckedLexer parseHeader(uint64_t offset)
    {
        ...
        UNWIND_CHECK(version_ == 5 || version_ == 4, "unexpected dwarf version {}", version_);
        ...
        UNWIND_CHECK(
            address_size == 8, "expected 64-bit dwarf but found address size {}", address_size);
        return L;
    }

    uint64_t readSegmentOffset(CheckedLexer& L) { return s_.readSegmentOffset(L, is_64bit_); }
    ...
    uint64_t readEncoded(CheckedLexer& L, uint64_t encoding)
    {
        ...
        case DW_FORM_addrx:
        {
            ...
            return s_.debug_addr.lexer(address_base_ + sizeof(uint64_t) * idx).read<uint64_t>();
        }
    ...
    void parseCompileUnit(CheckedLexer& L)
    {
        ...
        while (true)
        {
            ...
            {
                UNWIND_CHECK(form == DW_FORM_sec_offset, "unexpected rnglists_base form");
    ...
    CheckedLexer findAbbrev(uint64_t offset, uint64_t entry)
    {
        ...
        while (true)
        {
            ...
            UNWIND_CHECK(abbrev_code != 0, "could not find entry {} at offset {:x}", entry, offset);
            auto tag = L.readULEB128();
            L.read<uint8_t>();  // has children
            if (abbrev_code == entry)
            {
                UNWIND_CHECK(
                    tag == DW_TAG_compile_unit, "first entry was not a compile unit but {}", tag);
                return L;
            }
            while (true)
            {
