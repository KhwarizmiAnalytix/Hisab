...
struct Communicate
{
    ...
    {
        if (pipe(inpipe_.data()) < 0 || pipe(outpipe_.data()) < 0 || pipe(errpipe_.data()) < 0)
        {
        ...
        {  // parent process
            ...
            close(outpipe_[1]);
            close(errpipe_[1]);
            outbuf_ = std::make_unique<__gnu_cxx::stdio_filebuf<char>>(inpipe_[1], std::ios::out);
            inbuf_  = std::make_unique<__gnu_cxx::stdio_filebuf<char>>(outpipe_[0], std::ios::in);
            errbuf_ = std::make_unique<__gnu_cxx::stdio_filebuf<char>>(errpipe_[0], std::ios::in);
