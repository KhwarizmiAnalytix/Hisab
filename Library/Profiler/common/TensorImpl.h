...
struct PROFILER_API ExtraMeta
{
    ...
    std::unique_ptr<ExtraMeta> clone() const { return std::make_unique<ExtraMeta>(*this); }
};
...
public:
    ...
    bool unique() const { return version_counter_ ? 1 == version_counter_.use_count() : true; }
...
// it a non-error.
   ...
   */
    inline const Storage& unsafe_storage() const { return storage_; }
...
public:
    ...
    bool is_nested() const { return key_set_.has(DispatchKey::NestedTensor); }
...
public:
   ...
   */
    ...
    {
        return data_ptr_impl_impl<const T>([this]
...
public:
   ...
   */
    inline bool is_empty() const { return numel() == 0; }
   ...
   * ] for details.
   */
    bool allow_tensor_metadata_change() const { return allow_tensor_metadata_change_; }
...
public:
   ...
   */
    ...
    impl::PyObjectSlot* pyobj_slot() { return &pyobj_slot_; }

    const impl::PyObjectSlot* pyobj_slot() const { return &pyobj_slot_; }
...
public:
   ...
   */
    ...
    void ShareExternalPointer(
        DataPtr&& data_ptr, const caffe2::TypeMeta data_type, size_t size_bytes);
    ...
    inline void* raw_mutable_data(const caffe2::TypeMeta& meta)
    {
        ...
        if (data_type_ == meta && storage_initialized())
        {
            return static_cast<void*>(
                static_cast<char*>(storage_.mutable_data()) + storage_offset_ * meta.itemsize());
        }
        else
        {
   ...
   */
    bool dtype_initialized() const noexcept { return data_type_ != caffe2::TypeMeta(); }
   ...
   */
    ...
    // sizes/strides
    bool has_symbolic_sizes_strides() const { return has_symbolic_sizes_strides_; }
...
private:
    ...
    //
    // SetDims on array_ref is internally implemented as a template, so we can
    ...
    bool SetDims(array_ref<int64_t> s) { return SetDimsTemplate(s); }

    bool SetDims(array_ref<int> s) { return SetDimsTemplate(s); }

    bool SetDims(array_ref<size_t> s) { return SetDimsTemplate(s); }

    bool SetDims() { return SetDims(IntArrayRef{}); }

    bool SetDims(const int64_t d0) { return SetDims(IntArrayRef{d0}); }
...
private:
    void _set_is_contiguous(bool b) { is_contiguous_ = b; }

    void _set_is_channels_last_contiguous(bool b) { is_channels_last_contiguous_ = b; }

    void _set_is_channels_last_3d_contiguous(bool b) { is_channels_last_3d_contiguous_ = b; }

    void _set_is_channels_last(bool b) { is_channels_last_ = b; }

    void _set_is_channels_last_3d(bool b) { is_channels_last_3d_ = b; }

    void _set_is_non_overlapping_and_dense(bool b) { is_non_overlapping_and_dense_ = b; }
...
private:
    static void copy_tensor_metadata_except_version_counter(
        const TensorImpl* src_impl, TensorImpl* dest_impl, bool allow_tensor_metadata_change);
...
public:
    void set_storage_access_should_throw() { storage_access_should_throw_ = true; }
...
protected:
    ...
    void refresh_device_policy() { device_policy_ = custom_device_ || python_custom_device_; }

    void refresh_layout_policy() { layout_policy_ = custom_layout_ || python_custom_layout_; }
...
protected:
    ...
    // INVARIANT: When storage is non-null, this device_option must
    // agree with the type meta in storage.
    //
