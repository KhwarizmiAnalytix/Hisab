...
ApproximateClockToUnixTimeConverter::time_pairs ApproximateClockToUnixTimeConverter::measurePairs()
{
    ...
    for ([[maybe_unused]] const auto _ : profiler::irange(n_warmup))
    {
        getApproximateTime();
    ...
    time_pairs out;
    for (const auto i : profiler::irange(out.size()))
    {
        out[i] = measurePair();
    }
    return out;
...
std::function<time_t(approx_time_t)> ApproximateClockToUnixTimeConverter::makeConverter()
{
    ...
    for (const auto i : profiler::irange(replicates))
    {
    ...
    return [=](approx_time_t t_approx)
    {
        ...
        return t_approx > t0_approx
                   ? static_cast<time_t>(static_cast<double>(t_approx - t0_approx) * scale_factor) +
                         t0
                   : 0;
    };
}
