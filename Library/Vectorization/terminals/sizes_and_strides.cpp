...
void sizes_and_strides::resizeSlowPath(const size_t newSize, const size_t oldSize)
{
    if (newSize <= MAX_INLINE_SIZE)
    {
        VECTORIZATION_CHECK_DEBUG(
            !isInline(), "resizeSlowPath called when fast path should have been hit!");
        int64_t* tempStorage = outOfLineStorage_;
        memcpy(&inlineStorage_[0], &tempStorage[0], MAX_INLINE_SIZE * sizeof(inlineStorage_[0]));
        memcpy(
    ...
    else
    {
        if (isInline())
        {
            ...
            VECTORIZATION_CHECK(
                tempStorage, "Could not allocate memory to change Tensor sizes_and_strides!");
            const auto bytesToCopy = oldSize * sizeof(inlineStorage_[0]);
            const auto bytesToZero =
                (newSize > oldSize) ? (newSize - oldSize) * sizeof(tempStorage[0]) : 0;
