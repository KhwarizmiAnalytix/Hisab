...
struct packet
{
    ...
    // Applies f(integral_constant<size_t, I>) for each I in [0, N) via a constexpr comma fold.
    template <std::size_t... I, typename F>
    VECTORIZATION_FORCE_INLINE static void for_each_impl(F&& f, std::index_sequence<I...>)
    {
        (f(std::integral_constant<std::size_t, I>{}), ...);
    }

    template <typename F>
    VECTORIZATION_FORCE_INLINE static void for_each(F&& f)
    {
        for_each_impl(std::forward<F>(f), std::make_index_sequence<N>{});
    }
    ...
    {
        for_each([&](auto I) {
            data.data_[I] = simd<value_t>::load(from + static_cast<std::ptrdiff_t>(I * simd<value_t>::size));
        });
    }
    ...
    {
        for_each([&](auto I) {
            data.data_[I] = simd<value_t>::loadu(from + static_cast<std::ptrdiff_t>(I * simd<value_t>::size));
        });
    }
    ...
    {
        for_each([&](auto I) {
            simd<value_t>::store(data.data_[I], to + static_cast<std::ptrdiff_t>(I * simd<value_t>::size));
        });
    }
    ...
    {
        for_each([&](auto I) {
            simd<value_t>::storeu(data.data_[I], to + static_cast<std::ptrdiff_t>(I * simd<value_t>::size));
        });
    }

    VECTORIZATION_SIMD_RETURN_TYPE set(value_t a, array_simd_t& data)
    {
        for_each([&](auto I) { data.data_[I] = simd<value_t>::set(a); });
    }

    VECTORIZATION_SIMD_RETURN_TYPE setzero(array_simd_t& data)
    {
        for_each([&](auto I) { data.data_[I] = simd<value_t>::setzero(); });
    }

    VECTORIZATION_SIMD_RETURN_TYPE gather(value_t const* from, int const* offsets, array_simd_t& data)
    {
        for_each([&](auto I) {
            data.data_[I] = simd<value_t>::gather(from, offsets + static_cast<int>(I) * simd<value_t>::size);
        });
    }
    ...
    VECTORIZATION_SIMD_RETURN_TYPE gather(value_t const* from, int const offset, array_simd_t& data)
    {
        ...
        for_each([&](auto I) {
            data.data_[I] = simd<value_t>::gather(
                from + static_cast<std::ptrdiff_t>(static_cast<int>(I) * stride), offset);
        });
    }
    ...
    {
        for_each([&](auto I) {
            simd<value_t>::scatter(data.data_[I], offsets + static_cast<int>(I) * simd<value_t>::size, to);
        });
    }
