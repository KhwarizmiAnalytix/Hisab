...
struct packet
{
    ...
    VECTORIZATION_SIMD_RETURN_TYPE gather(value_t const* from, int const* offsets, array_simd_t& data)
    {
        simd_for_each_lane([&](auto ic) {
            constexpr std::size_t i = decltype(ic)::value;
            int const* lane_offsets = offsets + static_cast<int>(i) * simd<value_t>::size;
            simd<value_t>::gather(from, lane_offsets, data.data_[i]);
        });
    }
    ...
    VECTORIZATION_SIMD_RETURN_TYPE gather(value_t const* from, int const offset, array_simd_t& data)
    {
        ...
        simd_for_each_lane([&](auto ic) {
            constexpr std::size_t i = decltype(ic)::value;
            value_t const* ptr = from + static_cast<std::ptrdiff_t>(static_cast<int>(i) * stride);
            simd<value_t>::gather(ptr, offset, data.data_[i]);
        });
    }
    ...
    VECTORIZATION_SIMD_RETURN_TYPE scatter(array_simd_t const& data, int const* offsets, value_t* to)
    {
        simd_for_each_lane([&](auto ic) {
            constexpr std::size_t i = decltype(ic)::value;
            int const* lane_offsets = offsets + static_cast<int>(i) * simd<value_t>::size;
            simd<value_t>::scatter(data.data_[i], lane_offsets, to);
        });
    }
    ...
    VECTORIZATION_SIMD_RETURN_TYPE scatter(array_simd_t const& data, int const offset, value_t* to)
    {
        ...
        simd_for_each_lane([&](auto ic) {
            constexpr std::size_t i = decltype(ic)::value;
            value_t* ptr = to + static_cast<std::ptrdiff_t>(static_cast<int>(i) * stride);
            simd<value_t>::scatter(data.data_[i], offset, ptr);
        });
    }
