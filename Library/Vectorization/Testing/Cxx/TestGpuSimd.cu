...
__global__ void k_arith(
    const T* a, const T* b,
    T* r_add, T* r_sub, T* r_mul, T* r_div, T* r_fma,
    size_t n)
{
...
__global__ void k_unary(
    const T* a,
    T* r_sqrt, T* r_sqr, T* r_fabs, T* r_neg,
    T* r_ceil, T* r_floor, T* r_trunc,
    size_t n)
{
    size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
...
template <typename T>
__global__ void k_explog(
    const T* a,
    T* r_exp, T* r_log, T* r_exp2, T* r_log2, T* r_cbrt,
    size_t n)
{
    size_t i = (size_t)blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
