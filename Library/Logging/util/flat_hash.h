...
public:
    ...
    uint64_t count(const FindKey& key) const { return find(key) == end() ? 0 : 1; }
    ...
    uint64_t  size() const { return num_elements; }
...
private:
    ...
    emplace_new_key(
        int8_t distance_from_desired, EntryPointer current_entry, Key&& key, Args&&... args)
    {
        using std::swap;
...
struct fibonacci_hash_policy
{
    ...
    void commit(int8_t shift_) { shift = shift_; }
    void reset() { shift = 63; }
