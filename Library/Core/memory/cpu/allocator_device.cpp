...
void* allocator_device::allocate_raw(size_t alignment, size_t num_bytes)
{
    ...
    if QUARISMA_UNLIKELY (num_bytes == 0 || static_cast<std::ptrdiff_t>(num_bytes) < 0)
    {
        QUARISMA_THROW("allocating {} bytes", num_bytes);
    }
    ...
    {
        QUARISMA_THROW("CUDA error in allocator_device::allocate_raw: {}", std::to_string(result));
    }
#else
...
void allocator_device::deallocate_raw(void* ptr)
{
    ...
    if (result != cudaSuccess)
    {
        ...
        QUARISMA_LOG_ERROR("CUDA error in allocator_device::free: {}", std::to_string(result));
    }
#else
