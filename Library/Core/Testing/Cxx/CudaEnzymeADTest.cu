...
{
    __enzyme_autodiff((void*)square_impl, enzyme_dup, x, d_x, enzyme_dup, y, d_y);
}
...
#define CUDA_CHECK(call)                                                                         \
    ...
    {                                                                                            \
        ...
        ASSERT_EQ(_err, cudaSuccess) << "CUDA error at " #call ": " << cudaGetErrorString(_err); \
    } while (0)
...
void CudaEnzymeADTest::SetUp()
{
    ...
    ASSERT_GE(chosen, 0) << "Failed to select any usable CUDA device out of " << device_count
                         << " visible devices.";
